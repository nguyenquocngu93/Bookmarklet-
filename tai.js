// ==UserScript==
// @name         DevLib CDN Player - Glass UI Final
// @namespace    http://tampermonkey.net/
// @version      4.0
// @description  Fixed landscape + beautiful text buttons
// @author       You
// @match        *://*/*
// @grant        none
...
(function () {
    ...
    const CSS = `
        ...
        #dlp-prev-title { flex: 1; color: #fff; font-size: 13px; font-weight: 600; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
        .dlp-icon-btn {
            width: 34px;
            height: 34px;
            border-radius: 50%;
        ...
        #dlp-vid-wrap { background: #000; width: 100%; height: 230px; display: flex; align-items: center; justify-content: center; }
        #dlp-vid { width: 100%; height: 100%; object-fit: contain; transition: transform 0.3s; }

        /* ── ACTION BUTTONS - CHỈ CHỮ, ĐẸP ── */
        .dlp-actions {
            display: flex;
            flex-wrap: wrap;
            gap: 8px;
        ...
        .dlp-act:hover { transform: translateY(-2px); box-shadow: 0 8px 20px rgba(0,0,0,0.3); }
        .dlp-act:hover::after { opacity: 1; }
        .dlp-act:active { transform: scale(0.97); }

        .dlp-act.copy  { background: linear-gradient(135deg, #1e40af, #3b82f6); }
        .dlp-act.full  { background: linear-gradient(135deg, #991b1b, #ef4444); }
        .dlp-act.share { background: linear-gradient(135deg, #065f46, #10b981); }
        .dlp-act.dl    { background: linear-gradient(135deg, #5b21b6, #8b5cf6); }
        .dlp-act.pip   { background: linear-gradient(135deg, #9d174d, #ec4899); }
        .dlp-act.save  { background: linear-gradient(135deg, #b91c1c, #f43f5e); }
        .dlp-act.list  { background: linear-gradient(135deg, #115e59, #14b8a6); }
        .dlp-act.sub   { background: linear-gradient(135deg, #3730a3, #6366f1); }
        .dlp-act.rot   { background: linear-gradient(135deg, #155e75, #06b6d4); }
        .dlp-act.zoom  { background: linear-gradient(135deg, #92400e, #f59e0b); }
    ...
    function initUI() {
        ...
        document.body.insertAdjacentHTML('beforeend', `
            ...
            <div id="dlp-prev">
                ...
                <div class="dlp-actions" id="dlp-p-acts"></div>
            </div>
    ...
    function initLogic() {
        ...
        function triggerFullscreen() {
            ...
            if (req) {
                req.call(vid).then(() => {
                    // Lock landscape after entering fullscreen
                    setTimeout(() => {
                        if (screen.orientation && screen.orientation.lock) {
                            screen.orientation.lock('landscape').catch(() => {
                                // Fallback: try again with any orientation
        ...
        // Also handle video webkit fullscreen (iOS)
        const vid = $('dlp-vid');
        if (vid) {
            vid.addEventListener('webkitbeginfullscreen', () => {
                if (screen.orientation && screen.orientation.lock) {
                    screen.orientation.lock('landscape').catch(() => {});
                }
            });
        ...
        function openPrev(item) {
            ...
            $('dlp-p-acts').innerHTML = `
                <button class="dlp-act copy" id="dlp-pc-cp">Copy</button>
                <button class="dlp-act full" id="dlp-pc-fs">Fullscreen</button>
                <button class="dlp-act share" id="dlp-pc-share">Share</button>
                <button class="dlp-act dl" id="dlp-pc-dl">Download</button>
                <button class="dlp-act pip" id="dlp-pc-pip">PiP</button>
                <button class="dlp-act save" id="dlp-pc-bm">Save</button>
                <button class="dlp-act list" id="dlp-pc-pl">+Playlist</button>
                <button class="dlp-act sub" id="dlp-pc-sub">Subtitle</button>
                <button class="dlp-act rot" id="dlp-pc-rot">Rotate</button>
                <button class="dlp-act zoom" id="dlp-pc-zm">${zoom}x</button>
            `;
            ...
            const build = (t) => {
                ...
                const data = {
                    ...
                    ffmpeg: [
                        ...
                        { l:'Re-encode', c:`ffmpeg -referer "${REF}" -i "${url}" -c:v libx264 -c:a aac output.mp4` },
                    ],
                    termux: [
        ...
        $('dlp-btn-opt').onclick = (e) => {
            ...
            d.innerHTML = `
                ...
                <div class="dlp-di" id="dlp-m-pip">PiP Mode</div>
