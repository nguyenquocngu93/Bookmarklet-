 ...
 * Fixes: Clean UI without icons, Preview playback in History/Favorites
...
function getGlobalCSS(t) {
    return '' +
        ...
        '.__uvd_history_meta__ { color: ' + t.text3 + '; font-size: 9px; margin-bottom: 5px; }' +
        '.__uvd_history_url__ { color: ' + t.text2 + '; font-size: 9px; font-family: monospace; word-break: break-all; max-height: 25px; overflow: hidden; margin-bottom: 8px; }' +
        ...
        tsGetTorrentStat(hash, function(err, stat) {
            ...
            files.forEach(function(f, idx) {
                ...
                html += '<div class="__uvd_ts_file_card__">' +
                    '<div class="__uvd_ts_file_name__">' + escapeHtml(shortName) + '</div>' +
...
function renderFavorites(container) {
    ...
    data.favorites.forEach(function(fav, i) {
        ...
        var canPreview = ['M3U8', 'MP4', 'WEBM', 'MKV'].indexOf(fav.type) >= 0;
        ...
        card.innerHTML = 
            ...
            '<div class="__uvd_actions__">' +
                (canPreview ? '<button class="__uvd_act_btn__ __uvd_btn_preview__" data-url="' + encodeURIComponent(fav.url) + '" data-type="' + fav.type + '" data-action="preview"><span class="__uvd_btn_accent__"></span>Preview</button>' : '') +
...
function renderHistory(container) {
    ...
    history.forEach(function(h) {
        ...
        // Check if can preview
        var canPreview = ['M3U8', 'MP4', 'WEBM', 'MKV'].indexOf(h.type) >= 0;
        ...
        item.innerHTML = 
            ...
            '<div class="__uvd_actions__" style="margin-top:8px;">' +
                (canPreview ? '<button class="__uvd_act_btn__ __uvd_btn_preview__" data-url="' + encodeURIComponent(h.url) + '" data-type="' + h.type + '" data-action="preview"><span class="__uvd_btn_accent__"></span>Preview</button>' : '') +
                '<button class="__uvd_act_btn__ __uvd_btn_copy__" data-url="' + encodeURIComponent(h.url) + '" data-action="copy"><span class="__uvd_btn_accent__"></span>Copy</button>' +
            '</div>';
