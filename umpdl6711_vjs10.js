/**
 * Universal Media Player & Downloader - V6.7.11 PRO
 * - FIX NGHIÊM TRỌNG: tầng dò dự phòng (fallback, thêm ở 6.7.10) từng tự bấm
 *   TRÚNG chính các nút xuất "TXT/M3U/CSV/JSON" của bản thân script (khớp quy
 *   tắc "nhãn viết tắt in hoa ngắn"), gây tải nhầm file .txt. Đã loại trừ toàn
 *   bộ UI của chính script (mọi phần tử trong `.uvd-scope`) khỏi mọi vùng auto-
 *   click, kể cả tầng selector chuẩn lẫn tầng dự phòng.
 * - FIX NGHIÊM TRỌNG: biến màu giao diện (--text, --border, --bg, --accent...)
 *   trước định nghĩa trên `:root` — chính là thẻ <html>, DÙNG CHUNG với trang
 *   web đích. Site nào cũng khai báo cùng tên biến CSS generic đó trên :root
 *   là đè lên giá trị của script → toàn panel bị nhuộm theo màu web ("giao
 *   diện không giữ được màu"). Đã chuyển toàn bộ sang scope riêng `.uvd-scope`
 *   (tự gắn cho panel/overlay/toast qua __uvdAppendRoot), không còn đụng :root.
 * - FIX: `*{box-sizing:border-box}` trước đó áp dụng lên TOÀN BỘ trang web chứ
 *   không chỉ panel — có thể phá layout site đích. Đã giới hạn về `.uvd-scope`.
 * - MỚI: "Cài đặt" giờ mở dạng overlay chồng lớp trượt lên từ dưới (giống app),
 *   thay vì là 1 tab chật trong tabbar — bấm icon ⚙ ở header. Tabbar chính chỉ
 *   còn Streams / Nút đã click / Trình phát nên đỡ chật.
 * - MỚI: setting "Cho phép nút thu nhỏ Script" (▼) — tắt được nếu hay gặp panel
 *   glitch lúc thu nhỏ, mặc định vẫn bật.
 * - FIX: đóng script (nút ×) giờ giải phóng bộ nhớ đầy đủ hơn — tự đóng player
 *   overlay nếu đang mở (destroy hls.js, đóng AudioContext, clear video src),
 *   xoá Map danh sách link đã quét, gỡ luôn thẻ <style> đã tiêm vào <head>.
 * - (6.7.10) Auto-click rơi về tầng dò theo text (server/stream/host, viết tắt
 *   VI/MD/DS/ST...) khi selector chuẩn không khớp gì.
 * - (6.7.9) Nút ⏭ thử lần lượt từng server tới khi ra link.
 * - (6.7.8) Tab "Nút đã click" ghi + chặn theo từng nút cụ thể.
 * - FIX hiệu năng/giật lag trên site nặng quảng cáo (panelObserver, autoplay
 *   observer gộp theo rAF, ripple listener scope panel, animation nền tạm
 *   dừng khi ẩn tab).
 * - FIX: header panel giật/nhìn như "trôi theo" trang khi cuộn trên Chrome Android.
 * - Tính năng "Tải Blob" cho link blob: (video/audio).
  ...
  delays.forEach(function(delay, idx) {
    setTimeout(function() {
      ...
      if (found > 0 && reportedAt === -1) {
        ...
        buildUI(); // có thêm luồng mới ở lần quét sau, cập nhật UI im lặng
...
.uvd-settings-sheet{width:100%;max-width:1000px;max-height:92dvh;display:flex;flex-direction:column;transform:translateY(100%);transition:transform .32s cubic-bezier(.4,0,.2,1);border-radius:24px 24px 0 0;overflow:hidden;background:var(--glass);backdrop-filter:blur(var(--uvd-blur)) saturate(130%);-webkit-backdrop-filter:blur(var(--uvd-blur)) saturate(130%);border:1px solid var(--border);box-shadow:0 -20px 50px rgba(0,0,0,0.8);}
...
.uvd-settings-title{font-weight:700;font-size:16px;}
