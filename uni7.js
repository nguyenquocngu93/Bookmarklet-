/**
 * Universal Media Player v3.0
 * - Code quét của mình (chuẩn, không bắt nhầm)
 * - UI đẹp + tính năng từ userscript
 * - Bypass cho Turbovid...
 */
(function() {
    'use strict';
    ...
    style.textContent = 
        ...
        '#__ump_list_content__ .ump-item { background:#2a2a2a;padding:12px;margin:8px 0;border-radius:6px;cursor:pointer;transition:all 0.2s;border:2px solid transparent; }' +
        '#__ump_list_content__ .ump-item:hover { background:#333;border-color:#ff6b6b; }' +
        '#__ump_list_content__ .ump-item-title { font-size:13px;font-weight:500;margin-bottom:5px;word-break:break-all;color:#fff; }' +
        '#__ump_list_content__ .ump-item-info { color:#888;font-size:11px;display:flex;gap:8px;flex-wrap:wrap; }' +
        '#__ump_list_content__ .ump-badge { padding:2px 8px;border-radius:3px;font-size:10px;font-weight:bold;color:#fff; }' +
        '#__ump_list_content__ .ump-badge.hls { background:#9c27b0; }' +
        '#__ump_list_content__ .ump-badge.dash { background:#2196f3; }' +
        '#__ump_list_content__ .ump-badge.mp4 { background:#FF9800; }' +
        '#__ump_list_content__ .ump-badge.other { background:#607D8B; }' +
    ...
    scanBtn.onclick = function() { scanMedia(); };
    document.body.appendChild(scanBtn);
    
    // Backdrop
    var backdrop = document.createElement('div');
    backdrop.id = '__ump_backdrop__';
    backdrop.onclick = function() { closeAll(); };
    ...
    mediaList.innerHTML = 
        '<div id="__ump_list_header__">' +
            '<div id="__ump_list_title__">📹 Danh sách Media</div>' +
    ...
    // ========== EVENT LISTENERS ==========
    document.getElementById('__ump_close_list__').onclick = function() { closeMediaList(); };
    document.getElementById('__ump_close_modal__').onclick = function() { closeModal(); };
    document.getElementById('__ump_copy_cmd__').onclick = function() {
        copy(document.getElementById('__ump_command__').value);
        showToast('✅ Đã copy!');
    };
    
    // ========== QUÉT MEDIA (CODE CỦA BẠN) ==========
    var allUrls = [];
    var mediaItems = [];
    ...
    function scanMedia() {
        ...
        allUrls = [...urls.entries()].map(function(e) {
            return { url: e[0], type: e[1].type, source: e[1].source, priority: e[1].priority };
        }).sort(function(a, b) { return a.priority - b.priority; });
        
        // Lọc bỏ iframe, chỉ lấy playable
        mediaItems = allUrls.filter(function(x) { return x.type !== 'IFRAME'; });
        ...
        if (mediaItems.length > 0) {
            showToast('✅ Tìm thấy ' + mediaItems.length + ' media');
            showMediaList();
        } else {
            showToast('❌ Không tìm thấy media nào!');
        }
    }
    ...
    function showMediaList() {
        ...
        if (mediaItems.length === 0) {
            content.innerHTML = '<div style="color:#888;text-align:center;padding:40px;">Không có media nào</div>';
        } else {
            mediaItems.forEach(function(item, i) {
                var badgeClass = item.mediaType === 'M3U8' ? 'hls' : 
                                item.mediaType === 'MPD' ? 'dash' : 
                                item.mediaType === 'MP4' ? 'mp4' : 'other';
                ...
                div.innerHTML = 
                    ...
                    '<div class="ump-item-info">' +
                        '<span class="ump-badge ' + badgeClass + '">' + item.mediaType + '</span>' +
                        '<span class="ump-badge other">' + item.quality + '</span>' +
                        '<span class="ump-badge other">' + item.source + '</span>' +
                    '</div>';
    ...
    function openPlayer(streamInfo) {
        ...
        topBar.innerHTML = 
            '<div style="color:#fff;font-size:12px;flex:1;overflow:hidden;padding-right:10px;">' +
                ...
                '<div style="opacity:0.7;font-size:9px;">' + streamInfo.type + ' · ' + streamInfo.quality + '</div>' +
            '</div>' +
            '<button id="__ump_fullscreen_btn__" style="background:rgba(255,255,255,0.2);color:#fff;border:0;width:32px;height:32px;border-radius:50%;font-size:16px;margin-right:4px;cursor:pointer;">⛶</button>' +
        ...
        controls.innerHTML = 
            ...
            '<div style="display:grid;grid-template-columns:repeat(5,1fr);gap:4px;margin-bottom:4px;">' +
                ...
                '<button class="__ump_ctrl__" data-a="pip" style="background:rgba(255,255,255,0.15);color:#fff;border:0;padding:8px;border-radius:4px;font-size:10px;cursor:pointer;">📺</button>' +
            '</div>' +
            '<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:4px;margin-bottom:4px;">' +
                ...
                '<button class="__ump_ctrl__" data-a="loop" style="background:rgba(255,255,255,0.15);color:#fff;border:0;padding:8px;border-radius:4px;font-size:10px;cursor:pointer;">🔁</button>' +
            '</div>' +
        ...
        controls.querySelectorAll('.__ump_ctrl__').forEach(function(b) {
            b.onclick = function() {
                ...
                else if (a === 'mute') { video.muted = !video.muted; showToast(video.muted ? '🔇 Muted' : '🔊 Unmuted'); }
                else if (a === 'pip') {
                    if (document.pictureInPictureElement) document.exitPictureInPicture();
                    else video.requestPictureInPicture().catch(function(){});
                }
                else if (a === 'loop') { video.loop = !video.loop; showToast(video.loop ? '🔁 Loop ON' : '🔁 Loop OFF'); }
                else if (a === 'share') shareUrl(url);
                else if (a === 'cmd') showCommandModal(streamInfo);
                else if (a === 'bypass-direct') { video.src = ''; if(hlsInstance)hlsInstance.destroy(); loadVideoWithBypass('direct'); }
                else if (a === 'bypass-proxy') { video.src = ''; if(hlsInstance)hlsInstance.destroy(); loadVideoWithBypass('proxy'); }
        ...
        // Fullscreen
        document.getElementById('__ump_fullscreen_btn__').onclick = function() {
        ...
        overlay.addEventListener('click', function(e) {
            if (e.target === video || e.target === overlay) { if (video.paused) video.play(); else video.pause(); }
            showControls();
        });
        showControls();
    ...
    function showMenu(streamInfo, video) {
        ...
        menu.querySelectorAll('.__ump_m__').forEach(function(b) {
            b.onclick = function() {
                ...
                if (a === 'copy-url') { copy(url); showToast('✓ Copied'); }
                    ...
                    copy(cmd); showToast('✓ FFmpeg copied');
                }
    ...
    function showCommandModal(streamInfo) {
        var url = streamInfo.url;
        var cmd = 
            '# yt-dlp\nyt-dlp --referer "' + pageInfo.referer + '" -f best -o "' + pageInfo.title + '.%(ext)s" "' + url + '"\n\n' +
            '# yt-dlp + aria2c\nyt-dlp --external-downloader aria2c --external-downloader-args "-x 16 -k 1M" "' + url + '"\n\n' +
            '# FFmpeg\nffmpeg -headers "Referer: ' + pageInfo.referer + '" -i "' + url + '" -c copy "' + pageInfo.title + '.mp4"';
    ...
    function shareUrl(url) {
        ...
        else { copy(url); showToast('✓ Copied'); }
    }
    ...
    console.log('🎬 UMP v3.0 loaded - Scan button ở góc phải bên dưới');
})();