/**
Universal Media Player & Downloader - V5.5
Overlay player với toolbar + Giả lập Play
...
var pageInfo = {
  ...
  url: location.href, host: host, referer: profile.referer,
  origin: location.origin, userAgent: profile.userAgent || navigator.userAgent
};
var urls = new Map();
var patterns = [
...
function stopMonitor() { window.fetch = originalFetch; XMLHttpRequest.prototype.open = originalXHROpen; }
scan(document, 'main');
try { performance.getEntriesByType('resource').forEach(function(e) { findUrls(e.name, 'network:perf'); }); } catch(e) {}
installMonitor();

function simulatePlay() {
  toast('Đang giả lập click play...', '#8b5cf6');
  var clicked = 0;
  var tried = new Set();
  function safeClick(el, label) {
    if (!el || tried.has(el)) return;
    tried.add(el);
    try {
      ...
      clicked++;
      console.log('[UMP DL] Clicked:', label, el);
    } catch(e) { console.warn('[UMP DL] Click fail:', e); }
  }
  document.querySelectorAll('video').forEach(function(v){ if (v.paused) safeClick(v, 'video'); });
  var playSelectors = [
    '.jw-icon-display', '.jw-display-icon-container', '.jw-button-container .jw-icon',
    '.vjs-big-play-button', '.vjs-play-control', '.vjs-control.vjs-play-control',
    '[data-plyr="play"]', '.plyr__control--overlaid', '.fp-ui', '.fp-player',
    '[class*="play" i]', '[class*="player" i]', '[class*="btn-play" i]',
    '[id*="play" i]', '[id*="player" i]', '.play-button', '.play-btn', '.video-play', '.icon-play',
    '#play-button', '#play-btn', '#videoPlay', '.video-overlay', '.player-overlay', '.video-wrapper',
    '[data-action="play"]', '[data-trigger="play"]', '[onclick*="play" i]', '[onclick*="Play" i]'
  ];
  playSelectors.forEach(function(sel){
    try {
      document.querySelectorAll(sel).forEach(function(el){
        var rect = el.getBoundingClientRect();
        if (rect.width > 20 && rect.height > 20 && rect.top > -rect.height) safeClick(el, sel);
      });
    } catch(e) {}
  });
  document.querySelectorAll('iframe').forEach(function(f){
    try {
      if (f.contentDocument) {
        f.contentDocument.querySelectorAll('video, .jw-icon-display, .vjs-big-play-button, [class*="play" i]')
          .forEach(function(el){ safeClick(el, 'iframe:' + f.src); });
      }
    } catch(e) { safeClick(f, 'iframe'); }
  });
  var all = document.querySelectorAll('button, div, a, span');
  all.forEach(function(el){
    var t = (el.textContent || '').trim().toLowerCase();
    if (t === 'play' || t === '▶' || t === '►' || t === 'play video') {
      var rect = el.getBoundingClientRect();
      if (rect.width > 30 && rect.height > 30) safeClick(el, 'text:' + t);
    }
  });
  setTimeout(function(){
    scan(document, 'after-simulate');
    toast('Đã click ' + clicked + ' nút • Đang quét lại...', '#22c55e');
    if (document.getElementById('uvd')) buildUI();
  }, 1500);
  setTimeout(function(){
    scan(document, 'after-simulate-late');
    if (document.getElementById('uvd')) buildUI();
  }, 4000);
}
...
var playerState = {
  ...
  isMinimized: false, url: '', type: '', resolution: '',
...
function showVideoPlayer(url, type) {
  ...
  titleInfo.innerHTML =
    ...
    '<div style="font-size:11px;color:#aaa;margin-top:2px;">' + type + (playerState.resolution ? ' · ' + playerState.resolution : '') + '</div>';
...
function buildUI() {
  ...
  header.innerHTML =
    ...
    '<div style="display:flex;gap:6px;">' +
      '<button class="uvd-btn-icon uvd-ripple-btn" id="uvd_simplay" title="Giả lập Play (cho trang lazy-load)" style="background:rgba(139,92,246,0.25);"><span style="font-size:16px;">▶</span></button>' +
      '<button class="uvd-btn-icon uvd-ripple-btn" id="uvd_minimize" title="Thu nhỏ"><span style="font-size:18px;">−</span></button>' +
      '<button class="uvd-btn-icon uvd-ripple-btn" id="uvd_refresh__" title="Làm mới"><span style="font-size:16px;">↻</span></button>' +
  ...
  document.getElementById('uvd_simplay').onclick = function() { simulatePlay(); };
...
function renderStreams(container, arr) {
  if (!arr.length) {
    container.innerHTML = '<div style="text-align:center;padding:40px;color:var(--text2);">Không phát hiện stream nào.<br><span style="font-size:12px;margin-top:8px;display:block;color:var(--text3);">Thử bấm nút ▶ tím để giả lập play</span></div>';
    return;
  }
...
function renderSettings(container) {
  ...
  var avatarUrl = 'https://i0.wp.com/thatnhucuocsong.com.vn/wp-content/uploads/2022/02/Anh-meo-cute.jpg?ssl=1';
  var bookmarkletCode = "javascript:(function(){var s=document.createElement('script');s.src='[LINK_CỦA_BẠN]';document.head.appendChild(s);})();";
  
  container.innerHTML =
    '<div class="uvd-profile-card">' +
      '<div class="uvd-profile-avatar" style="background-image:url(' + avatarUrl + ');background-size:cover;background-position:center;"></div>' +
    ...
    '<div class="uvd-card uvd-timeline-card">' +
      ...
      '<div style="font-size:12px;color:var(--accent2);margin-top:8px;">' +
        '<strong>🔁 Force update:</strong> Thêm <code>?v=' + Date.now() + '</code> vào cuối link script để tránh cache.' +
      '</div>' +
    '</div>' +
    ...
    '<div class="uvd-card uvd-timeline-card">' +
      '<div style="font-size:13px;color:var(--text2);line-height:1.7;">' +
        ...
        '• Nếu không thấy link → bấm nút <strong style="color:#8b5cf6;">▶ tím</strong> để giả lập play<br>' +
...
console.log('V5.5 UMP DL - Overlay Player with Simulate Play Ready');
toast('V5.5 sẵn sàng! Bấm ▶ để giả lập play');
