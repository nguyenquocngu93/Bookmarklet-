 ...
 * - MỚI: PerformanceObserver theo dõi LIÊN TỤC mọi request tài nguyên (thay vì
 *   chỉ chụp 1 lần lúc chèn script) — bắt được link thật kể cả khi nó chỉ
 *   được tải về SAU khi hết đếm ngược quảng cáo, hoặc từ iframe khác gốc
 *   (URL của resource luôn hiện ra qua Resource Timing API dù cross-origin).
 * - MỚI: vá thêm fetch/XHR bên trong các iframe CÙNG GỐC hiện có trên trang
 *   (trước đây chỉ vá được window chính) — bắt được link do chính script bên
 *   trong iframe cùng gốc gọi tới.
 * - GIỚI HẠN THẬT (không có cách lách): iframe KHÁC GỐC bị Same-Origin Policy
 *   của trình duyệt chặn cứng — không đọc được DOM/script/network nội bộ của
 *   nó bằng bất kỳ thủ thuật JS nào từ trang cha. PerformanceObserver vẫn thấy
 *   được URL request (không phải nội dung), đó là cách hợp lệ duy nhất.
 * - Video player: trả lại đúng bản gốc, giữ fix BLOB (mượn video gốc) + Tải Blob.
