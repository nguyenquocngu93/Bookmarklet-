/**
 * Universal Video Downloader V3.1 — Android Optimized
 * Features: Glass UI optimized, Debounced search, Lighter animations,
 *           Larger preview, Touch-friendly, Performance mode
...
(function() {
    ...
    // ========== THEMES (Optimized for Android) ==========
    var themes = {
        'glass-dark': {
            name: 'Glass Dark',
            bg: 'rgba(15, 15, 20, 0.92)',
            bg2: 'rgba(25, 25, 35, 0.88)',
            bg3: 'rgba(35, 35, 50, 0.75)',
        ...
        'glass-light': {
            ...
            bg2: 'rgba(245, 245, 255, 0.88)',
            bg3: 'rgba(235, 235, 245, 0.75)',
        ...
        'glass-purple': {
            name: 'Glass Purple',
            bg: 'rgba(20, 5, 40, 0.92)',
            bg2: 'rgba(35, 10, 60, 0.88)',
            bg3: 'rgba(50, 15, 80, 0.75)',
            ...
            text3: '#8060a0',
            primary: '#BB86FC',
            primaryGlow: 'rgba(187, 134, 252, 0.25)',
            accent: '#03DAC5',
            accentGlow: 'rgba(3, 218, 197, 0.25)',
            danger: '#CF6679',
            success: '#66BB6A',
            warning: '#FFB74D'
        },
        'glass-matrix': {
            name: 'Glass Matrix',
            bg: 'rgba(0, 5, 0, 0.92)',
            bg2: 'rgba(0, 15, 0, 0.88)',
            bg3: 'rgba(0, 25, 0, 0.75)',
        ...
        'glass-cyber': {
            ...
            bg2: 'rgba(15, 15, 45, 0.88)',
            bg3: 'rgba(25, 25, 60, 0.75)',
                ...
                '<div class="__uvd_header_left__">' +
                    ...
                    '<div>' +
                        ...
                        '<div class="__uvd_subtitle__">' +
                            ...
                            '<span class="__uvd_count_video__">' + videoCount + '</span>🎬 ' +
                            '<span class="__uvd_count_photo__">' + photoCount + '</span>🖼️ ' +
                            '<span class="__uvd_count_script__">' + scriptCount + '</span>📜' +
                        '</div>' +
                    '</div>' +
                '</div>' +
    ...
    function getGlobalCSS(t) {
        // Optimize: disable heavy animations on mobile
        var animDuration = isMobile ? '0.15s' : '0.25s';
        var blurSize = isMobile ? '8px' : '12px';
        ...
        '.__uvd_logo__ { font-size: 24px; }' +
        '.__uvd_title__ { font-size: 15px; font-weight: 700; }' +
        '.__uvd_version__ { font-size: 9px; background: ' + t.primary + '; color: #fff; padding: 1px 5px; border-radius: 3px; vertical-align: middle; }' +
        '.__uvd_subtitle__ { font-size: 10px; color: ' + t.text2 + '; margin-top: 2px; display: flex; align-items: center; gap: 3px; }' +
        '.__uvd_live_dot__ { width: 5px; height: 5px; background: ' + t.success + '; border-radius: 50%; display: inline-block; animation: uvdPulse 1.5s infinite; }' +
        ...
        '.__uvd_fav_btn__ {' +
            'background: transparent; border: 0; font-size: 16px; cursor: pointer; padding: 2px 4px;' +
            '-webkit-tap-highlight-color: transparent;' +
        '}' +
        ...
        '.__uvd_btn_primary__ {' +
            ...
            'font-weight: 700; font-size: 12px; cursor: pointer; flex: 1;' +
            '-webkit-tap-highlight-color: transparent;' +
        '}' +
        ...
        '.__uvd_btn_ghost__ {' +
            ...
            'color: ' + t.text + '; padding: 10px 16px; border-radius: 8px;' +
            'font-weight: 600; font-size: 12px; cursor: pointer; flex: 1;' +
        '}' +
        ...
        '.__uvd_theme_btn__ {' +
            ...
            'color: ' + t.text + '; padding: 10px; border-radius: 8px;' +
        ...
        '.__uvd_profile_card__ {' +
            ...
            'padding: 8px; margin-bottom: 5px; border-radius: 6px; font-size: 11px;' +
        '}' +
        ...
        '.__uvd_player_container__ {' +
            ...
            'width: 100%;' +
        '}' +
        '.__uvd_player__ { width: 100%; max-height: 70vh; display: block; background: #000; }' +
        '.__uvd_player_status__ {' +
            'position: absolute; bottom: 8px; left: 50%; transform: translateX(-50%);' +
            'background: rgba(0,0,0,0.75); padding: 5px 12px; border-radius: 15px; font-size: 10px; color: #fff;' +
        '}' +
        '.__uvd_player_controls__ { display: flex; gap: 5px; flex-wrap: wrap; margin-top: 8px; }' +
        '.__uvd_player_btn__ {' +
            'background: ' + t.bg3 + '; border: 1px solid ' + t.glassBorder + ';' +
            'color: ' + t.text + '; padding: 7px 10px; border-radius: 6px;' +
    ...
    function bindGlobalEvents() {
        // Tabs - use event delegation for better performance
    ...
    // ========== PREVIEW PLAYER (Larger for Android) ==========
    function showPreview(url) {
        var t = getTheme();
        var overlay = document.createElement('div');
        overlay.className = '__uvd_overlay__';
        overlay.innerHTML = 
            '<div class="__uvd_overlay_box__" style="padding:10px;">' +
                ...
                '<div class="__uvd_player_container__">' +
                    '<video id="__uvd_pv__" class="__uvd_player__" controls autoplay playsinline webkit-playsinline style="max-height:75vh;"></video>' +
                ...
                '<div class="__uvd_player_controls__" id="__uvd_pv_controls__">' +
                    '<button class="__uvd_player_btn__" id="__uvd_pv_copy__">📋 Copy</button>' +
                    ...
                    '<button class="__uvd_player_btn__" id="__uvd_pv_fullscreen__">⛶ Full</button>' +
                '</div>' +
                '<div id="__uvd_pv_quality_list__" style="margin-top:8px;"></div>' +
            '</div>';
        document.body.appendChild(overlay);
        ...
        document.getElementById('__uvd_pv_close__').addEventListener('click', function() {
            video.pause(); video.src = ''; overlay.remove();
        });
        document.getElementById('__uvd_pv_copy__').addEventListener('click', function() { copy(url); toast('✓ Đã copy', 'success'); });
        document.getElementById('__uvd_pv_share__').addEventListener('click', function() { shareUrl(url); });
        document.getElementById('__uvd_pv_cmd__').addEventListener('click', function() { overlay.remove(); showCommandPicker(url, 'M3U8'); });
        document.getElementById('__uvd_pv_fullscreen__').addEventListener('click', function() {
        ...
        function initHls() {
            if (Hls.isSupported()) {
                ...
                currentHls.on(Hls.Events.MANIFEST_PARSED, function(event, data) {
                    status.innerText = '✅ HLS · ' + data.levels.length + ' levels';
                    
                    if (data.levels.length > 1) {
                        var qHtml = '<div style="color:' + t.text2 + ';font-size:10px;margin-bottom:5px;font-weight:600;">🎞️ Chất lượng:</div>';
                        qHtml += '<div style="display:flex;gap:4px;flex-wrap:wrap;">';
                        ...
                        qualityList.addEventListener('click', function(e) {
                            var btn = e.target.closest('.__uvd_player_btn__');
                            if (btn) {
                                ...
                                toast('🎞️ Quality: ' + btn.innerText, 'info');
                            }
                        });
                    }
                });
                ...
                video.addEventListener('waiting', function() { status.innerText = '⏳ Buffering...'; });
                video.addEventListener('error', function() { status.innerText = '❌ Lỗi phát video'; });
    ...
    function renderSettings(container) {
            ...
            profiles.forEach(function(p) {
                html += 
                    '<div class="__uvd_profile_card__">' +
                        ...
                        '<button class="__uvd_btn_danger__ __uvd_delprofile__" data-host="' + p + '" style="padding:3px 8px;font-size:10px;margin-top:5px;">🗑️ Xóa</button>' +
                    '</div>';
            });
        }
        html += '</div>';
        ...
        html += '<button class="__uvd_btn_primary__" id="__uvd_restore__" style="flex:1;background:' + t.accent + ';">📥 Import</button>';
        ...
        html += '<div style="color:' + t.text2 + ';font-size:10px;line-height:1.7;">';
        html += '<div>📦 Version: 3.1 — Android Optimized</div>';
        ...
        container.addEventListener('click', function(e) {
            ...
            if (themeBtn) {
                data.theme = themeBtn.dataset.theme;
                storage.set(data);
                buildUI();
            ...
            if (delBtn) {
                delete data.siteProfiles[delBtn.dataset.host];
                storage.set(data);
    ...
    console.log('✅ Universal DL V3.1 loaded! Found', urls.size, 'streams initially');
    toast('✅ V3.1 Ready!', 'success');
})();