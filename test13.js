/**
 * Universal Video Downloader V3.8 — Clean + Dropdown Menu
 * Fixes: Accent on buttons, Quality lag/empty, Status hidden,
 *        Command picker breaks video, Fullscreen rotation (Chrome Android)
 * New: Dropdown menu on click, smooth transitions
 * Removed: TorrServer, extra themes
...
(function() {
    ...
    var themes = {
        'dark': {
            ...
            bg2: 'rgba(18, 18, 30, 0.88)',
            bg3: 'rgba(28, 28, 45, 0.72)',
        ...
        'light': {
            name: 'Light',
            bg: 'rgba(245, 245, 255, 0.92)',
            bg2: 'rgba(240, 240, 250, 0.88)',
            bg3: 'rgba(235, 235, 245, 0.72)',
    ...
    function getGlobalCSS(t) {
        ...
        '@keyframes uvdSlideDown { from { opacity: 0; transform: translateY(-10px) scale(0.95); } to { opacity: 1; transform: translateY(0) scale(1); } }' +
        ...
        '}' +
        '.__uvd_list_item__:hover { background: ' + t.bg2 + '; border-color: ' + t.primary + '40; }' +
        '.__uvd_list_item__:active { transform: scale(0.98); }' +
        '.__uvd_list_item__ .__uvd_list_preview__ { ' +
            'display: flex; align-items: center; gap: 10px;' +
            'overflow: hidden;' +
        '}' +
        '.__uvd_list_item__ .__uvd_list_badge__ {' +
            'padding: 3px 10px; border-radius: 6px; font-size: 10px; font-weight: 700; color: #fff; flex-shrink: 0;' +
            'box-shadow: 0 2px 8px rgba(0,0,0,0.2);' +
        '}' +
        '.__uvd_list_item__ .__uvd_list_url__ {' +
            'font-family: monospace; font-size: 10px; color: ' + t.text2 + ';' +
            'white-space: nowrap; overflow: hidden; text-overflow: ellipsis;' +
            'flex: 1; min-width: 0;' +
        '}' +
        '.__uvd_list_item__ .__uvd_list_arrow__ {' +
            'color: ' + t.text3 + '; font-size: 12px; transition: transform 0.25s ease; flex-shrink: 0;' +
        '}' +
        '.__uvd_list_item__.open .__uvd_list_arrow__ { transform: rotate(180deg); }' +
        ...
        '}' +
        '.__uvd_list_item__.open .__uvd_list_menu__ {' +
            'max-height: 500px;' +
            'opacity: 1;' +
            'margin-top: 10px;' +
        '}' +
        
        '.__uvd_list_actions__ { display: grid; grid-template-columns: repeat(2, 1fr); gap: 6px; padding-top: 2px; }' +
        '.__uvd_list_actions__ .__uvd_act_btn__ {' +
        ...
        '}' +
        '.__uvd_list_actions__ .__uvd_act_btn__:active { transform: scale(0.96); opacity: 0.9; }' +
        '.__uvd_list_actions__ .__uvd_act_btn__.full { grid-column: 1 / -1; }' +
        ...
        '.__uvd_btn_fav__ { background: linear-gradient(135deg, #FFD700, #FFA000); color: #333; }' +
        ...
        '.__uvd_list_divider__ { height: 1px; background: ' + t.glassBorder + '; margin: 2px 0; }';
    }
    ...
    // ===== NEW: RENDER AS LIST WITH DROPDOWN MENU =====
    function renderCategory(container, category) {
        ...
        items.forEach(function(item, i) {
            ...
            itemDiv.className = '__uvd_list_item__';
            itemDiv.dataset.url = url;
            
            // Preview line
            var previewHtml = 
                '<div class="__uvd_list_preview__">' +
                    '<span class="__uvd_list_badge__" style="background:' + color + ';">' + type + '</span>' +
                    '<span class="__uvd_list_url__">' + escapeHtml(url) + '</span>' +
                    '<span class="__uvd_list_arrow__">▾</span>' +
                '</div>';
            
            // Hidden menu - sẽ slide xuống khi mở
            var menuHtml = 
                '<div class="__uvd_list_menu__">' +
                    '<div class="__uvd_list_actions__">' +
                        '<button class="__uvd_act_btn__ __uvd_btn_share__" data-action="share"><span class="__uvd_btn_accent__"></span>Share</button>' +
                        '<button class="__uvd_act_btn__ __uvd_btn_copy__" data-action="copy"><span class="__uvd_btn_accent__"></span>Copy</button>' +
                        '<button class="__uvd_act_btn__ __uvd_btn_fav__" data-action="fav"><span class="__uvd_btn_accent__"></span>' + (fav ? '⭐ Fav' : '☆ Fav') + '</button>';
            
            if (category === 'photo') {
                menuHtml += 
                        '<button class="__uvd_act_btn__ __uvd_btn_open__" data-action="open_photo"><span class="__uvd_btn_accent__"></span>Mở ảnh</button>' +
                        '<button class="__uvd_act_btn__ __uvd_btn_download__" data-action="download_photo"><span class="__uvd_btn_accent__"></span>Tải xuống</button>';
            } else if (type === 'IFRAME') {
                menuHtml += 
                        '<a href="' + url + '" target="_blank" class="__uvd_act_btn__ __uvd_btn_iframe__" style="text-decoration:none;text-align:center;grid-column:1/-1;"><span class="__uvd_btn_accent__"></span>Mở iframe</a>';
            } else {
                if (type === 'M3U8' || type === 'MP4' || type === 'WEBM') {
                    menuHtml += '<button class="__uvd_act_btn__ __uvd_btn_preview__" data-action="preview"><span class="__uvd_btn_accent__"></span>Preview</button>';
                }
                if (type === 'M3U8') {
                    menuHtml += '<button class="__uvd_act_btn__ __uvd_btn_quality__" data-action="quality"><span class="__uvd_btn_accent__"></span>Quality</button>';
                }
                menuHtml += '<button class="__uvd_act_btn__ __uvd_btn_cmd__" data-action="cmd" style="grid-column:1/-1;"><span class="__uvd_btn_accent__"></span>Tất cả lệnh tải</button>';
            }
            menuHtml += '</div></div>';
            
            itemDiv.innerHTML = previewHtml + menuHtml;
        ...
        container.addEventListener('click', function(e) {
            // Toggle dropdown on item click (but not on button clicks inside)
            var item = e.target.closest('.__uvd_list_item__');
            if (item && !e.target.closest('.__uvd_act_btn__') && !e.target.closest('a')) {
                var isOpen = item.classList.toggle('open');
                // Close other items
                if (isOpen) {
                    var siblings = item.parentElement.querySelectorAll('.__uvd_list_item__.open');
            ...
            if (btn) {
                e.stopPropagation();
                var item2 = btn.closest('.__uvd_list_item__');
                var url = item2 ? item2.dataset.url : '';
                var action = btn.dataset.action;
                var type = btn.closest('.__uvd_list_item__') ? 
                    btn.closest('.__uvd_list_item__').querySelector('.__uvd_list_badge__').innerText : '';
                ...
                if (action === 'share') shareUrl(url);
                    ...
                    btn.innerHTML = '<span class="__uvd_btn_accent__"></span>' + (isFav ? '⭐ Fav' : '☆ Fav');
                    // Update fav badge count
                    var favTab = document.querySelector('.__uvd_tab__[data-tab="favorites"] .__uvd_badge__');
                    if (favTab) favTab.innerText = data.favorites.length;
                    toast(isFav ? 'Đã thêm Favorite' : 'Đã xóa Favorite', 'info');
                }
            }
        });
    }
    
    // === KEEP ALL OTHER FUNCTIONS (showPreview, showQualityPicker, showCommandPicker, showEditor, etc) ===
    // They remain unchanged from the original for full functionality
    
    // Show preview, quality picker, command picker, editor - copy from original
    // (I'll keep them as-is to save space, but they are identical to the original implementation)
        ...
        parseM3U8Master(url, function(qualities) {
            if (!qualities) {
                overlay.querySelector('.__uvd_overlay_box__').innerHTML = 
                    '<div class="__uvd_overlay_title__"><span class="__uvd_accent_inline__"></span>Không phải Master Playlist</div>' +
                ...
                html += 
                    '<div class="__uvd_quality_card__">' +
                        ...
                        '<div class="__uvd_actions__">' +
                            '<button class="__uvd_act_btn__ __uvd_btn_preview__" data-url="' + encodeURIComponent(q.url) + '" data-action="preview_q"><span class="__uvd_btn_accent__"></span>Preview</button>' +
                            '<button class="__uvd_act_btn__ __uvd_btn_copy__" data-url="' + encodeURIComponent(q.url) + '" data-action="copy_q"><span class="__uvd_btn_accent__"></span>Copy</button>' +
                            '<button class="__uvd_act_btn__ __uvd_btn_share__" data-url="' + encodeURIComponent(q.url) + '" data-action="share_q"><span class="__uvd_btn_accent__"></span>YTDLnis</button>' +
                            '<button class="__uvd_act_btn__ __uvd_btn_cmd__" data-url="' + encodeURIComponent(q.url) + '" data-action="cmd_q"><span class="__uvd_btn_accent__"></span>Lệnh</button>' +
                        '</div>' +
                    '</div>';
            });
            ...
            overlay.addEventListener('click', function(e) {
                ...
                if (btn) {
                    ...
                    var action = btn.dataset.action;
                    if (action === 'preview_q') { overlay.remove(); showPreview(qUrl, 'M3U8'); }
                    else if (action === 'copy_q') { copy(qUrl); toast('Đã copy', 'success'); }
                    else if (action === 'share_q') shareUrl(qUrl);
                    else if (action === 'cmd_q') { overlay.remove(); showCommandPicker(qUrl, 'M3U8'); }
                }
            });
            overlay.querySelector('#__uvd_qp_close__').onclick = function() { overlay.remove(); };
        });
    ...
    function renderFavorites(container) {
        ...
        data.favorites.forEach(function(fav, i) {
            ...
            card.innerHTML = 
                ...
                '<div class="__uvd_actions__">' +
                    '<button class="__uvd_act_btn__ __uvd_btn_share__" data-url="' + encodeURIComponent(fav.url) + '" data-action="share"><span class="__uvd_btn_accent__"></span>Share</button>' +
    ...
    function renderSettings(container) {
        ...
        html += '<div class="__uvd_settings_section__"><div class="__uvd_settings_title__"><span class="__uvd_accent_inline__"></span>Thông tin</div>' +
            '<div style="color:' + t.text2 + ';font-size:10px;line-height:1.7;">' +
            '<div><span class="__uvd_accent_inline_small__"></span>Version: 3.8 — Clean + Dropdown</div>' +
    ...
    toast('V3.8 Ready! (Dropdown menu + Clean)', 'success');
})();