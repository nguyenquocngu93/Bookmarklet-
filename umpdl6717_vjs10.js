/**
 * Universal Media Player & Downloader - V6.7.17 PRO
 * - THỬ NGHIỆM (cần bạn test lại, mình không chạy được Video.js v10 thật để
 *   kiểm chứng trực tiếp): <video-player> trước bị ép height:100% = cao hết
 *   khung chứa (rất cao vì video ngang trong màn hình dọc) → video tự
 *   letterbox NGAY BÊN TRONG chính nó, thanh điều khiển overlay (nếu có) sẽ
 *   bám theo mép khối to đó, tạo khoảng đen thừa giữa video thật và thanh điều
 *   khiển — đúng như ảnh bạn chụp. Đổi sang dùng `aspect-ratio` (mặc định
 *   16:9, tự cập nhật đúng tỉ lệ thật khi biết videoWidth/videoHeight qua
 *   loadedmetadata) + max-height thay vì ép height:100%, để component tự co
 *   khít theo khung hình thật — giống "chế độ fluid" mà tài liệu Video.js mô
 *   tả — thay vì phải tự đoán mò layout Shadow DOM nội bộ của thư viện.
 * - (6.7.16) Header của CHÍNH script tự nổi đè vài px lên đỉnh video khi video
 *   ngang, giãn bình thường khi video đứng (không đụng CSS/skin Video.js).
 * - (6.7.15) Fix nghiêm trọng: z-index vượt giới hạn 32-bit khiến panel chồng
 *   lên player; rescan nền dừng khi đang xem; panel tự ẩn khi player mở.
...
function __uvdMountVjs10(wrapper, video) {
  ...
  function wrapWithSkin() {
    ...
    try {
      ...
      player.style.cssText = 'width:100%;max-height:100%;display:block;aspect-ratio:16/9;margin:auto;';
