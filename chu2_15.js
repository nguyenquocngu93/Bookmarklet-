/**
 * Universal Media Player & Downloader - V6.7.16 PRO (mod)
 * - Thay tab "Trình phát" bằng "Nút đã click"
 * - Lưu selector các nút đã click tự động, cho phép chặn/bỏ chặn
 * - Tác giả: nguyenquocngu91
 */
(function() {
'use strict';

// ========== VERSION ==========
var VERSION = '6.7.16-mod';
...
data.clickedSelectors = data.clickedSelectors || [];   // danh sách selector đã click
data.blockedSelectors = data.blockedSelectors || [];   // selector bị chặn
...
function autoClickServersAndScan() {
  ...
  function clickNext() {
    ...
    // Bỏ qua nếu selector đã bị chặn
    if (data.blockedSelectors.indexOf(selector) !== -1) {
      console.log('[UMP DL] Bỏ qua nút đã chặn:', selector);
      setTimeout(clickNext, 100);
      return;
    }
    
    // Lưu selector vào danh sách đã click (nếu chưa có)
...
function renderClickedSelectors(container) {
  ...
  list.forEach(function(sel) {
    ...
    html += '<div style="display:flex;align-items:center;gap:8px;padding:6px 0;border-bottom:1px solid var(--border);">' +
      '<button class="uvd-toggle-switch' + (isBlocked ? ' uvd-toggle-on' : '') + '" data-selector="' + encodeURIComponent(sel) + '" style="flex-shrink:0;"><span class="uvd-toggle-knob"></span></button>' +
  ...
  container.querySelectorAll('.uvd-toggle-switch').forEach(function(btn) {
    btn.onclick = function() {
      ...
      if (isOn) {
        if (data.blockedSelectors.indexOf(sel) === -1) {
          data.blockedSelectors.push(sel);
        }
      } else {
      ...
      storage.set(data);
      toast(isOn ? 'Đã chặn nút này' : 'Đã bỏ chặn');
    };
  });
}
...
// ========== START ==========
buildUI();
console.log('V' + VERSION + ' UMP DL PRO (mod) - Đã thêm tab Nút đã click');
