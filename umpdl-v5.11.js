/**
 * Universal Media Player & Downloader - V5.9 (Ad-block & Portrait Fix)
...
data.filterlist = data.filterlist || []; // Mảng pattern lọc quảng cáo
  ...
  patterns.forEach(function(p) {
    ...
    if (matches) {
      matches.forEach(function(u) {
        ...
        // Kiểm tra quảng cáo trước khi thêm
        if (isAdUrl(u)) {
          __uvdAdBlockedCount++;
          return; // bỏ qua URL quảng cáo
        }
...
// ========== OVERLAY PLAYER WITH TOOLBAR (FIX PORTRAIT) ==========
function showVideoPlayer(url, type) {
  ...
  // FIX PORTRAIT: dùng max-width/height thay vì width/height 100%
...
console.log('V5.9 UMP DL - Persistent Popup Block & Ad Filter Ready');
toast('V5.9 sẵn sàng — đã bật chặn popup + lọc quảng cáo!');
