  ...
  return (
    '<div class="uvd-card" data-type="' + escapeHtml(item.type) + '" data-url="' + escapeHtml(item.url) + '">' +
      '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">' +
        ...
        '<button class="uvd-block-btn" data-url="' + encodeURIComponent(item.url) + '" style="background:none;border:none;font-size:16px;cursor:pointer;color:var(--text);opacity:0.5;" title="Chặn link này">⛔</button>' +
      '</div>' +
      '<div class="uvd-url-box">' + escapeHtml(item.url) + '</div>' +
      '<div class="' + (item.type === 'M3U8' ? 'uvd-grid-2' : '') + '" style="margin-top:8px;">' +
        (item.type === 'IFRAME' ?
          '<button class="uvd-btn uvd-btn-sm" data-action="iframe" data-url="' + encodeURIComponent(item.url) + '" style="width:100%;background:rgba(109,140,255,0.25);">Mở iframe</button>' :
          '<button class="uvd-btn uvd-btn-sm" data-action="play" data-url="' + encodeURIComponent(item.url) + '" data-type="' + escapeHtml(item.type) + '" style="width:100%;background:rgba(109,140,255,0.25);">▶ Xem</button>' +
          (item.type === 'M3U8' ? '<button class="uvd-btn uvd-btn-sm" data-action="quality" data-url="' + encodeURIComponent(item.url) + '">Chất lượng</button>' : '')
        ) +
      '</div>' +
    '</div>'
  );
