 ...
 * - Trả lại giao diện/UI trình phát video (video wrapper, footer trạng thái +
 *   dung lượng, video.js v10 skin, khung preview thu nhỏ) ĐÚNG NHƯ bản gốc —
 *   đã bỏ hết các thử nghiệm nền gradient/ambient glow/thanh tua tự vẽ/bo góc.
 * - GIỮ LẠI: nút "Xem" hoạt động bình thường, và fix quan trọng cho link BLOB —
 *   script tự tìm lại đúng thẻ <video> gốc trên trang (kể cả trong iframe cùng
 *   gốc) đang giữ blob: MediaSource đó để "mượn" phát lại, thay vì tạo video
 *   mới (sẽ không load được). Đóng player sẽ trả video về đúng vị trí DOM cũ,
 *   không phá luồng phát của trang gốc.
 * - GIỮ LẠI: tính năng "Tải Blob" cho các link blob: là Blob tĩnh thật.
 * - FIX: header panel giật khi cuộn trên Chrome Android (không liên quan trình
 *   phát video — ép GPU layer cố định cho panel ngay khi tạo).
 * - FIX: khôi phục backdrop-filter khi thu nhỏ panel.
