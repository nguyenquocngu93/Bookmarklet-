/**
 * Universal Media Player & Downloader - V8.2 (Video.js)
 * - Sửa giao diện control bar thành cục tròn 2 hàng
 * - Bắt buộc hiển thị nút chất lượng
 * - Thêm seek buttons, screenshot
 * - Giữ hiệu ứng kính mờ
 */
(function() {
  'use strict';
  ...
  // ========== CSS (glass effect + 2-row bubble control bar) ==========
...
/* ===== GLASS EFFECT CHO VIDEO.JS CONTROL BAR - Dạng cục tròn 2 hàng ===== */
...
/* Hàng trên: progress bar */
#__uvd_player_overlay__ .video-js .vjs-progress-control {
  width: 100% !important;
  order: -1 !important;
  margin: 0 !important;
  padding: 0 !important;
...
/* Hàng dưới: các nút - sắp xếp thành hàng ngang */
#__uvd_player_overlay__ .video-js .vjs-control-bar > *:not(.vjs-progress-control) {
...
/* Hàng dưới: bọc các nút vào 1 container row */
#__uvd_player_overlay__ .video-js .vjs-control-bar {
  flex-wrap: nowrap !important;
}

/* Các nút trong hàng dưới - căn giữa và giãn đều */
#__uvd_player_overlay__ .video-js .vjs-control-bar .vjs-button,
#__uvd_player_overlay__ .video-js .vjs-control-bar .vjs-control {
  color: #fff !important;
...
#__uvd_player_overlay__ .video-js .vjs-control-bar .vjs-button:hover,
#__uvd_player_overlay__ .video-js .vjs-control-bar .vjs-control:hover {
  background: rgba(255,255,255,0.1) !important;
}

/* Các thành phần hiển thị thời gian */
#__uvd_player_overlay__ .video-js .vjs-current-time-display,
#__uvd_player_overlay__ .video-js .vjs-duration-display,
#__uvd_player_overlay__ .video-js .vjs-time-divider {
...
#__uvd_player_overlay__ .video-js .vjs-remaining-time-display,
#__uvd_player_overlay__ .video-js .vjs-live-display {
  display: none !important;
}

/* Nút play */
#__uvd_player_overlay__ .video-js .vjs-play-control {
  font-size: 18px !important;
}

/* Nút tua */
#__uvd_player_overlay__ .video-js .vjs-seek-button {
  font-size: 16px !important;
}

/* Nút chất lượng - hiển thị luôn */
#__uvd_player_overlay__ .video-js .vjs-quality-menu-button {
  font-size: 13px !important;
  background: rgba(255,255,255,0.08) !important;
  border-radius: 12px !important;
  padding: 2px 10px !important;
}
#__uvd_player_overlay__ .video-js .vjs-quality-menu-button .vjs-icon-placeholder {
  font-size: 13px !important;
}

/* Nút tốc độ */
#__uvd_player_overlay__ .video-js .vjs-playback-rate .vjs-playback-rate-value {
  font-size: 13px !important;
  font-weight: 600 !important;
}

/* Nút screenshot */
#__uvd_player_overlay__ .video-js .vjs-screenshot-button {
  font-size: 16px !important;
}

/* Nút fullscreen */
#__uvd_player_overlay__ .video-js .vjs-fullscreen-control {
  font-size: 16px !important;
}

/* Big play button ở giữa */
...
#__uvd_player_overlay__ .video-js .vjs-progress-holder .vjs-load-progress {
  background: rgba(255,255,255,0.2) !important;
}
#__uvd_player_overlay__ .video-js .vjs-slider-handle {
  border-color: #6d8cff !important;
}

/* Điều chỉnh kích thước các nút để vừa 1 hàng */
#__uvd_player_overlay__ .video-js .vjs-control-bar .vjs-control {
  width: 2.8em !important;
}
#__uvd_player_overlay__ .video-js .vjs-control-bar .vjs-play-control {
  width: 3em !important;
}
#__uvd_player_overlay__ .video-js .vjs-control-bar .vjs-seek-button {
  width: 2.6em !important;
}
#__uvd_player_overlay__ .video-js .vjs-control-bar .vjs-quality-menu-button {
  width: auto !important;
  padding: 0 8px !important;
}
#__uvd_player_overlay__ .video-js .vjs-control-bar .vjs-playback-rate {
  width: 2.6em !important;
}
#__uvd_player_overlay__ .video-js .vjs-control-bar .vjs-screenshot-button {
  width: 2.6em !important;
}
#__uvd_player_overlay__ .video-js .vjs-control-bar .vjs-fullscreen-control {
  width: 2.6em !important;
}
#__uvd_player_overlay__ .video-js .vjs-control-bar .vjs-volume-panel {
  width: 2.6em !important;
}
`;
  ...
  var __videoJsLoaded = false;
  var __videoJsLoading = false;
  var __videoJsCallbacks = [];

  function loadVideoJsAndPlugins(callback) {
    if (__videoJsLoaded && typeof videojs !== 'undefined') {
      callback();
      return;
    }
    __videoJsCallbacks.push(callback);
    if (__videoJsLoading) return;
    __videoJsLoading = true;
    ...
    function onLoad() {
      ...
      if (loaded >= scripts.length) {
        __videoJsLoaded = true;
        __videoJsLoading = false;
        registerScreenshotPlugin();
        // Đảm bảo quality menu được đăng ký
        if (typeof videojs !== 'undefined' && !videojs.getPlugin('qualityMenu')) {
          console.warn('qualityMenu plugin chưa được đăng ký, thử tự đăng ký');
          // Một số CDN có thể không tự đăng ký, cần gọi thủ công
          try {
            if (videojs.contrib && videojs.contrib.qualityMenu) {
              videojs.registerPlugin('qualityMenu', videojs.contrib.qualityMenu);
            }
          } catch(e) {}
        }
        var cbs = __videoJsCallbacks.slice();
        __videoJsCallbacks = [];
    ...
    scripts.forEach(function(src) {
      ...
      s.onload = onLoad;
      s.onerror = function() {
        console.warn('Lỗi tải script:', src);
        onLoad();
      };
  ...
  function showVideoPlayer(url, type) {
    ...
    wrapper.style.cssText = 'flex:1;display:flex;align-items:center;justify-content:center;background:#000;position:relative;height:100%;min-height:0;';
    ...
    function initPlayer() {
      if (typeof videojs === 'undefined') {
        toast('Video.js chưa sẵn sàng, thử lại');
        console.error('videojs undefined');
        return;
      }
      ...
      try {
        ...
        player.on('error', function(e) {
          ...
          toast('Lỗi phát video: ' + (player.error().message || 'không rõ'));
        });
        ...
        console.error('Lỗi khởi tạo Video.js:', err);
        toast('Không thể khởi tạo player: ' + err.message);
      }
    }
    ...
    if (typeof videojs !== 'undefined') {
      ...
      loadVideoJsAndPlugins(initPlayer);
    }
  }
  ...
  function closePlayer() {
    if (playerState.player) {
      try { playerState.player.dispose(); } catch(e) {}
