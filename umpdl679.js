...
function __uvdMountVjs10(wrapper, video) {
  ...
  function wrapWithSkin() {
    ...
    try {
      ...
      skin.style.cssText = 'width:100%;height:100%;display:block;--media-color-primary:var(--accent);--media-border-radius:16px;';
      if (video.parentNode) video.parentNode.removeChild(video);
      skin.appendChild(video);
      player.appendChild(skin);
      wrapper.appendChild(player);
      // QUAN TRỌNG: v10 (beta) bị vỡ UI nặng khi CONTAINER có tỉ lệ DỌC (hẹp-cao)
      // — control surface render sai kích thước. Bug nằm trong Shadow DOM của
      // skin, không sửa trực tiếp được. Cách né: ép container luôn có tỉ lệ
      // NGANG chuẩn (16:9) bất kể video dọc hay ngang — video dọc sẽ hiện giữa
      // có viền đen 2 bên (pillarbox) qua object-fit:contain có sẵn, còn control
      // bar thì luôn nằm trong 1 container hình dạng "bình thường" nên v10 tính
      // đúng, y hệt cách nó chạy tốt với video ngang.
      function checkOrientation() {
        ...
        if (video.videoWidth && video.videoHeight && video.videoHeight > video.videoWidth) {
          console.warn('[UMP DL] Video dọc (' + video.videoWidth + 'x' + video.videoHeight + ') — ép container 16:9 để tránh bug layout v10');
          player.style.aspectRatio = '16 / 9';
          player.style.height = 'auto';
          player.style.maxHeight = '100%';
          skin.style.aspectRatio = '16 / 9';
          skin.style.height = 'auto';
          skin.style.maxHeight = '100%';
        }
      }
