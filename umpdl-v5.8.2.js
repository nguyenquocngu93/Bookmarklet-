/**
 * Universal Media Player & Downloader - V5.8.2
 * - Giao diện gọn, bỏ icon thừa
 * - Mở iframe trong sandbox, không mất script
 * - Chặn popup cực mạnh (mixdrop, ok.ru...)
 * - Tối ưu thu nhỏ, ít lag
 * - Đồng bộ JSONBin an toàn
...
data.jsonbin = data.jsonbin || { apiKey: '', binId: '', autoSync: false };
...
function blockAllPopups() {
  ...
  window.open = function(url, name, specs) {
    console.log('[UMP] Chặn popup:', url);
    return null;
  };

  // Chặn click vào link target _blank
  document.addEventListener('click', function(e) {
    var target = e.target.closest('a');
    if (target && target.target && target.target !== '_self' && target.target !== '_top' && target.target !== '_parent') {
      e.preventDefault();
      e.stopPropagation();
      console.log('[UMP] Chặn click link popup:', target.href);
      return false;
    }
  }, true);

  // Bắt các sự kiện mở cửa sổ mới từ script
  ...
  document.createElement = function(tag) {
    ...
    if (tag === 'a') {
      ...
      el.setAttribute = function(attr, val) {
        if (attr === 'target' && (val === '_blank' || val !== '_self' && val !== '_top' && val !== '_parent')) {
          console.log('[UMP] Chặn setAttribute target popup');
          return;
        }
        return originalSetAttribute.call(el, attr, val);
      };
      Object.defineProperty(el, 'target', {
        set: function(val) {
          if (val !== '_self' && val !== '_top' && val !== '_parent') {
            console.log('[UMP] Chặn target popup');
            return;
          }
        },
  ...
  // MutationObserver để phát hiện iframe/script cố tình mở popup
  var observer = new MutationObserver(function(mutations) {
    mutations.forEach(function(mut) {
      if (mut.type === 'childList') {
        mut.addedNodes.forEach(function(node) {
          if (node.tagName === 'IFRAME') {
            // Chặn iframe cố tình mở popup
            try {
              var doc = node.contentDocument || node.contentWindow.document;
              if (doc) {
                doc.addEventListener('click', function(e) {
                  var t = e.target.closest('a');
                  if (t && t.target && t.target !== '_self') {
                    e.preventDefault();
                    e.stopPropagation();
                    console.log('[UMP] Chặn popup trong iframe');
                  }
                }, true);
              }
            } catch(e) {}
...
// ========== POPUP / NEW-TAB BLOCKER (dành cho auto-click) ==========
function blockPopupsDuring(durationMs, fn) {
  ...
  document.addEventListener('click', killBlankLinks, true);

  try { fn(); }
  finally {
    setTimeout(function() {
      window.open = origOpen;
      document.removeEventListener('click', killBlankLinks, true);
    }, durationMs);
  }
}
...
function runAutoClickAndRescan(silent) {
  ...
  setTimeout(function() {
    ...
    if (found > 0) {
      toast('Tu dong Play: tim them ' + found + ' luong moi');
      if (document.getElementById('__uvd__')) buildUI();
      setTimeout(function() {
        var n = pauseAllPlayingVideos();
        if (n > 0) toast('Tam dung video goc, xem qua player script');
      }, 800);
    } else if (!silent) {
      toast(clicked > 0 ? 'Da bam Play nhung chua thay link moi' : 'Khong tim thay nut Play');
    }
  }, 1200);
}
...
function makeCommands(url, type, title) {
  ...
  return {
    'yt-dlp': { label: 'yt-dlp (co ban)', cmd: 'yt-dlp --referer "' + ref + '" -o "' + t + '.%(ext)s" "' + url + '"' },
...
function shareUrl(url) {
  if (navigator.share) navigator.share({ title: pageInfo.title, url: url }).catch(function() { copy(url); toast('Da sao chep'); });
  else { copy(url); toast('Da sao chep'); }
}
...
function toggleFavorite(url, type) {
  ...
  if (idx >= 0) { data.favorites.splice(idx, 1); toast('Da xoa khoi yeu thich'); }
  else { data.favorites.unshift({ url, type, title: pageInfo.title, host: pageInfo.host, timestamp: Date.now() }); toast('Da them vao yeu thich'); }
  storage.set(data);
...
function exportData(format) {
  ...
  toast('Da xuat ' + format.toUpperCase());
}
...
// ========== MỞ IFRAME TRONG SANDBOX (không mất script) ==========
function openIframeInSandbox(url) {
  var overlay = document.createElement('div');
  overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:2147483647;display:flex;flex-direction:column;padding:20px;';
  
  var header = document.createElement('div');
  header.style.cssText = 'display:flex;justify-content:space-between;align-items:center;padding:10px;color:#fff;background:rgba(0,0,0,0.5);border-radius:8px 8px 0 0;';
  header.innerHTML = '<span style="font-weight:bold;">Sandbox Iframe</span><button id="__uvd_iframe_close__" style="background:rgba(255,0,0,0.3);border:none;color:#fff;padding:6px 12px;border-radius:6px;cursor:pointer;">Dong</button>';
  overlay.appendChild(header);
  
  var iframe = document.createElement('iframe');
  iframe.sandbox = 'allow-scripts allow-same-origin allow-forms allow-modals allow-popups-to-escape-sandbox';
  iframe.src = url;
  iframe.style.cssText = 'flex:1;width:100%;border:none;background:#fff;border-radius:0 0 8px 8px;';
  overlay.appendChild(iframe);
  
  document.body.appendChild(overlay);
  
  document.getElementById('__uvd_iframe_close__').onclick = function() {
    overlay.remove();
  };
  
  // Chặn popup trong iframe này
  try {
...
function showVideoPlayer(url, type) {
  ...
  overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:2147483646;display:flex;flex-direction:column;animation:uvdFadeIn 0.3s ease;';
  document.body.appendChild(overlay);
  ...
  minVideoBtn.textContent = 'Thu nho';
  ...
  if (type === 'MP4' || type === 'MKV' || type === 'WEBM') {
    ...
    downloadBtn.textContent = 'Tai xuat';
  ...
  footer.innerHTML = '<span id="__uvd_player_status__">Dang tai...</span><span id="__uvd_player_time__"></span>';
  ...
  function showQualitySubMenu() {
    ...
    panel.innerHTML = '<div style="color:#fff;font-weight:600;margin-bottom:12px;">Chon chat luong</div>';
  ...
  function showSpeedSubMenu() {
    ...
    var panel = document.createElement('div');
    panel.style.cssText = 'background:rgba(20,22,30,0.95);border-radius:16px;padding:20px;min-width:250px;max-width:90%;border:1px solid rgba(255,255,255,0.15);';
    panel.innerHTML = '<div style="color:#fff;font-weight:600;margin-bottom:12px;">Chon toc do</div>';
  ...
  function onMetadataLoaded() {
    ...
    document.getElementById('__uvd_player_status__').textContent = 'Dang phat';
    ...
    if (isHls && playerState.qualities.length === 0) {
      parseM3U8Master(url, function(qualities) {
        if (qualities && qualities.length > 0) playerState.qualities = qualities;
      });
    }
  }
...
function minimizePlayer() {
  ...
  mini.style.cssText = 'position:fixed;bottom:20px;right:20px;width:160px;height:90px;background:#000;border-radius:12px;z-index:2147483645;cursor:pointer;box-shadow:0 8px 30px rgba(0,0,0,0.8);border:2px solid rgba(255,255,255,0.2);overflow:hidden;transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1);';
...
function restorePlayer() {
  ...
  if (mini) {
    ...
    setTimeout(function() { mini.remove(); }, 250);
...
// ========== JSONBIN SYNC ==========
var JSONBIN_API_URL = 'https://api.jsonbin.io/v3/b/';

function syncToJsonBin(apiKey, binId, onSuccess, onError) {
  apiKey = apiKey.trim();
  binId = binId.trim();
  if (!apiKey || !binId) {
    toast('Thieu API Key hoac Bin ID', '#ff5d72');
    return;
  }
  ...
  .then(function(res) {
    if (!res.ok) {
      if (res.status === 401) throw new Error('Sai API Key hoac Bin ID (401)');
  ...
  .then(function(json) {
    ...
    var histCount = data.history.length;
    toast('Da tai len JSONBin! (' + favCount + ' yeu thich, ' + histCount + ' lich su)', '#34c759');
    if (onSuccess) onSuccess(json);
  })
  .catch(function(err) {
    toast('Loi tai len: ' + err.message, '#ff5d72');
    if (onError) onError(err);
  });
}

function syncFromJsonBin(apiKey, binId, onSuccess, onError) {
  apiKey = apiKey.trim();
  binId = binId.trim();
  if (!apiKey || !binId) {
    toast('Thieu API Key hoac Bin ID', '#ff5d72');
  ...
  if (!confirm('Ban co muon tai du lieu tu JSONBin ve? Du lieu local se bi ghi de!')) {
    return;
  }
  fetch(JSONBIN_API_URL + binId + '/latest', {
    method: 'GET',
    headers: {
      'X-Master-Key': apiKey,
      'Content-Type': 'application/json'
  ...
  .then(function(json) {
    var remote = json.record;
    if (!remote) throw new Error('Du lieu rong');
    var hasData = (remote.favorites && remote.favorites.length > 0) || (remote.history && remote.history.length > 0);
    if (!hasData) {
      toast('Cloud chua co du lieu, giu nguyen local', '#ffb84d');
      return;
    }
    data.favorites = remote.favorites || [];
    data.siteProfiles = remote.siteProfiles || {};
    data.history = remote.history || [];
    if (remote.jsonbin) {
      data.jsonbin = remote.jsonbin;
    }
    storage.set(data);
    toast('Da tai du lieu tu JSONBin! (' + data.favorites.length + ' yeu thich, ' + data.history.length + ' lich su)', '#34c759');
    if (onSuccess) onSuccess(json);
    buildUI();
  })
  .catch(function(err) {
    toast('Loi tai xuong: ' + err.message, '#ff5d72');
    if (onError) onError(err);
  });
}
...
function autoSyncIfEnabled() {
  var jb = data.jsonbin;
  if (jb && jb.autoSync && jb.apiKey && jb.binId) {
    toast('Dang dong bo du lieu tu JSONBin...', '#6d8cff');
    syncFromJsonBin(jb.apiKey, jb.binId, function() {
      toast('Dong bo thanh cong!', '#34c759');
    }, function() {
      toast('Khong the dong bo, dung local', '#ffb84d');
    });
  }
}
...
.uvd-overlay {
  ...
  backdrop-filter:blur(18px) saturate(120%); z-index:2147483647;
...
#__uvd_min_float__ {
  ...
  z-index:2147483645; cursor:pointer; display:flex; align-items:center;
...
function buildUI() {
  ...
  panel.style.cssText = 'position:fixed;top:15px;left:15px;right:15px;bottom:15px;z-index:2147483645;animation:uvdScaleIn 0.4s ease;';
  ...
  header.innerHTML = 
    '<div style="display:flex;align-items:center;gap:8px;">' +
      ...
      '<span style="font-weight:700;font-size:16px;letter-spacing:-0.01em;">UMP DL V' + VERSION + '</span>' +
    '</div>' +
    '<div style="display:flex;gap:6px;">' +
      '<button class="uvd-btn-icon uvd-ripple-btn" id="__uvd_autoplay__" title="Tu dong bam Play">Play</button>' +
      '<button class="uvd-btn-icon uvd-ripple-btn" id="__uvd_minimize__" title="Thu nho">Thu nho</button>' +
      '<button class="uvd-btn-icon uvd-ripple-btn" id="__uvd_refresh__" title="Lam moi">Lam moi</button>' +
      '<button class="uvd-btn-icon uvd-ripple-btn" id="__uvd_close__" title="Dong">Dong</button>' +
    '</div>';
  ...
  var tabList = [
    ...
    { id: 'favorites', text: 'Yeu thich (' + data.favorites.length + ')' },
    { id: 'history', text: 'Lich su (' + (data.history||[]).length + ')' },
    { id: 'settings', text: 'Cai dat' }
  ];
  ...
  info.innerHTML = 
    ...
    '<span id="__uvd_playsel__" style="color:var(--accent2);font-family:monospace;text-decoration:underline;cursor:pointer;font-size:11px;">' + (savedPlaySel || '(chua dat · bam de them)') + '</span>';
  ...
  document.getElementById('__uvd_refresh__').onclick = function() { buildUI(); toast('Da lam moi'); };
  document.getElementById('__uvd_autoplay__').onclick = function() {
    toast('Dang thu bam Play tren trang...');
    window.__uvd_autoClickPlay();
  };
  
  document.getElementById('__uvd_title__').onclick = function() {
    var newTitle = prompt('Ten file:', pageInfo.title);
  ...
  document.getElementById('__uvd_referer__').onclick = function() {
    ...
    if (newRef) {
      ...
      storage.set(data);
      toast('Da luu referer cho ' + pageInfo.host);
    }
  };
  ...
  document.getElementById('__uvd_playsel__').onclick = function() {
    ...
    var newSel = prompt('CSS selector cua nut Play (vd: .video-play-button):', current);
    if (newSel !== null) {
      newSel = newSel.trim();
      data.siteProfiles[pageInfo.host] = Object.assign({}, data.siteProfiles[pageInfo.host], { playSelector: newSel });
      storage.set(data);
      this.textContent = newSel || '(chua dat · bam de them)';
      if (newSel) {
        toast('Da luu selector cho ' + pageInfo.host);
        setTimeout(function() { buildUI(); }, 1000);
      } else {
        toast('Da xoa selector rieng');
      }
    }
  };
  
  // Nút thu nhỏ panel chính
...
function renderStreams(container, arr) {
  if (!arr.length) {
    container.innerHTML = '<div style="text-align:center;padding:40px;color:var(--text2);">Khong phat hien stream nao.</div>';
    return;
  }
  ...
  arr.forEach(function(item, i) {
    ...
    card.innerHTML = 
      '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">' +
        ...
        '<button class="uvd-fav-btn uvd-ripple-btn" data-url="' + encodeURIComponent(item.url) + '" data-type="' + item.type + '" style="background:none;border:1px solid var(--border);border-radius:4px;padding:4px 8px;font-size:12px;cursor:pointer;color:var(--text2);">' + (fav ? 'Yeu thich' : 'Them yeu thich') + '</button>' +
      '</div>' +
      '<div class="uvd-url-box">' + item.url + '</div>' +
      '<div class="uvd-grid-2" style="margin-top:8px;">' +
        '<button class="uvd-btn uvd-btn-sm uvd-ripple-btn" data-action="share" data-url="' + encodeURIComponent(item.url) + '" style="background:rgba(139,92,246,0.2);">Chia se</button>' +
        '<button class="uvd-btn uvd-btn-sm uvd-ripple-btn" data-action="copy" data-url="' + encodeURIComponent(item.url) + '">Sao chep</button>' +
        (item.type === 'IFRAME' ? 
          '<button class="uvd-btn uvd-btn-sm uvd-ripple-btn" data-action="iframe" data-url="' + encodeURIComponent(item.url) + '" style="grid-column:1/3;background:rgba(255,165,0,0.2);">Mo iframe (sandbox)</button>' :
          (item.type === 'M3U8' ?
            '<button class="uvd-btn uvd-btn-sm uvd-ripple-btn" data-action="quality" data-url="' + encodeURIComponent(item.url) + '">Chat luong</button>' +
            ...
            '<button class="uvd-btn uvd-btn-sm uvd-ripple-btn" data-action="cmd" data-url="' + encodeURIComponent(item.url) + '" data-type="' + item.type + '" style="grid-column:1/3;">Lenh tai</button>' :
            '<button class="uvd-btn uvd-btn-sm uvd-ripple-btn" data-action="play" data-url="' + encodeURIComponent(item.url) + '" data-type="' + item.type + '" style="background:rgba(109,140,255,0.25);">Xem</button>' +
            '<button class="uvd-btn uvd-btn-sm uvd-ripple-btn" data-action="cmd" data-url="' + encodeURIComponent(item.url) + '" data-type="' + item.type + '">Lenh tai</button>'
          )
        ) +
      '</div>';
  ...
  container.querySelectorAll('.uvd-fav-btn').forEach(function(b) {
    b.onclick = function() {
      ...
      this.textContent = isFav ? 'Yeu thich' : 'Them yeu thich';
    };
  });
  ...
  container.querySelectorAll('.uvd-btn[data-action]').forEach(function(b) {
    b.onclick = function() {
      ...
      else if (action === 'copy') { copy(u); toast('Da sao chep!'); }
      else if (action === 'iframe') openIframeInSandbox(u);
...
function showCommandPicker(url, type) {
  ...
  html += '<div style="font-weight:700;margin-bottom:12px;">Chon lenh tai</div>';
  ...
  Object.keys(cmds).forEach(function(k) {
    ...
    html += '<div class="uvd-card">' +
      ...
      '<button class="uvd-btn uvd-btn-sm cmd-select" data-cmd="' + encodeURIComponent(c.cmd) + '" style="width:100%;">Chinh sua & Copy</button>' +
    '</div>';
  });
  
  html += '</div><button class="uvd-btn uvd-btn-sm close-overlay-btn" style="width:100%;margin-top:10px;background:var(--danger);">Dong</button></div>';
...
function showEditor(text) {
  ...
  overlay.innerHTML = 
    '<div class="uvd-glass-panel" style="max-width:600px;margin:auto;">' +
      '<div style="font-weight:700;margin-bottom:8px;">Chinh sua lenh</div>' +
      ...
      '<div class="uvd-grid-2" style="margin-top:12px;">' +
        '<button class="uvd-btn uvd-btn-sm" id="__uvd_ed_copy__">Sao chep</button>' +
        '<button class="uvd-btn uvd-btn-sm" id="__uvd_ed_share__" style="background:rgba(139,92,246,0.3);">Chia se</button>' +
      '</div>' +
      '<button class="uvd-btn uvd-btn-sm close-editor" style="width:100%;margin-top:8px;background:var(--danger);">Dong</button>' +
    '</div>';
  document.body.appendChild(overlay);
...
function showQualityPicker(url) {
  var overlay = document.createElement('div');
  overlay.className = 'uvd-overlay';
  overlay.innerHTML = '<div class="uvd-glass-panel" style="max-width:600px;margin:auto;">Dang phan tich M3U8...</div>';
  document.body.appendChild(overlay);
  
  parseM3U8Master(url, function(qualities) {
    if (!qualities) {
      overlay.innerHTML = '<div class="uvd-glass-panel" style="max-width:600px;margin:auto;text-align:center;">' +
        '<div style="color:var(--danger);">Khong phai Master Playlist</div>' +
        '<button class="uvd-btn uvd-btn-sm close-overlay-btn" style="margin-top:12px;background:var(--danger);width:100%;">Dong</button></div>';
      overlay.querySelector('.close-overlay-btn').onclick = function() { overlay.remove(); };
      return;
    }
    
    var html = '<div class="uvd-glass-panel" style="max-width:600px;margin:auto;">';
    html += '<div style="font-weight:700;margin-bottom:12px;">Chon chat luong (' + qualities.length + ')</div>';
    html += '<div style="overflow-y:auto;max-height:60vh;">';
    
    qualities.forEach(function(q, idx) {
      getFileSize(q.url, function(size) {
        var sizeStr = size ? formatFileSize(size) : '? MB';
        var card = overlay.querySelector('.q-card[data-idx="' + idx + '"]');
        if (card) {
          var sizeSpan = card.querySelector('.q-size');
          if (sizeSpan) sizeSpan.textContent = sizeStr;
        }
      });
      
      html += '<div class="uvd-card q-card" data-idx="' + idx + '">' +
        '<div style="display:flex;justify-content:space-between;">' +
          '<b>' + q.label + '</b>' +
          '<span style="color:var(--text3);">' + Math.round(q.bandwidth/1000) + 'kbps</span>' +
        '</div>' +
        '<div><span style="color:var(--text2);">Dung luong: </span><span class="q-size" style="color:var(--accent2);">Dang tinh...</span></div>' +
        '<div class="uvd-grid-3" style="margin-top:8px;">' +
          '<button class="uvd-btn uvd-btn-sm q-act" data-url="' + encodeURIComponent(q.url) + '" data-action="share" style="background:rgba(139,92,246,0.2);">Chia se</button>' +
          ...
          '<button class="uvd-btn uvd-btn-sm q-act" data-url="' + encodeURIComponent(q.url) + '" data-action="cmd">Lenh</button>' +
        '</div>' +
      '</div>';
    });
...
function renderFavorites(container) {
  if (!data.favorites.length) {
    container.innerHTML = '<div style="text-align:center;padding:30px;color:var(--text2);">Chua co yeu thich.</div>';
    return;
  }
  ...
  data.favorites.forEach(function(f, i) {
    ...
    card.innerHTML = 
      ...
      '<div class="uvd-grid-3">' +
        '<button class="uvd-btn uvd-btn-sm fav-act" data-url="' + encodeURIComponent(f.url) + '" data-action="share" style="background:rgba(139,92,246,0.2);">Chia se</button>' +
        '<button class="uvd-btn uvd-btn-sm fav-act" data-url="' + encodeURIComponent(f.url) + '" data-action="copy">Copy</button>' +
        '<button class="uvd-btn uvd-btn-sm fav-del" data-idx="' + i + '" style="background:var(--danger);">Xoa</button>' +
      '</div>';
    container.appendChild(card);
  });
...
function renderHistory(container) {
  ...
  if (!hist.length) {
    container.innerHTML = '<div style="text-align:center;padding:30px;color:var(--text2);">Chua co lich su.</div>';
    return;
  }
  
  container.innerHTML = '<button class="uvd-btn uvd-btn-sm" id="__uvd_clear_hist__" style="width:100%;margin-bottom:10px;background:var(--danger);">Xoa toan bo lich su</button>';
  ...
  document.getElementById('__uvd_clear_hist__').onclick = function() {
    if (confirm('Xoa toan bo lich su?')) { data.history = []; storage.set(data); renderHistory(container); }
  };
...
function renderSettings(container) {
  ...
  var jb = data.jsonbin || { apiKey: '', binId: '', autoSync: false };
  ...
  container.innerHTML =
    ...
    '<div class="uvd-profile-stats">' +
      ...
      '<div class="uvd-stat"><div class="uvd-stat-num">' + data.favorites.length + '</div><div class="uvd-stat-label">Yeu thich</div></div>' +
      '<div class="uvd-stat"><div class="uvd-stat-num">' + (data.history||[]).length + '</div><div class="uvd-stat-label">Lich su</div></div>' +
    '</div>' +

    '<div class="uvd-card" style="border-color:rgba(109,140,255,0.5);">' +
      '<div style="font-weight:700;margin-bottom:8px;color:var(--accent);">Dong bo du lieu (JSONBin.io)</div>' +
      '<div style="font-size:12px;color:var(--text2);margin-bottom:8px;">Nhap API Key va Bin ID de dong bo yeu thich & lich su giua cac website.</div>' +
      '<div style="display:grid;gap:8px;">' +
        '<label style="font-size:12px;color:var(--text2);">API Key</label>' +
        '<input id="__uvd_jb_apikey__" type="text" style="width:100%;padding:8px;background:rgba(0,0,0,0.4);border:1px solid var(--border);border-radius:8px;color:#fff;font-size:13px;" placeholder="Paste your API Key" value="' + jb.apiKey + '">' +
        '<label style="font-size:12px;color:var(--text2);">Bin ID</label>' +
        '<input id="__uvd_jb_binid__" type="text" style="width:100%;padding:8px;background:rgba(0,0,0,0.4);border:1px solid var(--border);border-radius:8px;color:#fff;font-size:13px;" placeholder="Paste your Bin ID" value="' + jb.binId + '">' +
        '<div style="display:flex;gap:6px;flex-wrap:wrap;">' +
          '<button class="uvd-btn uvd-btn-sm" id="__uvd_jb_save__" style="background:rgba(109,140,255,0.3);">Luu cau hinh</button>' +
          '<button class="uvd-btn uvd-btn-sm" id="__uvd_jb_push__" style="background:rgba(34,197,94,0.3);">Tai len</button>' +
          '<button class="uvd-btn uvd-btn-sm" id="__uvd_jb_pull__" style="background:rgba(139,92,246,0.3);">Tai xuong</button>' +
        '</div>' +
        '<div style="display:flex;align-items:center;gap:8px;margin-top:4px;">' +
          '<input type="checkbox" id="__uvd_jb_autosync__" ' + (jb.autoSync ? 'checked' : '') + '>' +
          '<label for="__uvd_jb_autosync__" style="font-size:12px;color:var(--text2);">Tu dong dong bo khi mo script (tai xuong)</label>' +
        '</div>' +
      '</div>' +
    '</div>' +

    '<div class="uvd-card">' +
      '<div style="font-weight:600;margin-bottom:8px;">Sao luu cuc bo</div>' +
      '<button class="uvd-btn uvd-btn-sm" id="__uvd_backup__" style="width:100%;margin-bottom:6px;">Xuat du lieu (JSON)</button>' +
      '<button class="uvd-btn uvd-btn-sm" id="__uvd_restore__" style="width:100%;margin-bottom:6px;">Nhap du lieu (JSON)</button>' +
      '<button class="uvd-btn uvd-btn-sm" id="__uvd_reset__" style="width:100%;background:var(--danger);">Dat lai tat ca</button>' +
    '</div>' +

    '<div class="uvd-section-title"><span class="uvd-section-num">1</span> Cai dat Bookmarklet</div>' +
    '<div class="uvd-card uvd-timeline-card">' +
      ...
      '<div style="font-size:12px;color:var(--text3);line-height:1.7;margin-top:10px;">' +
        '<strong style="color:var(--text2);">Cach cai tren Chrome Android:</strong><br>' +
        '➊ Copy toan bo dong code tren (bat dau bang <code>javascript:</code>)<br>' +
        '➋ Mo trang web bat ky, bam vao thanh dia chi (URL)<br>' +
        '➌ Dan code vao va bam Go → trang web se load script<br>' +
        '➍ De luu lai, bam vao bieu tuong ⭐ (Bookmark) → chon "Them trang nay vao bookmark"<br>' +
        '➎ Sau do, moi lan muon dung, chi can bam vao bookmark do tu menu bookmark.<br>' +
        '<span style="color:var(--text2);">💡 Neu khong thay bookmark, vao Settings → Bookmarks.</span>' +
      '</div>' +
      '<div style="font-size:12px;color:var(--accent2);margin-top:8px;">' +
        '<strong>🔁 Force update:</strong> Link da tu them tham so <code>?force=</code> kem thoi gian hien tai nen luon tai ban moi nhat, khong bi cache.' +
      '</div>' +
    '</div>' +

    '<div class="uvd-section-title"><span class="uvd-section-num">2</span> Su dung</div>' +
    '<div class="uvd-card uvd-timeline-card">' +
      '<div style="font-size:13px;color:var(--text2);line-height:1.7;">' +
        '• Mo trang web co video<br>' +
        '• Bam vao bookmark da tao<br>' +
        '• Chon stream va bam <strong style="color:var(--accent);">Xem</strong> de mo player overlay<br>' +
        '• Trong player: thanh cong cu gom <strong style="color:var(--accent);">Chat luong</strong>, <strong style="color:var(--accent);">Toc do</strong>, <strong style="color:var(--accent);">Fullscreen</strong><br>' +
        '• Nut <strong style="color:var(--accent);">Thu nho</strong> canh nut dong de thu nho video<br>' +
        '• Nut <strong style="color:var(--accent);">Thu nho</strong> tren header chinh de thu nho toan bo script<br>' +
        '• Nut <strong style="color:var(--accent);">Play</strong> tren header: tu dong bam giup cac nut Play an (eporner, JAV...) de link stream lo ra. Neu khong an, dat CSS selector rieng o dong "Play selector" ben duoi ten file.' +
      '</div>' +
    '</div>' +

    '<div class="uvd-section-title"><span class="uvd-section-num">3</span> Tai video voi yt-dlp va Termux</div>' +
    '<div class="uvd-card uvd-timeline-card">' +
      '<div style="font-size:13px;color:var(--text2);line-height:1.7;">' +
        '<strong style="color:var(--text);">Cai dat yt-dlp tren Termux:</strong><br>' +
        ...
        '<strong style="color:var(--text);">Sau khi cai dat, copy lenh tu tab "Lenh tai":</strong><br>' +
        '• Mo tab <strong style="color:var(--accent);">Streams</strong>, chon stream can tai<br>' +
        '• Bam <strong style="color:var(--accent);">Lenh tai</strong> → chon lenh phu hop<br>' +
        '• Copy lenh, mo Termux, dan vao va bam Enter de tai.<br><br>' +
        '<strong style="color:var(--text);">Luu y:</strong> Nho cap quyen luu file cho Termux (Android 11+):<br>' +
  ...
  document.getElementById('__uvd_jb_save__').onclick = function() {
    var key = document.getElementById('__uvd_jb_apikey__').value.trim();
    var id = document.getElementById('__uvd_jb_binid__').value.trim();
    var auto = document.getElementById('__uvd_jb_autosync__').checked;
    data.jsonbin = { apiKey: key, binId: id, autoSync: auto };
    storage.set(data);
    toast('Da luu cau hinh JSONBin');
  };

  document.getElementById('__uvd_jb_push__').onclick = function() {
    var key = document.getElementById('__uvd_jb_apikey__').value.trim();
    var id = document.getElementById('__uvd_jb_binid__').value.trim();
    syncToJsonBin(key, id);
  };

  document.getElementById('__uvd_jb_pull__').onclick = function() {
    var key = document.getElementById('__uvd_jb_apikey__').value.trim();
    var id = document.getElementById('__uvd_jb_binid__').value.trim();
    syncFromJsonBin(key, id);
  };
  ...
  document.getElementById('__uvd_restore__').onclick = function() {
    ...
    inp.onchange = function(e) {
      ...
      reader.onload = function(ev) {
        try { data = Object.assign(data, JSON.parse(ev.target.result)); storage.set(data); toast('Da nhap!'); buildUI(); }
        catch(ex) { toast('File khong hop le','var(--danger)'); }
      };
  ...
  document.getElementById('__uvd_reset__').onclick = function() {
    if (confirm('Xoa toan bo du lieu?')) { localStorage.removeItem(STORAGE_KEY); data = {favorites:[],siteProfiles:{},history:[],jsonbin:{apiKey:'',binId:'',autoSync:false}}; storage.set(data); buildUI(); }
  };
}
...
console.log('V5.8.2 UMP DL - Sandbox iframe + Popup blocker');
toast('V5.8.2 san sang! (Popup da duoc chan)');
