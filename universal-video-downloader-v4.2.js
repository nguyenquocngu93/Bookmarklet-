/**
 * Universal Video Downloader V4.2 – Consistent Glass UI & Real Fullscreen
 * - Unified type scale (no more random 11/12/13/16/18px mixed ad-hoc)
 * - Icon buttons in header now actually styled (were unstyled before)
 * - Fullscreen rebuilt on native :fullscreen CSS instead of manual rotate hacks,
 *   so the player genuinely fills the screen with working controls
...
(function() {
    ...
    function showInlinePlayer(url, container) {
        ...
        // Fullscreen: rely on native :fullscreen CSS (see stylesheet) instead of
        // manual rotate/width hacks, which is what broke fullscreen before.
        ...
        function requestFullscreen() {
            // Toggle: if already fullscreen, exit instead of re-requesting.
            ...
            if (!req) { toast('Fullscreen not supported', 'var(--danger)'); return; }
            ...
            if (result && result.then) result.then(lockLandscape).catch(function(){});
            else lockLandscape();
        }
    ...
    style.textContent = `
        ...
        :root {
            ...
            /* Unified type scale — every piece of UI text below pulls from here
               so nothing ends up randomly 11px next to 18px again. */
            --fs-xs: 11px;
            --fs-sm: 12px;
            --fs-base: 13px;
            --fs-md: 14px;
        ...
        * { box-sizing: border-box; }
        ...
        /* Real fullscreen: browser makes .uvd-video-wrapper fill the screen when
           requestFullscreen() is called on it. These rules remove the 55vh cap
           and lay the controls over the bottom, instead of the old JS rotate hack. */
        ...
        .uvd-time { color:var(--text2); font-size:var(--fs-sm); min-width:90px; text-align:right; }
        ...
        /* Header icon buttons (minimize/refresh/close) — previously unstyled,
           which is why they looked like plain browser buttons at the wrong size. */
        .uvd-btn-icon {
            background:rgba(255,255,255,0.08); border:1px solid rgba(255,255,255,0.15);
            color:var(--text); width:32px; height:32px; border-radius:var(--radius-sm);
            font-size:var(--fs-md); font-weight:600; cursor:pointer;
            display:inline-flex; align-items:center; justify-content:center;
            transition: all 0.2s; backdrop-filter:blur(8px); position:relative; overflow:hidden;
            line-height:1;
        }
        .uvd-btn-icon:hover { background:rgba(255,255,255,0.18); border-color:rgba(255,255,255,0.3); }
        ...
        .uvd-type-badge {
            ...
            font-size:var(--fs-xs); font-weight:700; background:rgba(59,130,246,0.2);
            color:var(--accent); border:1px solid rgba(59,130,246,0.3);
            letter-spacing:0.02em;
        }
        .uvd-url-box {
            background:rgba(0,0,0,0.4); border-radius:var(--radius-sm); padding:12px;
    ...
    function buildUI() {
        ...
        header.innerHTML = 
            '<div style="display:flex;align-items:center;gap:8px;">' +
                ...
                '<span style="font-weight:700;font-size:16px;">Universal DL <span style="color:var(--accent);">V4.2</span></span>' +
            '</div>' +
    ...
    function renderSettings(container) {
        container.innerHTML = 
            ...
            '<div class="uvd-card" style="margin-top:10px;color:var(--text2);">Version 4.2<br>Favorites: ' + data.favorites.length + ' · History: ' + (data.history||[]).length + '</div>';
        container.querySelectorAll('.uvd-btn').forEach(function(b) { b.addEventListener('click', addRipple); });
    ...
    console.log('V4.2 Consistent Glass UI ready');
    toast('V4.2 Ready');
})();
