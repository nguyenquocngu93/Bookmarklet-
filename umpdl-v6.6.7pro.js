/**
 * Universal Media Player & Downloader - V6.4 PRO (bản 1 + mute + tự động reduce khi xem)
 * - Giao diện giống bản 1: ít trong suốt, hiệu ứng đầy đủ
 * - Thêm nút Mute, tự động tắt animation khi xem để đỡ nóng (vẫn giữ nền đẹp)
 * - Fix highlight tab Settings (không dính mép phải)
 * - Fix lỗi hiển thị hướng dẫn trong tab Settings
 * Author: nguyenquocngu91 (fixed)
 */
(function() {
'use strict';

// ========== INIT ==========
var VERSION = '6.4-fixed';
...
function showVideoPlayer(url, type) {
  ...
  // Tạm thời bật reduceMotion để giảm tải khi xem video (chỉ tắt animation, giữ nguyên nền)
  ...
  muteBtn.style.cssText = 'background:rgba(255,255,255,0.1);color:#fff;font-size:12px;';
...
function closePlayer() {
  if (playerState.overlay) {
    ...
    // Khôi phục trạng thái reduceMotion
...
// ========== CSS (GIỐNG BẢN 1, SỬA LỖI HIỂN THỊ TAB SETTINGS) ==========
...
/* Reduce motion: chỉ tắt animation, KHÔNG thay đổi nền hay blur */
.uvd-reduce-motion .uvd-liquid-bg{animation:none}
.uvd-reduce-motion *{animation:none!important;transition:none!important}
.uvd-reduce-motion .uvd-glass-panel{backdrop-filter:blur(14px) saturate(130%)!important; -webkit-backdrop-filter:blur(14px) saturate(130%)!important} /* giữ nguyên blur */
...
.uvd-tab{position:relative;z-index:1;flex:0 0 auto;background:transparent;border:none;color:var(--text2);font-weight:600;font-size:var(--fs-sm);padding:9px 16px;border-radius:999px;cursor:pointer;transition:color 0.25s;white-space:nowrap}
...
/* Fix cho hướng dẫn trong tab Settings */
...
function buildUI() {
  ...
  function moveIndicatorTo(btn) {
    if (!btn) return;
    // Lấy padding để indicator không bị dính mép phải
    var padding = 4;
    var width = btn.offsetWidth + padding * 2;
    indicator.style.width = width + 'px';
    indicator.style.transform = 'translateX(' + (btn.offsetLeft - padding) + 'px)';
...
function renderSettings(container) {
  ...
  container.innerHTML =
    ...
    '<div class="uvd-card uvd-timeline-card">' +
      ...
      '<div class="uvd-step"><span class="uvd-step-num">•</span><span class="uvd-step-text">Trong player: thanh công cụ gồm <strong style="color:var(--accent);">Chất lượng</strong>, <strong style="color:var(--accent);">Tốc độ</strong>, <strong style="color:var(--accent);">Toàn màn hình</strong>, <strong style="color:var(--accent);">PiP</strong>, <strong style="color:var(--accent);">Hẹn giờ</strong>, <strong style="color:var(--accent);">Boost</strong>, <strong style="color:var(--accent);">Mute</strong></span></div>' +
...
console.log('V' + VERSION + ' UMP DL PRO - Giao diện bản 1 + Mute + tự động giảm tải khi xem');
