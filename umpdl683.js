/**
 * Universal Media Player & Downloader - V6.7.5 PRO
 * - FIX THẬT SỰ: gỡ bỏ hoàn toàn hack visualViewport translateY từng khiến
 *   panel bị "trôi/cuộn theo" khi Chrome co giãn thanh địa chỉ lúc cuộn trang.
 *   position:fixed để tự lo, không can thiệp JS nữa.
 * - FIX: lỗi CSS biến --btn-bg tự tham chiếu chính nó (gây vỡ icon/nút toàn bộ panel)
 ...
 * - FIX: chế độ hiệu suất nền đặc, không trong suốt
 * - SỬA: khi thu nhỏ panel, header ở trên cùng, nội dung co lại mờ dần (hiệu ứng cuộn đẹp)
...
// ========== COMMANDS ==========
// ĐÃ BỎ makeCommands/showCommandPicker (yt-dlp/ffmpeg/curl) — tập trung vào xem trực tiếp, không phải tool tải
...
function toast(msg, color) {
  ...
  el.textContent = msg;
  el.style.cssText = 'position:fixed;top:20px;left:50%;transform:translateX(-50%);background:' + (color || '#3b82f6') + ';color:var(--on-dark);padding:12px 24px;border-radius:30px;z-index:2147483649;font:600 13px Segoe UI;box-shadow:0 4px 15px rgba(0,0,0,0.4);animation:uvdSlideIn 0.3s ease;';
  __uvdAppendRoot(el);
...
// ĐÃ BỎ exportData (TXT/JSON/M3U/CSV) — tập trung vào xem trực tiếp, không phải tool tải/export
...
function showSleepMenu() {
  ...
  panel.innerHTML = '<div style="color:var(--on-dark);font-weight:600;margin-bottom:12px;">⏰ Hẹn giờ ngủ</div>';
...
function showSubtitlePanel(video) {
  ...
  panel.innerHTML =
    '<div style="color:var(--on-dark);font-weight:600;margin-bottom:4px;">💬 Phụ đề <span style="font-size:10px;color:var(--gold);font-weight:400;">(thử nghiệm)</span></div>' +
    ...
    '<div style="display:flex;gap:6px;margin-bottom:8px;">' +
      '<input type="text" id="__uvd_sub_query__" placeholder="Tên phim..." value="' + escapeHtml(pageInfo.title) + '" style="flex:1;padding:9px;background:rgba(0,0,0,0.4);color:var(--on-dark);border:1px solid var(--border);border-radius:10px;font-size:12px;">' +
    ...
    '<details style="margin-top:12px;">' +
      ...
      '<input type="text" id="__uvd_sub_apikey__" placeholder="Dán API key cá nhân (subdl.com)" value="' + escapeHtml(data.settings.subdlApiKey || '') + '" style="width:100%;margin-top:8px;padding:9px;background:rgba(0,0,0,0.4);color:var(--on-dark);border:1px solid var(--border);border-radius:10px;font-size:11px;">' +
  ...
  panel.querySelector('#__uvd_sub_search__').onclick = function() {
    ...
    searchSubDL(q, function(list) {
      ...
      list.slice(0, 10).forEach(function(item) {
        ...
        row.innerHTML = '<div style="font-size:12px;color:var(--on-dark);">' + escapeHtml(title) + '</div>' +
...
function showGestureHint(text) {
  ...
  if (!el) {
    ...
    el.style.cssText = 'position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);background:rgba(0,0,0,0.7);color:var(--on-dark);padding:10px 18px;border-radius:12px;font-size:14px;font-weight:600;z-index:5;pointer-events:none;';
...
function __uvdMountVjs10(wrapper, video) {
  ...
  function wrapWithSkin() {
    ...
    try {
      ...
      // QUAN TRỌNG: ĐÃ THỬ cho player/skin tự co theo kích thước thật của video
      // (width/height:auto) để control bar không rơi vào vùng letterbox đen —
      // NHƯNG Web Components không có hành vi "replaced element" như <video>/
      // <img>, nên khi cả container lẫn video đều auto, chúng không tự tính
      // được kích thước lẫn nhau => sập về 0, đen thui không hiện gì. ĐÃ REVERT
      // về full 100% (ổn định, đã test chạy được với video ngang). Chấp nhận
      // control bar có thể lệch vùng letterbox ở vài tỉ lệ video hiếm gặp,
      // còn hơn vỡ hoàn toàn.
      // Watchdog: nếu sau 1.5s player vẫn render kích thước 0 (dấu hiệu vỡ
      // tương tự lỗi trên), tự fallback về controls gốc thay vì đen thui im lặng.
      setTimeout(function() {
        if (done && player.parentNode && (player.offsetWidth === 0 || player.offsetHeight === 0)) {
          console.error('[UMP DL] Video.js v10 render kích thước 0, tự fallback về controls gốc');
          if (video.parentNode) video.parentNode.removeChild(video);
...
function showVideoPlayer(url, type) {
  ...
  titleInfo.innerHTML = '<div style="font-weight:600;font-size:14px;color:var(--on-dark);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">▶ ' + escapeHtml(pageInfo.title) + '</div><div style="font-size:11px;color:#aaa;margin-top:2px;">' + escapeHtml(type) + '</div>';
  ...
  minVideoBtn.style.cssText = 'background:rgba(255,255,255,0.15);color:var(--on-dark);font-size:12px;';
  ...
  closeBtn.style.cssText = 'background:var(--btn-danger-bg);color:var(--on-dark);border:1px solid var(--btn-danger-border);';
  ...
  qualityBtn.style.cssText = 'background:var(--btn-bg);color:var(--on-dark);font-size:12px;';
  ...
  var fsBtn = null; // ĐÃ BỎ: v10 skin có sẵn nút Toàn màn hình trong control bar

  // ĐÃ BỎ nút PiP riêng: v10 skin có sẵn nút PiP trong control bar

  var sleepBtn = document.createElement('button');
  sleepBtn.className = 'uvd-btn uvd-btn-sm';
  sleepBtn.style.cssText = 'background:var(--btn-bg);color:var(--on-dark);font-size:12px;';
  ...
  boostBtn.style.cssText = 'background:' + (data.settings.volumeBoost ? 'var(--btn-gold-bg)' : 'var(--btn-bg)') + ';color:var(--on-dark);font-size:12px;';
  ...
  // ĐÃ BỎ nút Mute riêng: v10 skin có sẵn nút mute + volume slider trong control bar

  var autoNextBtn = document.createElement('button');
  autoNextBtn.className = 'uvd-btn uvd-btn-sm';
  autoNextBtn.id = '__uvd_autonext_btn__';
  autoNextBtn.style.cssText = 'background:' + (data.settings.autoNext ? 'var(--btn-accent-bg)' : 'var(--btn-bg)') + ';color:var(--on-dark);font-size:12px;';
  ...
  pinBtn.style.cssText = 'background:var(--btn-bg);color:var(--on-dark);font-size:12px;';
  pinBtn.textContent = '📌';
  ...
  screenshotBtn.style.cssText = 'background:var(--btn-bg);color:var(--on-dark);font-size:12px;';
  ...
  subtitleBtn.style.cssText = 'background:var(--btn-bg);color:var(--on-dark);font-size:12px;';
  ...
  // ĐÃ BỎ nút Tải xuống MP4/MKV/WEBM — tập trung vào xem trực tiếp, không phải tool tải
  if (type === 'M3U8' || type === 'MPD') {
    var shareBtn = document.createElement('button');
    shareBtn.className = 'uvd-btn uvd-btn-sm';
    shareBtn.style.cssText = 'background:var(--btn-purple-bg);color:var(--on-dark);font-size:12px;';
  ...
  function createMenuPanel(title, options, callback) {
    ...
    var panel = document.createElement('div');
    panel.style.cssText = 'background:rgba(20,22,30,0.95);border-radius:16px;padding:20px;min-width:250px;max-width:90%;border:1px solid rgba(255,255,255,0.15);';
    panel.innerHTML = '<div style="color:var(--on-dark);font-weight:600;margin-bottom:12px;">' + escapeHtml(title) + '</div>';
  ...
  function updateTitleDisplay() {
    ...
    if (playerState._displayedResolution !== currentRes) {
      ...
      infoDiv.innerHTML = '<div style="font-weight:600;font-size:14px;color:var(--on-dark);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">▶ ' + escapeHtml(pageInfo.title) + '</div><div style="font-size:11px;color:#aaa;margin-top:2px;">' + escapeHtml(sub) + '</div>';
    }
  }
...
function minimizePlayer() {
  ...
  label.style.cssText = 'position:absolute;bottom:4px;left:8px;color:var(--on-dark);font-size:11px;font-weight:600;text-shadow:0 2px 4px rgba(0,0,0,0.8);background:rgba(0,0,0,0.5);padding:2px 8px;border-radius:4px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:90%;';
...
.uvd-glass-panel{background:var(--glass);backdrop-filter:blur(var(--uvd-blur)) saturate(130%);-webkit-backdrop-filter:blur(var(--uvd-blur)) saturate(130%);border:1px solid var(--border);border-radius:var(--radius-lg);box-shadow:0 24px 60px rgba(90,40,120,0.18),0 2px 10px rgba(90,40,120,0.08),0 1px 0 rgba(255,255,255,0.6) inset;color:var(--text);font-family:-apple-system,BlinkMacSystemFont,'SF Pro Display','Segoe UI',Roboto,sans-serif;font-size:var(--fs-base);padding:16px;width:100%;position:relative;overflow:hidden;max-width:1000px;margin:auto;transition: backdrop-filter var(--uvd-transition), background var(--uvd-transition);}
...
.uvd-reduce-motion .uvd-glass-panel {
  ...
  border-color: rgba(90,40,120,0.18);
}
...
.uvd-tab.uvd-tab-active{color:var(--text);text-shadow:0 1px 4px rgba(0,0,0,0.3)}

:root{--bg:rgba(252,247,255,0.92);--glass:rgba(255,255,255,0.6);--glass-hi:rgba(255,255,255,0.5);--border:rgba(90,40,120,0.14);--text:#3a1e5c;--text2:#7a5c96;--text3:#a893c0;--on-dark:#f3f5ff;--accent:#9b5cff;--accent2:#ff8fd6;--danger:#ff5d8a;--gold:#e8a333;--success:#22b56e;--card-bg:rgba(255,255,255,0.55);--fs-xs:11px;--fs-sm:12px;--fs-base:13px;--fs-md:14px;--fs-lg:16px;--radius-sm:14px;--radius-md:20px;--radius-lg:28px;--grad-liquid:linear-gradient(135deg,var(--accent),var(--accent2));--glow-px:0px;--glow-op:0;--btn-bg:rgba(155,92,255,0.1);--btn-danger-bg:rgba(255,93,138,0.16);--btn-danger-border:rgba(255,93,138,0.35);--btn-success-bg:rgba(34,181,110,0.16);--btn-success-border:rgba(34,181,110,0.35);--btn-accent-bg:rgba(155,92,255,0.18);--btn-purple-bg:rgba(255,143,214,0.18);--btn-gold-bg:rgba(232,163,51,0.18)}
...
.uvd-btn{background:var(--glass-hi);border:1px solid var(--border);color:var(--text);padding:9px 16px;border-radius:var(--radius-md);font-weight:600;font-size:var(--fs-base);cursor:pointer;text-align:center;position:relative;overflow:hidden;display:inline-block;box-shadow:0 4px 14px rgba(90,40,120,0.12),0 1px 0 rgba(255,255,255,0.6) inset;line-height:1.3;transition:all var(--uvd-transition);}
.uvd-btn:active{transform:scale(0.96)}
.uvd-btn-sm{padding:7px 12px;font-size:var(--fs-sm);border-radius:var(--radius-sm)}
.uvd-btn-icon{background:var(--glass-hi);border:1px solid var(--border);color:var(--text);width:36px;height:36px;border-radius:999px;cursor:pointer;display:inline-flex;align-items:center;justify-content:center;position:relative;overflow:hidden;box-shadow:0 4px 14px rgba(90,40,120,0.14),0 1px 0 rgba(255,255,255,0.6) inset;transition:all var(--uvd-transition);}
.uvd-btn-icon:active{transform:scale(0.92)}
.uvd-card{background:var(--card-bg);border:1px solid var(--border);border-radius:var(--radius-md);padding:14px;margin-bottom:10px;font-size:var(--fs-base);box-shadow:0 8px 24px rgba(90,40,120,0.08),0 1px 0 rgba(255,255,255,0.5) inset;animation:uvdCardEnter 0.4s ease both;color:var(--text);transition:all var(--uvd-transition);}
...
.uvd-meta-card{padding:12px 14px;}
.uvd-meta-title{font-weight:700;font-size:15px;color:var(--text);line-height:1.3;cursor:pointer;flex:1;overflow:hidden;text-overflow:ellipsis;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;}
.uvd-meta-toggle{background:none;border:none;color:var(--text3);font-size:11px;font-weight:600;padding:6px 0 0;cursor:pointer;display:block;}
.uvd-meta-row{display:flex;justify-content:space-between;gap:10px;padding:6px 0;border-top:1px solid var(--border);font-size:11px;}
.uvd-meta-label{color:var(--text3);flex-shrink:0;}
.uvd-meta-value{color:var(--accent2);font-family:monospace;cursor:pointer;text-align:right;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}
...
.uvd-listing-url{font-size:11px;color:var(--text3);font-family:'SFMono-Regular',Consolas,monospace;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-bottom:10px;cursor:pointer;}
.uvd-cta-primary{width:100%;background:var(--grad-liquid);color:#fff;border:none;padding:11px;border-radius:999px;font-weight:700;font-size:13px;cursor:pointer;box-shadow:0 6px 16px rgba(155,92,255,0.35);transition:all var(--uvd-transition);position:relative;overflow:hidden;}
.uvd-cta-primary:active{transform:scale(0.97);}
.uvd-cta-secondary{width:100%;background:transparent;border:1px solid var(--border);color:var(--accent);padding:8px;border-radius:999px;font-weight:600;font-size:12px;cursor:pointer;margin-top:6px;position:relative;overflow:hidden;}
...
.uvd-profile-avatar{flex-shrink:0;width:56px;height:56px;border-radius:50%;background:var(--grad-liquid);color:var(--text);font-weight:700;font-size:18px;display:flex;align-items:center;justify-content:center;box-shadow:0 6px 18px rgba(109,140,255,0.4),0 0 0 3px rgba(255,255,255,0.08)}
...
.uvd-section-num{width:20px;height:20px;border-radius:50%;background:var(--grad-liquid);color:var(--text);font-size:11px;display:inline-flex;align-items:center;justify-content:center;flex-shrink:0;box-shadow:0 2px 8px rgba(109,140,255,0.4)}
...
.uvd-step-num{flex-shrink:0;width:24px;height:24px;border-radius:50%;background:var(--grad-liquid);color:var(--text);font-size:12px;font-weight:700;display:flex;align-items:center;justify-content:center;box-shadow:0 2px 8px rgba(109,140,255,0.4)}
...
// Không liên quan đến vị trí (đã fix ở trên). Đây là lỗi compositing riêng của
// Chrome: layer backdrop-filter (nền kính) và layer nội dung bị "chồng hình"
// với khung hình cũ của trang sau khi cuộn khiến toolbar ẩn/hiện — nhìn như
// panel trong suốt / icon tách khỏi nền. offsetHeight/willChange KHÔNG đủ mạnh
// để ép Chrome tái compositing layer backdrop-filter — phải nudge 'transform'
// qua 2 khung hình (kỹ thuật chuẩn để buộc GPU vẽ lại layer). Đây là ép vẽ lại
// MỘT LẦN, không track vị trí liên tục, nên không lặp lại bug trôi panel cũ.
...
// Cuộn trang là nguyên nhân chính khiến Chrome tự ẩn/hiện toolbar (không chỉ
// tap tay vào thanh địa chỉ) — bắt thêm sự kiện này, debounce nặng vì scroll
// bắn liên tục.
...
function buildUI() {
  ...
  info.className = 'uvd-card uvd-meta-card';
  info.style.cssText = 'margin-bottom:10px;flex-shrink:0;';
  var savedPlaySel = (data.siteProfiles[pageInfo.host] && data.siteProfiles[pageInfo.host].playSelector) || '';
  info.innerHTML =
    '<div style="display:flex;align-items:center;gap:8px;">' +
      '<span id="__uvd_title__" class="uvd-meta-title" title="Chạm để sửa tên">' + escapeHtml(pageInfo.title) + '</span>' +
    '</div>' +
    '<button class="uvd-meta-toggle" id="__uvd_meta_toggle__">Chi tiết kỹ thuật <span id="__uvd_meta_caret__">▾</span></button>' +
    '<div id="__uvd_meta_details__" style="display:none;margin-top:8px;">' +
      '<div class="uvd-meta-row"><span class="uvd-meta-label">Referer</span><span id="__uvd_referer__" class="uvd-meta-value">' + escapeHtml(pageInfo.referer) + '</span></div>' +
      '<div class="uvd-meta-row"><span class="uvd-meta-label">Play selector</span><span id="__uvd_playsel__" class="uvd-meta-value">' + escapeHtml(savedPlaySel || 'chưa đặt · chạm để thêm') + '</span></div>' +
    '</div>';
  content.appendChild(info);
  info.querySelector('#__uvd_meta_toggle__').onclick = function() {
    var d = document.getElementById('__uvd_meta_details__');
    var caret = document.getElementById('__uvd_meta_caret__');
  ...
  // ĐÃ BỎ footer export TXT/JSON/M3U/CSV — tập trung vào xem trực tiếp
  ...
  return (
    '<div class="uvd-card" data-type="' + escapeHtml(item.type) + '" data-url="' + escapeHtml(item.url) + '">' +
      '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">' +
        ...
        '<button class="uvd-block-btn" data-url="' + encodeURIComponent(item.url) + '" style="background:none;border:none;font-size:15px;cursor:pointer;color:var(--text);opacity:0.4;" title="Chặn link này">⛔</button>' +
      '</div>' +
      '<div class="uvd-listing-url" data-copy-url="' + encodeURIComponent(item.url) + '" title="Chạm để sao chép">' + escapeHtml(item.url) + '</div>' +
      (item.type === 'IFRAME' ?
        '<button class="uvd-cta-primary" data-action="iframe" data-url="' + encodeURIComponent(item.url) + '">Mở iframe</button>' :
        '<button class="uvd-cta-primary" data-action="play" data-url="' + encodeURIComponent(item.url) + '" data-type="' + escapeHtml(item.type) + '">▶ Xem</button>' +
        (item.type === 'M3U8' ? '<button class="uvd-cta-secondary" data-action="quality" data-url="' + encodeURIComponent(item.url) + '">Chọn chất lượng</button>' : '')
      ) +
    '</div>'
  );
}
...
function renderStreams(container, arr) {
  ...
  container.onclick = function(e) {
    var urlLine = e.target.closest('.uvd-listing-url');
    if (urlLine) {
      copy(decodeURIComponent(urlLine.dataset.copyUrl));
    ...
    if (actionBtn) {
      ...
      if (action === 'quality') showQualityPicker(u2);
...
function showEditor(text) {
  ...
  panel.innerHTML = 
    ...
    '<textarea style="width:100%;height:120px;background:rgba(0,0,0,0.5);border:1px solid var(--border);border-radius:10px;color:var(--on-dark);padding:12px;font-family:monospace;">' + escapeHtml(text) + '</textarea>' +
...
function showQualityPicker(url) {
  ...
  parseM3U8Master(url, function(qualities) {
    ...
    qualities.forEach(function(q) {
      ...
      card.style.cssText = 'cursor:pointer;display:flex;justify-content:space-between;align-items:center;gap:10px;';
      card.innerHTML = '<span><b>' + escapeHtml(q.label) + '</b> <span style="color:var(--text3);">' + Math.round(q.bandwidth/1000) + 'kbps</span></span><span style="color:var(--accent);font-weight:700;">▶ Xem</span>';
      card.onclick = function() { overlay.remove(); window.__uvd_showPlayer(q.url, 'M3U8'); };
...
function renderPlayerSettings(container) {
  ...
  container.innerHTML =
    '<div class="uvd-card">' +
      ...
      '<select id="__uvd_set_speed__" style="width:100%;padding:10px;background:rgba(0,0,0,0.4);color:var(--on-dark);border:1px solid var(--border);border-radius:10px;margin-bottom:12px;">' +
      ...
      '<select id="__uvd_set_quality__" style="width:100%;padding:10px;background:rgba(0,0,0,0.4);color:var(--on-dark);border:1px solid var(--border);border-radius:10px;">' +
    ...
    '<div class="uvd-card">' +
      ...
      '<input type="number" id="__uvd_doubletap_seconds__" min="1" max="60" step="1" value="' + s.doubleTapSeconds + '" style="width:100%;padding:10px;background:rgba(0,0,0,0.4);color:var(--on-dark);border:1px solid var(--border);border-radius:10px;">' +
    '</div>' +
    ...
    '<div class="uvd-card">' +
      ...
      '<input type="number" id="__uvd_hide_delay__" min="1" max="30" step="1" value="' + s.hideDelay + '" style="width:100%;padding:10px;background:rgba(0,0,0,0.4);color:var(--on-dark);border:1px solid var(--border);border-radius:10px;">' +
    '</div>' +
...
function renderSettings(container) {
  ...
  container.innerHTML =
    ...
    '<div class="uvd-profile-stats" style="grid-template-columns:repeat(4,1fr);">' +
      ...
      '<div class="uvd-stat"><div class="uvd-stat-num" style="color:var(--danger);">' + __uvdBlockedCount + '</div><div class="uvd-stat-label">Đã chặn popup</div></div>' +
    '</div>' +
    ...
    '<div class="uvd-card">' +
      ...
      '</div>' +
      '<textarea id="__uvd_filter_text__" style="width:100%;height:80px;background:rgba(0,0,0,0.5);border:1px solid var(--border);border-radius:10px;color:var(--on-dark);padding:12px;font-size:12px;">' + escapeHtml((data.filterlist||[]).join('\n')) + '</textarea>' +
    ...
    '<div class="uvd-card uvd-timeline-card">' +
      ...
      '<div class="uvd-step"><span class="uvd-step-num">•</span><span class="uvd-step-text">Chọn stream, chọn <strong style="color:var(--accent);">Chất lượng</strong> nếu là M3U8, bấm <strong style="color:var(--accent);">▶ Xem</strong> để mở player</span></div>' +
      '<div class="uvd-step"><span class="uvd-step-num">•</span><span class="uvd-step-text">Play/tạm dừng, tua, tốc độ, âm lượng, toàn màn hình, PiP: dùng thanh điều khiển kính của trình phát (chạm vào video để hiện) — chạm giữ nút ⚙ để mở menu Chất lượng/Tốc độ/Phụ đề chi tiết hơn</span></div>' +
      '<div class="uvd-step"><span class="uvd-step-num">•</span><span class="uvd-step-text">Thanh công cụ riêng phía trên gồm những gì trình phát chưa có sẵn: <strong style="color:var(--accent);">Chất lượng</strong>, <strong style="color:var(--accent);">Hẹn giờ</strong> ngủ, <strong style="color:var(--accent);">Boost</strong> âm lượng, <strong style="color:var(--accent);">Tự động phát tiếp</strong>, <strong style="color:var(--accent);">📌 Ghim</strong> ẩn tự động, <strong style="color:var(--accent);">📷 Screenshot</strong>, <strong style="color:var(--accent);">💬 Phụ đề</strong> (SubDL), <strong style="color:var(--accent);">Chia sẻ</strong></span></div>' +
