...
function autoClickPlayButtons(root, depth) {
  root = root || document;
  depth = depth || 0;
  var clicked = 0;
  ...
  // Thử gọi .play() trực tiếp trên các thẻ <video> có sẵn (bỏ qua nếu bị chặn)
  try {
  ...
  if (depth < 2) {
    try {
      root.querySelectorAll('iframe').forEach(function(f) {
        try { if (f.contentDocument) clicked += autoClickPlayButtons(f.contentDocument, depth + 1); }
        catch(e) {}
      });
    } catch(e) {}
  }
...
// phỏng thao tác đó rồi quét lại sau một khoảng chờ ngắn.
function runAutoClickAndRescan(silent) {
  var beforeCount = urls.size;
  var clicked = autoClickPlayButtons();
  setTimeout(function() {
...
console.log('V5.5 UMP DL - Auto-click Play + Overlay Player Ready');
toast('V5.5 sẵn sàng!');
