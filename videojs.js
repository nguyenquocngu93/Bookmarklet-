/**
 * UMP DL - Video.js Edition (giữ tab Cài đặt)
 * - Quét link video/m3u8/mp4...
 * - Phát bằng Video.js v10 (giao diện kính mờ)
 * - Giữ tab Streams và Cài đặt (bỏ tab Trình phát)
 * - Giữ nguyên các tính năng: filterlist, backup, restore, hiệu suất, glow...
 */
(function() {
  'use strict';
  ...
  var profile = data.siteProfiles[host] || {
  ...
  // ========== VIDEO.JS PLAYER (thay thế hoàn toàn player cũ) ==========
  var playerState = {
    overlay: null,
    player: null,
    videoEl: null,
  ...
  function showVideoPlayer(url, type) {
    ...
    header.style.cssText = 'padding:10px 16px;background:rgba(14,16,22,0.92);display:flex;flex-shrink:0;border-bottom:1px solid rgba(255,255,255,0.2);justify-content:space-between;align-items:center;';
    var titleSpan = document.createElement('span');
    titleSpan.style.cssText = 'font-weight:600;font-size:14px;color:#fff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;';
    titleSpan.textContent = '▶ ' + pageInfo.title;
    var closeBtn = document.createElement('button');
    closeBtn.textContent = '✕';
    closeBtn.style.cssText = 'background:rgba(255,255,255,0.15);border:none;color:#fff;padding:6px 12px;border-radius:8px;cursor:pointer;';
    ...
    loadVideoJS(function() {
      ...
      player.one('loadedmetadata', function() {
        ...
        // Lấy chất lượng nếu HLS
        if (isHls) {
          parseM3U8Master(url, function(qualities) {
            if (qualities && qualities.length > 0) {
              playerState.qualities = qualities;
              // Áp dụng default quality (chỉ thông báo, VHS không hỗ trợ switch đơn giản)
      ...
      player.on('ended', function() {
        ...
        if (data.settings.autoNext) {
          ...
          if (nextUrl) { toast('⏭ Phát tiếp...'); setTimeout(function() { showVideoPlayer(nextUrl.url, nextUrl.type); }, 800); }
        }
      });
  ...
  function closePlayer() {
    if (playerState.overlay) {
      if (data.settings.resumePlayback && playerState.url && playerState.videoEl) {
        savePlaybackPosition(playerState.url, playerState.videoEl);
      }
  ...
  // ========== CSS (giữ nguyên từ bản cũ, thêm glass cho Video.js) ==========
  ...
  function buildUI() {
    ...
    // Chỉ giữ 2 tab: Streams và Settings (bỏ tab Player)
    var tabList = [
  ...
  // ====== RENDER SETTINGS (giữ nguyên từ bản cũ) ======
  function renderSettings(container) {
    ...
    container.innerHTML =
      '<div class="uvd-profile-card">' +
        ...
        '<div class="uvd-profile-info">' +
          ...
          '<div class="uvd-profile-tags">' +
            ...
            '<span class="uvd-tag">Video.js</span>' +
          '</div>' +
        '</div>' +
      '</div>' +
      ...
      '<div class="uvd-card">' +
        ...
        '<div style="font-size:11px;color:var(--text3);margin-top:6px;">Video/audio do chính trang web tự bật sẽ bị tạm dừng. Video mở qua UMP DL không bị ảnh hưởng.</div>' +
      '</div>' +
      ...
      '<div class="uvd-card uvd-timeline-card">' +
        ...
        '<div class="uvd-callout"><span class="uvd-callout-icon">💡</span><span>Từ lần sau, bạn chỉ cần gõ tên bookmark (<strong style="color:var(--accent);">UMP DL</strong>) vào thanh địa chỉ rồi chọn nó để kích hoạt.</span></div>' +
      '</div>' +
      ...
      '<div class="uvd-card uvd-timeline-card">' +
        ...
        '<div class="uvd-step"><span class="uvd-step-num">•</span><span class="uvd-step-text">Chọn stream và bấm <strong style="color:var(--accent);">Xem</strong> để mở player</span></div>' +
        '<div class="uvd-step"><span class="uvd-step-num">•</span><span class="uvd-step-text">Trình phát sử dụng Video.js v10 với giao diện kính mờ</span></div>' +
  ...
  // ========== START ==========
  buildUI();
  console.log('V' + VERSION + ' UMP DL with Video.js - Glass UI, giữ tab Cài đặt');
