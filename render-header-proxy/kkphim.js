...
(function() {
  ...
  function showIntro() {
    ...
    el.querySelector('.uvd-kk-intro-go').onclick = function() { if (search) search.style.display = 'flex'; loadHome(); };
  }
  function close() {
  ...
  function bindMovieCards(root) { root.querySelectorAll('[data-slug]').forEach(function(btn) { btn.onclick = function() { loadMovie(btn.dataset.slug); }; }); }
  function loadHome() {
    state.query = ''; state.detail = null;
    loading('Đang chuẩn bị góc phim cho cưng...');
    var rowRequests = HOME_ROWS.map(function(row) { return json(API + '/danh-sach/' + row.id + '?page=1').then(function(data) { return { row: row, items: data.items || [] }; }).catch(function() { return { row: row, items: [] }; }); });
    Promise.all([json(API + '/the-loai').catch(function() { return {}; })].concat(rowRequests)).then(function(results) {
      state.categories = (results[0].data && results[0].data.items) || [];
      renderHome(results.slice(1));
    }).catch(function(err) { showError('Không lấy được trang chủ KPhim: ' + err.message); });
  }
  function renderHome(rows) {
    var el = content(); if (!el) return;
    el.innerHTML = '<div class="uvd-kk-category-row">' + state.categories.slice(0, 16).map(function(cat) { return '<button class="uvd-kk-category" data-category="' + esc(cat.slug) + '">' + esc(cat.name) + '</button>'; }).join('') + '</div>' + rows.map(function(group) {
      return '<section class="uvd-kk-home-section"><div class="uvd-kk-home-section-head"><strong>' + group.row.icon + ' ' + group.row.label + '</strong><button class="uvd-kk-home-more" data-browse="' + group.row.id + '">Xem thêm →</button></div><div class="uvd-kk-home-strip">' + group.items.slice(0, 10).map(cardHtml).join('') + '</div></section>';
    }).join('');
    bindMovieCards(el);
    el.querySelectorAll('[data-browse]').forEach(function(btn) { btn.onclick = function() { browseList('danh-sach/' + btn.dataset.browse, btn.textContent.replace('Xem thêm →', '') || 'Danh sách'); }; });
    el.querySelectorAll('[data-category]').forEach(function(btn) { btn.onclick = function() { browseCategory(btn.dataset.category, btn.textContent); }; });
    var p = pager(); if (p) p.innerHTML = '';
  }
  function browseList(path, title, page) {
    state.query = ''; state.page = page || 1;
    loading('Đang lấy ' + (title || 'phim') + '...');
    json(API + '/' + path + '?page=' + state.page).then(function(data) { renderList(data.items || [], data.pagination || {}, title, path); }).catch(function(err) { showError('Không lấy được danh sách: ' + err.message); });
  }
  function browseCategory(slug, title) {
    state.query = '';
    loading('Đang lấy thể loại ' + title + '...');
    json(API + '/v1/api/the-loai/' + encodeURIComponent(slug) + '?page=1&limit=24').then(function(data) { renderList((data.data && data.data.items) || data.items || [], (data.data && data.data.params && data.data.params.pagination) || {}, title, 'v1/api/the-loai/' + slug); }).catch(function(err) { showError('Không lấy được thể loại: ' + err.message); });
  }
  function loadRecent(page) { browseList('danh-sach/phim-moi-cap-nhat', 'Phim mới', page || 1); }
  ...
  function renderList(items, meta, title, path) {
    var el = content(); if (!el) return;
    if (!items.length) { el.innerHTML = '<div class="uvd-kk-loading">🥺 Chưa thấy phim này. Thử tên ngắn hơn nha.</div>'; return; }
    el.innerHTML = (title ? '<button class="uvd-kk-back uvd-kk-home-back">← Trang chủ KPhim</button><div class="uvd-kk-section-title">' + esc(title) + '</div>' : '') + '<div class="uvd-kk-grid">' + items.slice(0, 24).map(function(item) {
      var image = item.thumb_url || item.poster_url || '';
      return '<button class="uvd-kk-card" data-slug="' + esc(item.slug) + '"><span class="uvd-kk-poster">' + (image ? '<img src="' + esc(image) + '" alt="">' : '🎬') + '</span><span class="uvd-kk-card-name">' + esc(item.name) + '</span><span class="uvd-kk-card-meta">' + esc(item.year || '') + (item.origin_name ? ' · ' + esc(item.origin_name) : '') + '</span></button>';
    }).join('') + '</div>';
    bindMovieCards(el);
    var homeBack = el.querySelector('.uvd-kk-home-back'); if (homeBack) homeBack.onclick = loadHome;
    var p = pager();
    if (p) {
      ...
      if (prev) prev.onclick = function() { browseList(path || 'danh-sach/phim-moi-cap-nhat', title, current - 1); };
      if (next) next.onclick = function() { browseList(path || 'danh-sach/phim-moi-cap-nhat', title, current + 1); };
    }
  }
  function tmdbDetails(movie) {
    ...
    return (proxy ? json(proxy + '/' + kind + '/' + encodeURIComponent(tmdb.id)) : Promise.reject()).catch(direct).catch(function() { return null; });
  }
  ...
  function renderMovie(data, tmdb) {
    ...
    el.querySelector('.uvd-kk-back').onclick = loadHome;
  ...
  window.__uvdKkphim = { version: MODULE_VERSION, open: mount, close: close };
})();
