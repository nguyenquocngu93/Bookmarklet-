 ...
 * - Kế thừa giao diện V6.1 (bo góc, không full‑screen)
 * - Thu nhỏ kiểu mới: ẩn nội dung, chỉ giữ header
 * - Responsive: panel tự điều chỉnh trên màn hình nhỏ
 * - Nút filter dùng chữ (Lưu / Import file)
  ...
  playerState.guardInterval = setInterval(function() {
    ...
    try {
      ...
      for (var i = 0; i < allVideos.length; i++) {
        var v = allVideos[i];
        if (v !== playerState.video && !v.paused) {
          v.pause();
        }
      }
    } catch(e) {}
...
// ========== BUILD UI (CSS GỐC V6.1 + RESPONSIVE + MINIMIZE MỚI) ==========
...
.uvd-panel-header{display:flex;justify-content:space-between;align-items:center;padding-bottom:12px;border-bottom:1px solid var(--border);margin-bottom:10px;flex-shrink:0}
.uvd-minimized .uvd-panel-content > *:not(.uvd-panel-header){display:none}
.uvd-minimized .uvd-panel-content{overflow:hidden}
.uvd-minimized .uvd-glass-panel{padding-bottom:0}
...
@media (max-width:600px){.uvd-glass-panel{border-radius:var(--radius-md);padding:12px;margin:8px;max-width:100%}.uvd-btn{padding:7px 12px;font-size:var(--fs-sm)}.uvd-btn-sm{padding:5px 10px;font-size:var(--fs-xs)}.uvd-tab{font-size:11px;padding:7px 4px}.uvd-profile-card{flex-direction:column;text-align:center}.uvd-profile-stats{grid-template-columns:repeat(2,1fr)}}
`;
document.head.appendChild(style);

// ========== BUILD UI MAIN (RESPONSIVE + MINIMIZE MỚI) ==========
function buildUI() {
  ...
  panel.style.cssText = 'position:fixed;top:15px;left:50%;transform:translateX(-50%);width:calc(100% - 30px);max-width:1000px;bottom:15px;z-index:2147483647;animation:uvdScaleIn 0.4s ease;';
  ...
  // Header với 4 nút (bao gồm nút minimize mới)
  var header = document.createElement('div');
...
// ========== CÁC HÀM RENDER (giữ nguyên từ V6.1/V6.2) ==========
// (Bạn chèn toàn bộ các hàm renderStreams, showCommandPicker, showEditor, showQualityPicker, renderFavorites, renderHistory, renderSettings từ V6.2 vào đây)
...
console.log('V6.3 UMP DL - Responsive, minimize mới, filter chữ');
toast('V6.3 sẵn sàng!');
})();