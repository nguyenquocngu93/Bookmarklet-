 ...
 * - MỚI: nút ⏭ "Thử lần lượt server" — khác với nút ▶ (bấm HẾT các nút server
 *   cùng lúc, dễ dính nút sai như "thêm vào playlist"), nút ⏭ bấm TỪNG server
 *   MỘT, chờ ~1.8s xem có ra link mới không rồi mới bấm server kế tiếp, dừng
 *   ngay khi tìm được link. Kết hợp với tab "Nút đã click" để chặn server hay
 *   bấm nhầm, càng dùng càng ít phải thử lại từ đầu.
 * - MỚI (bản trước): tab "Nút đã click" — ghi lại từng nút cụ thể auto-click đã
 *   bấm trúng theo từng site, có toggle CHẶN/CHO PHÉP.
 ...
 * - FIX auto-click: quét lại nhiều lần trong ~4s sau khi bấm (site load link
 *   chậm). Lưu ý: click giả lập luôn isTrusted=false, vài site chặn theo cờ
 *   này — không script nào giả mạo được, có thể vẫn phải bấm tay ở site khó.
