 ...
 * - FIX QUAN TRỌNG: tính năng "tự quét server nếu chưa thấy media" (thêm ở
 *   6.7.15) tự bấm lung tung chỉ sau 2.5s — đúng lúc video thật (blob) còn
 *   đang khởi tạo, bấm trúng gì đó trong chính player thật làm nó reset/mất
 *   trước khi kịp lộ link → đây là NGUYÊN NHÂN THẬT khiến tính năng blob "biến
 *   mất". Giờ: tăng lên 5s + THÊM ĐIỀU KIỆN không tự quét nếu đã thấy <video>
 *   có nguồn/đang load (dấu hiệu player thật đang khởi tạo, kể cả blob).
 * - CỦNG CỐ khoá điều hướng: thêm chặn cả <form> submit (event 'submit' CÓ
 *   thể preventDefault, nhưng gọi thẳng form.submit() qua JS thì KHÔNG bắn
 *   event này — đã vá thêm chính hàm submit() trên prototype). Sửa lỗi so
 *   sánh điều hướng chỉ so pathname, bỏ sót link kiểu ?actor=xyz (cùng path,
 *   khác query vẫn là điều hướng thật).
 * - Icon header đồng bộ, JAV play-button selector, "Mở tại chỗ" cho IFRAME,
 *   postMessage passive capture, video player giữ bản gốc + fix BLOB + Tải Blob,
 *   PerformanceObserver liên tục, fix giật panel khi cuộn.
...
function autoClickServersAndScan() {
  ...
  // Trước đây chỉ kiểm tra khi CHÍNH phần tử là <a> — bỏ sót trường hợp phần
  // tử được bấm (vd tên diễn viên) nằm LỒNG BÊN TRONG một <a href> (ảnh, span,
  // text con...), click vẫn nổi bọt lên <a> cha và điều hướng bình thường.
  function isRealNavAnchor(el) {
    ...
    try {
      ...
      // So cả pathname LẪN query string — trước đây chỉ so pathname nên bỏ
      // sót link kiểu ?actor=xyz (cùng path, khác query vẫn là điều hướng thật).
      if (abs.pathname + abs.search !== curPath + location.search) return true;
    } catch (e) {}
    return false;
  }
  ...
  document.querySelectorAll('body *').forEach(function(el) {
    ...
    if (isRealNavAnchor(el)) return; // loại cả phần tử lồng trong link điều hướng thật
    var cs = getComputedStyle(el);
    var clickable = cs.cursor === 'pointer' || el.tagName === 'A' || el.tagName === 'BUTTON' || !!el.onclick;
    if (!clickable) return;
  ...
  // ===== KHOÁ ĐIỀU HƯỚNG TUYỆT ĐỐI TRONG LÚC TỰ BẤM =====
  // Không tin tưởng bộ lọc trước là đủ (site có thể điều hướng kiểu SPA không
  // qua href) — chặn cứng mọi đường có thể rời trang trong suốt lúc quét, dù
  // lỡ bấm trúng gì cũng không bay khỏi trang. Khôi phục lại y nguyên sau đó.
  var navGuardActive = true;
  ...
  // 'submit' event có thể preventDefault được, nhưng gọi thẳng form.submit()
  // qua JS thì KHÔNG bắn sự kiện 'submit' (bypass hoàn toàn) — phải vá thêm
  // chính hàm submit() trên prototype mới chặn được trường hợp này.
  function navSubmitGuard(e) {
    if (navGuardActive) e.preventDefault();
  }
  document.addEventListener('submit', navSubmitGuard, true);
  var origFormSubmit = window.HTMLFormElement && window.HTMLFormElement.prototype.submit;
  if (origFormSubmit) {
    try { window.HTMLFormElement.prototype.submit = function() { if (!navGuardActive) origFormSubmit.apply(this, arguments); }; } catch (e) {}
  }

  var origPushState = history.pushState;
  var origReplaceState = history.replaceState;
  var origAssign = null, origReplace = null;
  try { origAssign = location.assign; location.assign = function() {}; } catch (e) {}
  try { origReplace = location.replace; location.replace = function() {}; } catch (e) {}
  ...
  function restoreNav() {
    ...
    document.removeEventListener('submit', navSubmitGuard, true);
    if (origFormSubmit) { try { window.HTMLFormElement.prototype.submit = origFormSubmit; } catch (e) {} }
    history.pushState = origPushState;
    history.replaceState = origReplaceState;
    if (origAssign) { try { location.assign = origAssign; } catch (e) {} }
    if (origReplace) { try { location.replace = origReplace; } catch (e) {} }
    window.removeEventListener('beforeunload', onBeforeUnload);
  }
  // Lưới an toàn cuối: dù logic bên dưới có lỗi bất ngờ, vẫn tự mở khoá sau
  // một khoảng thời gian hợp lý thay vì khoá điều hướng của trang vĩnh viễn.
  setTimeout(restoreNav, candidates.length * 900 + 6000);

  toast('Tìm thấy ' + candidates.length + ' nút — đang tự bấm lần lượt (đã khoá điều hướng, không rời trang), đợi vài giây...');
  var i = 0;
  function clickNext() {
...
// ========== TỰ ĐỘNG QUÉT SERVER NẾU KHÔNG THẤY MEDIA NÀO ==========
// Nếu sau khi trang tải xong (+ các nguồn quét ở trên) mà chưa tìm thấy link
// media nào (M3U8/MPD/MP4/WEBM/MKV/FLV/TS/BLOB) — khả năng cao đây là site
// kiểu "chọn server" cần bấm mới lộ link — tự động chạy quét server luôn,
// đỡ phải bấm tay nút 🖥.
var __uvdMediaTypes = { M3U8: 1, MPD: 1, MP4: 1, WEBM: 1, MKV: 1, FLV: 1, TS: 1, BLOB: 1 };
function __uvdHasAnyMedia() {
  var found = false;
  urls.forEach(function(v) { if (__uvdMediaTypes[v.type]) found = true; });
  return found;
}
function __uvdRealPlayerLikelyInitializing() {
  // Có <video> đã gán nguồn hoặc đã bắt đầu load — rất có thể là player thật
  // (kể cả blob: MediaSource) đang khởi tạo. Tự bấm lung tung lúc này dễ làm
  // nó bị reset/thay thế trước khi kịp lộ link thật — đúng lỗi từng gặp.
  var vids = document.querySelectorAll('video');
  for (var i = 0; i < vids.length; i++) {
    var v = vids[i];
    if (v.currentSrc || v.src || v.readyState > 0 || v.querySelector('source')) return true;
  }
  return false;
}
setTimeout(function() {
  if (!document.getElementById('__uvd__')) return;
  if (__uvdHasAnyMedia()) return;
  if (__uvdRealPlayerLikelyInitializing()) return;
  toast('Chưa thấy link media nào — tự động quét server...');
  try { autoClickServersAndScan(); } catch (e) {}
}, 5000);
