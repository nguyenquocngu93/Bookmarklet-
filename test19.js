/**
 * Universal Video Downloader V3.4 – Mobile Optimized, Enhanced Monitor, Fullscreen Fix
 * - Fully responsive UI for mobile devices
 * - Advanced monitoring (MediaSource, createObjectURL, periodic scan)
 * - Fullscreen now forces landscape orientation
 * - All previous Pro features retained (glass UI, ripple, etc.)
...
(function() {
    ...
    function installMonitor() {
        ...
        XMLHttpRequest.prototype.open = function(method, url) {
            if (url && monitorEnabled) findUrls(url, 'xhr:live');
            return originalXHROpen.apply(this, arguments);
        };

        // MediaSource interceptor – theo dõi source buffers
        if (originalMediaSource) {
            var origAddSourceBuffer = originalMediaSource.prototype.addSourceBuffer;
            originalMediaSource.prototype.addSourceBuffer = function(type) {
                var sb = origAddSourceBuffer.call(this, type);
                // Không trực tiếp lấy URL nhưng ta sẽ dùng cách khác
                return sb;
            };
            // Hook createObjectURL để bắt blob URL được tạo từ MediaSource
            URL.createObjectURL = function(obj) {
                var url = originalCreateObjectURL.call(this, obj);
                if (obj instanceof originalMediaSource) {
                    // Lưu lại URL để sau này có thể quét
                    // Khi source được gán vào video, video.src sẽ được quét
                    var videoElements = document.querySelectorAll('video');
                    videoElements.forEach(function(v) {
                        if (v.src === url || v.currentSrc === url) {
                            // Có thể gán sự kiện
                            findUrls(url, 'mse:createObjectURL');
                        }
                    });
                }
                return url;
        ...
        // Periodic DOM scan (mỗi 4 giây) để bắt stream động
        var periodicScan = setInterval(function() {
            if (!monitorEnabled || !document.getElementById('__uvd__')) {
                clearInterval(periodicScan);
                return;
            }
            scan(document, 'periodic');
            // Cập nhật UI nếu cần
        }, 4000);
    }
    ...
    // ========== PRO PLAYER (with orientation lock) ==========
    ...
    function showPreview(url) {
        ...
        async function requestFullscreenWithOrientation() {
            try {
        ...
        function exitFullscreenHandler() {
            if (document.fullscreenElement === null && screen.orientation && screen.orientation.unlock) {
                screen.orientation.unlock();
            }
        }

        fsBtn.addEventListener('click', requestFullscreenWithOrientation);
    ...
    // ========== BUILD UI WITH MOBILE OPTIMIZATION ==========
    ...
    style.textContent = `
        ...
        .uvd-ripple {
            ...
            transform:scale(0); animation:uvdRipple 0.6s linear; pointer-events:none;
        }
        ...
        @media (max-width: 768px) {
            ...
            .uvd-player-panel { max-width:100% !important; }
            .uvd-btn { padding:10px 12px; font-size:14px; }
            .uvd-type-badge { font-size:10px; }
            .uvd-controls { gap:6px; padding:8px; }
            .uvd-timeline { min-width:60px; }
            .uvd-time { font-size:10px; min-width:70px; }
            .uvd-volume-slider { width:50px; }
            .uvd-speed-select, .uvd-quality-select { font-size:10px; padding:4px 4px; }
        }
        @media (max-width: 480px) {
            .uvd-overlay { padding:8px; }
            .uvd-card { padding:10px; }
            .uvd-btn { padding:12px 8px; font-size:15px; border-radius:20px; }
            .uvd-grid-2, .uvd-grid-3 { gap:4px; }
            .uvd-controls { gap:4px; }
            .uvd-timeline { min-width:40px; }
        }
    `;
    ...
    function buildUI() {
        ...
        header.innerHTML = 
            '<div style="display:flex;align-items:center;gap:8px;">' +
                ...
                '<span style="font-weight:700;font-size:16px;">DL <span style="color:var(--accent);">V3.4</span></span>' +
            '</div>' +
        ...
        tabList.forEach(function(t) {
            ...
            b.style.cssText = 'flex:1;background:transparent;border:none;padding:10px 4px;font-size:12px;border-radius:0;border-bottom:2px solid transparent;min-width:60px;';
            tabs.appendChild(b);
        });
        ...
        info.innerHTML = 
            ...
            '<span id="__uvd_referer__" style="color:var(--accent2);font-family:monospace;text-decoration:underline;cursor:pointer;word-break:break-all;">' + pageInfo.referer + '</span>';
    ...
    function showCommandPicker(url, type) {
        ...
        var html = '<div class="uvd-glass-panel" style="max-width:600px;margin:auto;display:flex;flex-direction:column;max-height:80vh;">';
    ...
    function renderSettings(container) {
        container.innerHTML = 
            ...
            '<div class="uvd-card" style="margin-top:10px;font-size:12px;color:var(--text2);">' +
                'Version 3.4 · nguyenquocngu93<br>' +
    ...
    console.log('Universal DL V3.4 mobile optimized ready');
    toast('V3.4 Mobile Ready');
})();