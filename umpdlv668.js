/**
 * Universal Media Player & Downloader - V6.7.7 PRO
 * - Redesign UMP toolbar theo phong cách Video.js (vàng cam, glass, icon trắng)
 * - Video.js control bar chồng lên video
 * - Bỏ footer dung lượng
 * - Thêm nút ẩn toolbar & thoát player
  ...
  .then(function(json) {
    finish(function() {
      if (json && json.status === false) {
        ...
        toast('SubDL: ' + (json.message || 'yêu cầu bị từ chối'));
        cb([]); return;
      }
...
function showSubtitlePanel(video) {
  ...
  panel.innerHTML =
    ...
    '<details style="margin-top:12px;">' +
      ...
      '<div style="font-size:10px;color:var(--text3);margin-top:4px;">Đăng ký free tại subdl.com/panel/api để lấy API key cá nhân.</div>' +
    '</details>' +
...
// ========== VIDEO.JS V10 MOUNT (có fallback) ==========
function __uvdMountVjs10(wrapper, video) {
  ...
  function wrapWithSkin() {
    ...
    try {
      ...
      player.className = 'vjs-fluid';
      player.style.cssText = 'width:100%;height:100%;display:block;';
      var skin = document.createElement('video-skin');
      skin.className = 'vjs-fluid';
      skin.style.cssText = 'width:100%;height:100%;display:block;--media-color-primary:#ffb300;--media-border-radius:16px;';
  ...
  if (!window.__uvdVjs10Loading) {
    ...
    s.onerror = function() { console.error('[UMP DL] Không tải được Video.js v10'); };
...
function showVideoPlayer(url, type) {
  ...
  // ===== TOOLBAR UMP DL – PHONG CÁCH VIDEO.JS =====
  var umpToolbar = document.createElement('div');
  umpToolbar.id = '__uvd_ump_toolbar__';
  umpToolbar.className = 'uvd-ump-toolbar';
  videoWrapper.appendChild(umpToolbar);

  // Các nút chức năng (icon trắng, dạng text)
  var btnConfigs = [
    { id: 'quality', icon: '⚙️', label: '', tooltip: 'Chất lượng' },
    { id: 'speed_down', icon: '⏪', label: '', tooltip: 'Giảm tốc' },
    { id: 'speed_up', icon: '⏩', label: '', tooltip: 'Tăng tốc' },
    { id: 'sleep', icon: '⏰', label: '', tooltip: 'Hẹn giờ ngủ' },
    { id: 'boost', icon: '🔊', label: '', tooltip: 'Boost âm lượng' },
    { id: 'subtitle', icon: '💬', label: '', tooltip: 'Phụ đề' },
    { id: 'screenshot', icon: '📷', label: '', tooltip: 'Chụp màn hình' },
    { id: 'pin', icon: '📌', label: '', tooltip: 'Ghim controls' },
    { id: 'autonext', icon: '⏭', label: '', tooltip: 'Tự động phát tiếp' },
    { id: 'share', icon: '📤', label: '', tooltip: 'Chia sẻ' },
  ];
  if (type === 'MP4' || type === 'MKV' || type === 'WEBM') {
    btnConfigs.push({ id: 'download', icon: '⬇️', label: '', tooltip: 'Tải xuống' });
  }

  var btns = {};
  btnConfigs.forEach(function(cfg) {
    var btn = document.createElement('button');
    btn.className = 'uvd-ump-btn';
    btn.id = '__uvd_ump_' + cfg.id;
    btn.title = cfg.tooltip || '';
    btn.textContent = cfg.icon;
    umpToolbar.appendChild(btn);
    btns[cfg.id] = btn;
  });

  // Separator
  var sep = document.createElement('span');
  sep.className = 'uvd-ump-sep';
  umpToolbar.appendChild(sep);

  // Label tốc độ
  var speedLabel = document.createElement('span');
  speedLabel.id = '__uvd_ump_speed__';
  speedLabel.className = 'uvd-ump-speed';
  speedLabel.textContent = '1x';
  umpToolbar.appendChild(speedLabel);

  // Nút ẩn toolbar
  var hideToolbarBtn = document.createElement('button');
  hideToolbarBtn.className = 'uvd-ump-btn';
  hideToolbarBtn.id = '__uvd_ump_hide__';
  hideToolbarBtn.title = 'Ẩn toolbar';
  hideToolbarBtn.textContent = '▼';
  hideToolbarBtn.onclick = function() {
    umpToolbar.style.display = 'none';
    // Hiện lại bằng cách click vào video (hoặc có thể thêm nút hiện ở góc)
    toast('Toolbar đã ẩn. Click vào video để hiện lại.');
  };
  umpToolbar.appendChild(hideToolbarBtn);

  // Nút thoát player
  var exitBtn = document.createElement('button');
  exitBtn.className = 'uvd-ump-btn';
  exitBtn.id = '__uvd_ump_exit__';
  exitBtn.title = 'Thoát player';
  exitBtn.textContent = '✕';
  exitBtn.onclick = function() {
    closePlayer();
    document.getElementById('__uvd_stream_list__').style.display = 'block';
  };
  umpToolbar.appendChild(exitBtn);

  // ---- GẮN SỰ KIỆN CHO CÁC NÚT ----
  btns.quality.onclick = function() {
    if (playerState.qualities.length > 0) showQualitySubMenu();
    else toast('Không có chất lượng để chọn');
  };
  btns.speed_down.onclick = function() {
    var v = playerState.video;
    if (!v) return;
    var rates = [0.25, 0.5, 0.75, 1, 1.25, 1.5, 2];
    var idx = rates.indexOf(v.playbackRate);
    if (idx > 0) { v.playbackRate = rates[idx-1]; speedLabel.textContent = v.playbackRate + 'x'; }
  };
  btns.speed_up.onclick = function() {
    var v = playerState.video;
    if (!v) return;
    var rates = [0.25, 0.5, 0.75, 1, 1.25, 1.5, 2];
    var idx = rates.indexOf(v.playbackRate);
    if (idx < rates.length-1) { v.playbackRate = rates[idx+1]; speedLabel.textContent = v.playbackRate + 'x'; }
  };
  btns.sleep.onclick = showSleepMenu;
  btns.boost.onclick = function() {
    ...
    btns.boost.classList.toggle('uvd-ump-active', data.settings.volumeBoost);
  };
  btns.boost.classList.toggle('uvd-ump-active', data.settings.volumeBoost);
  btns.subtitle.onclick = function() { showSubtitlePanel(playerState.video); };
  btns.screenshot.onclick = function() {
    var v = playerState.video;
  ...
  function resetHideTimer() {
    ...
    playerState.hideTimeout = setTimeout(function() {
      ...
      umpToolbar.style.opacity = '0';
    }, delay);
  }

  videoWrapper.addEventListener('click', function() {
    // Nếu toolbar đang bị ẩn bởi nút "▼" thì hiện lại
    if (umpToolbar.style.display === 'none') {
      umpToolbar.style.display = '';
      toast('Toolbar hiện lại');
    }
    resetHideTimer();
  });
  ...
  umpToolbar.addEventListener('click', resetHideTimer);
  ...
  function onMetadataLoaded() {
    ...
    if (data.settings.autoFullscreen && !document.fullscreenElement) {
      var fsReq = videoWrapper && (videoWrapper.requestFullscreen || videoWrapper.webkitRequestFullscreen);
...
/* ===== VIDEO.JS CONTROL BAR – NỔI LÊN VIDEO, MÀU VÀNG CAM ===== */
...
video-skin .vjs-button {
  color: #ffb300 !important;
}
video-skin .vjs-button:hover {
  color: #ffca28 !important;
}
video-skin .vjs-slider {
  background: rgba(255, 255, 255, 0.25) !important;
}
video-skin .vjs-slider .vjs-slider-handle {
...
video-skin .vjs-volume-panel .vjs-volume-control {
  background: rgba(0, 0, 0, 0.5);
  border-radius: 8px;
}
video-skin .vjs-quality-selector .vjs-icon-placeholder {
  color: #ffb300 !important;
}
video-skin.vjs-fluid {
  width: 100% !important;
...
function renderSettings(container) {
  ...
  container.innerHTML =
    ...
    '<div class="uvd-card">' +
      ...
      '<div style="font-size:11px;color:var(--text3);margin-top:6px;">Video/audio do chính trang web tự bật sẽ bị tạm dừng ngay. Video mở qua UMP DL Player không bị ảnh hưởng.</div>' +
    '</div>' +

    '<div class="uvd-card">' +
      '<div style="font-weight:600;margin-bottom:8px;">🛡️ Lọc quảng cáo (Filterlist)</div>' +
      '<div style="font-size:12px;color:var(--text2);margin-bottom:8px;">' +
        'Nhập mỗi dòng một từ khóa hoặc domain (vd: <code>doubleclick.net</code>). Hỗ trợ regex nếu bắt đầu bằng <code>regex:</code>.' +
      '</div>' +
    ...
    '<div class="uvd-card uvd-timeline-card">' +
      '<div class="uvd-step"><span class="uvd-step-num">1</span><span class="uvd-step-text">Mở trang web, bấm vào ⭐ Bookmark trên thanh địa chỉ.</span></div>' +
      ...
      '<div class="uvd-callout"><span class="uvd-callout-icon">💡</span><span>Từ lần sau, gõ tên bookmark (<strong style="color:var(--accent);">UMP DL</strong>) vào thanh địa chỉ và chọn nó để kích hoạt.</span></div>' +
    '</div>' +

    '<div class="uvd-section-title"><span class="uvd-section-num">2</span> Sử dụng</div>' +
    '<div class="uvd-card uvd-timeline-card">' +
      '<div class="uvd-step"><span class="uvd-step-num">•</span><span class="uvd-step-text">Mở trang video, gõ tên bookmark và chọn</span></div>' +
      ...
      '<div class="uvd-step"><span class="uvd-step-num">•</span><span class="uvd-step-text">Toolbar UMP DL nằm ở góc trên phải, control bar Video.js ở dưới video</span></div>' +
      '<div class="uvd-step"><span class="uvd-step-num">•</span><span class="uvd-step-text">Chạm đúp trái/phải để tua nhanh</span></div>' +
      '<div class="uvd-callout"><span class="uvd-callout-icon">▶</span><span>Nút <strong style="color:var(--accent);">▶</strong> trên header script giúp tự động bấm Play để lấy link.</span></div>' +
    '</div>' +
...
console.log('V' + VERSION + ' UMP DL PRO - Redesigned UI');
