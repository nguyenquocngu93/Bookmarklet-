...
function __uvdShowHlsLogPanel() {
  var old = document.getElementById('__uvd_hls_log_panel__');
  if (old) { old.remove(); return; }
  var text = __uvdFormatHlsLog();
  try { copy(text); } catch(e) {}
  var panel = document.createElement('div');
  panel.id = '__uvd_hls_log_panel__';
  panel.style.cssText = 'position:fixed;left:8px;right:8px;top:44px;bottom:60px;z-index:2147483647;overflow:auto;padding:10px 12px;border-radius:12px;background:rgba(10,10,10,.96);color:#0f0;font:11px/1.4 monospace;box-shadow:0 8px 26px rgba(0,0,0,.6);white-space:pre-wrap;word-break:break-all;';
  var closeBar = document.createElement('div');
  closeBar.textContent = '[đóng] (đã copy log vào clipboard)';
  closeBar.style.cssText = 'color:#f66;font-weight:bold;margin-bottom:8px;position:sticky;top:0;background:rgba(10,10,10,.96);padding-bottom:6px;';
  closeBar.onclick = function() { panel.remove(); };
  panel.appendChild(closeBar);
  var body = document.createElement('div');
  body.textContent = text;
  panel.appendChild(body);
  (document.body || document.documentElement).appendChild(panel);
  // Video HTML5 trên Chrome Android có thể bị đứng hình khi gặp alert()/confirm()
  // chặn luồng chính, nên panel này chỉ là DOM thường — không chặn playback.
  var watcher = setInterval(function() {
    if (!document.body.contains(panel)) { clearInterval(watcher); return; }
    if (!document.getElementById('__uvd_hls_log_panel__')) document.body.appendChild(panel);
  }, 1000);
}
...
var __uvdM3u8Cache = new Map();
function __uvdCacheM3u8(url, body) {
  try {
    if (!url || !body || body.indexOf('#EXTM3U') === -1) return;
    __uvdM3u8Cache.set(url, { text: body, time: Date.now() });
    if (__uvdM3u8Cache.size > 20) {
      var oldestKey = __uvdM3u8Cache.keys().next().value;
      __uvdM3u8Cache.delete(oldestKey);
    }
  } catch(e) {}
}
function __uvdGetCachedM3u8(url) {
  var entry = __uvdM3u8Cache.get(url);
  // Chỉ dùng cache trong 30s đầu — tránh phát video cũ/lỗi thời nếu server có đổi nội dung.
  if (entry && (Date.now() - entry.time) < 30000) return entry.text;
  return null;
}
...
function __uvdPauseSuspiciousVideos(root, depth) {
  // Giống pauseAllPlayingVideos nhưng bỏ qua video đang phát thật (có tiến
  // trình phát rõ ràng) — chỉ pause video vừa mới autoplay (currentTime ~0).
  root = root || document;
  depth = depth || 0;
  var pausedCount = 0;
  try {
    root.querySelectorAll('video').forEach(function(v) {
      if (!v.paused && v.currentTime <= 0.3) {
  ...
  if (depth < 2) {
    try {
      root.querySelectorAll('iframe').forEach(function(f) {
        try { if (f.contentDocument) pausedCount += __uvdPauseSuspiciousVideos(f.contentDocument, depth + 1); }
        catch(e) {}
      });
    } catch(e) {}
  }
...
function __uvdPausePageAfterMediaFound() {
  ...
  // Script đang ẩn = người dùng cố ý dùng UVD chỉ để chặn popup và xem video
  // gốc bằng UI của trang — không được đụng vào playback lúc này.
  if (typeof __uvdScriptHidden !== 'undefined' && __uvdScriptHidden) return;
  clearTimeout(__uvdMediaPauseTimer);
  __uvdMediaPauseTimer = setTimeout(function() {
    try { __uvdPauseSuspiciousVideos(); } catch(e) {}
    setTimeout(function() { try { __uvdPauseSuspiciousVideos(); } catch(e) {} }, 700);
  }, 80);
}
      ...
      try {
        ...
        if (response && response.clone && inspectResponseBody) {
          response.clone().text().then(function(body) {
            ...
            __uvdCacheM3u8(responseUrl, body);
          }).catch(function() {});
        }
      } catch(e) {}
    }).catch(function() {});
    ...
    if (!this.__uvdBodyHooked) {
      ...
      this.addEventListener('load', function() {
        try {
          ...
          if (this.responseURL) __uvdCacheM3u8(this.responseURL, xhrBody);
        } catch(e) {}
      });
    }
...
function __uvdNeutralizeMedia(el) {
  ...
  // Không đụng vào video đang thực sự phát dở (đã có tiến trình phát thật,
  // không phải vừa mới autoplay) — tránh pause nhầm video người dùng đang xem
  // ngay lúc script vừa load.
  if (!el.paused && el.currentTime > 0.3) return;
  try {
  ...
  if (!HlsCtor.__uvdPngStripLoader) {
    HlsCtor.__uvdPngStripLoader = class UvdPngStripLoader extends BaseLoader {
      load(context, config, callbacks) {
        // Nếu chính trang gốc đã tải thành công đúng URL này (m3u8) trước đó,
        // dùng lại nội dung đó thay vì gọi mạng lần 2 — nhiều server "hlsmod"
        // coi request lần 2 là dùng-1-lần/hết hạn và trả 404 giả.
        var cachedText = context && context.url ? __uvdGetCachedM3u8(context.url) : null;
        if (cachedText != null) {
          var now = performance.now();
          setTimeout(function() {
            try {
              callbacks.onSuccess(
                { url: context.url, data: cachedText },
                { trequest: now, tfirst: now, tload: now, loaded: cachedText.length, total: cachedText.length },
                context,
                null
              );
            } catch(e) {}
