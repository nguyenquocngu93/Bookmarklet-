/**
 * Universal Media Player & Downloader - V6.7.2 PRO
...
// === SỬA LỖI SubDL (search + xử lý lỗi) ===
function searchSubDL(query, cb) {
  var apiKey = (data.settings.subdlApiKey || '').trim();
  if (!apiKey) {
    toast('Chưa có SubDL API Key, xem hướng dẫn trong bảng Phụ đề');
    cb([], { message: 'missing_key' });
    return;
  }
  var controller = new AbortController();
  var timeout = setTimeout(function() { controller.abort(); }, 15000);
  ...
  .then(function(json) {
    var results = (json && (json.results || json.subtitles || json.data)) || [];
    cb(results, null);
  })
  .catch(function(err) {
    clearTimeout(timeout);
    console.error('SubDL error:', err);
    var msg = err.message || 'network_error';
    if (msg.includes('abort')) msg = 'timeout';
    cb([], { message: msg });
  });
}
...
function showSubtitlePanel(video) {
  ...
  // === SỬA LỖI: xử lý kết quả tìm kiếm + hiển thị lỗi rõ ràng ===
  panel.querySelector('#__uvd_sub_search__').onclick = function() {
    ...
    searchSubDL(q, function(list, err) {
      if (err) {
        if (err.message === 'missing_key') {
          box.innerHTML = '<div style="font-size:11px;color:#ffb84d;padding:8px 0;">⚠️ Chưa nhập SubDL API Key. Vui lòng nhập ở phần "API Key SubDL" bên dưới.</div>';
        } else if (err.message === 'timeout') {
          box.innerHTML = '<div style="font-size:11px;color:#ff5d72;padding:8px 0;">⏱️ Kết nối tới SubDL quá lâu. Vui lòng thử lại.</div>';
        } else if (err.message && err.message.includes('HTTP')) {
          box.innerHTML = '<div style="font-size:11px;color:#ff5d72;padding:8px 0;">❌ Lỗi HTTP từ SubDL. Kiểm tra API Key hoặc thử lại sau.</div>';
        } else {
          box.innerHTML = '<div style="font-size:11px;color:#ff5d72;padding:8px 0;">❌ Không thể tìm phụ đề (CORS / mạng). Nếu dùng Chrome Android, hãy thử cài extension "CORS Unblock" hoặc dùng proxy.</div>';
        }
        return;
      }
      if (!list.length) {
        box.innerHTML = '<div style="font-size:11px;color:var(--text3);padding:8px 0;">Không tìm thấy kết quả.</div>';
        return;
      }
      box.innerHTML = '';
...
// ========== CSS (giữ nguyên UI, sửa lỗi thu nhỏ) ==========
...
/* ===== SỬA LỖI THU NHỎ: giữ blur nhẹ + overflow hidden ===== */
.uvd-panel-minimized.uvd-glass-panel{backdrop-filter:blur(6px) saturate(130%)!important;-webkit-backdrop-filter:blur(6px) saturate(130%)!important;background:rgba(10,11,16,0.95)!important;overflow:hidden!important}
