// ==UserScript==
// @name         DevLib CDN Player - Glass UI Final v5
// @namespace    http://tampermonkey.net/
// @version      5.0
// @description  Video title from page + compact menu
// @author       You
// @match        *://*/*
// @grant        none
...
(function () {
    ...
    const CSS = `
        ...
        .dlp-icon-btn.primary { background: #8b5cf6; border-color: #8b5cf6; }
        ...
        .dlp-action-bar .dlp-act:hover { transform: translateY(-1px); }
        ...
        document.body.insertAdjacentHTML('beforeend', `
            ...
            <div id="dlp-prev">
                <div id="dlp-prev-bar">
                    ...
                    <button class="dlp-icon-btn" id="dlp-btn-opt" title="Menu">⋮</button>
    ...
    function initLogic() {
        ...
        function openPrev(item) {
            ...
            // Compact action bar - chỉ 3 nút chính
        ...
        function showMainMenu(e) {
            ...
            // Bind all menu items
            $('dlp-m-cp').onclick = () => { cp(cur.url); d.classList.remove('on'); };
            $('dlp-m-share').onclick = () => {
                if (navigator.share) {
            ...
            $('dlp-m-dl').onclick = () => { openCmd(cur.url); d.classList.remove('on'); };
            $('dlp-m-bm').onclick = () => {
            ...
            $('dlp-m-qs').onclick = () => {
                ...
                fetch(cur.url).then(r => r.text()).then(text => {
                    ...
                    const d2 = $('dlp-drop');
                    d2.innerHTML = `<div style="padding:10px 16px;color:rgba(255,255,255,0.4);font-size:11px;font-weight:700;">QUALITY</div>` +
                        qs.map(q => `<div class="dlp-di" data-url="${q.url}">${q.label}</div>`).join('');
                    d2.classList.add('on');
                    d2.querySelectorAll('.dlp-di').forEach(qi => qi.onclick = () => {
        ...
        document.addEventListener('click', (e) => {
            const d = $('dlp-drop');
            if (d.classList.contains('on') && !d.contains(e.target) && e.target.id !== 'dlp-pc-menu' && e.target.id !== 'dlp-btn-opt') {
                d.classList.remove('on');
            }
        });
