/**
Universal Media Player & Downloader - V5.7 (Fixed & Optimized)
Overlay player với toolbar
Chọn chất lượng, tốc độ, fullscreen, thu nhỏ
Hiệu ứng thu nhỏ mượt mà (GPU Accelerated)
Giữ referer, hỗ trợ HLS
Tự động bấm nút Play cho các site lazy-load (eporner, JAV site...)
Chặn popup/tab lạ khi mô phỏng click; không tự play video ngoài ý muốn
Tự pause video gốc sau khi đã lấy được link, tránh phát nền song song
...
function lockOrientation(video) {
  ...
  try {
    if (screen.orientation && screen.orientation.lock) screen.orientation.lock(target).catch(function(e){});
    else if (screen.lockOrientation) screen.lockOrientation(target);
    else if (screen.mozLockOrientation) screen.mozLockOrientation(target);
    else if (screen.msLockOrientation) screen.msLockOrientation(target);
  } catch(e) {}
}
...
var playerState = {
  ...
  qualities: [], currentQuality: 0, speed: 1, isMinimized: false,
  url: '', type: '', resolution: '', onFullscreenChange: null, menuOpen: false
};
...
function showVideoPlayer(url, type) {
  ...
  footer.innerHTML = '<span id="uvd_player_status">Đang tải...</span><span id="uvd_player_time"></span>';
  ...
  function showQualitySubMenu() {
    ...
    sel.onchange = function() {
      ...
      if (q && playerState.hls) {
        ...
        playerState.resolution = q.label;
        ...
        if (infoDiv) {
          infoDiv.innerHTML = '<div style="font-weight:600;font-size:14px;color:#fff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">▶ ' + pageInfo.title + '</div>' +
            '<div style="font-size:11px;color:#aaa;margin-top:2px;">' + playerState.type + ' · ' + q.label + '</div>';
        }
      }
      overlay2.remove();
    };
...
function minimizePlayer() {
  ...
  mini.style.cssText = 'position:fixed;bottom:20px;right:20px;width:160px;height:90px;background:#000;border-radius:12px;z-index:2147483647;cursor:pointer;box-shadow:0 8px 30px rgba(0,0,0,0.8);border:2px solid rgba(255,255,255,0.2);overflow:hidden;transition: all 0.4s cubic-bezier(0.4, 0, 0.2, 1);';
  var canvas = document.createElement('canvas');
  canvas.width = 160; canvas.height = 90;
  var ctx = canvas.getContext('2d');
  ctx.fillStyle = '#000'; ctx.fillRect(0, 0, 160, 90);
  if (video.videoWidth) ctx.drawImage(video, 0, 0, 160, 90);
  ...
  overlay.style.transition = 'transform 0.4s cubic-bezier(0.4,0,0.2,1), opacity 0.4s ease';
  overlay.style.transform = 'scale(0.8) translateZ(0)';
...
function restorePlayer() {
  ...
  overlay.style.willChange = 'transform, opacity';
  overlay.style.transition = 'transform 0.4s cubic-bezier(0.4,0,0.2,1), opacity 0.4s ease';
  overlay.style.transform = 'scale(1) translateZ(0)';
  overlay.style.opacity = '1';
  setTimeout(function() { overlay.style.willChange = 'auto'; }, 400);
...
.uvd-glass-panel {
  ...
  font-size:var(--fs-base); padding:16px; width:100%; position:relative; overflow:hidden;
...
.uvd-liquid-bg {
  ...
  filter: blur(50px); animation: uvdLiquidDrift 16s ease-in-out infinite;
}
...
.uvd-btn {
  ...
  box-shadow:0 3px 10px rgba(0,0,0,0.4), 0 1px 0 rgba(255,255,255,0.08) inset; line-height:1.3;
}
.uvd-btn:hover {
...
.uvd-card:hover {
  ...
  background:rgba(255,255,255,0.05); border-color:rgba(109,140,255,0.25);
}
.uvd-type-badge {
...
#uvd_min_float {
  ...
  transition: transform 0.3s; animation:uvdFloatBtnIn 0.3s ease; backdrop-filter:blur(10px);
}
...
.uvd-profile-avatar {
  ...
  box-shadow:0 6px 18px rgba(109,140,255,0.4), 0 0 0 3px rgba(255,255,255,0.08); letter-spacing:0.02em;
}
...
function buildUI() {
  ...
  header.innerHTML =
    ...
    '<div style="display:flex;gap:6px;">' +
      '<button class="uvd-btn-icon uvd-ripple-btn" id="uvd_autoplay" title="Tự động bấm Play"><span style="font-size:15px;">▶</span></button>' +
      '<button class="uvd-btn-icon uvd-ripple-btn" id="uvd_minimize" title="Thu nhỏ"><span style="font-size:18px;">−</span></button>' +
      '<button class="uvd-btn-icon uvd-ripple-btn" id="uvd_refresh" title="Làm mới"><span style="font-size:16px;">↻</span></button>' +
      '<button class="uvd-btn-icon uvd-ripple-btn" id="uvd_close" title="Đóng"><span style="font-size:16px;">×</span></button>' +
    '</div>';
  ...
  info.innerHTML =
    ...
    '<span id="uvd_title" style="color:var(--accent);text-decoration:underline;cursor:pointer;">' + pageInfo.title + '</span> ' +
    '<span style="color:var(--text3);">(sửa)</span> <br>' +
    '<span style="color:var(--text2);">Referer: </span>' +
    '<span id="uvd_referer" style="color:var(--accent2);font-family:monospace;text-decoration:underline;cursor:pointer;font-size:11px;">' + pageInfo.referer + '</span> <br>' +
    '<span style="color:var(--text2);">Play selector: </span>' +
    '<span id="uvd_playsel" style="color:var(--accent2);font-family:monospace;text-decoration:underline;cursor:pointer;font-size:11px;">' + (savedPlaySel || '(chưa đặt · bấm để thêm)') + '</span>';
  ...
  document.getElementById('uvd_minimize').onclick = function() {
    ...
    panelEl.style.transition = 'transform 0.4s cubic-bezier(0.4,0,0.2,1), opacity 0.4s ease';
    panelEl.style.transform = 'scale(0.7) translateZ(0)';
    panelEl.style.opacity = '0';
    setTimeout(function() {
      panelEl.style.display = 'none';
      panelEl.style.willChange = 'auto';
      var floatBtn = document.getElementById('uvd_min_float');
      if (!floatBtn) {
        floatBtn = document.createElement('button');
        floatBtn.id = 'uvd_min_float';
        ...
        floatBtn.onclick = function() {
          ...
          panelEl.style.willChange = 'transform, opacity';
          panelEl.style.transition = 'transform 0.4s cubic-bezier(0.4,0,0.2,1), opacity 0.4s ease';
          panelEl.style.transform = 'scale(1) translateZ(0)';
          panelEl.style.opacity = '1';
          setTimeout(function() { panelEl.style.willChange = 'auto'; }, 400);
          floatBtn.remove();
        };
...
function showEditor(text) {
  ...
  overlay.innerHTML =
    ...
    '<div class="uvd-grid-2" style="margin-top:12px;">' +
      '<button class="uvd-btn uvd-btn-sm" id="uvd_ed_copy">Sao chép</button>' +
      '<button class="uvd-btn uvd-btn-sm" id="uvd_ed_share" style="background:rgba(139,92,246,0.3);">Chia sẻ</button>' +
    '</div>' +
...
function renderHistory(container) {
  ...
  container.innerHTML = '<button class="uvd-btn uvd-btn-sm" id="uvd_clear_hist" style="width:100%;margin-bottom:10px;background:var(--danger);">Xóa toàn bộ lịch sử</button>';
...
function renderSettings(container) {
  ...
  container.innerHTML =
    ...
    '<div class="uvd-card">' +
      ...
      '<button class="uvd-btn uvd-btn-sm" id="uvd_backup" style="width:100%;margin-bottom:6px;">Xuất dữ liệu</button>' +
      '<button class="uvd-btn uvd-btn-sm" id="uvd_restore" style="width:100%;margin-bottom:6px;">Nhập dữ liệu</button>' +
      '<button class="uvd-btn uvd-btn-sm" id="uvd_reset" style="width:100%;background:var(--danger);">Đặt lại tất cả</button>' +
    '</div>' +
...
console.log('V5.7 UMP DL - Fixed & Optimized Ready');
toast('V5.7 sẵn sàng!');
})();