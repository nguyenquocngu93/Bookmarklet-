...
.uvd-app-shell .uvd-tab{color:#5d7f7a!important;font-weight:700}
...
function buildStreamCardHTML(item, i) {
  if (String(item.type || '').toUpperCase() === 'IFRAME') {
    return '<div class="uvd-card uvd-iframe-card" data-type="IFRAME" data-url="' + escapeHtml(item.url) + '">' +
      ...
      '<div class="uvd-iframe-actions"><button class="uvd-btn uvd-btn-sm" data-action="iframe-window" data-url="' + encodeURIComponent(item.url) + '">↗ Mở cửa sổ mới</button><button class="uvd-btn uvd-btn-sm" data-action="iframe" data-url="' + encodeURIComponent(item.url) + '">Mở iframe</button><button class="uvd-btn uvd-btn-sm" data-action="copy" data-url="' + encodeURIComponent(item.url) + '">Sao chép</button></div>' +
      '</div>';
  }
  var actionsHtml;
  if (item.type === 'BLOB') {
