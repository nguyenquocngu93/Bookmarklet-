 ...
 * - FIX QUAN TRỌNG: "Quét server" lỡ bấm trúng link điều hướng thật (vd link
 *   "diễn viên") làm bay khỏi trang. Nguyên nhân: bản trước chỉ kiểm tra khi
 *   CHÍNH phần tử bấm là thẻ <a> — bỏ sót trường hợp phần tử (ảnh/span/chữ)
 *   nằm LỒNG BÊN TRONG một <a href>, click vẫn nổi bọt lên thẻ cha và điều
 *   hướng bình thường. Giờ dò cả tổ tiên gần nhất là <a href> thật.
 * - THÊM AN TOÀN TUYỆT ĐỐI: trong suốt lúc tự bấm, khoá cứng mọi đường có thể
 *   rời trang dù lỡ trúng gì — chặn default của mọi <a href> thật (click bắt
 *   ở capture phase), vô hiệu hoá tạm history.pushState/replaceState và
 *   location.assign/replace, cộng thêm beforeunload làm lưới an toàn cuối.
 *   Khôi phục lại y nguyên ngay sau khi quét xong (hoặc tự mở khoá sau timeout
 *   phòng lỗi). KHÔNG áp dụng cho nút "Mở tab mới" của IFRAME — vẫn mở bình
 *   thường như trước vì đó là hành động cố ý của người dùng.
 * - MỚI: tự động chạy "Quét server" nếu 2.5s sau khi tải trang mà chưa tìm
 *   thấy link media nào — đỡ phải bấm tay nút 🖥 trên các site "chọn server".
 * - MỚI: thêm selector `button[id^="video-host-"]` cho nút Play (site JAV
 *   dùng id kiểu này khá phổ biến).
 * - Icon header đồng bộ (SVG line-icon cùng nét/size/màu).
...
function autoClickServersAndScan() {
  ...
  function isRealNavAnchor(el) {
    ...
    try {
      ...
      if (abs.pathname !== curPath) return true;
    } catch (e) {}
    return false;
  }
