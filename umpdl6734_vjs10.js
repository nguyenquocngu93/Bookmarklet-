/**
 * Universal Media Player & Downloader - V6.7.24 PRO (video dọc lấp khung, bỏ hiệu ứng trượt lag)
...
function showVideoPlayer(url, type) {
  ...
  function __uvdApplyPlayerLayout() {
    ...
    if (fs) {
      ...
      // Video dọc: không bo góc, giãn lấp đầy khung có lề đều 4 phía
      videoWrapper.style.padding = PORTRAIT_INSET;
...
.uvd-settings-sheet{width:100%;max-width:1000px;max-height:92dvh;display:flex;flex-direction:column;border-radius:32px 32px 0 0;overflow:hidden;background:var(--glass);backdrop-filter:blur(var(--uvd-blur)) saturate(130%);-webkit-backdrop-filter:blur(var(--uvd-blur)) saturate(130%);border:1px solid var(--border);box-shadow:0 -20px 50px rgba(0,0,0,0.8)}
