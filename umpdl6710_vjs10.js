/**
 * Universal Media Player & Downloader - V6.7.10 PRO
 * - MỚI: khi CẢ nút ▶ lẫn ⏭ đều không khớp được selector chuẩn nào (0 nút),
 *   tự động rơi về tầng dự phòng: đoán nút server theo TEXT (chữ server/
 *   stream/host, tên host phổ biến, hoặc nhãn viết tắt in hoa kiểu VI/MD/DS/
 *   ST). ▶ sẽ bấm HẾT các nút đoán được (đã có chặn popup toàn cục từ INIT
 *   nên an toàn), ⏭ vẫn bấm lần lượt từng nút. Mọi nút bấm kiểu đoán được gắn
 *   badge "🔍 đoán" trong tab "Nút đã click" để rà lại, bỏ bớt nút sai.
 * - FIX: tabbar đã có overflow-x:auto sẵn nhưng .uvd-tab dùng flex:1 khiến các
 *   tab bị ép co chật lại thay vì tràn ra cho cuộn ngang — đổi sang flex:0 0
 *   auto để tab giữ đúng độ rộng chữ, tabbar tự cuộn được khi nhiều tab.
 * - MỚI (bản trước): nút ⏭ thử lần lượt từng server tới khi ra link; tab "Nút
 *   đã click" ghi + chặn theo từng nút cụ thể.
 * - FIX hiệu năng/giật lag trên site nặng quảng cáo (panelObserver theo dõi sai
 *   phạm vi, observer chặn tự phát xử lý không gộp khung hình, ripple listener
 *   gắn lên toàn document, animation nền không tạm dừng khi ẩn tab).
 * - FIX auto-click: quét lại nhiều lần trong ~4s sau khi bấm. Lưu ý: click giả
 *   lập luôn isTrusted=false, vài site chặn theo cờ này — không script nào giả
 *   mạo được, có thể vẫn phải bấm tay ở site khó.
 * - FIX: header panel giật/nhìn như "trôi theo" trang khi cuộn trên Chrome Android.
 * - FIX: khôi phục lại backdrop-filter (kính mờ) khi thu nhỏ panel.
