/**
 * Universal Media Player & Downloader - V6.8.1 PRO
 * - FIX: bỏ hẳn việc mount skin video.js v10 (<video-player>/<video-skin>) cho
 *   player thẻ nổi — component này tự vẽ thêm 1 thanh điều khiển riêng, gây
 *   trùng thanh tua, tràn/dính ra ngoài thẻ bo góc, và chặn click nút Đóng.
 *   Giờ dùng thẳng <video> gốc + UI tối giản tự viết (nút play/pause riêng
 *   được thêm lại cạnh thanh tua vì không còn nút play của skin cũ).
 * - MỚI: thêm lớp nền tối mờ (blur) phía sau thẻ video nổi để dễ tập trung,
 *   bấm ra ngoài nền mờ sẽ đóng menu tính năng nếu đang mở.
 * - V6.8.0: Player dạng thẻ nổi bo góc (không còn phủ đen toàn màn hình) —
 *   chỉ còn video + thanh tua, nổi giữa trang web; gộp toolbar cũ vào 1 nút
 *   menu (⋮) duy nhất ở góc phải trên video; tự ẩn UI chính (__uvd__) khi
 *   đang phát video.
...
var oldPlayerOverlay = document.getElementById('__uvd_player_overlay__');
if (oldPlayerOverlay) oldPlayerOverlay.remove();
var oldPlayerBackdrop = document.getElementById('__uvd_player_backdrop__');
if (oldPlayerBackdrop) oldPlayerBackdrop.remove();
var oldPlayerMini = document.getElementById('__uvd_player_mini__');
if (oldPlayerMini) oldPlayerMini.remove();
...
function showVideoPlayer(url, type) {
  ...
  // Ẩn UI chính của script khi video đang phát (chỉ hiện lại khi đóng player)
  var __uvdMainPanel = document.getElementById('__uvd__');
  if (__uvdMainPanel) {
    playerState._mainPanelPrevDisplay = __uvdMainPanel.style.display || '';
    __uvdMainPanel.style.display = 'none';
  }

  var backdrop = document.createElement('div');
  backdrop.id = '__uvd_player_backdrop__';
  backdrop.style.cssText = 'position:fixed;inset:0;background:rgba(2,3,6,0.72);backdrop-filter:blur(6px) saturate(110%);-webkit-backdrop-filter:blur(6px) saturate(110%);z-index:2147483647;animation:uvdFadeIn 0.25s ease;';
  __uvdAppendRoot(backdrop);
  playerState.backdrop = backdrop;
  backdrop.onclick = function() { toggleMenu(false); };

  var overlay = document.createElement('div');
  overlay.id = '__uvd_player_overlay__';
  overlay.style.cssText = 'position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);width:min(94vw,560px);aspect-ratio:16/9;max-height:80vh;background:#000;z-index:2147483648;border-radius:18px;overflow:hidden;box-shadow:0 24px 70px rgba(0,0,0,0.65),0 0 0 1px rgba(255,255,255,0.08);animation:uvdScaleIn 0.3s ease;';
  __uvdAppendRoot(overlay);
  playerState.overlay = overlay;
  applyEffectsPref(overlay);
  applyMotionPref(overlay);

  // NÚT MENU DUY NHẤT Ở GÓC PHẢI TRÊN VIDEO
  var cornerBtn = document.createElement('button');
  cornerBtn.id = '__uvd_player_menu_btn__';
  cornerBtn.style.cssText = 'position:absolute;top:10px;right:10px;width:34px;height:34px;border-radius:50%;background:rgba(20,22,30,0.55);backdrop-filter:blur(6px);-webkit-backdrop-filter:blur(6px);border:1px solid rgba(255,255,255,0.18);color:#fff;font-size:18px;line-height:1;cursor:pointer;z-index:7;display:flex;align-items:center;justify-content:center;transition:opacity 0.3s ease;';
  cornerBtn.textContent = '⋮';
  cornerBtn.title = 'Menu tính năng';

  // HEADER (nay là bảng menu tính năng thả xuống, ẩn mặc định, mở bằng nút góc phải)
  var header = document.createElement('div');
  header.id = '__uvd_player_header__';
  header.style.cssText = 'position:absolute;top:50px;right:10px;width:min(82%,300px);max-height:calc(100% - 60px);overflow-y:auto;padding:12px 14px;background:rgba(14,16,22,0.96);border-radius:14px;border:1px solid rgba(255,255,255,0.15);box-shadow:0 12px 34px rgba(0,0,0,0.55);z-index:7;display:none;';
  ...
  overlay.appendChild(cornerBtn);

  var menuOpen = false;
  function toggleMenu(forceState) {
    menuOpen = (typeof forceState === 'boolean') ? forceState : !menuOpen;
    header.style.display = menuOpen ? 'block' : 'none';
  }
  ...
  videoWrapper.style.cssText = 'position:absolute;inset:0;display:flex;align-items:center;justify-content:center;background:#000;overflow:hidden;z-index:1;';
  ...
  // Không mount skin video.js v10 nữa trong chế độ thẻ nổi: component này tự vẽ
  // thêm 1 thanh điều khiển riêng (gây trùng thanh tua, tràn ra ngoài thẻ bo góc,
  // và chặn click nút đóng). Dùng thẳng <video> + UI tối giản tự viết là đủ.

  // FOOTER: chỉ còn thanh tua (seek bar) + thời gian, bỏ hết thông tin phụ (status, dung lượng...)
  var footer = document.createElement('div');
  footer.id = '__uvd_player_footer__';
  footer.style.cssText = 'position:absolute;left:0;right:0;bottom:0;padding:8px 12px 10px;background:linear-gradient(to top, rgba(0,0,0,0.75), transparent);display:flex;align-items:center;gap:8px;z-index:6;transition: opacity 0.3s ease;';
  footer.innerHTML =
    '<button id="__uvd_playpause_btn__" style="flex-shrink:0;width:28px;height:28px;border-radius:50%;background:rgba(255,255,255,0.16);border:none;color:#fff;font-size:12px;cursor:pointer;display:flex;align-items:center;justify-content:center;">▶</button>' +
    '<span id="__uvd_player_time__" style="color:#fff;font-size:11px;min-width:36px;text-align:right;cursor:pointer;">00:00</span>' +
    '<div id="__uvd_seek_track__" style="flex:1;height:14px;display:flex;align-items:center;cursor:pointer;">' +
      '<div style="width:100%;height:4px;background:rgba(255,255,255,0.28);border-radius:2px;position:relative;">' +
        '<div id="__uvd_seek_fill__" style="position:absolute;left:0;top:0;bottom:0;width:0%;background:var(--accent);border-radius:2px;"></div>' +
        '<div id="__uvd_seek_thumb__" style="position:absolute;top:50%;left:0%;width:11px;height:11px;background:#fff;border-radius:50%;transform:translate(-50%,-50%);box-shadow:0 1px 4px rgba(0,0,0,0.5);"></div>' +
      '</div>' +
    '</div>';
  overlay.appendChild(footer);

  var playPauseBtn = document.getElementById('__uvd_playpause_btn__');
  function syncPlayPauseIcon() { playPauseBtn.textContent = video.paused ? '▶' : '❚❚'; }
  ...
  // Chạm 1 lần vào giữa video cũng để play/pause (không đụng vùng double-tap tua 2 bên)
  ...
  var seekTrack = document.getElementById('__uvd_seek_track__');
  var seekFill = document.getElementById('__uvd_seek_fill__');
  var seekThumb = document.getElementById('__uvd_seek_thumb__');
  ...
  if (seekTrack) {
    var __seeking = false;
    seekTrack.addEventListener('click', function(e) { seekToClientX(e.clientX); });
    ...
    seekTrack.addEventListener('touchmove', function(e) {
      if (__seeking && e.touches[0]) seekToClientX(e.touches[0].clientX);
    }, { passive: true });
    seekTrack.addEventListener('touchend', function() { __seeking = false; });
  }
  ...
  function updateTimeDisplay() {
    ...
    if (seekFill && seekThumb && !__seeking) {
      var pct = Math.min(100, Math.max(0, (t / d) * 100));
...
function minimizePlayer() {
  ...
  if (playerState.backdrop) playerState.backdrop.style.opacity = '0';
  overlay.style.transition = 'opacity 0.25s ease';
  overlay.style.opacity = '0';
  setTimeout(function() {
    overlay.style.display = 'none';
    if (playerState.backdrop) playerState.backdrop.style.display = 'none';
  }, 260);
...
function restorePlayer() {
  ...
  if (playerState.backdrop) {
    playerState.backdrop.style.display = 'block';
    playerState.backdrop.style.opacity = '1';
  }
...
function closePlayer() {
  if (playerState.overlay) {
    ...
    if (playerState.backdrop) { playerState.backdrop.remove(); playerState.backdrop = null; }
    ...
    var __uvdMainPanelRestore = document.getElementById('__uvd__');
    if (__uvdMainPanelRestore) __uvdMainPanelRestore.style.display = playerState._mainPanelPrevDisplay || '';
    applyMotionPref(__uvdMainPanelRestore);
    storage.set(data);
  }
}
