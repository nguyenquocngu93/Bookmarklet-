/**
 * Universal Media Player & Downloader - V6.8.2 PRO
 * - FIX: player thẻ nổi bị "trôi" xuống góc sau khi cuộn trang — do neo giữa
 *   màn hình bằng top:50% (tính theo layout viewport, đổi theo khi Chrome
 *   co giãn thanh địa chỉ lúc cuộn). Đổi sang neo cứng theo mép trên bằng px
 *   cố định (giống panel chính), ổn định hơn nhiều trên Chrome Android.
 * - THEO YÊU CẦU: dùng lại thanh điều khiển gốc của skin video.js v10 (play,
 *   tua 10s, volume, gear, fullscreen, thanh tua...) thay vì thanh tua tự vẽ
 *   tối giản — bỏ hẳn thanh tua/nút play tự chế để không còn bị lặp 2 thanh.
 *   Nút ⋮ góc phải trên video vẫn giữ để mở nhanh các tính năng phụ (chất
 *   lượng, tốc độ, PiP, hẹn giờ, boost, phụ đề, tải xuống...).
 * - V6.8.1: bỏ mount skin v10 (đã revert ở bản này theo phản hồi).
 * - V6.8.0: Player dạng thẻ nổi bo góc, nổi giữa trang web, nền tối mờ phía
 *   sau; tự ẩn UI chính (__uvd__) khi đang phát video.
...
function showVideoPlayer(url, type) {
  ...
  // Neo theo mép trên bằng giá trị px cố định (không dùng top:50%) vì top:50% tính
  // theo layout viewport — khi Chrome co giãn thanh địa chỉ lúc cuộn trang, layout
  // viewport đổi chiều cao khiến điểm 50% nhảy theo => player bị "trôi". Neo cứng
  // theo mép trên ổn định hơn nhiều trên Chrome Android.
  overlay.style.cssText = 'position:fixed;top:64px;left:50%;transform:translateX(-50%);width:min(94vw,560px);aspect-ratio:16/9;max-height:min(70vh,70dvh);background:#000;z-index:2147483648;border-radius:18px;overflow:hidden;box-shadow:0 24px 70px rgba(0,0,0,0.65),0 0 0 1px rgba(255,255,255,0.08);animation:uvdScaleIn 0.3s ease;';
  ...
  // Dùng lại skin video.js v10 (nút điều khiển đẹp, có sẵn thanh tua/thời gian) —
  // không tự vẽ thêm thanh tua/nút play riêng nữa để khỏi bị trùng/lặp 2 thanh.
  __uvdMountVjs10(videoWrapper, video);

  // ===== AUTO-HIDE (chỉ áp dụng cho nút menu ⋮ của mình; skin v10 tự lo phần
  // ẩn/hiện thanh điều khiển riêng của nó) =====
  ...
  // ===== TIỆN ÍCH THỜI GIAN (chỉ dùng cho toast "tiếp tục từ..."; thanh tua/thời
  // gian hiển thị trên video giờ do skin video.js v10 tự vẽ, không tự vẽ nữa) =====
