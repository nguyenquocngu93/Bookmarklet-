...
function showNativePreview(url, type) {
  ...
  headerDiv.innerHTML = 
    '<div style="min-width:0;">' +
      ...
      '<div style="font-size:11px;color:var(--text3);">Định dạng: <span style="color:var(--accent);">' + type + '</span> · <span id="__uvd_orient_badge__" style="color:var(--accent2);">Đang phát hiện…</span></div>' +
    '</div>' +
    '<button class="uvd-btn uvd-btn-sm uvd-ripple-btn" id="__uvd_back_btn__" style="background:var(--glass-hi);flex-shrink:0;">← Quay lại</button>';
  ...
  video.addEventListener('loadedmetadata', function() {
    var badge = document.getElementById('__uvd_orient_badge__');
    if (!badge) return;
  ...
  btnFullscreen.onclick = function() {
    enterFullscreenAuto(videoWrapper, video);
  };
...
function lockAutoOrientation(video) {
  ...
  // Nếu đã có metadata (video đã load xong), khóa xoay ngay.
  if (video.videoWidth && video.videoHeight) {
    applyLock();
  } else {
    // Chờ metadata, kèm fallback đề phòng sự kiện không bắn kịp.
