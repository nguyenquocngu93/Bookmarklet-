...
function toast(msg, color) {
  ...
  el.textContent = msg;
  el.style.cssText = 'position:fixed;top:22px;left:50%;transform:translateX(-50%);background:' + (color || 'linear-gradient(135deg,#ff9fb4,#f76c8c)') + ';color:#fff;padding:12px 22px;border-radius:999px;z-index:2147483647;font:700 13px Segoe UI;box-shadow:0 6px 20px rgba(247,108,140,.4),inset 0 1px 0 rgba(255,255,255,.4);border:1px solid rgba(255,255,255,.5);animation:uvdSlideIn 0.3s cubic-bezier(.22,1,.36,1) both;transition:opacity .22s ease,transform .22s ease;';
  __uvdAppendRoot(el);
  setTimeout(function() {
    el.style.opacity = '0';
...
function __uvdOpenMediaLinksPopup(streams) {
  ...
  streams.slice(0, 8).forEach(function(stream, index) {
    ...
    if (hasMeta) {
      // Link chất lượng cao: bê nguyên cục card stream (thumbnail to + cảnh khác + metadata)
      ...
      card.innerHTML =
        '<div class="uvd-plcard-preview" data-thumb-url="' + escapeHtml(stream.url) + '">' +
          ...
          '<span class="uvd-plcard-badge">✨ chất lượng</span>' +
          '<button class="uvd-plrow-watch" title="Xem">▶</button>' +
        '</div>' +
        '<div class="uvd-plcard-head">' + metaLine + '</div>' +
        '<div class="uvd-plcard-note">link này chất lượng nè, xem ngay đi 🎬</div>' +
