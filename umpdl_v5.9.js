/**
 * Universal Media Player & Downloader - V5.9
...
// ========== POPUP / NEW-TAB BLOCKER (bật liên tục suốt phiên dùng script) ==========
// Nhiều site (vd: midrop và các site chia sẻ file/quảng cáo nặng) không chỉ
// gắn popup ngay lúc bấm play — chúng còn bắn quảng cáo theo TIMER riêng,
// hoặc lắng nghe click ở BẤT KỲ đâu trên trang. Chặn 2-3 giây quanh lúc mô
// phỏng click là không đủ. Giờ ta bật chặn NGAY khi mở script và giữ nguyên
// cho đến khi đóng panel, thay vì chỉ bật tạm thời.
//
// Kỹ thuật phổ biến nhất của mạng quảng cáo là tạo thẻ <a target="_blank">
// ẩn rồi tự gọi .click() bằng JS (né việc trình duyệt chặn window.open() gọi
// trực tiếp từ code). Bắt sự kiện ở pha CAPTURE trên document sẽ bắt được cả
// những click "giả" kiểu này, không chỉ click thật của người dùng.
...
function killBlankLinks(e) {
  ...
  while (t && t !== document) {
    if (t && t.tagName === 'A') {
      var tg = t.target;
      // Chỉ chặn khi target trỏ ra ngoài tab hiện tại (_blank hoặc tên
      // popunder tùy biến) — không đụng vào điều hướng bình thường.
...
// Giữ tên hàm cũ để không phải sửa chỗ gọi (autoClickPlayButtons...) — giờ
// chỉ cần đảm bảo block đang bật (nó bật sẵn từ đầu phiên), không cần
// bật/tắt riêng theo từng lần click mô phỏng nữa.
function blockPopupsDuring(durationMs, fn) {
  installPopupBlock();
  fn();
}

// ========== AUTO-CLICK PLAY (cho site lazy-load link khi bấm Play) ==========
// Nhiều site (eporner, một số JAV site...) chỉ nhúng link .m3u8/.mp4 thật
// vào DOM/network SAU KHI người dùng bấm nút Play. Danh sách selector dưới
// đây phủ các player phổ biến nhất: FluidPlayer (eporner dùng), JWPlayer,
// video.js, Plyr, MediaElement.js, và các nút play tùy biến chung chung.
...
function autoClickPlayButtons(root, depth, allowVideoPlayFallback) {
  ...
  // CHỈ gọi .play() trực tiếp trên <video> khi được yêu cầu rõ ràng (bấm nút
  // ▶ thủ công) — không tự động làm việc này ở lần chạy im lặng, vì nhiều
  // site vốn không cần giả lập gì cả, video vẫn đang paused chờ người dùng,
  // và việc tự phát sẽ gây phiền (tự nhảy phát có tiếng ngoài ý muốn).
  // Khi thực sự áp dụng, video được tắt tiếng + phát thử trong thời gian
  // ngắn rồi tự dừng lại, để không làm gián đoạn trải nghiệm xem của người dùng.
  ...
  // Đệ quy vào iframe cùng origin — nhiều site nhúng player qua iframe riêng
  if (depth < 2) {
    try {
...
// ========== TỰ PAUSE VIDEO GỐC SAU KHI ĐÃ LẤY ĐƯỢC LINK ==========
// Sau khi mô phỏng bấm Play để lấy link, video gốc trên trang vẫn tiếp tục
// phát nền (có tiếng) trong khi người dùng lại xem qua player riêng của
// script -> bị phát trùng 2 nơi. Hàm này tìm mọi <video> đang phát (kể cả
// trong iframe cùng origin) và pause lại, để chỉ còn player của script hoạt động.
...
// Bấm thử các nút Play ngay khi mở script — với site lazy-load, link thật
// (m3u8/mp4/blob) chỉ xuất hiện SAU khi người dùng "bấm play", nên ta mô
// phỏng thao tác đó rồi quét lại sau một khoảng chờ ngắn. Toàn bộ thao tác
// được bọc trong blockPopupsDuring() để tránh dính popup/nhảy tab do quảng
// cáo gắn trên nút play của site.
function runAutoClickAndRescan(silent) {
  var beforeCount = urls.size;
  var clicked = 0;
  blockPopupsDuring(2500, function() {
    // Lần chạy im lặng (tự động khi mở script): chỉ bấm các nút Play khớp
    // selector, KHÔNG tự ý play() mọi <video> — tránh làm phiền các site
    // vốn không cần giả lập gì. Khi người dùng chủ động bấm nút ▶, mới cho
    // phép thêm fallback play() tắt tiếng.
  ...
  setTimeout(function() {
    ...
    if (found > 0) {
      ...
      // Đợi thêm chút để video gốc kịp thật sự bắt đầu phát rồi mới pause,
      // tránh trường hợp pause quá sớm khi player chưa kịp gán nguồn.
      setTimeout(function() {
...
// ========== AUTO CLICK PLAY ==========
// Nhiều site (eporner, các trang JAV...) chỉ gắn link stream thật vào
// DOM/network SAU KHI người dùng bấm nút play. Ta giả lập click các nút
// play thường gặp để trigger request đó; fetch/xhr monitor ở trên sẽ
// tự động bắt được link mới xuất hiện.
...
function autoClickPlay(root, depth) {
  ...
  // Thử gọi .play() trực tiếp trên các thẻ <video> có sẵn
  try {
    root.querySelectorAll('video').forEach(function(v) {
  ...
  // Selector tùy chỉnh riêng cho từng site (đặt trong tab Cài đặt) được ưu tiên trước
  ...
  // Đệ quy vào iframe cùng gốc (same-origin), một số site nhúng player qua iframe
  try {
    root.querySelectorAll('iframe').forEach(function(f) {
...
// Player nhiều site load bất đồng bộ nên thử bấm play lặp lại vài lần
...
function showVideoPlayer(url, type) {
  ...
  // An toàn thêm: mỗi khi mở player riêng của script, pause luôn video gốc
  // của trang (nếu đang phát) để tránh phát trùng 2 nơi cùng lúc.
  ...
  function showQualitySubMenu() {
    ...
    sel.onchange = function() {
      ...
      if (q && playerState.hls) {
        ...
        // Không cần tự set resolution/label ở đây nữa — sự kiện
        // Hls.Events.LEVEL_SWITCHED sẽ tự cập nhật khi level thực sự đổi,
        // đảm bảo label luôn khớp với video đang thật sự phát.
  ...
  // ===== HIỂN THỊ TIÊU ĐỀ / ĐỘ PHÂN GIẢI (dùng chung, tránh nơi ghi đè nơi) =====
  ...
  function updateSizeEstimate() {
    ...
    if (isHls) {
      ...
      // File trực tiếp (MP4/WEBM/MKV) — thử HEAD request lấy Content-Length.
  ...
  function onMetadataLoaded() {
    ...
    // Chỉ lấy resolution từ kích thước video thật ở LẦN ĐẦU load — không ghi
    // đè mỗi khi 'loadedmetadata' bắn lại (một số trình duyệt/hls.js bắn lại
    // sự kiện này khi đổi chất lượng), tránh việc label bị đè về giá trị cũ
    // ngay sau khi người dùng vừa chọn chất lượng khác.
  ...
  if (isHls) {
    if (window.Hls && Hls.isSupported()) {
      ...
      // Nguồn xác thực DUY NHẤT cho việc đổi chất lượng — bắn ra dù người
      // dùng chọn tay hay hls.js tự động ABR, nên label luôn đúng thực tế.
...
function minimizePlayer() {
  ...
  // Pause NGAY trước khi tạo animation — nguyên nhân chính gây lag là video
  // vẫn tiếp tục decode/phát trong lúc overlay đang animate thu nhỏ cùng lúc.
  video.pause();
  ...
  // Chỉ animate opacity, KHÔNG dùng transform:scale trên overlay lớn — scale
  // buộc trình duyệt phải vẽ lại/composite toàn bộ layer video mỗi khung
  // hình, đây là phần tốn tài nguyên nhất gây giật trên máy yếu.
...
function buildUI() {
  ...
  document.getElementById('__uvd_minimize__').onclick = function() {
    ...
    // Panel dùng backdrop-filter blur + nền liquid animate vô hạn — animate
    // transform:scale trên layer này rất nặng (trình duyệt phải composite
    // lại toàn bộ blur mỗi khung hình) và là nguyên nhân chính gây lag.
    // Tạm dừng animation nền + chỉ fade opacity (bỏ hẳn scale) cho nhẹ hơn.
...
console.log('V5.9 UMP DL - Persistent Popup Block Ready');
toast('V5.9 sẵn sàng — đã bật chặn popup liên tục!');
