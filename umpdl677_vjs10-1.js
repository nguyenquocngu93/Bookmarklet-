/**
 * Universal Media Player & Downloader - V6.7.8 PRO
 * - FIX QUAN TRỌNG: link BLOB không xem được. Nguyên nhân: blob: từ MediaSource
 *   (hls.js/dash.js/Shaka...) chỉ phát được trên ĐÚNG thẻ <video> đã gắn nó —
 *   gán src cho video mới sẽ không load được gì. Giờ script tự tìm lại đúng
 *   thẻ <video> gốc trên trang (kể cả trong iframe cùng gốc) và "mượn" chính
 *   nó để phát, đóng player sẽ trả video về đúng chỗ cũ, không phá trang gốc.
 * - UI player: nền video đổi từ đen phẳng sang gradient (kể cả "ambient glow"
 *   lấy màu trung bình khung hình hiện tại, tự tắt an toàn nếu video bị CORS
 *   chặn canvas), giúp hiệu ứng kính mờ của thanh trạng thái nổi bật hơn.
 * - Thanh trạng thái dưới cùng giờ chồng lên video (kính mờ, tự ẩn/hiện theo
 *   controls) thay vì chiếm riêng một dải bên dưới video.
 * - Khung preview thu nhỏ (mini player): bo góc lớn hơn, đổ bóng dồn xuống
 *   đáy thay vì đều 4 phía, nền gradient lấy từ màu trung bình khung hình.
 * - FIX: header panel giật khi cuộn trên Chrome Android (bỏ hack nudge transform
 *   theo scroll, ép GPU layer cố định ngay khi tạo panel).
 * - FIX: khôi phục backdrop-filter khi thu nhỏ panel (từng bị tắt lúc debug).
 * - Tính năng "Tải Blob" cho các link blob: là Blob tĩnh thật (không phải
 *   MediaSource) — tải trực tiếp trong trang, lưu file thật.
...
function showVideoPlayer(url, type) {
  ...
  footer.style.cssText = 'position:absolute;left:0;right:0;bottom:0;padding:10px 16px;background:linear-gradient(to top,rgba(0,0,0,0.75),rgba(0,0,0,0.35) 70%,transparent);backdrop-filter:blur(16px) saturate(140%);-webkit-backdrop-filter:blur(16px) saturate(140%);border-top:1px solid rgba(255,255,255,0.1);font-size:12px;color:#ddd;display:flex;justify-content:space-between;gap:8px;z-index:3;transition: opacity 0.3s ease;';
