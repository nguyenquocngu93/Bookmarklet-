...
* - Thêm giả lập nút play cho eporner, JAV sites
* - Auto-click play button detection
* - Manual play trigger
...
var defaultProfiles = {
  ...
  'eporner.com': { referer: 'https://www.eporner.com/', userAgent: '' }
};
...
function findAndClickPlay() {
  ...
  // Priority 1: Click video element
  ...
  videos.forEach(function(v) {
    if (v.paused && !clicked) {
      try {
        v.play();
        clicked = true;
        console.log('[UMP DL] Auto-played video element');
      } catch(e) {}
    }
  });
  
  // Priority 2: Click play buttons
  if (!clicked) {
    selectors.forEach(function(sel) {
      ...
      try {
        ...
        elements.forEach(function(el) {
          ...
          // Check if element contains play icon
          var hasPlayIcon = 
            el.textContent.includes('▶') ||
            el.textContent.includes('play') ||
            el.innerHTML.includes('play') ||
            (el.querySelector('svg') && el.querySelector('svg').innerHTML.includes('polygon')) ||
            el.querySelector('i[class*="fa-play"]');
          
          if (hasPlayIcon || sel === 'video') {
            try {
              el.click();
              clicked = true;
              playClicked = true;
              console.log('[UMP DL] Clicked play button:', sel);
              toast('🎬 Đã bấm Play tự động');
            } catch(e) {}
          }
        });
      } catch(e) {}
...
// Delayed scan for lazy-loaded content
setTimeout(function() { scan(document, 'delayed-2s'); }, 2000);
setTimeout(function() { scan(document, 'delayed-5s'); }, 5000);
setTimeout(function() { scan(document, 'delayed-10s'); }, 10000);
...
function showVideoPlayer(url, type) {
  ...
  function showQualitySubMenu() {
    ...
    sel.style.cssText = 
      'width:100%;padding:10px;background:rgba(255,255,255,0.1);color:#fff;border:1px solid rgba(255,255,255,0.2);border-radius:8px;font-size:14px;';
...
function minimizePlayer() {
  ...
  mini.style.cssText = 
    'position:fixed;bottom:20px;right:20px;width:160px;height:90px;background:#000;border-radius:12px;z-index:2147483647;cursor:pointer;box-shadow:0 8px 30px rgba(0,0,0,0.8);border:2px solid rgba(255,255,255,0.2);overflow:hidden;transition: all 0.6s cubic-bezier(0.4, 0, 0.2, 1);';
...
function buildUI() {
  ...
  header.innerHTML = 
    ...
    '<button class="uvd-btn-icon uvd-ripple-btn" id="__uvd_trigger_play__" title="Trigger Play"><span style="font-size:16px;">🎬</span></button>' +
  ...
  // Trigger Play button
  document.getElementById('__uvd_trigger_play__').onclick = function() {
    var clicked = findAndClickPlay();
    if (clicked) {
      toast('🎬 Đã kích hoạt Play', '#22c55e');
      // Rescan after click
      setTimeout(function() { 
        scan(document, 'post-play'); 
        buildUI();
      }, 2000);
    } else {
      toast('⚠️ Không tìm thấy nút Play', '#f59e0b');
    }
  };
...
function renderSettings(container) {
  ...
  container.innerHTML =
    ...
    '<span class="uvd-tag">Auto Play</span>' +
    '</div>' +
    '</div>' +
    '</div>' +
    ...
    '• Script sẽ <strong style="color:#22c55e;">TÂN CÔNG BẤM PLAY</strong> (nếu cần) → stream sẽ xuất hiện<br>' +
    '• Nếu không tự động, bấm nút <strong style="color:var(--gold);">🎬 Trigger Play</strong> trên header<br>' +
...
console.log('V5.5 UMP DL - Auto Play Button Simulation Ready');
toast('🎬 V5.5 sẵn sàng!');
