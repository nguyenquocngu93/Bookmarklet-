/**
 * Universal Media Player & Downloader - V6.7 PRO
 * - Tối ưu code: gom nút toolbar qua helper dùng chung, giảm lặp code
 * - Chặn mạnh video/audio tự phát trên trang chủ sau khi chạy script
 * - Bỏ hệ thống đổi màu accent (dead code), đồng bộ 1 bảng màu cố định
 * - Đồng bộ lại màu các nút đỏ/xanh (đóng, tải xuống...) đỡ chói, hài hoà UI kính mờ
 * - Thêm hiệu ứng phát sáng (glow) có thể bật/tắt + chỉnh cường độ trong Cài đặt
 * - Thêm Phụ đề (thử nghiệm): tải file .srt/.vtt cục bộ + tìm trên OpenSubtitles
...
data.settings = Object.assign({
  ...
  openSubtitlesApiKey: '' // API key OpenSubtitles (thử nghiệm)
}, data.settings || {});
...
// ========== PHỤ ĐỀ (THỬ NGHIỆM) ==========
// Tải file .srt/.vtt cục bộ hoặc tìm/tải từ OpenSubtitles. Tính năng đang thử nghiệm.
...
function searchOpenSubtitles(query, cb) {
  var apiKey = (data.settings.openSubtitlesApiKey || '').trim();
  if (!apiKey) { toast('Chưa có OpenSubtitles API Key, xem hướng dẫn trong bảng Phụ đề'); cb([]); return; }
  fetch('https://api.opensubtitles.com/api/v1/subtitles?query=' + encodeURIComponent(query) + '&languages=vi,en', {
    headers: { 'Api-Key': apiKey, 'Content-Type': 'application/json' }
  })
  .then(function(r) { return r.json(); })
  .then(function(json) { cb((json && json.data) || []); })
  .catch(function() { toast('Lỗi kết nối OpenSubtitles (có thể do CORS/mạng)'); cb([]); });
}

function downloadOpenSubtitleFile(fileId, cb) {
  var apiKey = (data.settings.openSubtitlesApiKey || '').trim();
  ...
  .then(function(r) { return r.json(); })
  .then(function(json) {
    if (!json || !json.link) { toast('Không lấy được link phụ đề'); return; }
    return fetch(json.link).then(function(r2) { return r2.text(); });
  })
  .then(function(text) {
    if (!text) return;
    var vtt = srtToVtt(text);
    var blobUrl = URL.createObjectURL(new Blob([vtt], { type: 'text/vtt' }));
    attachSubtitleTrack(playerState.video, blobUrl, 'OpenSubtitles');
    if (cb) cb(true);
  })
  .catch(function() { toast('Lỗi tải phụ đề từ OpenSubtitles'); if (cb) cb(false); });
}
...
function showSubtitlePanel(video) {
  ...
  panel.innerHTML =
    ...
    '<div style="font-size:11px;color:var(--text3);margin-bottom:12px;">Tải file có sẵn hoặc tìm trên OpenSubtitles.</div>' +
    ...
    '<div style="font-size:12px;color:var(--text2);margin-bottom:6px;">Tìm trên OpenSubtitles</div>' +
    ...
    '<details style="margin-top:12px;">' +
      '<summary style="font-size:11px;color:var(--text3);cursor:pointer;">API Key OpenSubtitles</summary>' +
      '<input type="text" id="__uvd_sub_apikey__" placeholder="Dán API key (opensubtitles.com)" value="' + escapeHtml(data.settings.openSubtitlesApiKey || '') + '" style="width:100%;margin-top:8px;padding:9px;background:rgba(0,0,0,0.4);color:#fff;border:1px solid var(--border);border-radius:10px;font-size:11px;">' +
      '<div style="font-size:10px;color:var(--text3);margin-top:4px;">Đăng ký miễn phí tại opensubtitles.com/consumers để lấy API key.</div>' +
    '</details>' +
  ...
  panel.querySelector('#__uvd_sub_apikey__').onchange = function() {
    data.settings.openSubtitlesApiKey = this.value.trim();
    storage.set(data);
  };
  ...
  panel.querySelector('#__uvd_sub_search__').onclick = function() {
    ...
    searchOpenSubtitles(q, function(list) {
      ...
      list.slice(0, 10).forEach(function(item) {
        ...
        row.innerHTML = '<div style="font-size:12px;color:#fff;">' + escapeHtml(attr.release || attr.feature_details && attr.feature_details.title || q) + '</div>' +
          '<div style="font-size:10px;color:var(--text3);">' + escapeHtml((attr.language || '').toUpperCase()) + '</div>';
        row.onclick = function() {
          var f = attr.files && attr.files[0];
          if (!f) { toast('Không có file để tải'); return; }
          toast('Đang tải phụ đề...');
          downloadOpenSubtitleFile(f.file_id, function(ok) { if (ok) overlay2.remove(); });
        };
        box.appendChild(row);
      });
    });
...
:root{--bg:rgba(3,4,8,0.97);--glass:rgba(12,14,20,0.85);--glass-hi:rgba(255,255,255,0.06);--border:rgba(255,255,255,0.08);--text:#f3f5ff;--text2:#9ca3bd;--text3:#5d6377;--accent:#6d8cff;--accent2:#b98bff;--danger:#ff5d72;--gold:#ffb84d;--success:#34d399;--card-bg:rgba(255,255,255,0.03);--fs-xs:11px;--fs-sm:12px;--fs-base:13px;--fs-md:14px;--fs-lg:16px;--radius-sm:12px;--radius-md:16px;--radius-lg:26px;--grad-liquid:linear-gradient(135deg,var(--accent),var(--accent2));--glow-px:0px;--glow-op:0;--btn-bg:var(--btn-bg);--btn-danger-bg:rgba(255,93,114,0.22);--btn-danger-border:rgba(255,93,114,0.4);--btn-success-bg:rgba(52,211,153,0.2);--btn-success-border:rgba(52,211,153,0.4);--btn-accent-bg:rgba(109,140,255,0.28);--btn-purple-bg:rgba(167,139,250,0.24);--btn-gold-bg:rgba(255,184,77,0.26)}
...
/* Thanh tìm kiếm */
.uvd-search-box{display:flex;gap:6px;margin-bottom:10px;}
.uvd-search-box input{flex:1;padding:8px 12px;background:rgba(0,0,0,0.4);border:1px solid var(--border);border-radius:var(--radius-sm);color:var(--text);font-size:var(--fs-sm);}
.uvd-search-box input::placeholder{color:var(--text3);}
.uvd-search-box input:focus{outline:none;border-color:var(--accent);}
`;
...
if (window.visualViewport) {
  ...
  addCleanup(function() {
    window.visualViewport.removeEventListener('resize', __uvdSyncViewport);
    window.visualViewport.removeEventListener('scroll', __uvdSyncViewport);
  });
}
...
function buildUI() {
  ...
  searchBox.style.cssText = 'display:flex;gap:6px;margin-bottom:10px;flex-shrink:0;';
  var searchInput = document.createElement('input');
  searchInput.type = 'text';
  searchInput.placeholder = '🔍 Tìm kiếm stream...';
  searchInput.id = '__uvd_search_input__';
  searchInput.style.cssText = 'flex:1;padding:8px 12px;background:rgba(0,0,0,0.4);border:1px solid var(--border);border-radius:var(--radius-sm);color:var(--text);font-size:var(--fs-sm);';
  searchInput.addEventListener('input', function() {
    ...
    // Ẩn nút "Xem thêm" nếu đã filter
    var more = streamList.querySelector('.uvd-more-btn');
  ...
  streamList.style.cssText = 'overflow-y:auto;height:calc(100% - 50px);padding-right:4px;';
...
function renderSettings(container) {
  ...
  container.innerHTML =
    ...
    '<div class="uvd-card uvd-timeline-card">' +
      ...
      '<div class="uvd-step"><span class="uvd-step-num">•</span><span class="uvd-step-text">Dùng thanh tìm kiếm ở tab Streams để lọc theo tên hoặc loại.</span></div>' +
