/**
 * Universal Media Player & Downloader - V6.4 PRO (FIXED)
 * - Giữ nguyên hiệu ứng, thêm Mute, sửa Reduce Motion, tự động giảm hiệu ứng khi xem video
 * Author: nguyenquocngu91 (fixed by bạn)
 */
(function() {
'use strict';
...
var playerState = {
  ...
  sleepEndAt: 0,
  savePosTimer: null,
  wasReduceMotion: false   // lưu trạng thái reduceMotion trước khi mở player
};
...
function showVideoPlayer(url, type) {
  ...
  // Tạm thời bật reduceMotion để giảm tải khi xem video (nếu chưa bật)
...
/* Fix cho reduce motion: thêm nền đậm, tắt blur và animation */
.uvd-reduce-motion .uvd-liquid-bg{animation:none;display:none}
.uvd-reduce-motion .uvd-glass-panel{backdrop-filter:none!important;-webkit-backdrop-filter:none!important;background:rgba(8,10,16,0.96)!important}
.uvd-reduce-motion .uvd-overlay{backdrop-filter:none!important}
...
function buildUI() {
  ...
  function moveIndicatorTo(btn) {
    if (!btn) return;
    // Lấy padding để indicator không bị dính sát mép phải
    var padding = 4;
...
console.log('V' + VERSION + ' UMP DL PRO - Resume, tua chạm đúp, PiP, boost, hẹn giờ, tab cuộn ngang, Mute');
