...
(function() {
            ...
            '<div class="__uvd_header__">' +
                '<div class="__uvd_header_left__">' +
                    '<div class="__uvd_header_text__">' +
                        '<div class="__uvd_title__"><span class="__uvd_accent_inline__"></span>Universal DL <span class="__uvd_version__">V3.7</span></div>' +
    ...
    function getGlobalCSS(t) {
        ...
        '.__uvd_accent_inline_small__ {' +
            ...
            'opacity: 0.8;' +
        '}' +
        /* Accent bar trước text nút action */
        '.__uvd_btn_accent__ {' +
        ...
        /* Quality card - optimized, no heavy blur */
        ...
        /* STATUS - đẩy lên cao hơn để không bị controls che */
    ...
    function showPreview(url, type) {
        ...
        // FIX: Lệnh tải - KHÔNG cleanup video, chỉ hide preview tạm thời
        document.getElementById('__pv_cmd__').addEventListener('click', function(e) {
            e.stopPropagation();
            // Hide preview overlay nhưng KHÔNG destroy video
            overlay.style.display = 'none';
            // Pause video để không chạy ngầm
            video.pause();
            showCommandPicker(url, type || 'M3U8', function() {
                // Callback khi đóng command picker -> show lại preview
        ...
        // FIX: Fullscreen với orientation lock - fullscreen cả wrap (không chỉ video)
        // để giữ controls overlay, và lock orientation SAU KHI đã vào fullscreen
        // (Chrome Android từ chối lock orientation nếu chưa ở fullscreen).
    ...
    // FIX: Fullscreen với orientation lock đúng thứ tự (enter fullscreen TRƯỚC, lock SAU)
    // Chrome Android chỉ cho phép screen.orientation.lock() khi document đang ở fullscreen,
    // nên gọi lock() trước khi fullscreen thực sự bắt đầu sẽ luôn bị từ chối im lặng.
    function enterFullscreen(element) {
        ...
        if (!request) {
            // iOS Safari fallback: no Fullscreen API on generic elements,
            // only native video fullscreen is available, and it has no orientation control.
        ...
        // If no Promise support, doLock() will still fire via the fullscreenchange listener above.
    }
    
    // FIX: showCommandPicker nhận callback để restore preview
    ...
    function tsListTorrents(callback) {
        ...
        .catch(function(err) {
            // Fallback: some TorrServer builds expose GET /torrents
            fetch(tsBase() + '/torrents')
                .then(function(r) { return r.json(); })
                .then(function(list) { callback(null, list || []); })
    ...
    function renderTorrServer(container) {
        ...
        html += '<div class="__uvd_ts_input_row__">' +
            '<input type="text" class="__uvd_ts_input__" id="__uvd_ts_magnet__" placeholder="Dán magnet link hoặc .torrent URL..." />' +
            '</div>' +
            ...
            '</div>' +
            '<div style="color:' + t.text3 + ';font-size:10px;margin-bottom:12px;">Server: <span class="__uvd_clickable__" id="__uvd_ts_edit_host__">' + escapeHtml(tsBase()) + '</span></div>' +
        ...
        function loadTorrServerList() {
            ...
            tsListTorrents(function(err, list) {
                ...
                if (!listEl) return;
                if (err) {
                    listEl.innerHTML = '<div class="__uvd_empty__"><div class="__uvd_empty_icon__">⚠️</div>' +
    ...
    function renderSettings(container) {
        ...
        html += '<div class="__uvd_settings_section__"><div class="__uvd_settings_title__"><span class="__uvd_accent_inline__"></span>Thông tin</div>' +
            ...
            '<div><span class="__uvd_accent_inline_small__"></span>Version: 3.7 — Full Fix + TorrServer</div>' +
    ...
    buildUI();
    startAutoRefresh();
    console.log('✅ Universal DL V3.7 loaded! Found', urls.size, 'streams initially');
    toast('V3.7 Ready! (Fullscreen fix + TorrServer)', 'success');
})();
