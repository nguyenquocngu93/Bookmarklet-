 ...
 *   Nguyên nhân thật là bản trước tự ép "nudge" transform mỗi lần scroll/resize —
 *   chính thao tác đó gây giật hình. Đã thay bằng cách ép panel có RIÊNG một GPU
 *   compositing layer cố định (isolation + contain + will-change) ngay khi tạo,
 *   không còn can thiệp theo sự kiện scroll nữa.
 * - FIX: khôi phục lại backdrop-filter (kính mờ) khi thu nhỏ panel — bản trước
 *   từng bị tắt hẳn (blur:0) trong lúc debug, làm panel minimized thành nền đặc.
 * - MỚI: tính năng "Tải Blob" — với các link blob: phát hiện được từ thẻ
 *   video/audio (thường không dùng được với Chia sẻ/Sao chép/Lệnh tải), giờ có
 *   nút tải trực tiếp trong trang (fetch blob rồi lưu file thật). Có cảnh báo rõ
 *   khi blob là MediaSource (HLS/DASH) không thể tải trọn vẹn kiểu này.
