/**
 * Universal Media Player & Downloader - V6.4 PRO (FINAL)
 * - Hiệu ứng chuyển động mượt hơn
 * - Chỉ giữ tab: Streams, Player, Settings
 * - Settings Player: gợi ý thông minh (tooltip)
 * - Settings hiệu suất: điều chỉnh blur và tốc độ transition
 * - Fix header minimize khi Chrome ẩn thanh địa chỉ
 * - Nút "Chặn" trong mỗi stream -> tự động thêm vào filter
...
data.settings = Object.assign({
  ...
  reduceMotion: false,
  accent: 'blue',
  // Cài đặt hiệu suất nâng cao
  blurIntensity: 14,          // px
  transitionSpeed: 0.3,       // giây
...
function applyMotionPref(el) {
  ...
  // Áp dụng blur và transition từ settings
...
// ========== CSS (tăng hiệu ứng, thêm biến, sửa lỗi header) ==========
...
/* Reduce motion: tắt animation và giảm blur */
.uvd-reduce-motion .uvd-liquid-bg{animation:none}
.uvd-reduce-motion *{animation:none!important;transition:none!important}
.uvd-reduce-motion .uvd-glass-panel{backdrop-filter:blur(0px)!important; -webkit-backdrop-filter:blur(0px)!important}

/* Tăng hiệu ứng chuyển động cho các thành phần */
.uvd-card, .uvd-btn, .uvd-btn-icon, .uvd-tab {
  transition: all var(--uvd-transition) !important;
}
.uvd-card:hover {
  transform: translateY(-5px) scale(1.01);
  box-shadow: 0 18px 40px rgba(0,0,0,0.8), 0 0 0 1px rgba(109,140,255,0.4) inset;
}
.uvd-btn:hover {
...
.uvd-card{background:var(--card-bg);border:1px solid var(--border);border-radius:var(--radius-md);padding:14px;margin-bottom:10px;font-size:var(--fs-base);box-shadow:0 1px 0 rgba(255,255,255,0.04) inset;animation:uvdCardEnter 0.4s ease both;color:var(--text)}
...
.uvd-tab{position:relative;z-index:1;flex:0 0 auto;background:transparent;border:none;color:var(--text2);font-weight:600;font-size:var(--fs-sm);padding:9px 16px;border-radius:999px;cursor:pointer;white-space:nowrap}
...
// ========== SCRIPT MINIMIZE / RESTORE (fix lỗi khi Chrome ẩn thanh địa chỉ) ==========
function __uvdSyncViewport() {
  var panel = document.getElementById('__uvd__');
  if (!panel) return;
  // Dùng visualViewport để điều chỉnh offset khi thanh địa chỉ ẩn/hiện
  if (window.visualViewport) {
    var offset = window.visualViewport.offsetTop || 0;
    panel.style.transform = 'translateZ(0) translateY(' + offset + 'px)';
  }
}
if (window.visualViewport) {
  window.visualViewport.addEventListener('resize', __uvdSyncViewport);
  window.visualViewport.addEventListener('scroll', __uvdSyncViewport);
}
...
function restoreScriptPanel() {
  ...
  panel.addEventListener('transitionend', function onEnd(e) {
    ...
    // Cập nhật vị trí sau khi restore
    __uvdSyncViewport();
  });
...
function buildUI() {
  ...
  // Chỉ giữ 3 tab: Streams, Player, Settings
  var tabList = [
    ...
    '<div class="uvd-card">' +
      '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">' +
        ...
        '<div style="display:flex;gap:4px;">' +
          ...
          '<button class="uvd-block-btn" data-url="' + encodeURIComponent(item.url) + '" style="background:none;border:none;font-size:16px;cursor:pointer;color:var(--danger);" title="Chặn link này">⛔</button>' +
        '</div>' +
      '</div>' +
...
function renderStreams(container, arr) {
  ...
  container.onclick = function(e) {
    ...
    if (blockBtn) {
      ...
      // Trích xuất domain hoặc toàn bộ URL
      ...
      if (confirm('Chặn tất cả stream chứa "' + pattern + '" ?')) {
        ...
        setTimeout(function() { blockBtn.textContent = '⛔'; }, 1500);
      }
      return;
    }
...
function renderSettings(container) {
  ...
  document.getElementById('__uvd_save_filter__').onclick = function() {
    ...
    buildUI(); // Refresh để áp dụng filter mới
  };
...
console.log('V' + VERSION + ' UMP DL PRO - Hiệu ứng mượt, tối ưu, block link, fix header');
