 ...
 * - SỬA: Hàm quét server giờ ƯU TIÊN dùng playSelector nếu có,
 *   không còn bỏ qua nó nữa. Nếu không có, vẫn dò nhóm nhưng loại trừ
 *   các nút chứa từ khóa diễn viên/đạo diễn (actress, actor, director, tag...)
 * - Thêm bảo vệ chặn chuyển trang mạnh hơn khi quét.
 * - Tối ưu phát hiện link qua PerformanceObserver.
 * - Áp dụng cho mọi trang web (không chỉ jav.ninja).
...
var defaultProfiles = {
  ...
  'jav.ninja': { referer: 'https://jav.ninja/', userAgent: '' }
};
...
// ========== TỰ BẤM CÁC NÚT "SERVER" RỒI QUÉT LẠI (ĐÃ SỬA) ==========
function autoClickServersAndScan() {
  __uvd_scanProtectStart();

  // Lấy selector riêng nếu có (ưu tiên)
  var customSel = (data.siteProfiles[pageInfo.host] && data.siteProfiles[pageInfo.host].playSelector) || '';
  var candidates = [];

  if (customSel) {
    try {
      document.querySelectorAll(customSel).forEach(function(el) {
        if (el.closest && (el.closest('#__uvd__') || el.closest('#__uvd_player_overlay__') || el.closest('#__uvd_inline_iframe_overlay__'))) return;
        candidates.push(el);
      });
    } catch(e) {}
    if (candidates.length === 0) {
      toast('Selector "' + customSel + '" không tìm thấy phần tử nào, thử dò nhóm tự động...');
    } else {
      toast('Dùng selector riêng: tìm thấy ' + candidates.length + ' nút server.');
    }
  }

  // Nếu không có selector hoặc không tìm thấy, dùng cơ chế nhóm (có lọc)
  if (candidates.length === 0) {
    var groups = {};
    var curPath = location.pathname;
    var excludeKeywords = ['diễn viên', 'actress', 'actor', 'đạo diễn', 'director', 'tag', 'tags', 'thể loại', 'category'];
    ...
    document.querySelectorAll('body *').forEach(function(el) {
      ...
      for (var k = 0; k < excludeKeywords.length; k++) {
        if (txt.indexOf(excludeKeywords[k]) !== -1) return;
      }
      
      var elClasses = (el.className || '').toLowerCase();
      var parentClasses = (el.parentElement && el.parentElement.className || '').toLowerCase();
      var combined = elClasses + ' ' + parentClasses;
      for (var k2 = 0; k2 < excludeKeywords.length; k2++) {
        if (combined.indexOf(excludeKeywords[k2]) !== -1) return;
      }
      ...
      if (isAnchor) {
        ...
        if (href && href.indexOf('#') !== 0 && href.toLowerCase().indexOf('javascript:') !== 0) {
          try {
            ...
            if (abs.pathname !== curPath) return;
          } catch (e) {}
        }
      }
    ...
    Object.keys(groups).forEach(function(k) {
      var g = groups[k];
      if (g.els.length >= 2 && g.els.length <= 10) {
        var isServerGroup = false;
        for (var i = 0; i < g.els.length; i++) {
          var el = g.els[i];
          var id = (el.id || '').toLowerCase();
          var cls = (el.className || '').toLowerCase();
          if (id.indexOf('host') !== -1 || id.indexOf('server') !== -1 || id.indexOf('quality') !== -1 ||
              cls.indexOf('host') !== -1 || cls.indexOf('server') !== -1 || cls.indexOf('quality') !== -1) {
            isServerGroup = true;
            break;
          }
        }
        if (isServerGroup) {
          filteredCandidates = filteredCandidates.concat(g.els);
        }
      }
    ...
    if (filteredCandidates.length === 0) {
      toast('Không tìm thấy nhóm nút server nào (đã lọc diễn viên) — dùng selector thủ công trong Cài đặt.');
      __uvd_scanProtectEnd();
      return;
    }
    candidates = filteredCandidates;
    toast('Tìm thấy ' + candidates.length + ' nút server — đang tự bấm lần lượt, đợi vài giây...');
  }
