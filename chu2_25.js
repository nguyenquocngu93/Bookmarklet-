/**
 * Universal Media Player & Downloader - V6.7.22 PRO (theme sáng thật, header player gọn)
...
function showVideoPlayer(url, type) {
  ...
  // ─── HEADER: chữ "Video" ở bên trái, không icon ───
  var sheetHeader = document.createElement('div');
  sheetHeader.className = 'uvd-settings-header';
  sheetHeader.id = '__uvd_player_header__';
  sheetHeader.style.cssText = 'display:flex; align-items:center; gap:10px; padding:14px 16px; border-bottom:1px solid var(--border); flex-shrink:0;';
  ...
  backBtn.style.cssText = 'background:var(--glass-hi); border:1px solid var(--border); color:var(--text); width:34px; height:34px; border-radius:var(--radius-sm); cursor:pointer; display:inline-flex; align-items:center; justify-content:center; flex-shrink:0;';

  var titleSpan = document.createElement('span');
  titleSpan.textContent = 'Video';
  titleSpan.style.cssText = 'font-weight:700; font-size:16px; color:var(--accent); text-shadow:0 0 12px rgba(255,47,200,0.5); margin-left:6px;';

  var menuBtn = document.createElement('button');
  menuBtn.className = 'uvd-icon-btn uvd-icon-btn-wide';
  menuBtn.textContent = '⋮';
  menuBtn.title = 'Tuỳ chọn';
  menuBtn.style.cssText = 'flex-shrink:0; background:var(--btn-bg); border:1px solid var(--border); color:var(--accent2); width:36px; height:36px; border-radius:var(--radius-sm); cursor:pointer; display:inline-flex; align-items:center; justify-content:center; font-size:16px; flex-shrink:0; position:relative; overflow:hidden; transition:all var(--uvd-transition);';

  sheetHeader.appendChild(backBtn);
  sheetHeader.appendChild(titleSpan);   // Video nằm sát bên trái
  sheetHeader.appendChild(menuBtn);     // Menu bên phải
  ...
  // ─── VIDEO: scale 95%, không dính mép, đổ bóng dưới và phải ───
  video.style.cssText = 'max-width:95%; max-height:95%; width:auto; height:auto; display:block; object-fit:contain; border-radius:12px; box-shadow:8px 12px 30px rgba(0,0,0,0.7), -2px -2px 10px rgba(255,255,255,0.05); margin:auto;';
  ...
  // ─── HÀM CẬP NHẬT CHIỀU RỘNG (video dọc cũng 95%) ───
  ...
  function __uvdUpdatePlayerWidth() {
    ...
    if (!playerEl) return;
    var isFullscreen = __uvdIsFullscreenNow();
    playerEl.style.width = isFullscreen ? '100%' : '95%';
  }
  ...
  function createMenuPanel(title, options, callback) {
    ...
    titleDiv.style.cssText = 'color:var(--text);font-weight:600;font-size:15px;margin-bottom:14px;text-align:center;';
    titleDiv.textContent = title;
    panel.appendChild(titleDiv);
    var content = document.createElement('div');
    content.style.cssText = 'max-height:60vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px;';
    options.forEach(function(opt) {
      var btn = document.createElement('button');
      btn.className = 'uvd-btn uvd-btn-sm';
      // ─── NÚT CHỌN CHẤT LƯỢNG: BÓNG NHẸ, KHÔNG DÍNH ───
      btn.style.cssText = 'width:100%; padding:12px 10px; background:rgba(155,61,255,0.08); border:1px solid var(--border); border-radius:12px; color:var(--text); font-weight:600; font-size:14px; cursor:pointer; transition:all 0.15s ease; box-shadow:0 2px 8px rgba(0,0,0,0.1); text-align:center;';
      btn.textContent = opt.label;
      btn.addEventListener('mouseenter', function() {
        this.style.background = 'rgba(155,61,255,0.18)';
        this.style.boxShadow = '0 4px 12px rgba(155,61,255,0.2)';
      });
      btn.addEventListener('mouseleave', function() {
        this.style.background = 'rgba(155,61,255,0.08)';
    ...
    // ─── NÚT ĐÓNG CŨNG CÓ BÓNG NHẸ ───
    closeBtn.style.cssText = 'width:100%; margin-top:12px; padding:12px 10px; background:rgba(255,93,114,0.12); border:1px solid var(--border); border-radius:12px; color:var(--text); font-weight:600; font-size:14px; cursor:pointer; transition:all 0.15s ease; box-shadow:0 2px 8px rgba(0,0,0,0.1); text-align:center;';
    closeBtn.addEventListener('mouseenter', function() {
      this.style.background = 'rgba(255,93,114,0.25)';
      this.style.boxShadow = '0 4px 12px rgba(255,93,114,0.2)';
    });
    closeBtn.addEventListener('mouseleave', function() {
      this.style.background = 'rgba(255,93,114,0.12)';
      this.style.boxShadow = '0 2px 8px rgba(0,0,0,0.1)';
    });
...
/* Cải thiện độ tương phản và dễ đọc */
.uvd-scope{color:var(--text);font-family:-apple-system,BlinkMacSystemFont,'SF Pro Display','Segoe UI',Roboto,sans-serif;--bg:rgba(255,246,251,0.98);--glass:rgba(255,250,253,0.92);--glass-hi:rgba(155,61,255,0.10);--border:rgba(155,61,255,0.25);--text:#1a1024;--text2:#4a3b5a;--text3:#7c6a8f;--accent:#ff2fc8;--accent2:#9b3dff;--danger:#ff5d72;--gold:#e0900a;--success:#1fa97a;--card-bg:rgba(255,255,255,0.78);--fs-xs:11px;--fs-sm:12px;--fs-base:13px;--fs-md:14px;--fs-lg:16px;--radius-sm:14px;--radius-md:20px;--radius-lg:32px;--grad-liquid:linear-gradient(135deg,var(--accent),var(--accent2));--glow-px:0px;--glow-op:0;--btn-bg:rgba(255,47,200,0.12);--btn-danger-bg:rgba(255,93,114,0.16);--btn-danger-border:rgba(255,93,114,0.35);--btn-success-bg:rgba(31,169,122,0.14);--btn-success-border:rgba(31,169,122,0.35);--btn-accent-bg:rgba(255,47,200,0.16);--btn-purple-bg:rgba(155,61,255,0.16);--btn-gold-bg:rgba(224,144,10,0.16)}
...
.uvd-card:hover{transform:translateY(-5px) scale(1.01);box-shadow:0 18px 40px rgba(0,0,0,0.8),0 0 0 1px rgba(255,47,200,0.4) inset}
...
.uvd-profile-card{display:flex;align-items:center;gap:14px;background:linear-gradient(135deg,rgba(255,47,200,0.14),rgba(155,61,255,0.08));border:1px solid rgba(155,61,255,0.25);border-radius:var(--radius-lg);padding:16px;margin-bottom:10px;animation:uvdCardEnter .4s ease both}
