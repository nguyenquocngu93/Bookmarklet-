/**
 * Universal Media Player V3.1 - Fixed
 * - Nút Fullscreen riêng (không auto)
 * - Fix controls & seek bar trong fullscreen
 * - Fix video player tự biến mất
 * - Chặn quảng cáo iframe
 */
(function() {
    'use strict';
    ...
    function openPlayer(streamInfo) {
        ...
        controlsContainer.style.cssText = 'position:absolute;inset:0;pointer-events:none;z-index:10;';
        ...
        topBar.style.cssText = 'position:absolute;top:0;left:0;right:0;background:linear-gradient(180deg,rgba(0,0,0,0.7),transparent);padding:12px 15px;display:flex;justify-content:space-between;align-items:center;transition:opacity 0.3s;pointer-events:auto;z-index:20;';
        topBar.innerHTML = 
            ...
            '</div>' +
            '<button id="__ump_fullscreen_btn__" style="background:rgba(255,255,255,0.15);color:#fff;border:0;padding:10px;border-radius:50%;font-size:18px;margin-right:6px;pointer-events:auto;cursor:pointer;">⛶</button>' +
            '<button id="__ump_menu_btn__" style="background:rgba(255,255,255,0.15);color:#fff;border:0;padding:10px 14px;border-radius:50%;font-size:18px;margin-right:6px;pointer-events:auto;cursor:pointer;">⋮</button>' +
            '<button id="__ump_close_player__" style="background:rgba(244,67,54,0.9);color:#fff;border:0;padding:10px 14px;border-radius:50%;font-size:16px;font-weight:bold;pointer-events:auto;cursor:pointer;">✕</button>';
        controlsContainer.appendChild(topBar);
        
        // ========== CENTER PLAY BUTTON ==========
        var centerPlay = document.createElement('div');
        centerPlay.id = '__ump_center_play__';
        centerPlay.style.cssText = 'position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);pointer-events:auto;transition:opacity 0.3s,transform 0.2s;z-index:15;';
        centerPlay.innerHTML = '<button id="__ump_center_btn__" style="background:rgba(0,0,0,0.6);color:#fff;border:2px solid #fff;width:70px;height:70px;border-radius:50%;font-size:28px;cursor:pointer;display:flex;align-items:center;justify-content:center;pointer-events:auto;">▶</button>';
        ...
        seekLeft.style.cssText = 'position:absolute;top:50%;left:15%;transform:translateY(-50%);background:rgba(0,0,0,0.5);color:#fff;border:0;width:55px;height:55px;border-radius:50%;font-size:18px;pointer-events:auto;transition:opacity 0.3s;line-height:1;z-index:15;cursor:pointer;';
        ...
        seekRight.style.cssText = 'position:absolute;top:50%;right:15%;transform:translateY(-50%);background:rgba(0,0,0,0.5);color:#fff;border:0;width:55px;height:55px;border-radius:50%;font-size:18px;pointer-events:auto;transition:opacity 0.3s;line-height:1;z-index:15;cursor:pointer;';
        controlsContainer.appendChild(seekRight);
        
        // ========== BOTTOM BAR ==========
        var bottomBar = document.createElement('div');
        bottomBar.id = '__ump_bottombar__';
        bottomBar.style.cssText = 'position:absolute;bottom:0;left:0;right:0;background:linear-gradient(0deg,rgba(0,0,0,0.8),transparent);padding:20px 15px 15px;transition:opacity 0.3s;pointer-events:auto;z-index:20;';
        bottomBar.innerHTML = 
            '<div style="display:flex;align-items:center;gap:10px;">' +
                '<span id="__ump_time_cur__" style="color:#fff;font-size:12px;font-family:monospace;min-width:40px;">0:00</span>' +
                '<div style="flex:1;position:relative;height:20px;display:flex;align-items:center;" id="__ump_seek_wrapper__">' +
                    '<div style="width:100%;height:4px;background:rgba(255,255,255,0.3);border-radius:2px;position:relative;pointer-events:none;">' +
                    ...
                    '</div>' +
                    '<input type="range" id="__ump_seek__" min="0" max="100" value="0" step="0.1" style="position:absolute;inset:0;opacity:0;cursor:pointer;z-index:25;pointer-events:auto;">' +
                '</div>' +
        ...
        document.getElementById('__ump_fullscreen_btn__').onclick = function(e) {
            ...
            if (!isFullscreen) {
                if (overlay.requestFullscreen) {
                    overlay.requestFullscreen({ navigationUI: 'hide' }).then(function() {
                        isFullscreen = true;
                    }).catch(function() {
                        isFullscreen = true;
                    });
        ...
        video.addEventListener('loadedmetadata', function() {
            document.getElementById('__ump_loading__').style.display = 'none';
        ...
        video.addEventListener('waiting', function() {
            document.getElementById('__ump_loading__').style.display = 'block';
        });
        ...
        video.addEventListener('error', function() {
            ...
            loadEl.innerHTML = '<div style="color:#f44336;text-align:center;background:rgba(0,0,0,0.8);padding:20px;border-radius:10px;pointer-events:auto;"><div style="font-size:30px;">❌</div><div>Video lỗi</div></div>';
        ...
        document.getElementById('__ump_center_btn__').onclick = function(e) {
            e.stopPropagation();
            togglePlay();
        };
        
        document.getElementById('__ump_seek_left__').onclick = function(e) {
            e.stopPropagation();
            video.currentTime = Math.max(0, video.currentTime - 10);
            flashSeekIndicator('left', '-10s');
        };
        
        document.getElementById('__ump_seek_right__').onclick = function(e) {
            e.stopPropagation();
        ...
        document.getElementById('__ump_seek__').oninput = function(e) {
            e.stopPropagation();
        ...
        // ========== TOP BAR HANDLERS ==========
        document.getElementById('__ump_close_player__').onclick = function(e) {
            e.stopPropagation();
            video.pause();
            video.src = '';
    ...
    console.log('🎬 UMP V3.1 loaded - Fixed');
})();