/**
 * Universal Media Player & Downloader - V6.7.20 PRO
 * - MỚI: player giờ dùng ĐÚNG kiểu bottom-sheet như overlay Cài đặt — trượt
 *   lên từ dưới, nền kính hồng theo theme XUYÊN SUỐT từ header xuống quanh
 *   video (trước đó header có nền riêng màu tối che mất nền kính, nhìn như
 *   đen). Chạm ra ngoài (nền đen mờ dần) để đóng, giống hệt Cài đặt. Video có
 *   khung viền hồng-kính bao quanh (margin 12px, bo góc 14px) thay vì dính
 *   sát mép card.
 * - FIX: "get dung lượng" hiện số vô lý kiểu "97 B" cho video dài — do khi
 *   HLS đang ở chế độ auto (currentLevel = -1, ABR chưa chốt level), code cũ
 *   rơi về playerState.bandwidth có thể đang bằng 0/cũ. Giờ lấy bitrate CAO
 *   NHẤT trong các level đã biết làm ước tính đại diện, kèm ngưỡng hợp lý
 *   (dưới ~64kbps tương đương thì coi là chưa đáng tin, hiện "đang ước tính"
 *   thay vì số sai).
 * - (6.7.19) Card bo góc 2 góc trên, cao theo nội dung; toolbar gọn còn ⋮
 *   (Chất lượng+Phụ đề); xoá footer/mini-player/thu nhỏ Script.
...
function showVideoPlayer(url, type) {
  ...
  overlay.style.cssText = 'position:fixed;inset:0;z-index:2147483647;display:flex;align-items:flex-end;justify-content:center;background:rgba(0,0,0,0);';
  ...
  // CARD — dùng đúng kiểu bottom-sheet trượt lên như overlay Cài đặt: bo góc
  // 2 góc trên, nền kính hồng theo theme XUYÊN SUỐT (header không còn nền
  // riêng để lộ nền kính này, giống hệt .uvd-settings-header). Cao theo NỘI
  // DUNG (header + video co theo tỉ lệ thật), không ép flex:1 chiếm hết màn
  // hình — tự loại bỏ khoảng trống thừa quanh video từ gốc.
  var card = document.createElement('div');
  card.className = 'uvd-player-card';
  overlay.appendChild(card);
  overlay.addEventListener('click', function(e) { if (e.target === overlay) closePlayer(); }); // chạm ra ngoài để đóng, giống Cài đặt
  requestAnimationFrame(function() {
    overlay.classList.add('uvd-open');
    card.classList.add('uvd-open');
  });

  // HEADER — chỉ còn: tiêu đề + nút ⋮ (Chất lượng/Phụ đề) + nút đóng. Dòng 2
  // gộp loại/độ phân giải/dung lượng (trước nằm ở footer, đã xoá footer).
  // KHÔNG đặt nền riêng cho header — để nền kính hồng của card lộ ra xuyên
  // suốt từ header xuống viền quanh video, giống hệt cách Settings làm.
  var header = document.createElement('div');
  header.id = '__uvd_player_header__';
  header.style.cssText = 'padding:14px 16px 10px;display:flex;flex-direction:column;flex-shrink:0;transition: opacity 0.3s ease;position:relative;z-index:6;';
  ...
  titleInfo.innerHTML = '<div style="font-weight:700;font-size:15px;color:var(--accent);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;text-shadow:0 0 12px rgba(255,79,160,0.4);">▶ ' + escapeHtml(pageInfo.title) + '</div><div style="font-size:11px;color:var(--text2);margin-top:2px;">' + escapeHtml(type) + '</div>';
  titleRow.appendChild(titleInfo);

  var btnGroup = document.createElement('div');
  btnGroup.style.cssText = 'display:flex;gap:6px;flex-shrink:0;position:relative;';
  var menuBtn = document.createElement('button');
  menuBtn.className = 'uvd-back-btn';
  menuBtn.textContent = '⋮';
  menuBtn.title = 'Thêm tuỳ chọn';
  ...
  // VIDEO WRAPPER — nằm gọn trong card, co theo tỉ lệ thật của video.
  var videoWrapper = document.createElement('div');
  videoWrapper.id = '__uvd_video_wrapper__';
  videoWrapper.className = 'uvd-player-video-frame';
  videoWrapper.style.cssText = 'display:flex;align-items:center;justify-content:center;background:#000;position:relative;max-height:75dvh;';
  ...
  // Cập nhật đúng tỉ lệ khung hình thật khi biết videoWidth/videoHeight, để
  // <video-player> (và cả card bọc ngoài) co khít theo video — không còn
  // logic "header nổi lên video" nữa vì thiết kế card mới đã tự giải quyết
  // khoảng trống từ gốc.
  ...
  video.addEventListener('resize', __uvdUpdatePlayerAspect); // đổi chất lượng có thể đổi luôn videoWidth/Height
  __uvdUpdatePlayerAspect(); // phòng khi metadata đã có sẵn (ví dụ resume) trước khi listener kịp gắn
  ...
  // ===== formatTime (dùng lại cho toast "tiếp tục từ...") =====
  ...
  function updateTitleDisplay() {
    ...
    infoDiv.innerHTML = '<div style="font-weight:600;font-size:14px;color:#fff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">▶ ' + escapeHtml(pageInfo.title) + '</div><div style="font-size:11px;color:var(--text2);margin-top:2px;">' + escapeHtml(sub) + ' · <span id="__uvd_player_size__" style="color:#ff9ed6;">đang ước tính...</span></div>';
  ...
  function updateSizeEstimate() {
    ...
    if (isHls && playerState.hls) {
      var levels = playerState.hls.levels || [];
      var lvl = levels[playerState.hls.currentLevel];
      var bw = lvl ? lvl.bitrate : 0;
      if (!bw) {
        // currentLevel = -1 (auto, ABR chưa chốt) hoặc chưa có dữ liệu — lấy
        // bitrate CAO NHẤT trong các level đã biết làm ước tính đại diện, đỡ
        // hơn nhiều so với dùng playerState.bandwidth có thể đang cũ/bằng 0
        // (đây là nguyên nhân gây hiện số vô lý kiểu "97 B" cho video dài).
        for (var i = 0; i < levels.length; i++) {
          if (levels[i].bitrate > bw) bw = levels[i].bitrate;
        }
      }
      if (bw && video.duration) {
        var bytes = (bw / 8) * video.duration;
        // Ngưỡng hợp lý: dưới ~64kbps tương đương là bất thường đối với hầu
        // hết phim/video HLS — coi như dữ liệu bitrate chưa đáng tin, không
        // hiện số gây hiểu lầm.
        if (bytes < video.duration * 8000) {
...
.uvd-player-card{width:100%;max-width:1000px;margin:auto;display:flex;flex-direction:column;transform:translateY(100%);transition:transform .32s cubic-bezier(.4,0,.2,1);border-radius:22px 22px 0 0;overflow:hidden;background:var(--glass);backdrop-filter:blur(var(--uvd-blur)) saturate(130%);-webkit-backdrop-filter:blur(var(--uvd-blur)) saturate(130%);border:1px solid var(--border);box-shadow:0 -20px 50px rgba(0,0,0,0.8);max-height:94dvh;}
.uvd-player-card.uvd-open{transform:translateY(0);}
#__uvd_player_overlay__{transition:background .28s ease;}
#__uvd_player_overlay__.uvd-open{background:rgba(0,0,0,0.9);}
.uvd-player-video-frame{margin:0 12px 12px;border-radius:14px;overflow:hidden;}
...
.uvd-back-btn{background:var(--glass-hi);border:1px solid var(--border);color:var(--text);width:34px;height:34px;border-radius:var(--radius-sm);cursor:pointer;display:inline-flex;align-items:center;justify-content:center;flex-shrink:0;}
