...
function showVideoPlayer(url, type) {
  ...
  // ----- Header (có tiêu đề + thông tin phụ) -----
  var sheetHeader = document.createElement('div');
  sheetHeader.className = 'uvd-settings-header';
  sheetHeader.id = '__uvd_player_header__';
  sheetHeader.style.cssText = 'flex-shrink:0; flex-direction:column; align-items:stretch; gap:2px; transition: opacity 0.3s ease;';
  ...
  // Dòng thông tin phụ (định dạng · độ phân giải · dung lượng)
  ...
  sheetBody.style.cssText = 'padding:0; overflow:hidden; display:flex; align-items:center; justify-content:center; flex:1; min-height:0; background:transparent;';
  ...
  function updateInfoDisplay() {
    ...
    if (isHls && playerState.hls) {
      ...
      // Thử HEAD request cho file tĩnh
