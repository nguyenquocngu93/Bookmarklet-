 ...
 * - MỚI: nút "Mở tại chỗ" cho link IFRAME — nhúng iframe thật ngay trong
 *   trang hiện tại (không cần rời tab) để đếm ngược quảng cáo chạy tự nhiên.
 *   LƯU Ý THẬT: cách này KHÔNG giúp script đọc được link bên trong iframe
 *   khác gốc — Same-Origin Policy chặn cứng bất kể nhúng kiểu gì. Cách lấy
 *   được link thật: nhấn giữ trên video để dùng menu ngữ cảnh CỦA CHROME
 *   ("Sao chép địa chỉ video") — đây là tính năng trình duyệt, không phải
 *   JS của trang, nên không bị chặn.
 * - MỚI: bắt thụ động qua postMessage (kênh hợp lệ một số site dùng để giao
 *   tiếp iframe↔trang cha).
 * - Video player: giữ nguyên bản gốc + fix BLOB (mượn video gốc) + Tải Blob.
 * - PerformanceObserver theo dõi liên tục network của TRANG CHÍNH (không
 *   phải bên trong iframe khác gốc — đã đính chính lại cho đúng).
