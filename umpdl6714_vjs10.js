/**
 * Universal Media Player & Downloader - V6.7.14 PRO
 * - FIX NGHIÊM TRỌNG: tầng đoán rộng (fallback theo text) từng chạy CẢ TRONG
 *   lần quét ngầm tự động lúc mới mở script (setTimeout 400ms), khiến script
 *   tự bấm lung tung ngay cả khi người dùng chỉ đang lướt web bình thường,
 *   chưa hề bấm nút nào. Đã tách riêng cờ allowTextGuess: tầng đoán rộng giờ
 *   CHỈ chạy khi người dùng chủ động bấm ▶ (hoặc ⏭ vốn đã vậy từ trước) — lần
 *   quét ngầm tự động lúc mở script chỉ dùng selector chuẩn/tuỳ chỉnh, không
 *   bao giờ đoán rộng nữa.
 * - MỚI: footer player (trạng thái/dung lượng/thời gian) đổi từ 1 dòng cố định
 *   chiếm riêng không gian bên dưới video, sang lớp phủ kính (glass, backdrop-
 *   filter blur) đè lên đáy khung video — đỡ tốn chiều cao, nhất là với video
 *   ngang bị letterbox. `pointer-events` chỉ bật lại ở phần giờ (có thể bấm),
 *   phần còn lại trong suốt với thao tác để không chặn control gốc bên dưới.
 * - (6.7.13) Fix bug "UI chồng x2" do rescan so sai mốc; giảm giật lúc thu nhỏ;
 *   dọn nốt màu xanh còn sót; bo tròn thêm; nền kính ánh tím-hồng nhẹ.
 * - (6.7.12) Theme hồng sáng; fix chữ "Cài đặt" đen khó đọc; fix tab chia
 *   khoảng không đều.
 * - (6.7.11) Fix nghiêm trọng: tự click nhầm nút xuất TXT/M3U/CSV/JSON của
 *   chính mình; biến màu :root bị site đích đè lên.
...
function toast(msg, color) {
  ...
  el.textContent = msg;
  el.style.cssText = 'position:fixed;top:20px;left:50%;transform:translateX(-50%);background:' + (color || '#ff4fa0') + ';color:#fff;padding:12px 24px;border-radius:30px;z-index:2147483649;font:600 13px Segoe UI;box-shadow:0 4px 15px rgba(0,0,0,0.4);animation:uvdSlideIn 0.3s ease;';
  __uvdAppendRoot(el);
...
function showVideoPlayer(url, type) {
  ...
  // FOOTER — đè lớp kính lên đáy khung video thay vì chiếm hẳn 1 dòng riêng
  // bên dưới (đỡ tốn không gian dọc, đặc biệt với video ngang bị letterbox).
  var footer = document.createElement('div');
  footer.id = '__uvd_player_footer__';
  footer.style.cssText = 'position:absolute;left:0;right:0;bottom:0;z-index:5;padding:8px 16px;background:rgba(10,6,14,0.55);backdrop-filter:blur(14px) saturate(140%);-webkit-backdrop-filter:blur(14px) saturate(140%);border-top:1px solid rgba(255,143,214,0.18);font-size:12px;color:#ddd;display:flex;justify-content:space-between;pointer-events:none;transition: opacity 0.3s ease;';
  footer.innerHTML = '<span id="__uvd_player_status__">Đang tải...</span><span id="__uvd_player_size__" style="color:#ff9ed6;">Đang ước tính dung lượng...</span><span id="__uvd_player_time__" style="cursor:pointer;pointer-events:auto;">00:00</span>';
