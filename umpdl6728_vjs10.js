/**
 * Universal Media Player & Downloader - V6.7.21 PRO (hồng-tím, mượt hoá UI, ẩn script)
...
function showVideoPlayer(url, type) {
  ...
  sheet.style.cssText = 'display:flex; flex-direction:column; height:92dvh; max-height:92dvh; overflow:hidden; box-sizing:border-box; will-change: transform; backface-visibility: hidden; transition: transform .3s cubic-bezier(.22,1,.36,1); transform: translate3d(0,100%,0);';
  ...
  sheetHeader.style.cssText = 'flex-shrink:0; flex-direction:column; align-items:stretch; gap:2px;';
  ...
  titleSpan.style.cssText = 'flex:1; font-size:15px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;';
  ...
  function __uvdUpdatePlayerWidth() {
    var playerEl = document.getElementById('__uvd_player_el__');
    if (!playerEl) return;
    var isLandscape = video.videoWidth && video.videoHeight && video.videoWidth >= video.videoHeight;
    var shrink = isLandscape && !__uvdIsFullscreenNow();
    playerEl.style.width = shrink ? '90%' : '100%';
  }
  function __uvdUpdatePlayerAspect() {
    if (!video.videoWidth || !video.videoHeight) return;
    var playerEl = document.getElementById('__uvd_player_el__');
    if (playerEl) playerEl.style.aspectRatio = video.videoWidth + '/' + video.videoHeight;
    __uvdUpdatePlayerWidth();
  }
  ...
  playerState.updatePlayerWidth = __uvdUpdatePlayerWidth;
        ...
        fetch(video.src, { method: 'HEAD', headers: { 'Referer': pageInfo.referer } })
          .then(function(r) {
            ...
            if (len) {
              ...
              if (s) {
                var current = info.textContent;
                if (!current.includes('≈')) {
                  info.textContent = current + ' · ' + s;
                }
              }
            }
          })
...
.uvd-glass-panel::before{content:'';position:absolute;top:0;left:8%;right:8%;height:1px;z-index:2;background:linear-gradient(90deg,transparent,rgba(255,79,216,0.6),rgba(176,107,255,0.6),transparent);opacity:0.7}
...
.uvd-liquid-bg{position:absolute;inset:-20%;z-index:0;pointer-events:none;background:radial-gradient(closest-side,rgba(255,79,216,0.14),transparent 70%) 20% 25%/60% 60% no-repeat;filter:blur(28px);animation:uvdLiquidDrift 16s ease-in-out infinite}
...
.uvd-settings-title{font-weight:700;font-size:16px;color:var(--accent);text-shadow:0 0 12px rgba(255,79,216,0.5)}
...
.uvd-tab-indicator{position:absolute;top:4px;bottom:4px;left:0;width:0;border-radius:999px;background:var(--grad-liquid);z-index:0;box-shadow:0 3px 12px rgba(255,79,216,.45);transition:transform .4s cubic-bezier(.4,0,.2,1),width .4s cubic-bezier(.4,0,.2,1)}
...
.uvd-scope{color:var(--text);font-family:-apple-system,BlinkMacSystemFont,'SF Pro Display','Segoe UI',Roboto,sans-serif;--bg:rgba(12,5,20,0.97);--glass:rgba(28,12,38,0.86);--glass-hi:rgba(214,140,255,0.06);--border:rgba(255,141,232,0.16);--text:#f6f2ff;--text2:#b3a3cf;--text3:#6d5f89;--accent:#ff4fd8;--accent2:#b06bff;--danger:#ff5d72;--gold:#ffb84d;--success:#34d399;--card-bg:rgba(255,255,255,0.035);--fs-xs:11px;--fs-sm:12px;--fs-base:13px;--fs-md:14px;--fs-lg:16px;--radius-sm:14px;--radius-md:20px;--radius-lg:32px;--grad-liquid:linear-gradient(135deg,var(--accent),var(--accent2));--glow-px:0px;--glow-op:0;--btn-bg:rgba(255,255,255,0.1);--btn-danger-bg:rgba(255,93,114,0.22);--btn-danger-border:rgba(255,93,114,0.4);--btn-success-bg:rgba(52,211,153,0.2);--btn-success-border:rgba(52,211,153,0.4);--btn-accent-bg:rgba(255,79,216,0.28);--btn-purple-bg:rgba(176,107,255,0.28);--btn-gold-bg:rgba(255,184,77,0.26)}
.uvd-fx-on .uvd-btn{transition:box-shadow .25s ease,transform .15s ease}
.uvd-fx-on .uvd-btn:active{transform:scale(.95)}
.uvd-fx-on.uvd-glass-panel,.uvd-fx-on #__uvd_player_header__{box-shadow:0 0 var(--glow-px) rgba(255,79,216,var(--glow-op)),0 8px 30px rgba(0,0,0,0.5)}
...
.uvd-restore-btn span.uvd-restore-dot{width:8px;height:8px;border-radius:50%;background:var(--grad-liquid);box-shadow:0 0 8px rgba(255,79,216,0.7);animation:uvdPulse 2s infinite}
...
.uvd-card{background:var(--card-bg);border:1px solid var(--border);border-radius:var(--radius-md);padding:14px;margin-bottom:10px;font-size:var(--fs-base);box-shadow:0 1px 0 rgba(255,255,255,0.04) inset;animation:uvdCardEnter .28s cubic-bezier(.22,1,.36,1) both;color:var(--text);will-change:transform;transition:transform var(--uvd-transition),box-shadow var(--uvd-transition),border-color var(--uvd-transition)}
.uvd-card:hover{transform:translateY(-5px) scale(1.01);box-shadow:0 18px 40px rgba(0,0,0,0.8),0 0 0 1px rgba(255,79,216,0.4) inset}
.uvd-type-badge{display:inline-block;padding:4px 12px;border-radius:var(--radius-sm);font-size:var(--fs-xs);font-weight:700;background:linear-gradient(135deg,rgba(255,79,216,0.22),rgba(176,107,255,0.18));color:var(--accent);border:1px solid rgba(255,79,216,0.28);letter-spacing:.03em}
...
.uvd-profile-card{display:flex;align-items:center;gap:14px;background:linear-gradient(135deg,rgba(255,79,216,0.14),rgba(176,107,255,0.08));border:1px solid rgba(255,79,216,0.25);border-radius:var(--radius-lg);padding:16px;margin-bottom:10px;animation:uvdCardEnter .4s ease both}
.uvd-profile-avatar{flex-shrink:0;width:56px;height:56px;border-radius:50%;background:var(--grad-liquid);color:#fff;font-weight:700;font-size:18px;display:flex;align-items:center;justify-content:center;box-shadow:0 6px 18px rgba(255,79,216,0.4),0 0 0 3px rgba(255,255,255,0.08)}
...
.uvd-section-num{width:20px;height:20px;border-radius:50%;background:var(--grad-liquid);color:#fff;font-size:11px;display:inline-flex;align-items:center;justify-content:center;flex-shrink:0;box-shadow:0 2px 8px rgba(255,79,216,0.4)}
.uvd-timeline-card{border-left:2px solid rgba(255,79,216,0.3)}
...
.uvd-step-num{flex-shrink:0;width:24px;height:24px;border-radius:50%;background:var(--grad-liquid);color:#fff;font-size:12px;font-weight:700;display:flex;align-items:center;justify-content:center;box-shadow:0 2px 8px rgba(255,79,216,0.4)}
.uvd-step-text{font-size:13px;color:var(--text2);line-height:1.65;padding-top:2px}
.uvd-step-text strong{color:var(--text)}
.uvd-callout{display:flex;gap:10px;align-items:flex-start;background:rgba(255,79,216,0.1);border:1px solid rgba(255,79,216,0.25);border-left:3px solid var(--accent);border-radius:10px;padding:10px 12px;margin-top:10px;font-size:12px;color:var(--text2);line-height:1.6}
...
function buildUI() {
  ...
  header.innerHTML = 
    '<div style="display:flex;align-items:center;gap:8px;">' +
      '<span style="width:10px;height:10px;background:var(--grad-liquid);border-radius:50%;animation:uvdPulse 2s infinite;box-shadow:0 0 8px rgba(255,79,216,0.6);"></span>' +
...
function buildStreamCardHTML(item, i) {
  ...
  if (item.type === 'BLOB') {
    actionsHtml =
      '<button class="uvd-btn uvd-btn-sm" data-action="play" data-url="' + encodeURIComponent(item.url) + '" data-type="' + escapeHtml(item.type) + '" style="background:rgba(255,79,216,0.25);">Xem</button>' +
    ...
    actionsHtml =
      '<button class="uvd-btn uvd-btn-sm" data-action="share" data-url="' + encodeURIComponent(item.url) + '" style="background:rgba(176,107,255,0.22);">Chia sẻ</button>' +
...
function showQualityPicker(url) {
  ...
  parseM3U8Master(url, function(qualities) {
    ...
    qualities.forEach(function(q) {
      ...
      playBtn.style.background = 'rgba(255,79,216,0.25)';
