/**
 * Universal Media Player & Downloader - V6.0
 ...
 * - Chặn popup/tab lạ LIÊN TỤC suốt phiên (kể cả popunder theo timer/click-anywhere)
 ...
 * - Lọc video quảng cáo bằng filterlist (import pattern)
 ...
 * - Header player với hiệu ứng kính mờ
 * - Hướng dẫn tạo bookmarklet đơn giản
 * - Fix lỗi giao diện khi mở iframe (dùng button an toàn)
 * - Fix nút bị đổi màu trên một số web (CSS !important)
 * - Trạng thái buffering/đang phát
 * - Loại bỏ auto copy khi share
...
function killBlankLinks(e) {
  var t = e.target;
  // Bỏ qua các sự kiện từ giao diện của chúng ta
...
// ========== AUTO CLICK PLAY (duplicate) ==========
var AUTO_CLICK_SELECTORS = [
  '.jw-icon-playback', '.jw-display-icon-container', '.jw-icon-display',
  '.vjs-big-play-button', '.vjs-play-control',
  '.plyr__control[data-plyr="play"]',
  '.play-button', '.playBtn', '.btn-play', '.icon-play', '.play-icon',
  '.play-overlay', '.video-play-button', '.big-play-button',
  '.fp-play', '.mejs__play', '.mejs-overlay-play',
  'button.play', 'div.play', 'span.play-btn',
  '[aria-label="Play"]', '[aria-label="play"]', '[title="Play"]', '[title="play"]',
  '.flowplayer .fp-ui', '.player-poster', '.video-thumb-wrap', '.thumb-play'
];
...
function autoClickPlay(root, depth) {
  ...
  var selectors = customSel ? [customSel].concat(AUTO_CLICK_SELECTORS) : AUTO_CLICK_SELECTORS;
  ...
  try {
    root.querySelectorAll('iframe').forEach(function(f) {
      try { if (f.contentDocument) clicked += autoClickPlay(f.contentDocument, depth + 1); }
      catch(e) {}
    });
  } catch(e) {}
  return clicked;
...
// ========== OVERLAY PLAYER WITH TOOLBAR (ENHANCED) ==========
function showVideoPlayer(url, type) {
  ...
  // Cập nhật trạng thái buffering/playing
  var statusEl = document.getElementById('__uvd_player_status__');
  video.addEventListener('waiting', function() {
    if (statusEl) statusEl.textContent = '• Đang buffering...';
  });
  ...
  video.addEventListener('pause', function() {
    if (statusEl && !video.ended) statusEl.textContent = '• Tạm dừng';
  });
...
// (Phần 1 kết thúc - tiếp tục với Phần 2)
...
// ========== BUILD UI (CSS đã có !important để chống ghi đè) ==========
...
.uvd-glass-panel{background:var(--glass)!important;backdrop-filter:blur(28px) saturate(130%);-webkit-backdrop-filter:blur(28px) saturate(130%);border:1px solid var(--border)!important;border-radius:var(--radius-lg);box-shadow:0 20px 50px rgba(0,0,0,0.8),0 0 0 1px rgba(255,255,255,0.03) inset,0 1px 0 rgba(255,255,255,0.08) inset;color:var(--text)!important;font-family:-apple-system,BlinkMacSystemFont,'SF Pro Display','Segoe UI',Roboto,sans-serif;font-size:var(--fs-base);padding:16px;width:100%;position:relative;overflow:hidden;max-width:1000px;margin:auto}
...
.uvd-btn{background:var(--glass-hi)!important;border:1px solid var(--border)!important;color:var(--text)!important;padding:9px 16px;border-radius:var(--radius-md);font-weight:600;font-size:var(--fs-base);cursor:pointer;transition:all 0.2s;backdrop-filter:blur(10px);text-align:center;position:relative;overflow:hidden;display:inline-block;box-shadow:0 3px 10px rgba(0,0,0,0.4),0 1px 0 rgba(255,255,255,0.08) inset;line-height:1.3}
.uvd-btn:hover{background:rgba(255,255,255,0.10)!important;border-color:rgba(255,255,255,0.20)!important;transform:translateY(-1px);box-shadow:0 5px 14px rgba(0,0,0,0.5),0 1px 0 rgba(255,255,255,0.12) inset}
.uvd-btn:active{transform:scale(0.96)}
.uvd-btn-sm{padding:7px 12px;font-size:var(--fs-sm);border-radius:var(--radius-sm)}
.uvd-btn-icon{background:var(--glass-hi)!important;border:1px solid var(--border)!important;color:var(--text)!important;width:34px;height:34px;border-radius:var(--radius-sm);cursor:pointer;display:inline-flex;align-items:center;justify-content:center;transition:all 0.2s;backdrop-filter:blur(10px);position:relative;overflow:hidden;box-shadow:0 3px 8px rgba(0,0,0,0.35),0 1px 0 rgba(255,255,255,0.08) inset}
...
.uvd-card{background:var(--card-bg)!important;border:1px solid var(--border)!important;border-radius:var(--radius-md);padding:14px;margin-bottom:10px;font-size:var(--fs-base);backdrop-filter:blur(6px);box-shadow:0 1px 0 rgba(255,255,255,0.04) inset;transition:transform 0.3s cubic-bezier(.4,0,.2,1),box-shadow 0.3s ease,background 0.3s ease,border-color 0.3s ease;animation:uvdCardEnter 0.4s ease both;color:var(--text)!important}
.uvd-card:hover{transform:translateY(-3px);box-shadow:0 14px 30px rgba(0,0,0,0.7),0 0 0 1px rgba(109,140,255,0.30) inset,0 0 24px rgba(109,140,255,0.08);background:rgba(255,255,255,0.05)!important;border-color:rgba(109,140,255,0.25)!important}
.uvd-type-badge{display:inline-block;padding:4px 12px;border-radius:var(--radius-sm);font-size:var(--fs-xs);font-weight:700;background:linear-gradient(135deg,rgba(109,140,255,0.22),rgba(185,139,255,0.18));color:var(--accent)!important;border:1px solid rgba(109,140,255,0.28);letter-spacing:0.03em}
.uvd-url-box{background:rgba(0,0,0,0.5)!important;border-radius:var(--radius-sm);padding:12px;font-family:'SFMono-Regular',Consolas,monospace;font-size:var(--fs-sm);word-break:break-all;color:var(--text2)!important;max-height:100px;overflow-y:auto;line-height:1.5;border:1px solid rgba(255,255,255,0.04)}
...
.uvd-tabbar{position:relative;display:flex;gap:2px;padding:4px;background:rgba(255,255,255,0.04)!important;border:1px solid var(--border)!important;border-radius:999px;margin-bottom:10px;flex-shrink:0}
...
.uvd-tab{position:relative;z-index:1;flex:1;min-width:0;background:transparent!important;border:none!important;color:var(--text2)!important;font-weight:600;font-size:var(--fs-sm);padding:9px 6px;border-radius:999px;cursor:pointer;transition:color 0.25s;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
...
.uvd-profile-card{display:flex;align-items:center;gap:14px;background:linear-gradient(135deg,rgba(109,140,255,0.14),rgba(185,139,255,0.08));border:1px solid rgba(109,140,255,0.25)!important;border-radius:var(--radius-lg);padding:16px;margin-bottom:10px;animation:uvdCardEnter 0.4s ease both}
.uvd-profile-avatar{flex-shrink:0;width:56px;height:56px;border-radius:50%;background:var(--grad-liquid);color:#fff;font-weight:700;font-size:18px;display:flex;align-items:center;justify-content:center;box-shadow:0 6px 18px rgba(109,140,255,0.4),0 0 0 3px rgba(255,255,255,0.08);letter-spacing:0.02em}
.uvd-profile-info{min-width:0}
.uvd-profile-name{font-weight:700;font-size:15px;color:var(--text)!important}
.uvd-profile-role{font-size:11.5px;color:var(--text2)!important;margin-top:2px}
.uvd-profile-tags{display:flex;flex-wrap:wrap;gap:5px;margin-top:8px}
.uvd-tag{font-size:10px;font-weight:600;padding:3px 9px;border-radius:999px;background:rgba(255,255,255,0.08);border:1px solid var(--border);color:var(--text2)!important}
...
.uvd-stat-num{font-size:18px;font-weight:700;color:var(--accent)!important}
.uvd-stat-label{font-size:10px;color:var(--text3)!important;margin-top:2px}
.uvd-section-title{display:flex;align-items:center;gap:8px;font-weight:700;font-size:13px;color:var(--text)!important;margin:16px 0 8px}
...
.uvd-inline-code{display:inline-block;background:rgba(0,0,0,0.35);padding:3px 9px;border-radius:6px;color:var(--accent2)!important;font-family:'SFMono-Regular',Consolas,monospace;font-size:11px;border:1px solid rgba(255,255,255,0.06);margin:2px 0}
.uvd-profile-footer{text-align:center;font-size:11px;color:var(--text3)!important;margin-top:14px;padding-top:12px;border-top:1px solid var(--border)}
`;
  ...
  arr.forEach(function(item, i) {
    ...
    card.innerHTML = 
      ...
      '<div class="uvd-grid-2" style="margin-top:8px;">' +
        ...
        (item.type === 'IFRAME' ? 
          '<button class="uvd-btn uvd-btn-sm uvd-ripple-btn" data-action="iframe" data-url="' + encodeURIComponent(item.url) + '" style="text-align:center;grid-column:1/3;">Mở iframe</button>' :
          (item.type === 'M3U8' ?
...
console.log('V6.0 UMP DL - Buffering status, iframe fix, anti-style-override');
toast('V6.0 sẵn sàng — trạng thái buffering, iframe an toàn, giao diện ổn định!');
