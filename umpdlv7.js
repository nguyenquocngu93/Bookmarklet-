/**
 * Universal Media Player & Downloader - Video.js Edition
 * - Sử dụng Video.js v10 làm trình phát chính
 * - Giao diện Glassmorphism (kính mờ)
 * - Tất cả tính năng gom vào menu Settings (⚙️)
 * - Hỗ trợ HLS, MP4, WEBM, MKV...
 * - Giữ nguyên các chức năng: scan URL, ad filter, resume, auto-next, screenshot, PiP, boost, subtitle...
...
// ========== PLAYER STATE (Video.js) ==========
var playerState = {
  overlay: null,
  mini: null,
  video: null,          // thẻ video gốc
  player: null,         // instance Video.js
  url: '',
  type: '',
  isMinimized: false,
  pinned: false,
...
function setSleepTimer(minutes) {
  ...
  playerState.sleepTimerId = setTimeout(function() {
    if (playerState.player) playerState.player.pause();
...
function showSubtitlePanel() {
  ...
  panel.querySelector('#__uvd_sub_file__').onchange = function(e) {
    ...
    reader.onload = function() {
      ...
      var blobUrl = URL.createObjectURL(new Blob([vtt], { type: 'text/vtt' }));
      attachSubtitleTrack(playerState.video, blobUrl, file.name.replace(/\.(srt|vtt)$/i, ''));
      overlay2.remove();
    };
  ...
  panel.querySelector('#__uvd_sub_off__').onclick = function() {
    if (playerState.video && playerState.video.textTracks) {
      for (var i = 0; i < playerState.video.textTracks.length; i++) playerState.video.textTracks[i].mode = 'disabled';
    }
    toast('Đã tắt phụ đề');
    overlay2.remove();
  };
...
function showVideoPlayer(url, type) {
  ...
  overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:2147483648;display:flex;flex-direction:column;animation:uvdFadeIn 0.3s ease;';
  ...
  header.style.cssText = 'padding:10px 16px;background:rgba(14,16,22,0.92);display:flex;flex-direction:column;flex-shrink:0;border-bottom:1px solid rgba(255,255,255,0.2);box-shadow:0 4px 20px rgba(0,0,0,0.5);';
  ...
  // Footer (giữ nguyên để hiển thị trạng thái)
  var footer = document.createElement('div');
  footer.id = '__uvd_player_footer__';
  footer.style.cssText = 'padding:8px 16px;background:rgba(0,0,0,0.7);border-top:1px solid var(--btn-bg);font-size:12px;color:#aaa;display:flex;justify-content:space-between;flex-shrink:0;';
  footer.innerHTML = '<span id="__uvd_player_status__">Đang tải Video.js...</span><span id="__uvd_player_size__" style="color:#8ab4ff;">Đang ước tính...</span><span id="__uvd_player_time__" style="cursor:pointer;">00:00</span>';
  overlay.appendChild(footer);

  // TẢI VIDEO.JS VÀ PLUGIN
  var loadedVideoJS = false;
  function loadVideoJS(callback) {
    if (typeof videojs !== 'undefined') {
      // Kiểm tra xem videojs đã load chưa
      if (videojs && videojs.version && parseInt(videojs.version) >= 10) {
        loadedVideoJS = true;
        callback();
        return;
      }
  ...
  loadVideoJS(function() {
    ...
    var player = videojs(videoEl, {
      ...
      controlBar: {
        children: [
          ...
          'customSettingsButton', // sẽ đăng ký sau
          'fullscreenToggle'
        ]
      }
    });
    ...
    // Đăng ký nút Settings tùy chỉnh
    var SettingsButton = videojs.getComponent('Button');
    var CustomSettingsButton = videojs.extend(SettingsButton, {
      constructor: function() {
        SettingsButton.apply(this, arguments);
    ...
    // Thêm nút vào controlBar (nếu chưa có)
    var controlBar = player.getChild('controlBar');
    if (controlBar) {
      // Xóa nút cũ nếu có
      var oldBtn = controlBar.getChild('customSettingsButton');
      if (oldBtn) controlBar.removeChild(oldBtn);
      controlBar.addChild('customSettingsButton', {}, 9); // vị trí trước fullscreen
    }

    // Thiết lập giao diện Glass cho Video.js (CSS sẽ được thêm sau)
    // Thêm CSS glass
    ...
    addCleanup(function() { glassStyle.remove(); });
    ...
    // Sự kiện loadedmetadata
    player.one('loadedmetadata', function() {
      lockOrientation(videoEl);
      ...
      // Volume boost
      if (data.settings.volumeBoost) enableVolumeBoost(videoEl, data.settings.volumeBoostMax);
      // Auto fullscreen
    ...
    // Sự kiện timeupdate để lưu vị trí và cập nhật thời gian
    var lastSave = 0;
    player.on('timeupdate', function() {
      ...
      if (data.settings.resumePlayback && Date.now() - lastSave > 5000) {
        lastSave = Date.now();
        savePlaybackPosition(url, videoEl);
      }
      updateTimeDisplay(t, d);
      updateTitleDisplay();
    });
    ...
    // Sự kiện pause/play để ẩn/hiện controls
    player.on('play', resetHideTimer);
    ...
    // Xử lý chất lượng (nếu HLS)
    if (isHls) {
      player.ready(function() {
        var vhs = player.tech().vhs;
        if (vhs) {
          // Lấy danh sách chất lượng
          var levels = vhs.playlists.media().attributes;
          // Tuy nhiên VHS không cung cấp trực tiếp, ta dùng parseM3U8Master để lấy
          parseM3U8Master(url, function(qualities) {
            if (qualities && qualities.length > 0) {
              playerState.qualities = qualities;
              // Áp dụng default quality
              applyDefaultQualityPreference(player);
            }
          });
        }
      });
    ...
    function updateTimeDisplay(t, d) {
      var el = document.getElementById('__uvd_player_time__');
      if (!el) return;
      if (!d) { el.textContent = '00:00'; return; }
    ...
    function updateTitleDisplay() {
      ...
      var sub = playerState.type + (res ? ' · ' + res : '');
    ...
    function applyDefaultQualityPreference(p) {
      ...
      var targetIdx = 0;
      if (pref === 'highest') targetIdx = qualities.length - 1;
      else if (pref === 'lowest') targetIdx = 0;
      // VHS không hỗ trợ switch trực tiếp, ta có thể set quality bằng cách chọn variant
      // Có thể dùng vhs.playlists.media() để chọn, nhưng cách đơn giản là dùng parseM3U8Master để lấy url và chuyển
      // Tạm thời, ta chỉ hiển thị thông báo
      toast('Chất lượng mặc định: ' + pref);
    }

    // Hàm show/hide controls
    function showControls() {
      var headerEl = document.getElementById('__uvd_player_header__');
      var footerEl = document.getElementById('__uvd_player_footer__');
    ...
    function resetHideTimer() {
      ...
      playerState.hideTimeout = setTimeout(function() {
        if (playerState.player && playerState.player.paused()) return;
        if (playerState.pinned) return;
        var headerEl = document.getElementById('__uvd_player_header__');
        var footerEl = document.getElementById('__uvd_player_footer__');
        if (headerEl) headerEl.style.opacity = '0';
        if (footerEl) footerEl.style.opacity = '0.3';
        playerState.controlsVisible = false;
      }, delay);
    }

    // Sự kiện click vào video wrapper để reset timer
    videoWrapper.addEventListener('click', resetHideTimer);
    videoWrapper.addEventListener('touchstart', resetHideTimer);
    // Click vào thời gian để đổi chế độ
    document.getElementById('__uvd_player_time__').addEventListener('click', function() {
      playerState.timeMode = (playerState.timeMode + 1) % 3;
      updateTimeDisplay(player.currentTime(), player.duration());
      toast('Chế độ: ' + ['còn lại', 'đã qua', 'tổng'][playerState.timeMode]);
      resetHideTimer();
    });
    ...
    function onFullscreenChange() {
      ...
      if (isFullscreen) lockOrientation(videoEl);
    ...
    // Lưu tham chiếu
    playerState._resetHideTimer = resetHideTimer;
    playerState._showControls = showControls;
    playerState._videojsReady = true;
    // Khởi tạo ban đầu
    resetHideTimer();
  });

  // ===== HÀM MENU CÀI ĐẶT (POPUP) =====
  function showSettingsMenu() {
    if (document.getElementById('__uvd_settings_menu__')) return;
    var overlay2 = document.createElement('div');
    overlay2.id = '__uvd_settings_menu__';
    overlay2.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.6);z-index:2147483649;display:flex;align-items:center;justify-content:center;backdrop-filter:blur(6px);';
    var panel = document.createElement('div');
    panel.style.cssText = 'background:rgba(16,18,26,0.92);border-radius:24px;padding:24px;width:90%;max-width:400px;max-height:80vh;overflow-y:auto;border:1px solid rgba(255,255,255,0.12);box-shadow:0 20px 60px rgba(0,0,0,0.8);backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);';
    panel.innerHTML = `
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;">
        <span style="font-weight:700;font-size:17px;color:#fff;">⚙️ Cài đặt trình phát</span>
        <button class="uvd-btn uvd-btn-sm" id="__uvd_settings_close__" style="background:rgba(255,255,255,0.1);color:#fff;border:1px solid rgba(255,255,255,0.1);">✕</button>
      </div>
      <div id="__uvd_settings_content__" style="display:flex;flex-direction:column;gap:10px;">
        <!-- Các mục sẽ được render -->
      </div>
    `;
    overlay2.appendChild(panel);
    __uvdAppendRoot(overlay2);
    document.getElementById('__uvd_settings_close__').onclick = function() { overlay2.remove(); };
    overlay2.onclick = function(e) { if (e.target === overlay2) overlay2.remove(); };

    // Render các mục
    var content = document.getElementById('__uvd_settings_content__');
    renderSettingsMenu(content);
  }
  ...
  function renderSettingsMenu(container) {
    ...
    // 1. Tốc độ
    var currentSpeed = p.playbackRate ? p.playbackRate() : 1;
    var speedHtml = `
      <div style="display:flex;align-items:center;justify-content:space-between;padding:6px 0;border-bottom:1px solid rgba(255,255,255,0.06);">
        <span style="color:var(--text2);font-size:13px;">⏩ Tốc độ</span>
        <div style="display:flex;align-items:center;gap:8px;">
          <button class="uvd-btn uvd-btn-sm" id="__uvd_speed_dec__" style="padding:4px 10px;">−</button>
          <span id="__uvd_speed_display__" style="color:#fff;font-weight:600;min-width:40px;text-align:center;">${currentSpeed}x</span>
          <button class="uvd-btn uvd-btn-sm" id="__uvd_speed_inc__" style="padding:4px 10px;">+</button>
        </div>
      </div>
    `;
    items.push(speedHtml);

    // 2. Chất lượng (nếu có)
    if (playerState.qualities && playerState.qualities.length > 0) {
      var qHtml = `
        <div style="display:flex;align-items:center;justify-content:space-between;padding:6px 0;border-bottom:1px solid rgba(255,255,255,0.06);">
          <span style="color:var(--text2);font-size:13px;">📶 Chất lượng</span>
          <select id="__uvd_quality_select__" style="background:rgba(0,0,0,0.4);color:#fff;border:1px solid rgba(255,255,255,0.1);border-radius:8px;padding:4px 8px;font-size:12px;">
            ${playerState.qualities.map((q, idx) => `<option value="${idx}">${q.label}</option>`).join('')}
          </select>
        </div>
      `;
      items.push(qHtml);
    ...
    items.push(`
      <div style="display:flex;align-items:center;justify-content:space-between;padding:6px 0;border-bottom:1px solid rgba(255,255,255,0.06);">
        <span style="color:var(--text2);font-size:13px;">💬 Phụ đề</span>
        <button class="uvd-btn uvd-btn-sm" id="__uvd_subtitle_btn__" style="background:var(--btn-purple-bg);color:#fff;">Chọn</button>
      </div>
    `);
    ...
    items.push(`
      <div style="display:flex;align-items:center;justify-content:space-between;padding:6px 0;border-bottom:1px solid rgba(255,255,255,0.06);">
        <span style="color:var(--text2);font-size:13px;">⏰ Hẹn giờ ngủ</span>
        <button class="uvd-btn uvd-btn-sm" id="__uvd_sleep_btn__" style="background:var(--btn-bg);color:#fff;">Đặt</button>
      </div>
    `);

    // 5. Boost
    var boostOn = data.settings.volumeBoost;
    items.push(`
      <div style="display:flex;align-items:center;justify-content:space-between;padding:6px 0;border-bottom:1px solid rgba(255,255,255,0.06);">
        <span style="color:var(--text2);font-size:13px;">🔊 Tăng âm (${data.settings.volumeBoostMax}%)</span>
        <button id="__uvd_boost_toggle__" class="uvd-toggle-switch ${boostOn ? 'uvd-toggle-on' : ''}" style="width:36px;height:22px;border-radius:11px;"><span class="uvd-toggle-knob" style="width:16px;height:16px;top:3px;left:3px;"></span></button>
      </div>
    `);

    // 6. Mute
    var isMuted = p.muted ? p.muted() : false;
    items.push(`
      <div style="display:flex;align-items:center;justify-content:space-between;padding:6px 0;border-bottom:1px solid rgba(255,255,255,0.06);">
        <span style="color:var(--text2);font-size:13px;">🔇 Tắt tiếng</span>
        <button id="__uvd_mute_toggle__" class="uvd-toggle-switch ${isMuted ? 'uvd-toggle-on' : ''}" style="width:36px;height:22px;border-radius:11px;"><span class="uvd-toggle-knob" style="width:16px;height:16px;top:3px;left:3px;"></span></button>
      </div>
    `);
    ...
    if (document.pictureInPictureEnabled) {
      items.push(`
        <div style="display:flex;align-items:center;justify-content:space-between;padding:6px 0;border-bottom:1px solid rgba(255,255,255,0.06);">
          <span style="color:var(--text2);font-size:13px;">⛶ Picture-in-Picture</span>
          <button class="uvd-btn uvd-btn-sm" id="__uvd_pip_btn__" style="background:var(--btn-bg);color:#fff;">Mở</button>
        </div>
      `);
    }
    ...
    items.push(`
      <div style="display:flex;align-items:center;justify-content:space-between;padding:6px 0;border-bottom:1px solid rgba(255,255,255,0.06);">
        <span style="color:var(--text2);font-size:13px;">📷 Chụp ảnh</span>
        <button class="uvd-btn uvd-btn-sm" id="__uvd_screenshot_btn__" style="background:var(--btn-bg);color:#fff;">Chụp</button>
      </div>
    `);

    // 9. Ghim controls
    var pinned = playerState.pinned;
    items.push(`
      <div style="display:flex;align-items:center;justify-content:space-between;padding:6px 0;border-bottom:1px solid rgba(255,255,255,0.06);">
        <span style="color:var(--text2);font-size:13px;">📌 Ghim điều khiển</span>
        <button id="__uvd_pin_toggle__" class="uvd-toggle-switch ${pinned ? 'uvd-toggle-on' : ''}" style="width:36px;height:22px;border-radius:11px;"><span class="uvd-toggle-knob" style="width:16px;height:16px;top:3px;left:3px;"></span></button>
      </div>
    `);

    // 10. Auto-next
    var autoNext = data.settings.autoNext;
    items.push(`
      <div style="display:flex;align-items:center;justify-content:space-between;padding:6px 0;border-bottom:1px solid rgba(255,255,255,0.06);">
        <span style="color:var(--text2);font-size:13px;">⏭ Tự động phát tiếp</span>
        <button id="__uvd_autonext_toggle__" class="uvd-toggle-switch ${autoNext ? 'uvd-toggle-on' : ''}" style="width:36px;height:22px;border-radius:11px;"><span class="uvd-toggle-knob" style="width:16px;height:16px;top:3px;left:3px;"></span></button>
      </div>
    `);

    // 11. Resume
    var resume = data.settings.resumePlayback;
    items.push(`
      <div style="display:flex;align-items:center;justify-content:space-between;padding:6px 0;border-bottom:1px solid rgba(255,255,255,0.06);">
        <span style="color:var(--text2);font-size:13px;">🔄 Tiếp tục xem dở</span>
        <button id="__uvd_resume_toggle__" class="uvd-toggle-switch ${resume ? 'uvd-toggle-on' : ''}" style="width:36px;height:22px;border-radius:11px;"><span class="uvd-toggle-knob" style="width:16px;height:16px;top:3px;left:3px;"></span></button>
      </div>
    `);
    ...
    // Gán sự kiện
    // Tốc độ
    var speedDisplay = document.getElementById('__uvd_speed_display__');
    document.getElementById('__uvd_speed_dec__').onclick = function() {
      var rates = [0.25,0.5,0.75,1,1.25,1.5,1.75,2];
      var cur = p.playbackRate();
      var idx = rates.indexOf(cur);
      if (idx > 0) p.playbackRate(rates[idx-1]);
      else p.playbackRate(0.25);
      speedDisplay.textContent = p.playbackRate() + 'x';
      toast('Tốc độ: ' + p.playbackRate() + 'x');
    };
    document.getElementById('__uvd_speed_inc__').onclick = function() {
      var rates = [0.25,0.5,0.75,1,1.25,1.5,1.75,2];
      var cur = p.playbackRate();
      var idx = rates.indexOf(cur);
      if (idx < rates.length-1) p.playbackRate(rates[idx+1]);
      else p.playbackRate(2);
      speedDisplay.textContent = p.playbackRate() + 'x';
      toast('Tốc độ: ' + p.playbackRate() + 'x');
    };

    // Chất lượng
    var qualitySelect = document.getElementById('__uvd_quality_select__');
    if (qualitySelect) {
      qualitySelect.onchange = function() {
        var idx = parseInt(this.value);
        var q = playerState.qualities[idx];
        if (q && q.url) {
          // VHS không hỗ trợ switch đơn giản, ta sẽ reload player với url mới
          toast('Chuyển sang ' + q.label + ', đang tải lại...');
          p.src({ src: q.url, type: 'application/x-mpegURL' });
          p.load();
          p.play();
        }
      };
    ...
    // Phụ đề
    document.getElementById('__uvd_subtitle_btn__').onclick = function() {
      var menuEl = document.getElementById('__uvd_settings_menu__');
      if (menuEl) menuEl.remove();
      showSubtitlePanel();
    };

    // Hẹn giờ
    document.getElementById('__uvd_sleep_btn__').onclick = function() {
      var menuEl = document.getElementById('__uvd_settings_menu__');
      if (menuEl) menuEl.remove();
      showSleepMenu();
    };
    ...
    // Boost toggle
    document.getElementById('__uvd_boost_toggle__').onclick = function() {
      var isOn = this.classList.toggle('uvd-toggle-on');
      data.settings.volumeBoost = isOn;
      storage.set(data);
      if (isOn) { enableVolumeBoost(videoEl, data.settings.volumeBoostMax); toast('Đã bật Boost ' + data.settings.volumeBoostMax + '%'); }
      else { disableVolumeBoost(); toast('Đã tắt Boost'); }
    };

    // Mute toggle
    document.getElementById('__uvd_mute_toggle__').onclick = function() {
      var isOn = this.classList.toggle('uvd-toggle-on');
      p.muted(isOn);
      toast(isOn ? 'Đã tắt tiếng' : 'Đã bật tiếng');
    };

    // PiP
    var pipBtn = document.getElementById('__uvd_pip_btn__');
    if (pipBtn) {
      pipBtn.onclick = function() {
        var v = videoEl;
        if (!v) return;
        if (document.pictureInPictureElement) document.exitPictureInPicture().catch(function(){});
        else v.requestPictureInPicture().catch(function() { toast('Không hỗ trợ PiP'); });
      };
    }

    // Screenshot
    document.getElementById('__uvd_screenshot_btn__').onclick = function() {
      var v = videoEl;
    ...
    // Pin toggle
    document.getElementById('__uvd_pin_toggle__').onclick = function() {
      var isOn = this.classList.toggle('uvd-toggle-on');
      playerState.pinned = isOn;
      toast(isOn ? 'Đã ghim controls' : 'Bỏ ghim');
      if (isOn) {
        showControls();
        clearTimeout(playerState.hideTimeout);
      } else {
    ...
    // Auto-next toggle
    document.getElementById('__uvd_autonext_toggle__').onclick = function() {
      var isOn = this.classList.toggle('uvd-toggle-on');
      data.settings.autoNext = isOn;
      storage.set(data);
      toast(isOn ? 'Tự động phát tiếp: BẬT' : 'Tự động phát tiếp: TẮT');
    };

    // Resume toggle
    document.getElementById('__uvd_resume_toggle__').onclick = function() {
      var isOn = this.classList.toggle('uvd-toggle-on');
      data.settings.resumePlayback = isOn;
      storage.set(data);
      toast(isOn ? 'Nhớ vị trí: BẬT' : 'Nhớ vị trí: TẮT');
    };
  }

  // Hàm getNextStreamUrl (giữ nguyên)
...
function restorePlayer() {
  ...
  if (mini) { mini.style.transition = 'opacity 0.2s ease'; mini.style.opacity = '0'; setTimeout(function() { mini.remove(); }, 220); playerState.mini = null; }
  overlay.style.display = 'flex';
  overlay.style.transition = 'opacity 0.25s ease';
  overlay.style.opacity = '1';
  if (playerState.player) { playerState.player.play().catch(function(){}); if (playerState._resetHideTimer) playerState._resetHideTimer(); }
}
...
function closePlayer() {
  if (playerState.overlay) {
    ...
    playerState._videojsReady = false;
...
.uvd-panel-minimized .uvd-panel-content > *:not(#__uvd_header__){pointer-events:none;opacity:0;transform:scale(0.96);transition:opacity 0.25s ease, transform 0.3s ease;max-height:0;overflow:hidden;padding:0;margin:0;}
.uvd-panel-minimized .uvd-panel-content #__uvd_header__{margin-bottom:0;border-bottom:none;padding-bottom:4px;}
.uvd-panel-minimized .uvd-glass-panel{background:rgba(10,11,16,0.95)!important;backdrop-filter:blur(0)!important}
.uvd-reduce-motion .uvd-liquid-bg{animation:none}
.uvd-reduce-motion *{animation:none!important;transition:none!important}
.uvd-reduce-motion .uvd-glass-panel{backdrop-filter:blur(0px)!important;-webkit-backdrop-filter:blur(0px)!important;background:rgba(10,11,16,0.98)!important;border-color:rgba(255,255,255,0.12);}
...
.uvd-reduce-motion .uvd-glass-panel .uvd-card{background:rgba(255,255,255,0.06);}
.uvd-reduce-motion .uvd-glass-panel .uvd-btn{background:rgba(255,255,255,0.08);}
...
// ========== BUILD UI (giữ nguyên) ==========
// ... (phần buildUI, renderStreams, renderPlayerSettings, renderSettings, minimize script panel giữ nguyên như bản cũ)
// Vì dài quá, tôi sẽ copy phần buildUI từ bản cũ, nhưng đã sửa tên hàm và xóa search box.
...
function renderSettings(container) {
  ...
  container.innerHTML =
    ...
    '<div class="uvd-card uvd-timeline-card">' +
      ...
      '<div class="uvd-step"><span class="uvd-step-num">•</span><span class="uvd-step-text">Trong player: bấm nút <strong style="color:var(--accent);">⚙️</strong> để mở menu cài đặt với tất cả tính năng</span></div>' +
      '<div class="uvd-step"><span class="uvd-step-num">•</span><span class="uvd-step-text">Chạm đúp 2 lần vào nửa trái/phải video để tua lùi/tiến (số giây tùy chỉnh)</span></div>' +
      '<div class="uvd-step"><span class="uvd-step-num">•</span><span class="uvd-step-text">Click vào thời gian để chuyển đổi chế độ hiển thị</span></div>' +
      '<div class="uvd-callout"><span class="uvd-callout-icon">▶</span><span>Nút <strong style="color:var(--accent);">▶</strong> trên header: tự động bấm giúp các nút Play ẩn để link stream lộ ra.</span></div>' +
    '</div>' +
...
// ========== START ==========
buildUI();
console.log('V' + VERSION + ' UMP DL with Video.js - Glass UI');
