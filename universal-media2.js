/**
 * Universal Media Player v2
 * Fixed: Video fullscreen, light theme, compact controls
 */
(function() {
    'use strict';
    ...
    function openPlayer(streamInfo) {
        ...
        // Delay 1 chút để DOM sẵn sàng
        setTimeout(function() {
            if (overlay.requestFullscreen) {
                overlay.requestFullscreen({ navigationUI: 'hide' }).catch(function(err) {
                    console.log('Fullscreen failed:', err.message);
                    // Fallback: dùng CSS full viewport
                    overlay.style.width = '100vw';
                    overlay.style.height = '100vh';
                });
            }
            
            // Lock landscape if possible (for phone)
        ...
        if (streamInfo.type === 'M3U8') {
            if (window.Hls && Hls.isSupported()) {
                ...
                hlsScript.onerror = function() { video.src = url; };
        ...
        overlay.addEventListener('click', function(e) {
            // Click vào video (không phải controls) → toggle play/pause
    ...
    function activateGuard() {
        ...
        try {
            location.assign = function(url) { blocked++; console.log('🚫 assign:', url); updateGuardBadge(blocked); };
            location.replace = function(url) { blocked++; console.log('🚫 replace:', url); updateGuardBadge(blocked); };
        } catch(e) {}
    ...
    function updateGuardBadge(count) {
        // Badge tích hợp vào top bar (không cần badge riêng)
    }
