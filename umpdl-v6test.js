/**
 * Universal Media Player & Downloader - V6.0 (Tối ưu & Sửa lỗi)
 * - Overlay player với toolbar (glass + blur mạnh)
 * - Chọn chất lượng, tốc độ, fullscreen, thu nhỏ (đã tối ưu, bớt lag)
 * - Giữ referer, hỗ trợ HLS
 * - Tự động bấm nút Play cho các site lazy-load (giới hạn số lần, dừng khi có stream)
 * - Chặn popup/tab lạ nhưng cho phép mở iframe từ script
 ...
 * - Lọc video quảng cáo bằng filterlist (import pattern, thêm sẵn danh sách mặc định)
 * - Sửa lỗi video dọc bị mất nội dung trên trình phát
 * - Đồng bộ resolution tự động từ kích thước video (kể cả ABR không có RESOLUTION trong manifest)
 * - Cập nhật dung lượng khi chuyển chất lượng M3U8
 * - Header player hiệu ứng kính mờ nâng cao, thông báo trạng thái (đang buffering, đang phát)
 * - Loại bỏ tự động copy khi nhấn Share (chỉ dùng Web Share hoặc prompt)
 * - Fix nút công cụ bị đổi màu trên một số web (thêm !important vào CSS)
 * - Fix chặn iframe quá mạnh, giờ mở được bình thường
 * - Tối ưu scan, giảm auto-click, giảm lag khi phát
...
// Thêm filterlist mặc định nếu chưa có
...
// ========== URL DETECTION (Tối ưu: không quét outerHTML toàn trang) ==========
var urls = new Map();
var patterns = [
...
function scan(doc, src) {
  try {
    ...
    // Bỏ quét outerHTML để giảm tải CPU
...
// ========== POPUP / NEW-TAB BLOCKER (cho phép mở từ script) ==========
var __uvdPopupBlockActive = false;
var __uvdOriginalWindowOpen = null;
var __uvdBlockedCount = 0;
var __uvdAllowPopup = false;

function killBlankLinks(e) {
  var t = e.target;
  while (t && t !== document) {
    if (t && t.tagName === 'A') {
      // Bỏ qua các link do script tạo ra (có class chứa 'uvd-iframe-link')
      if (t.closest('.uvd-iframe-link')) return;
      var tg = t.target;
...
// ========== AUTO-CLICK PLAY (Gộp chung, bỏ duplicate) ==========
...
// Giới hạn auto-click: chỉ chạy nếu chưa có stream
var autoClickTimers = [];
function scheduleAutoClick() {
  if (urls.size > 0) return; // Đã có stream thì không cần auto-click nữa
  [800, 2000].forEach(function(delay) {
    autoClickTimers.push(setTimeout(function() {
      if (urls.size > 0) return;
      blockPopupsDuring(2000, function() {
        autoClickPlayButtons(document, 0, true);
      });
      setTimeout(function() {
...
function shareUrl(url) {
  if (navigator.share) {
    ...
    // Không tự động copy nữa, hiển thị prompt để người dùng tự copy
    prompt('URL để chia sẻ:', url);
  }
}
...
// ========== OVERLAY PLAYER WITH TOOLBAR (CẢI TIẾN) ==========
function showVideoPlayer(url, type) {
  ...
  overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.85);backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px);z-index:2147483648;display:flex;flex-direction:column;animation:uvdFadeIn 0.3s ease;';
  document.body.appendChild(overlay);
  playerState.overlay = overlay;

  // Header với glass mạnh
  var header = document.createElement('div');
  header.style.cssText = 'padding:12px 16px;background:rgba(0,0,0,0.45);backdrop-filter:blur(24px) saturate(180%);-webkit-backdrop-filter:blur(24px) saturate(180%);display:flex;flex-direction:column;flex-shrink:0;border-bottom:1px solid rgba(255,255,255,0.15);box-shadow:0 4px 15px rgba(0,0,0,0.5);';
  ...
  minVideoBtn.style.cssText = 'background:rgba(255,255,255,0.15);color:#fff;font-size:14px;border:1px solid rgba(255,255,255,0.2);';
  ...
  closeBtn.style.cssText = 'background:rgba(255,0,0,0.4);color:#fff;border:1px solid rgba(255,255,255,0.2);';
  ...
  qualityBtn.style.cssText = 'background:rgba(255,255,255,0.12);color:#fff;font-size:12px;border:1px solid rgba(255,255,255,0.15);';
  ...
  speedBtn.style.cssText = 'background:rgba(255,255,255,0.12);color:#fff;font-size:12px;border:1px solid rgba(255,255,255,0.15);';
  ...
  fsBtn.style.cssText = 'background:rgba(255,255,255,0.12);color:#fff;font-size:12px;border:1px solid rgba(255,255,255,0.15);';
  ...
  if (type === 'MP4' || type === 'MKV' || type === 'WEBM') {
    ...
    downloadBtn.style.cssText = 'background:rgba(34,197,94,0.35);color:#fff;font-size:12px;border:1px solid rgba(255,255,255,0.2);';
  ...
  if (type === 'M3U8' || type === 'MPD') {
    ...
    shareBtn.style.cssText = 'background:rgba(139,92,246,0.35);color:#fff;font-size:12px;border:1px solid rgba(255,255,255,0.2);';
  ...
  function showQualitySubMenu() {
    ...
    panel.style.cssText = 'background:rgba(20,22,30,0.96);border-radius:16px;padding:20px;min-width:250px;max-width:90%;border:1px solid rgba(255,255,255,0.15);';
  ...
  video.addEventListener('waiting', function() {
    document.getElementById('__uvd_player_status__').textContent = 'Đang buffering...';
  });
  video.addEventListener('playing', function() {
    document.getElementById('__uvd_player_status__').textContent = 'Đang phát';
  });
  video.addEventListener('seeking', function() {
    document.getElementById('__uvd_player_status__').textContent = 'Đang tua...';
  });
...
// ========== BUILD UI (CSS cải thiện chống ghi đè) ==========
...
.uvd-glass-panel {
  background:var(--glass) !important; backdrop-filter:blur(28px) saturate(130%) !important;
  ...
  color:var(--text) !important; font-family:-apple-system,BlinkMacSystemFont,'SF Pro Display','Segoe UI',Roboto,sans-serif !important;
  font-size:var(--fs-base) !important;
  padding:16px !important; width:100% !important; position:relative !important; overflow:hidden !important;
...
.uvd-scroll::-webkit-scrollbar-thumb{background:var(--accent) !important;border-radius:4px}
.uvd-scroll::-webkit-scrollbar-track{background:transparent !important}

.uvd-btn {
  background:var(--glass-hi) !important; border:1px solid var(--border) !important;
  color:var(--text) !important; padding:9px 16px !important; border-radius:var(--radius-md) !important;
  font-weight:600 !important; font-size:var(--fs-base) !important; cursor:pointer !important; transition: all 0.2s !important;
  backdrop-filter:blur(10px) !important; text-align:center !important; position:relative !important;
  overflow:hidden !important; display:inline-block !important;
  box-shadow:0 3px 10px rgba(0,0,0,0.4), 0 1px 0 rgba(255,255,255,0.08) inset !important;
  line-height:1.3 !important;
  text-decoration:none !important;
}

.uvd-btn:hover {
  background:rgba(255,255,255,0.10) !important; border-color:rgba(255,255,255,0.20) !important;
  transform:translateY(-1px) !important; box-shadow:0 5px 14px rgba(0,0,0,0.5), 0 1px 0 rgba(255,255,255,0.12) inset !important;
}

.uvd-btn:active { transform:scale(0.96) !important; }
...
.uvd-btn-icon {
  background:var(--glass-hi) !important; border:1px solid var(--border) !important;
  color:var(--text) !important; width:34px !important; height:34px !important; border-radius:var(--radius-sm) !important;
  cursor:pointer !important; display:inline-flex !important; align-items:center !important; justify-content:center !important;
  transition: all 0.2s !important; backdrop-filter:blur(10px) !important; position:relative !important; overflow:hidden !important;
  box-shadow:0 3px 8px rgba(0,0,0,0.35), 0 1px 0 rgba(255,255,255,0.08) inset !important;
}

.uvd-btn-icon:hover { background:rgba(255,255,255,0.10) !important; border-color:rgba(255,255,255,0.20) !important; }
.uvd-btn-icon:active { transform:scale(0.92) !important; }

.uvd-card {
  background:var(--card-bg) !important; border:1px solid var(--border) !important;
  border-radius:var(--radius-md) !important; padding:14px !important; margin-bottom:10px !important;
  font-size:var(--fs-base) !important; backdrop-filter:blur(6px) !important;
  box-shadow:0 1px 0 rgba(255,255,255,0.04) inset !important;
  transition: transform 0.3s cubic-bezier(.4,0,.2,1), box-shadow 0.3s ease, background 0.3s ease, border-color 0.3s ease !important;
...
.uvd-card:hover {
  ...
  box-shadow:0 14px 30px rgba(0,0,0,0.7), 0 0 0 1px rgba(109,140,255,0.30) inset, 0 0 24px rgba(109,140,255,0.08) !important;
  background:rgba(255,255,255,0.05) !important;
  border-color:rgba(109,140,255,0.25) !important;
}

.uvd-type-badge {
  display:inline-block !important; padding:4px 12px !important; border-radius:var(--radius-sm) !important;
  ...
  color:var(--accent) !important; border:1px solid rgba(109,140,255,0.28) !important;
  letter-spacing:0.03em !important; box-shadow:0 1px 0 rgba(255,255,255,0.06) inset !important;
}

.uvd-url-box {
  background:rgba(0,0,0,0.5) !important; border-radius:var(--radius-sm) !important; padding:12px !important;
  font-family:'SFMono-Regular',Consolas,monospace !important; font-size:var(--fs-sm) !important; word-break:break-all !important;
  color:var(--text2) !important; max-height:100px; overflow-y:auto; line-height:1.5 !important;
  border:1px solid rgba(255,255,255,0.04) !important;
}

.uvd-grid-2 { display:grid !important; grid-template-columns:1fr 1fr !important; gap:8px !important; }
.uvd-grid-3 { display:grid !important; grid-template-columns:1fr 1fr 1fr !important; gap:6px !important; }

.uvd-tabbar {
  position:relative !important; display:flex !important; gap:2px; padding:4px;
...
.uvd-tab-indicator {
  position:absolute !important; top:4px; bottom:4px; left:4px; width:0;
  border-radius:999px !important; background:var(--grad-liquid) !important; z-index:0;
...
.uvd-tab {
  position:relative !important; z-index:1; flex:1; min-width:0; background:transparent !important;
  border:none !important; color:var(--text2) !important; font-weight:600 !important; font-size:var(--fs-sm) !important;
  padding:9px 6px !important; border-radius:999px !important; cursor:pointer !important; transition:color 0.25s !important;
...
#__uvd_min_float__ {
  position:fixed !important; bottom:20px !important; right:20px !important; width:54px !important; height:54px !important;
  border-radius:50% !important; background:var(--grad-liquid) !important; color:#fff !important;
  border:1px solid rgba(255,255,255,0.25) !important; box-shadow:0 8px 22px rgba(0,0,0,0.6), 0 0 20px rgba(109,140,255,0.35) !important;
  z-index:2147483647 !important; cursor:pointer !important; display:flex; align-items:center;
  justify-content:center; font-weight:700 !important; font-size:var(--fs-lg) !important;
  transition: transform 0.3s !important; animation:uvdFloatBtnIn 0.3s ease;
  backdrop-filter:blur(10px) !important;
}

#__uvd_min_float__:hover { transform:scale(1.1) !important; }
...
.uvd-profile-card {
  ...
  background:linear-gradient(135deg, rgba(109,140,255,0.14), rgba(185,139,255,0.08)) !important;
  border:1px solid rgba(109,140,255,0.25) !important; border-radius:var(--radius-lg) !important;
  padding:16px !important; margin-bottom:10px !important; animation:uvdCardEnter 0.4s ease both;
}

.uvd-profile-avatar {
  flex-shrink:0; width:56px; height:56px; border-radius:50%;
  background:var(--grad-liquid) !important; color:#fff !important; font-weight:700 !important; font-size:18px !important;
  display:flex; align-items:center; justify-content:center;
...
.uvd-tag {
  font-size:10px !important; font-weight:600 !important; padding:3px 9px !important; border-radius:999px !important;
  background:rgba(255,255,255,0.08) !important; border:1px solid var(--border) !important; color:var(--text2) !important;
}

.uvd-profile-stats {
  display:grid !important; grid-template-columns:repeat(3,1fr) !important; gap:8px !important; margin-bottom:14px !important;
}

.uvd-stat {
  background:var(--card-bg) !important; border:1px solid var(--border) !important; border-radius:var(--radius-md) !important;
...
.uvd-section-num {
  width:20px; height:20px; border-radius:50%; background:var(--grad-liquid) !important;
...
.uvd-timeline-card { border-left:2px solid rgba(109,140,255,0.3) !important; }

.uvd-inline-code {
  display:inline-block; background:rgba(0,0,0,0.35) !important; padding:3px 9px !important; border-radius:6px !important;
  color:var(--accent2) !important; font-family:'SFMono-Regular',Consolas,monospace !important; font-size:11px !important;
  border:1px solid rgba(255,255,255,0.06) !important; margin:2px 0;
}
...
function buildUI() {
  ...
  document.getElementById('__uvd_autoplay__').onclick = function() {
    var n = autoClickPlayButtons(document, 0, true);
...
// ========== RENDER FUNCTIONS (Cập nhật nút Mở iframe) ==========
function renderStreams(container, arr) {
  ...
  arr.forEach(function(item, i) {
    ...
    card.innerHTML = 
      ...
      '<div class="uvd-grid-2" style="margin-top:8px;">' +
        ...
        (item.type === 'IFRAME' ? 
          '<button class="uvd-btn uvd-btn-sm uvd-ripple-btn uvd-iframe-link" data-action="openiframe" data-url="' + encodeURIComponent(item.url) + '" style="text-align:center;grid-column:1/3;">Mở iframe</button>' :
          (item.type === 'M3U8' ?
  ...
  container.querySelectorAll('.uvd-btn[data-action]').forEach(function(b) {
    b.onclick = function() {
      ...
      else if (action === 'openiframe') {
        // Cho phép popup từ script
        __uvdAllowPopup = true;
        window.open(u, '_blank');
        __uvdAllowPopup = false;
      }
    };
  });
}
  ...
  container.innerHTML =
    ...
    '<div class="uvd-card uvd-timeline-card">' +
      '<div style="font-size:13px;color:var(--text2);line-height:1.7;">' +
        ...
        '• Bấm <strong style="color:var(--accent);">−</strong> trên header chính để thu nhỏ toàn bộ script<br>' +
...
console.log('V6.0 UMP DL - Tối ưu hiệu năng, fix UI, thêm thông báo trạng thái');
toast('V6.0 sẵn sàng — Glass mượt, iframe mở được, không tự copy khi Share!');
