...
(function () {
    ...
    const HISTORY_KEY = 'dlp_history';
    const BOOKMARK_KEY = 'dlp_bookmarks';
    ...
    let history = loadSet(HISTORY_KEY);
    let bookmarks = loadSet(BOOKMARK_KEY);
    ...
    const CSS = `
        ...
        .dlp-pact.share { background: linear-gradient(135deg, rgba(5, 150, 105, 0.6), rgba(52, 211, 153, 0.6)); }
        ...
        .dlp-pact.pip { background: linear-gradient(135deg, rgba(245, 158, 11, 0.6), rgba(251, 191, 36, 0.6)); }
        .dlp-pact.bookmark { background: linear-gradient(135deg, rgba(239, 68, 68, 0.6), rgba(248, 113, 113, 0.6)); }
    ...
    function initLogic() {
        ...
        function renderPanel() {
            ...
            else if (currentPanelTab === 'history') renderHistory();
            else if (currentPanelTab === 'bookmarks') renderBookmarks();
        ...
        function renderHistory() {
            ...
            [...history].reverse().forEach(item => {
                try {
                    ...
                    div.innerHTML = `
                        <div class="dlp-li-top">
                            <span style="color:#60a5fa;font-size:12px;flex:1;">${data.name}</span>
                            <span style="color:rgba(255,255,255,0.4);font-size:10px;">${new Date(data.ts).toLocaleString()}</span>
                        </div>
                        <div class="dlp-li-url">${data.url}</div>
                    `;
                    div.onclick = () => openPrev({url: data.url, type: data.type});
                    pb.appendChild(div);
                } catch(e) {}
            });
        }
        ...
        function renderBookmarks() {
            ...
            if (!bookmarks.size) {
                pb.innerHTML = `
                    <div style="color:rgba(255,255,255,0.5);text-align:center;padding:30px;font-size:13px">
                        Chưa có bookmark nào.<br>
                        ⭐ Nhấn nút Bookmark khi preview để lưu.
                    </div>`;
                return;
            }
            ...
            [...bookmarks].forEach(item => {
                try {
                    ...
                    div.innerHTML = `
                        <div class="dlp-li-top">
                            <span style="color:#fbbf24;font-size:12px;flex:1;">⭐ ${data.name}</span>
                            <button class="dlp-btn" style="font-size:10px;padding:4px 8px;">Xóa</button>
                        </div>
                        <div class="dlp-li-url">${data.url}</div>
                    `;
        ...
        function openPrev(item) {
            ...
            $('dlp-p-acts').innerHTML = `
                ...
                <button class="dlp-pact share" id="dlp-pc-share">📤 Share</button>
                ...
                <button class="dlp-pact pip" id="dlp-pc-pip">🖼 PiP</button>
                <button class="dlp-pact bookmark" id="dlp-pc-bm">⭐ Lưu</button>
            `;

            $('dlp-pc-cp').onclick = () => cp(cur.url);
            $('dlp-pc-fs').onclick = () => triggerFullscreen();
            $('dlp-pc-share').onclick = () => shareToAndroid(cur.url, fname(cur.url));
            ...
            $('dlp-pc-pip').onclick = () => {
                if (document.pictureInPictureElement) {
                    ...
                    $('dlp-vid').requestPictureInPicture().catch(() => {
                        toast('❌ PiP không được hỗ trợ');
                    });
                }
            };
            $('dlp-pc-bm').onclick = () => {
                const bookmarkItem = JSON.stringify({
                    url: cur.url,
                    name: fname(cur.url),
                    type: cur.type,
                    ts: Date.now()
                ...
                saveSet(BOOKMARK_KEY, bookmarks);
                toast('⭐ Đã lưu bookmark!', 'rgba(245, 158, 11, 0.8)');
            };
        ...
        // ══════════════════════════════════════
        // SHARE TO ANDROID (MỚI)
        // ══════════════════════════════════════
        function shareToAndroid(url, name) {
            // Thử Web Share API trước
            if (navigator.share) {
                navigator.share({
                    title: name,
                    text: `Xem video: ${name}`,
                    ...
                    // Fallback nếu user cancel
                });
            } else {
                // Fallback: mở intent Android
                const intentUrl = `intent://${url.replace(/^https?:\/\//, '')}#Intent;scheme=https;action=android.intent.action.SEND;type=text/plain;S.android.intent.extra.TEXT=${encodeURIComponent(url)};end`;
        ...
        $('dlp-btn-opt').onclick = (e) => {
            ...
            d.innerHTML = `
                ...
                <div class="dlp-di" id="dlp-m-share">📤 Share Android</div>
                <div class="dlp-di" id="dlp-m-new">🌐 Mở tab mới</div>
                <div class="dlp-di" id="dlp-m-pip">🖼 Picture in Picture</div>
                <div class="dlp-di" id="dlp-m-bm">⭐ Bookmark</div>
            ...
            $('dlp-m-share').onclick = () => { d.classList.remove('on'); shareToAndroid(cur.url, fname(cur.url)); };
            $('dlp-m-new').onclick = () => { window.open(cur.url,'_blank'); d.classList.remove('on'); };
            $('dlp-m-pip').onclick = () => {
                d.classList.remove('on');
                $('dlp-vid').requestPictureInPicture().catch(() => toast('❌ PiP không hỗ trợ'));
            };
            $('dlp-m-bm').onclick = () => {
                d.classList.remove('on');
                const bookmarkItem = JSON.stringify({
                ...
                bookmarks.add(bookmarkItem);
                saveSet(BOOKMARK_KEY, bookmarks);
                toast('⭐ Đã lưu!', 'rgba(245, 158, 11, 0.8)');
            };
            $('dlp-m-block').onclick = () => {
                try {
