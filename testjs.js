/**
 * Universal Media Player & Downloader - V6.7.12 - VIDEO.JS EDITION
 * - GIỮ NGUYÊN: scan link, filter, fetch, auto-click, popup blocker, resume, boost
 * - THAY THẾ: trình phát custom bằng Video.js 7.21.0 full tính năng
 * - TÍCH HỢP: HLS (VHS), Quality Selector, PiP, Playback Rates, Phụ đề
 * Author: nguyenquocngu91 (Modified)
 */
(function() {
'use strict';

// ========== VERSION ==========
var VERSION = '6.7.12-VJS';
...
// ========== M3U8 MASTER PARSER (giữ để hiển thị chất lượng) ==========
...
function setSleepTimer(minutes) {
  ...
  playerState.sleepTimerId = setTimeout(function() {
    if (playerState.videojs) playerState.videojs.pause();
...
function attachSubtitleTrack(video, vttUrl, label) {
  if (!video || !playerState.videojs) return;
  // Xóa track cũ
  var oldTracks = video.querySelectorAll('track[data-uvd-sub="1"]');
  ...
  // Video.js sẽ tự động nhận track mới
  playerState.videojs.textTracks().forEach(function(tt) {
    if (tt.label === (label || 'Phụ đề')) tt.mode = 'showing';
  });
...
function showSubtitlePanel() {
  ...
  panel.innerHTML =
    '<div style="color:#fff;font-weight:600;margin-bottom:4px;">💬 Phụ đề</div>' +
  ...
  panel.querySelector('#__uvd_sub_off__').onclick = function() {
    if (playerState.videojs) {
      playerState.videojs.textTracks().forEach(function(tt) { tt.mode = 'disabled'; });
    }
    toast('Đã tắt phụ đề');
    overlay2.remove();
  };
...
// ========== VIDEO.JS PLAYER ==========
function loadVideoJSLibraries(callback) {
  ...
  css.rel = 'stylesheet';
  css.href = 'https://vjs.zencdn.net/7.21.0/video-js.css';
  document.head.appendChild(css);

  // Load Video.js core
  var script = document.createElement('script');
  script.src = 'https://vjs.zencdn.net/7.21.0/video.min.js';
  script.onload = function() {
    // Load VHS plugin
    var vhsScript = document.createElement('script');
    vhsScript.src = 'https://cdn.jsdelivr.net/npm/@videojs/http-streaming@3.13.0/dist/videojs-http-streaming.min.js';
    vhsScript.onload = function() {
      // Load Quality Selector plugin
      var qsScript = document.createElement('script');
      qsScript.src = 'https://cdn.jsdelivr.net/npm/videojs-quality-selector@2.0.1/dist/videojs-quality-selector.min.js';
      qsScript.onload = callback;
      qsScript.onerror = callback;
      document.head.appendChild(qsScript);
    };
    vhsScript.onerror = callback;
...
function showVideoPlayer(url, type) {
  ...
  overlay.style.cssText = 'position:fixed;inset:0;background:#000;z-index:2147483648;display:flex;flex-direction:column;animation:uvdFadeIn 0.3s ease;';
  __uvdAppendRoot(overlay);
  playerState.overlay = overlay;

  // Container cho video
  var container = document.createElement('div');
  container.style.cssText = 'flex:1;position:relative;background:#000;min-height:0;';
  ...
  closeBtn.style.cssText = 'position:absolute;top:12px;right:16px;z-index:10;background:rgba(0,0,0,0.6);color:#fff;border:none;border-radius:50%;width:36px;height:36px;font-size:20px;cursor:pointer;';
  ...
  playerState.closeBtn = closeBtn;

  // Khởi tạo Video.js
  loadVideoJSLibraries(function() {
    ...
    // Thêm Quality Selector
    if (window.videojsQualitySelector) {
    ...
    player.ready(function() {
      ...
      // Volume Boost
      if (data.settings.volumeBoost) {
        enableVolumeBoost(video, data.settings.volumeBoostMax);
      }
    ...
    player.on('timeupdate', function() {
      if (data.settings.resumePlayback) {
        clearTimeout(playerState.saveTimer);
        playerState.saveTimer = setTimeout(function() {
          var t = player.currentTime();
          var d = player.duration();
          if (t > 3 && d && (t/d) < 0.95) {
            data.playbackPositions[url] = { time: t, duration: d, updatedAt: Date.now() };
            storage.set(data);
          }
        }, 3000);
      }
    ...
    player.on('ended', function() {
      if (data.settings.resumePlayback) delete data.playbackPositions[url];
      if (data.settings.autoNext) {
        var next = getNextStreamUrl(url);
        if (next) {
          toast('⏭ Đang phát stream tiếp theo...');
          setTimeout(function() { showVideoPlayer(next.url, next.type); }, 800);
        }
      }
    });
    ...
    var onFsChange = function() {
      ...
      if (isFs) lockOrientation(video);
      else unlockOrientation();
    };
    document.addEventListener('fullscreenchange', onFsChange);
    document.addEventListener('webkitfullscreenchange', onFsChange);
    playerState.onFullscreenChange = onFsChange;
...
function minimizePlayer() {
  ...
  mini.style.cssText = 'position:fixed;bottom:20px;right:20px;width:160px;height:90px;background:#000;border-radius:12px;z-index:2147483647;cursor:pointer;box-shadow:0 8px 30px rgba(0,0,0,0.8);border:2px solid rgba(255,255,255,0.2);overflow:hidden;display:flex;align-items:center;justify-content:center;';
  ...
  if (playerState.video && playerState.video.videoWidth) {
    try { ctx.drawImage(playerState.video, 0, 0, 160, 90); } catch(e) {}
  }
  ...
  if (playerState.overlay) {
    playerState.overlay.style.transition = 'opacity 0.25s ease';
    playerState.overlay.style.opacity = '0';
    setTimeout(function() { if (playerState.overlay) playerState.overlay.style.display = 'none'; }, 260);
  }
...
function restorePlayer() {
  ...
  if (playerState.mini) {
    playerState.mini.style.transition = 'opacity 0.2s ease';
    playerState.mini.style.opacity = '0';
    setTimeout(function() { if (playerState.mini) playerState.mini.remove(); }, 220);
    playerState.mini = null;
  }
  if (playerState.overlay) {
    playerState.overlay.style.display = 'flex';
    playerState.overlay.style.transition = 'opacity 0.25s ease';
    playerState.overlay.style.opacity = '1';
  }
  if (playerState.videojs) {
    playerState.videojs.play().catch(function(){});
  }
}
...
function closePlayer() {
  if (playerState.overlay) {
    ...
    if (playerState.videojs) {
      playerState.videojs.dispose();
      playerState.videojs = null;
    }
...
function renderPlayerSettings(container) {
  ...
  container.innerHTML =
    ...
    '<div class="uvd-card">' +
      '<div style="font-weight:600;margin-bottom:10px;">🔄 Tua nhanh (Video.js đã hỗ trợ)</div>' +
      '<div style="font-size:12px;color:var(--text2);">Chạm đúp vào video để tua (khoảng 10s). Có thể dùng thanh kéo để tua nhanh hơn.</div>' +
    '</div>' +
...
function renderSettings(container) {
  ...
  container.innerHTML =
    '<div class="uvd-profile-card">' +
      ...
      '<div class="uvd-profile-info">' +
        ...
        '<div class="uvd-profile-tags">' +
          ...
          '<span class="uvd-tag">Resume · Video.js</span>' +
        '</div>' +
      '</div>' +
    '</div>' +
    ...
    '<div class="uvd-card">' +
      ...
      '<div style="font-size:11px;color:var(--text3);margin-top:6px;">Video/audio do chính trang web tự bật sẽ bị tạm dừng ngay.</div>' +
    '</div>' +

    '<div class="uvd-card">' +
      '<div style="font-weight:600;margin-bottom:8px;">🛡️ Lọc quảng cáo (Filterlist)</div>' +
      '<div style="font-size:12px;color:var(--text2);margin-bottom:8px;">' +
        'Nhập mỗi dòng một từ khóa hoặc domain. Hỗ trợ regex nếu bắt đầu bằng <code>regex:</code>.' +
      '</div>' +
    ...
    '<div class="uvd-card uvd-timeline-card">' +
      '<div class="uvd-step"><span class="uvd-step-num">1</span><span class="uvd-step-text">Mở trang web, bấm vào biểu tượng <strong>⭐ Bookmark</strong> trên thanh địa chỉ.</span></div>' +
      ...
      '<div class="uvd-step"><span class="uvd-step-num">4</span><span class="uvd-step-text"><strong>Xóa toàn bộ địa chỉ</strong> trong ô URL, dán đoạn code sau:</span></div>' +
      ...
      '<div class="uvd-callout"><span class="uvd-callout-icon">💡</span><span>Gõ tên bookmark (<strong style="color:var(--accent);">UMP DL</strong>) vào thanh địa chỉ để kích hoạt.</span></div>' +
    '</div>' +

    '<div class="uvd-section-title"><span class="uvd-section-num">2</span> Sử dụng Video.js</div>' +
    '<div class="uvd-card uvd-timeline-card">' +
      '<div class="uvd-step"><span class="uvd-step-num">•</span><span class="uvd-step-text">Mở trang web có video → chạy bookmarklet</span></div>' +
      '<div class="uvd-step"><span class="uvd-step-num">•</span><span class="uvd-step-text">Chọn stream và bấm <strong style="color:var(--accent);">Xem</strong></span></div>' +
      '<div class="uvd-step"><span class="uvd-step-num">•</span><span class="uvd-step-text">Video.js cung cấp: Play/Pause, Volume, Progress bar, Fullscreen, PiP, Tốc độ phát (Speed), Chất lượng (Quality Selector)</span></div>' +
      '<div class="uvd-step"><span class="uvd-step-num">•</span><span class="uvd-step-text">Hỗ trợ phụ đề (tải file .srt/.vtt hoặc tìm qua SubDL)</span></div>' +
      '<div class="uvd-step"><span class="uvd-step-num">•</span><span class="uvd-step-text">Tính năng Boost (tăng âm lượng) có thể bật trong tab Trình phát</span></div>' +
    '</div>' +
...
console.log('V' + VERSION + ' UMP DL PRO - Video.js Edition');
toast('V' + VERSION + ' PRO sẵn sàng! (Video.js)');
