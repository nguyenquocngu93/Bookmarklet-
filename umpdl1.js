/**
 * Universal Media Player & Downloader - V6.5 PRO
 * - Thêm nút Screenshot
 * - Auto-hide controls (tùy chọn)
 * - Hiển thị thời gian còn lại (click để đổi)
 * - Các tính năng cũ: resume, tua đúp, Boost, PiP, Sleep, Mute, quality, speed, ...
...
// ========== OVERLAY PLAYER (có auto-hide, screenshot, thời gian còn lại) ==========
function showVideoPlayer(url, type) {
  ...
  function resetHideTimer() {
    ...
    // Đặt timer ẩn sau 3 giây
    playerState.hideTimeout = setTimeout(function() {
      if (video.paused) return; // Không ẩn nếu đang tạm dừng
  ...
  // Thêm vào toolbar để reset timer khi tương tác với nút
  toolbar.addEventListener('click', resetHideTimer);
  header.addEventListener('click', resetHideTimer);

  // ===== HIỂN THỊ THỜI GIAN CÒN LẠI =====
  var timeMode = 0; // 0: remaining, 1: elapsed, 2: total
  ...
  function updateTimeDisplay() {
    ...
    var total = d;
    var text = '';
    if (data.settings.showRemainingTime && timeMode === 0) {
      text = '-' + format(remaining);
    } else if (timeMode === 1) {
      text = format(elapsed);
    } else {
      text = format(elapsed) + ' / ' + format(total);
    }
    timeEl.textContent = text;
  }
  // Click vào thời gian để chuyển chế độ
  timeEl.addEventListener('click', function() {
    timeMode = (timeMode + 1) % 3;
    updateTimeDisplay();
    toast(timeMode === 0 ? 'Hiển thị thời gian còn lại' : timeMode === 1 ? 'Hiển thị thời gian đã qua' : 'Hiển thị tổng thời gian');
  });

  video.addEventListener('timeupdate', updateTimeDisplay);
  video.addEventListener('loadedmetadata', function() {
    updateTimeDisplay();
    // Khởi tạo timer auto-hide sau khi metadata load
    resetHideTimer();
  });

  // ===== CÁC HÀM KHÁC (quality, speed, ...) =====
  ...
  function onMetadataLoaded() {
    ...
    // Khởi tạo auto-hide timer sau khi metadata load
    resetHideTimer();
  }
  ...
  playerState._updateTime = updateTimeDisplay;
  playerState._timeMode = timeMode;
}
...
function renderSettings(container) {
  ...
  container.innerHTML =
    ...
    '<div class="uvd-card uvd-timeline-card">' +
      ...
      '<div class="uvd-step"><span class="uvd-step-num">•</span><span class="uvd-step-text">Trong player: thanh công cụ gồm <strong style="color:var(--accent);">Chất lượng</strong>, <strong style="color:var(--accent);">Tốc độ</strong>, <strong style="color:var(--accent);">Toàn màn hình</strong>, <strong style="color:var(--accent);">PiP</strong>, <strong style="color:var(--accent);">Hẹn giờ</strong>, <strong style="color:var(--accent);">Boost</strong>, <strong style="color:var(--accent);">Mute</strong>, <strong style="color:var(--accent);">📷 Screenshot</strong></span></div>' +
...
console.log('V' + VERSION + ' UMP DL PRO - Thêm Screenshot, Auto-hide, Thời gian còn lại');
