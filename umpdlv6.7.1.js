/**
 * Universal Media Player & Downloader - V6.7.1 PRO
 * - FIX: lỗi CSS biến --btn-bg tự tham chiếu chính nó (gây vỡ icon/nút toàn bộ panel)
 * - FIX: header giật/vỡ hình khi thu nhỏ panel + cuộn trang (throttle qua requestAnimationFrame, tắt blur lúc thu nhỏ)
 * - Đổi phụ đề thử nghiệm sang SubDL API v2 (chỉ cần API key cá nhân, không cần app "consumer" riêng)
...
function searchSubDL(query, cb) {
  ...
  .then(function(json) { cb((json && (json.results || json.subtitles || json.data)) || []); })
  .catch(function() { toast('Lỗi kết nối SubDL (có thể do CORS/mạng)'); cb([]); });
}

function downloadSubDLFile(item, cb) {
  var apiKey = (data.settings.subdlApiKey || '').trim();
  // Nếu search có unpack=1 sẵn link file trực tiếp thì dùng luôn, đỡ phải gọi thêm download endpoint
  var directUrl = item.file_url || item.download_url || (item.files && item.files[0] && item.files[0].url);
  var nId = item.nId || item.n_id || item.id;
  var reqPromise;
  if (directUrl) {
...
.uvd-panel-minimized.uvd-glass-panel{backdrop-filter:blur(0px)!important;-webkit-backdrop-filter:blur(0px)!important;background:rgba(10,11,16,0.98)!important}
...
// ========== FIX HEADER MINIMIZE ==========
// Gộp qua requestAnimationFrame để tránh giật/lỗi vẽ lại khi cuộn nhanh
// (đặc biệt khi panel đang thu nhỏ + thanh địa chỉ Chrome co giãn).
var __uvdViewportRAF = null;
function __uvdSyncViewport() {
  if (__uvdViewportRAF) return;
  __uvdViewportRAF = requestAnimationFrame(function() {
    __uvdViewportRAF = null;
...
if (window.visualViewport) {
  ...
  addCleanup(function() {
    ...
    if (__uvdViewportRAF) cancelAnimationFrame(__uvdViewportRAF);
  });
}
