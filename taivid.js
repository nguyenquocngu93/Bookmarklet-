// ==UserScript==
// @name         DevLib CDN Player - Glass UI v3 Simple Fix
// @namespace    http://tampermonkey.net/
// @version      3.3
// @description  Fixed video playback - simple like original
// @author       You
// @match        *://*/*
// @grant        GM_getValue
// @grant        GM_setValue
...
(function () {
    ...
    // ══════════════════════════════════════════
    // GLASS UI CSS - ĐƠN GIẢN, KHÔNG OVERLAY
    // ══════════════════════════════════════════
    const CSS = `
        ...
        * { box-sizing: border-box; }
        ...
        .dlp-img-item:hover { transform: scale(1.04); }
        .dlp-img-thumb {
            width: 100%;
            height: 100%;
            object-fit: cover;
        ...
        .dlp-action-btn:hover { background: rgba(255,255,255,0.14); transform: translateY(-2px); }
        .dlp-action-btn .dlp-action-icon { font-size: 20px; }
        .dlp-action-btn .dlp-action-label { font-size: 10px; opacity: 0.85; }
        ...
        .dlp-icon-btn:hover { background: rgba(255,255,255,0.15); transform: scale(1.1); }
    ...
    function initLogic() {
        ...
        // ══════════════════════════════════════
        // FULLSCREEN - GIỮ NGUYÊN NHƯ BẢN GỐC
        // ══════════════════════════════════════
        function triggerFullscreen() {
            ...
            if (req) {
                req.call(vid).then(() => {
                    ...
                    toast('❌ Trình duyệt chặn fullscreen');
                });
            }
        }
        ...
        function renderStreams() {
            ...
            if (!items.length) {
                pb.innerHTML = `<div style="color:rgba(255,255,255,0.4);text-align:center;padding:50px 20px;font-size:13px;">
                    📡 No streams detected<br><span style="font-size:11px;">Play video → hit Scan</span></div>`;
                return;
            }
            ...
            items.slice(0, 60).forEach(item => {
                ...
                card.innerHTML = `
                    <img class="dlp-img-thumb" src="${item.url}" loading="lazy" 
                         onerror="this.style.display='none'" alt="${item.type}">
            ...
            playlist.forEach((item, idx) => {
                ...
                div.innerHTML = `
                    <div class="dlp-li-top">
                        ...
                        <button class="dlp-btn" style="font-size:10px;padding:3px 8px;">✕</button>
                    </div>
                    <div class="dlp-li-url">${item.url}</div>
                `;
        ...
        function openCmd(url) {
            const build = (t) => {
                ...
                $('dlp-cmd').innerHTML = `
                    ...
                    `).join('')}
                    <button style="width:100%;background:#c62828;color:white;padding:12px;border-radius:12px;
