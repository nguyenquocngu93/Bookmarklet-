/**
 * Universal Media Player & Downloader - V6.7.9 PRO
 * - FIX: 2 thanh điều khiển chồng lên nhau khi video.js v10 không tải được
 *   (rơi về controls gốc trình duyệt cộng với thanh kính mờ tự vẽ). Giờ luôn
 *   tắt hẳn controls gốc (video.controls = false, ép lại nhiều lần trong lúc
 *   video-skin load bất đồng bộ), chỉ còn DUY NHẤT thanh tua tự vẽ.
 * - BỎ hẳn dòng trạng thái "Đang tải/Tạm dừng" và "Đang ước tính dung lượng"
 *   theo yêu cầu — chỉ còn thanh tua (tiến độ) + giờ, gọn và không chồng chéo.
 * - Ở chế độ xem thường (vừa bấm "Xem", CHƯA bấm Toàn màn hình thật): video
 *   giờ bo góc 20px, đổ bóng như một thẻ nổi, thanh tua kính mờ chồng lên
 *   ngay trên đáy video. Khi bấm "Toàn màn hình" (Fullscreen API thật) thì tự
 *   bỏ bo góc/đổ bóng, tràn viền như bình thường; thoát fullscreen sẽ tự bo lại.
 * - FIX QUAN TRỌNG (6.7.8): link BLOB không xem được — mượn lại đúng thẻ
 *   <video> gốc của trang cho blob: MediaSource thay vì tạo video mới.
 * - Nền video: gradient + ambient glow lấy màu trung bình khung hình.
...
function __uvdMountVjs10(wrapper, video) {
  ...
  function fallbackToNative() {
    ...
    // KHÔNG bật video.controls native nữa — thanh tua kính mờ tự vẽ trong
    // footer đã đảm nhiệm việc này; bật thêm controls gốc sẽ tạo ra 2 thanh
    // tua chồng lên nhau (đúng lỗi người dùng gặp phải).
...
function showVideoPlayer(url, type) {
  ...
  videoWrapper.style.cssText = 'flex:1;display:flex;align-items:center;justify-content:center;background:radial-gradient(120% 90% at 50% 30%,#1c2233 0%,#0a0b10 70%);position:relative;overflow:hidden;';

  // Bo góc + đổ bóng như một "thẻ" ở chế độ xem thường; khi bấm Toàn màn hình
  // thật (Fullscreen API trên chính videoWrapper) thì bỏ bo góc, tràn viền.
  function applyNormalCardStyle() {
    videoWrapper.style.margin = '0 10px 10px';
    videoWrapper.style.borderRadius = '20px';
    videoWrapper.style.boxShadow = '0 16px 30px -8px rgba(0,0,0,0.7)';
  }
  ...
  video.controls = false; // luôn tắt controls gốc của trình duyệt — tự vẽ thanh tua riêng bên dưới, tránh 2 thanh chồng nhau
  ...
  // video-skin (nếu tải xong) có thể tự bật lại controls gốc — ép tắt lại
  // vài lần trong lúc nó khởi tạo bất đồng bộ (tối đa ~4s theo FALLBACK_MS).
  [200, 600, 1200, 2000, 3000, 4200].forEach(function(ms) {
    setTimeout(function() { try { video.controls = false; } catch (e) {} }, ms);
  });

  // AMBIENT GLOW: lấy màu trung bình khung hình hiện tại để nền gradient sống
  // động theo nội dung video, làm nổi bật hiệu ứng kính mờ của thanh điều
  // khiển bên dưới. Tự bỏ qua an toàn nếu canvas bị "tainted" (video CORS).
  (function setupAmbientGlow() {
    var ambCanvas = document.createElement('canvas');
    ambCanvas.width = 16; ambCanvas.height = 9;
    var ambCtx = ambCanvas.getContext('2d', { willReadFrequently: true });
    var ambBroken = false;
    var ambTimer = setInterval(function() {
      if (ambBroken || !video.videoWidth) return;
      try {
        ambCtx.drawImage(video, 0, 0, 16, 9);
        var d = ambCtx.getImageData(0, 0, 16, 9).data;
        var r = 0, g = 0, b = 0, n = d.length / 4;
        for (var i = 0; i < d.length; i += 4) { r += d[i]; g += d[i+1]; b += d[i+2]; }
        r = Math.round(r / n); g = Math.round(g / n); b = Math.round(b / n);
        videoWrapper.style.background = 'radial-gradient(120% 90% at 50% 30%, rgba(' + r + ',' + g + ',' + b + ',0.35) 0%, #0a0b10 75%)';
      } catch (e) {
        ambBroken = true;
        clearInterval(ambTimer);
      }
    }, 1200);
    playerState.ambientTimer = ambTimer;
    addCleanup(function() { clearInterval(ambTimer); });
  })();

  // FOOTER — chồng lên video (kính mờ) thay vì chiếm riêng một dải bên dưới
  var footer = document.createElement('div');
  footer.id = '__uvd_player_footer__';
  footer.style.cssText = 'position:absolute;left:0;right:0;bottom:0;padding:10px 16px 12px;background:linear-gradient(to top,rgba(0,0,0,0.75),rgba(0,0,0,0.35) 70%,transparent);backdrop-filter:blur(16px) saturate(140%);-webkit-backdrop-filter:blur(16px) saturate(140%);border-top:1px solid rgba(255,255,255,0.1);z-index:3;transition: opacity 0.3s ease;';
  footer.innerHTML =
    '<div style="display:flex;align-items:center;gap:10px;">' +
      '<div id="__uvd_seek_track__" style="position:relative;flex:1;height:16px;display:flex;align-items:center;cursor:pointer;touch-action:none;">' +
        '<div style="position:absolute;left:0;right:0;height:4px;border-radius:2px;background:rgba(255,255,255,0.25);"></div>' +
        '<div id="__uvd_seek_fill__" style="position:absolute;left:0;height:4px;border-radius:2px;background:var(--grad-liquid);width:0%;pointer-events:none;"></div>' +
        '<div id="__uvd_seek_knob__" style="position:absolute;left:0%;width:12px;height:12px;border-radius:50%;background:#fff;box-shadow:0 1px 4px rgba(0,0,0,0.6);transform:translateX(-50%);pointer-events:none;"></div>' +
      '</div>' +
      '<span id="__uvd_player_time__" style="cursor:pointer;font-size:11px;color:#ddd;flex-shrink:0;white-space:nowrap;">00:00</span>' +
    '</div>';
  videoWrapper.appendChild(footer);

  // ===== THANH TUA (SEEK BAR) =====
  var seekTrack = footer.querySelector('#__uvd_seek_track__');
  var seekFill = footer.querySelector('#__uvd_seek_fill__');
  var seekKnob = footer.querySelector('#__uvd_seek_knob__');
  var seekDragging = false;
  function updateSeekUI() {
    if (seekDragging || !video.duration || isNaN(video.duration)) return;
    var pct = Math.max(0, Math.min(1, video.currentTime / video.duration)) * 100;
  ...
  function onSeekEnd() { seekDragging = false; }
  seekTrack.addEventListener('mousedown', onSeekStart);
  seekTrack.addEventListener('touchstart', onSeekStart, { passive: false });
  ...
  function updateSizeEstimate() {
    // Đã bỏ hiển thị dung lượng theo yêu cầu — không cần gọi HEAD request nữa.
  }
  ...
  function onFullscreenChange() {
    ...
    var isFullscreen = !!fsEl;
    if (isFullscreen) lockOrientation(video);
    else unlockOrientation();
    // Chỉ bỏ bo góc khi CHÍNH videoWrapper là phần tử fullscreen thật —
    // giữ bo góc ở mọi trạng thái xem bình thường khác.
    if (fsEl === videoWrapper) applyFullscreenStyle();
    else applyNormalCardStyle();
  }
...
function minimizePlayer() {
  ...
  mini.style.cssText = 'position:fixed;bottom:20px;right:20px;width:160px;height:90px;background:#0a0b10;border-radius:16px;z-index:2147483647;cursor:pointer;box-shadow:0 18px 26px -6px rgba(0,0,0,0.75);border:1px solid rgba(255,255,255,0.12);overflow:hidden;transition: opacity 0.25s ease, transform 0.25s ease, box-shadow 0.4s ease;';

  var canvas = document.createElement('canvas');
  canvas.width = 160;
  canvas.height = 90;
  canvas.style.cssText = 'display:block;width:100%;height:100%;border-radius:16px;';
  ...
  if (video.videoWidth) {
    try {
      ...
      // Lấy màu trung bình khung hình để làm nền gradient ấm áp thay vì đen phẳng
      var d = ctx.getImageData(0, 0, 160, 90).data;
      var r = 0, g = 0, b = 0, n = d.length / 4;
      for (var i = 0; i < d.length; i += 4) { r += d[i]; g += d[i+1]; b += d[i+2]; }
      r = Math.round(r / n); g = Math.round(g / n); b = Math.round(b / n);
      mini.style.background = 'linear-gradient(to bottom, rgba(' + r + ',' + g + ',' + b + ',0.3), #05060a 85%)';
      mini.style.boxShadow = '0 18px 28px -4px rgba(' + Math.round(r*0.6) + ',' + Math.round(g*0.6) + ',' + Math.round(b*0.6) + ',0.55)';
    } catch (e) {}
  }
  ...
  label.style.cssText = 'position:absolute;bottom:4px;left:8px;color:#fff;font-size:11px;font-weight:600;text-shadow:0 2px 4px rgba(0,0,0,0.8);background:rgba(0,0,0,0.45);backdrop-filter:blur(6px);-webkit-backdrop-filter:blur(6px);padding:2px 8px;border-radius:8px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:90%;';
...
function closePlayer() {
  if (playerState.overlay) {
    clearInterval(playerState.ambientTimer);
