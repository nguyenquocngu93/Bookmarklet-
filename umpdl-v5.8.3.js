/**
 * Universal Media Player & Downloader - V5.8 (fixed)
 * - Tích hợp đồng bộ JSONBin
 * - Tự động upload khi đóng script
 * - Chặn popup cực mạnh (Mixdrop, ok.ru, ...)
 * - Xóa icon thừa trong thông báo
...
(function() {
  ...
  // Bắt các iframe mới và vô hiệu hóa popup trong chúng
            ...
            try {
              ...
              if (doc) {
                ...
                if (node.contentWindow) {
                  node.contentWindow.open = function() { return null; };
                }
              }
            } catch(e) {}
          }
  ...
  setTimeout(function() {
    ...
    if (found > 0) {
      ...
      setTimeout(function() {
        ...
        if (n > 0) toast('Đã tạm dừng video gốc, xem qua player script cho ổn định'); // bỏ icon
      }, 800);
    } else if (!silent) {
...
function syncToJsonBin(apiKey, binId, onSuccess, onError) {
  ...
  binId = binId.trim();
  if (!apiKey || !binId) {
    toast('❌ Thiếu API Key hoặc Bin ID', '#ff5d72');
    return;
  }
  ...
  .then(function(res) {
    if (!res.ok) {
      if (res.status === 401) throw new Error('Sai API Key hoặc Bin ID (401)');
  ...
  .then(function(json) {
    ...
    toast('✅ Đã tải lên JSONBin! (' + favCount + ' yêu thích, ' + histCount + ' lịch sử)', '#34c759');
    if (onSuccess) onSuccess(json);
  })
  .catch(function(err) {
    toast('❌ Lỗi tải lên: ' + err.message, '#ff5d72');
...
function syncFromJsonBin(apiKey, binId, onSuccess, onError) {
  ...
  if (!confirm('⚠️ Bạn có chắc muốn tải dữ liệu từ JSONBin về? Dữ liệu local sẽ bị ghi đè!')) {
    return;
  }
  ...
  .then(function(json) {
    ...
    if (!remote) throw new Error('Dữ liệu rỗng');
    var hasData = (remote.favorites && remote.favorites.length > 0) || (remote.history && remote.history.length > 0);
    if (!hasData) {
      toast('⚠️ Cloud chưa có dữ liệu, giữ nguyên dữ liệu local', '#ffb84d');
      return;
    }
    ...
    storage.set(data);
    toast('✅ Đã tải dữ liệu từ JSONBin! (' + data.favorites.length + ' yêu thích, ' + data.history.length + ' lịch sử)', '#34c759');
    if (onSuccess) onSuccess(json);
    buildUI();
  })
  .catch(function(err) {
    toast('❌ Lỗi tải xuống: ' + err.message, '#ff5d72');
    if (onError) onError(err);
  });
}
...
function autoSyncIfEnabled() {
  ...
  if (jb && jb.autoSync && jb.apiKey && jb.binId) {
    toast('☁️ Đang đồng bộ dữ liệu từ JSONBin...', '#6d8cff');
    syncFromJsonBin(jb.apiKey, jb.binId, function() {
      toast('✅ Đồng bộ thành công!', '#34c759');
    }, function() {
      toast('⚠️ Không thể đồng bộ, dùng dữ liệu local', '#ffb84d');
    });
  }
}
...
function buildUI() {
  ...
  // ===== NÚT ĐÓNG: TỰ ĐỘNG UPLOAD JSONBIN =====
  document.getElementById('__uvd_close__').onclick = function() {
    var jb = data.jsonbin;
    if (jb && jb.apiKey && jb.binId) {
      toast('Đang đồng bộ lên JSONBin...');
      syncToJsonBin(jb.apiKey, jb.binId, function() {
        toast('Đã đồng bộ và đóng script');
        stopMonitor();
        panel.remove();
      }, function() {
        toast('Lỗi đồng bộ, vẫn đóng script');
        stopMonitor();
        panel.remove();
      });
    } else {
  ...
  document.getElementById('__uvd_autoplay__').onclick = function() {
    toast('▶ Đang thử bấm Play trên trang...');
...
// ========== RENDER FUNCTIONS (giữ nguyên như bản 5.8) ==========
  ...
  parseM3U8Master(url, function(qualities) {
    ...
    qualities.forEach(function(q, idx) {
      ...
      html += '<div class="uvd-card q-card" data-idx="' + idx + '">' +
        ...
        '</div>' +
        '<div><span style="color:var(--text2);">Dung lượng: </span><span class="q-size" style="color:var(--accent2);">Đang tính...</span></div>' +
...
function renderSettings(container) {
  ...
  container.innerHTML =
    ...
    '<div class="uvd-card" style="border-color:rgba(109,140,255,0.5);">' +
      '<div style="font-weight:700;margin-bottom:8px;color:var(--accent);">☁️ Đồng bộ dữ liệu (JSONBin.io)</div>' +
      '<div style="font-size:12px;color:var(--text2);margin-bottom:8px;">Nhập API Key và Bin ID để đồng bộ yêu thích & lịch sử giữa các website.</div>' +
      '<div style="display:grid;gap:8px;">' +
        ...
        '<div style="display:flex;gap:6px;flex-wrap:wrap;">' +
          '<button class="uvd-btn uvd-btn-sm" id="__uvd_jb_save__" style="background:rgba(109,140,255,0.3);">💾 Lưu cấu hình</button>' +
          '<button class="uvd-btn uvd-btn-sm" id="__uvd_jb_push__" style="background:rgba(34,197,94,0.3);">📤 Tải lên</button>' +
          '<button class="uvd-btn uvd-btn-sm" id="__uvd_jb_pull__" style="background:rgba(139,92,246,0.3);">📥 Tải xuống</button>' +
        '</div>' +
        '<div style="display:flex;align-items:center;gap:8px;margin-top:4px;">' +
          '<input type="checkbox" id="__uvd_jb_autosync__" ' + (jb.autoSync ? 'checked' : '') + '>' +
          '<label for="__uvd_jb_autosync__" style="font-size:12px;color:var(--text2);">Tự động đồng bộ khi mở script (tải xuống)</label>' +
        '</div>' +
      '</div>' +
    '</div>' +
    ...
    '<div class="uvd-card">' +
      ...
      '<button class="uvd-btn uvd-btn-sm" id="__uvd_restore__" style="width:100%;margin-bottom:6px;">Nhập dữ liệu (JSON)</button>' +
  ...
  document.getElementById('__uvd_reset__').onclick = function() {
    if (confirm('Xóa toàn bộ dữ liệu?')) { localStorage.removeItem(STORAGE_KEY); data = {favorites:[],siteProfiles:{},history:[],jsonbin:{apiKey:'',binId:'',autoSync:false}}; storage.set(data); buildUI(); }
  };
}
...
console.log('V5.8 Fixed - Auto upload on close, strong popup blocker');
toast('V5.8 sẵn sàng! (Popup đã được chặn)');
