...
function searchSubDL(query, cb) {
  ...
  // QUAN TRỌNG: nếu mạng/CSP của trang chặn âm thầm request tới api.subdl.com
  // (không trả lỗi, không timeout tự nhiên), fetch có thể treo vô thời hạn và
  // "Đang tìm..." không bao giờ đổi. Dùng AbortController để tự huỷ sau 15s.
  ...
  .catch(function(err) {
    ...
    toast(isTimeout ? 'SubDL không phản hồi sau 15s (mạng có thể đang chặn api.subdl.com)' : 'Lỗi kết nối SubDL: ' + (err && err.message ? err.message : 'CORS/mạng'));
    cb([]);
  });
}
...
.uvd-panel-minimized.uvd-glass-panel{background:rgba(10,11,16,0.92)!important}
