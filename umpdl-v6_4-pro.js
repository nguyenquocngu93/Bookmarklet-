...
var playerState = {
  ...
  sleepEndAt: 0,
  savePosTimer: null,
  gesture: { active: false, mode: null, moved: false, startX: 0, startY: 0, startVal: 0 }
};
...
// ========== SWIPE GESTURE: brightness (trái) / âm lượng (phải) ==========
function attachPlayerGestures(wrapper, video) {
  ...
  function onStart(e) {
    ...
    w = wrapper.getBoundingClientRect();
    var isLeft = (t.clientX - w.left) < w.width / 2;
    playerState.gesture.active = true;
    playerState.gesture.moved = false;
    playerState.gesture.mode = isLeft ? 'brightness' : 'volume';
    playerState.gesture.startX = t.clientX;
    playerState.gesture.startY = t.clientY;
    playerState.gesture.startVal = isLeft
      ? (parseFloat((video.style.filter || '').replace(/[^\d.]/g, '')) || 100)
      : (video.volume * 100);
  }
  function onMove(e) {
    if (!playerState.gesture.active) return;
    var t = e.touches ? e.touches[0] : e;
    var dy = playerState.gesture.startY - t.clientY;
    if (Math.abs(dy) > 8 || Math.abs(t.clientX - playerState.gesture.startX) > 8) playerState.gesture.moved = true;
    if (!playerState.gesture.moved) return;
    var delta = (dy / w.height) * 150;
    if (playerState.gesture.mode === 'brightness') {
      var b = Math.max(40, Math.min(160, playerState.gesture.startVal + delta));
      video.style.filter = 'brightness(' + b + '%)';
      showGestureHint('☀️ Độ sáng video ' + Math.round(b) + '%');
    } else {
      var v = Math.max(0, Math.min(100, playerState.gesture.startVal + delta));
      video.volume = v / 100;
      showGestureHint('🔊 Âm lượng ' + Math.round(v) + '%');
    }
    e.preventDefault();
  }
  function onEnd(e) {
    var wasDrag = playerState.gesture.moved;
    playerState.gesture.active = false;
    hideGestureHintSoon();
    if (wasDrag) return; // đang vuốt chỉnh sáng/âm lượng, không tính là tap
