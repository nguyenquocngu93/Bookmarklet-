/**
 * Universal Media Player & Downloader - V5.8
 * - Tích hợp đồng bộ dữ liệu qua JSONBin.io
 * - Tối ưu thu nhỏ (giảm lag)
 * - Ước tính dung lượng video (hiển thị trong danh sách chất lượng)
 * - Cập nhật độ phân giải chính xác khi chọn quality
 * - Tự động đồng bộ khi mở script (tùy chọn)
...
// ========== GET FILE SIZE (HEAD request) ========== // NEW IN V5.8
  ...
  function showQualitySubMenu() {
    ...
    sel.onchange = function() {
      ...
      if (q && playerState.hls) {
        ...
        // Cập nhật resolution cho tiêu đề
...
// ========== MINIMIZE / RESTORE (tối ưu) ========== // IMPROVED IN V5.8
function minimizePlayer() {
  ...
  // Dùng opacity và transform nhẹ, không scale cả panel
...
// ========== JSONBIN SYNC FUNCTIONS ========== // NEW IN V5.8
...
function syncToJsonBin(apiKey, binId, onSuccess, onError) {
  ...
  .then(function(json) {
    toast('✅ Đã tải lên JSONBin thành công!');
...
function syncFromJsonBin(apiKey, binId, onSuccess, onError) {
  ...
  .then(function(json) {
    ...
    storage.set(data);
    toast('✅ Đã tải dữ liệu từ JSONBin!');
    if (onSuccess) onSuccess(json);
    buildUI(); // Refresh giao diện
  })
  .catch(function(err) {
...
function buildUI() {
  ...
  document.getElementById('__uvd_minimize__').onclick = function() {
    ...
    setTimeout(function() {
      ...
      panelEl.style.transform = ''; // reset để lần sau hiện lại
...
// ===== SHOW QUALITY PICKER (có ước tính dung lượng) ===== // UPDATED IN V5.8
function showQualityPicker(url) {
  ...
  parseM3U8Master(url, function(qualities) {
    ...
    qualities.forEach(function(q, idx) {
      // Lấy dung lượng cho từng stream (HEAD request)
      getFileSize(q.url, function(size) {
        var sizeStr = size ? formatFileSize(size) : '? MB';
        // Cập nhật card đã render (tìm card tương ứng)
...
// ===== RENDER SETTINGS (có phần JSONBin) ===== // UPDATED IN V5.8
...
console.log('V5.8 UMP DL - JSONBin Sync Ready');
toast('V5.8 sẵn sàng! (Đồng bộ JSONBin)');
