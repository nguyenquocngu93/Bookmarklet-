/**
 * Universal Media Player v2.3 - Video.js Edition (FIX)
 * - Tải Video.js từ nhiều CDN (có fallback)
...
(function() {
    ...
    function openPlayer(streamInfo) {
        ...
        loading.innerHTML = '<div style="color:#fff;text-align:center;"><div style="font-size:36px;">⏳</div><div style="font-size:12px;margin-top:5px;">Đang tải Video.js...</div></div>';
        ...
        var cdnSources = [
            ...
            'https://unpkg.com/video.js@7.21.5/dist/video.min.js'
        ];
        var cssSources = [
            'https://cdnjs.cloudflare.com/ajax/libs/video.js/7.21.5/video-js.min.css',
            'https://cdn.jsdelivr.net/npm/video.js@7.21.5/dist/video-js.min.css',
        ...
        function loadVideoJS(callback) {
            ...
            function tryLoad() {
                if (current >= cdnSources.length) {
                    loading.innerHTML = '<div style="color:#f44336;"><div style="font-size:30px;">❌</div><div>Không thể tải Video.js</div><button onclick="location.reload()" style="background:#fff;color:#000;border:0;padding:8px 16px;border-radius:4px;margin-top:8px;cursor:pointer;">🔄 Thử lại</button></div>';
                    return;
                }
                
                loading.innerHTML = '<div style="color:#fff;text-align:center;"><div style="font-size:36px;">⏳</div><div style="font-size:12px;">Đang tải Video.js từ CDN ' + (current + 1) + '...</div></div>';
                
                var script = document.createElement('script');
                script.src = cdnSources[current];
                script.onload = function() {
                    loaded = true;
                    // Kiểm tra videojs đã load chưa
                    if (typeof videojs !== 'undefined') {
                        callback();
                    } else {
                        current++;
        ...
        loadVideoJS(function() {
            loading.innerHTML = '<div style="color:#fff;text-align:center;"><div style="font-size:36px;">⏳</div><div style="font-size:12px;">Đang khởi tạo trình phát...</div></div>';
            ...
            try {
                ...
                player.on('error', function() {
                    ...
                    if (err && err.code === 4) msg = '❌ URL không tồn tại hoặc bị chặn CORS';
                    if (err && err.code === 2) msg = '❌ Lỗi mạng, kiểm tra kết nối';
                    if (err && err.code === 3) msg = '❌ Định dạng video không hỗ trợ';
                    
                    loading.innerHTML = '<div style="color:#f44336;text-align:center;background:rgba(0,0,0,0.8);padding:20px;border-radius:10px;"><div style="font-size:30px;">❌</div><div style="font-size:14px;font-weight:bold;">' + msg + '</div><div style="font-size:11px;margin-top:5px;opacity:0.7;">' + url.substring(0, 80) + '</div><button onclick="window.location.reload()" style="background:#fff;color:#000;border:0;padding:8px 16px;border-radius:4px;margin-top:10px;cursor:pointer;">🔄 Thử lại</button></div>';
                });
                
                // Fallback: nếu không load được sau 10s
                var timeoutId = setTimeout(function() {
                    if (loading.parentNode) {
                        loading.innerHTML = '<div style="color:#ff9800;text-align:center;background:rgba(0,0,0,0.8);padding:20px;border-radius:10px;"><div style="font-size:30px;">⏰</div><div style="font-size:14px;font-weight:bold;">Quá thời gian tải</div><div style="font-size:11px;margin-top:5px;opacity:0.7;">Thử dùng nút Bypass trong menu</div></div>';
                    }
                }, 10000);
                player.one('loadedmetadata', function() { clearTimeout(timeoutId); });
                ...
                loading.innerHTML = '<div style="color:#f44336;text-align:center;background:rgba(0,0,0,0.8);padding:20px;border-radius:10px;"><div style="font-size:30px;">❌</div><div style="font-size:14px;font-weight:bold;">Lỗi: ' + e.message + '</div><button onclick="window.location.reload()" style="background:#fff;color:#000;border:0;padding:8px 16px;border-radius:4px;margin-top:10px;cursor:pointer;">🔄 Thử lại</button></div>';
            }
        });
    ...
    function showMenu(streamInfo, video) {
        ...
        html += '<button class="__ump_m__" data-a="share-url" style="background:#FF6B6B;color:#fff;border:0;padding:10px;border-radius:5px;font-weight:bold;text-align:left;font-size:12px;cursor:pointer;">📱 Share</button>';
        ...
        html += '<button class="__ump_m__" data-a="dl-mp4" style="background:#FF9800;color:#fff;border:0;padding:10px;border-radius:5px;font-weight:bold;text-align:left;font-size:12px;cursor:pointer;">⬇️ Download</button>';
        html += '<button class="__ump_m__" data-a="dl-cmd" style="background:#9C27B0;color:#fff;border:0;padding:10px;border-radius:5px;font-weight:bold;text-align:left;font-size:12px;cursor:pointer;">💻 Copy yt-dlp</button>';
        html += '<button class="__ump_m__" data-a="dl-ffmpeg" style="background:#795548;color:#fff;border:0;padding:10px;border-radius:5px;font-weight:bold;text-align:left;font-size:12px;cursor:pointer;">🎬 Copy FFmpeg</button>';
        html += '<button class="__ump_m__" data-a="reload-video" style="background:#009688;color:#fff;border:0;padding:10px;border-radius:5px;font-weight:bold;text-align:left;font-size:12px;cursor:pointer;">🔄 Reload</button>';
        html += '<button class="__ump_m__" data-a="rename" style="background:#3F51B5;color:#fff;border:0;padding:10px;border-radius:5px;font-weight:bold;text-align:left;font-size:12px;cursor:pointer;">✏️ Rename</button>';
        html += '<button class="__ump_m__" data-a="bypass-menu" style="background:#FF5722;color:#fff;border:0;padding:10px;border-radius:5px;font-weight:bold;text-align:left;font-size:12px;cursor:pointer;">🛡️ Bypass</button>';
        html += '</div>';
    ...
    function showBypassMenu(streamInfo) {
        ...
        html += '<p style="font-size:11px;color:#666;margin-bottom:10px;">Dùng khi video bị chặn (CORS, Referer...)</p>';
        ...
        html += '<button class="__ump_bypass__" data-mode="proxy" style="background:#2196F3;color:#fff;border:0;padding:12px;border-radius:5px;width:100%;margin-bottom:6px;font-weight:bold;font-size:13px;cursor:pointer;">🔄 CORS Proxy</button>';
    ...
    console.log('🎬 UMP v2.3 - Video.js với fallback CDN');
})();
