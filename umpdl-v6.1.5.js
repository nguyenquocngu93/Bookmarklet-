 ...
 * Full screen + bo góc, minimize mới, filter chữ
...
var minBtn = document.getElementById('__uvd_min_float__'); if(minBtn) minBtn.remove();

var STORAGE_KEY = 'uvd_data_v54';
var storage = {
  get: function() { try { return JSON.parse(localStorage.getItem(STORAGE_KEY)) || {}; } catch(e) { return {}; } },
  set: function(d) { try { localStorage.setItem(STORAGE_KEY, JSON.stringify(d)); } catch(e) {} }
};
...
var pageInfo = {
  ...
  referer: profile.referer, origin: location.origin,
...
function isAdUrl(url) {
  ...
  for(var i=0;i<data.filterlist.length;i++) {
    var p = data.filterlist[i].trim().toLowerCase(); if(!p) continue;
    if(p.startsWith('regex:')) { try { if(new RegExp(p.slice(6),'i').test(url)) return true; } catch(e){} }
    else if(lower.indexOf(p)!==-1) return true;
  }
  return false;
}
...
function findUrls(text,source) {
  ...
  patterns.forEach(function(p) {
    ...
    m.forEach(function(u) {
      ...
      if(isAdUrl(u)) { __uvdAdBlockedCount++; return; }
...
function scan(doc,src) {
  try {
    doc.querySelectorAll('video,source,audio').forEach(function(v){ if(v.src) findUrls(v.src,src+':element'); if(v.currentSrc) findUrls(v.currentSrc,src+':current'); });
    ...
    doc.querySelectorAll('iframe').forEach(function(i,idx) {
      if(i.src) {
        if(!isAdUrl(i.src)) urls.set(i.src,{type:'IFRAME',source:'iframe#'+idx,priority:99,timestamp:Date.now()});
...
// ========== POPUP BLOCKER ==========
var __uvdPopupBlockActive = false, __uvdOriginalWindowOpen = null, __uvdBlockedCount = 0;
window.__uvdSafeOpen = function(url) {
  return __uvdOriginalWindowOpen ? __uvdOriginalWindowOpen(url,'_blank') : window.open(url,'_blank');
};
function killBlankLinks(e) {
  var t = e.target;
  if(t.closest && (t.closest('#__uvd__')||t.closest('#__uvd_player_overlay__'))) return;
  while(t && t!==document) {
    if(t && t.tagName==='A') { var tg = t.target; if(tg && tg!=='_self'&&tg!=='_top'&&tg!=='_parent') { e.preventDefault(); e.stopPropagation(); __uvdBlockedCount++; return; } }
    t = t.parentNode;
  }
}
function installPopupBlock() {
  if(__uvdPopupBlockActive) return; __uvdPopupBlockActive = true;
  __uvdOriginalWindowOpen = window.open; window.open = function(){ __uvdBlockedCount++; return null; };
  ['click','mousedown','pointerdown','auxclick'].forEach(function(t){ document.addEventListener(t,killBlankLinks,true); });
}
function uninstallPopupBlock() {
  if(!__uvdPopupBlockActive) return; __uvdPopupBlockActive = false;
  if(__uvdOriginalWindowOpen) window.open = __uvdOriginalWindowOpen;
  ['click','mousedown','pointerdown','auxclick'].forEach(function(t){ document.removeEventListener(t,killBlankLinks,true); });
}
function blockPopupsDuring(dur,fn){ installPopupBlock(); fn(); }
...
function simulateClick(el) {
  try {
    ...
    if(!rect.width||!rect.height){ x=0; y=0; }
    ['pointerdown','mousedown','pointerup','mouseup','click'].forEach(function(type) {
      var ev; try { ev = new MouseEvent(type,{bubbles:true,cancelable:true,view:window,clientX:x,clientY:y}); } catch(e) { ev = document.createEvent('MouseEvent'); ev.initMouseEvent(type,true,true,window,0,0,0,x,y,false,false,false,false,0,null); }
      el.dispatchEvent(ev);
    });
...
function autoClickPlayButtons(root,depth,allowVideoPlayFallback) {
  ...
  var clicked = 0;
  AUTO_PLAY_SELECTORS.forEach(function(sel){ try { root.querySelectorAll(sel).forEach(function(el){ if(simulateClick(el)) clicked++; }); } catch(e){} });
  if(allowVideoPlayFallback) {
    try { root.querySelectorAll('video').forEach(function(v){ if(v.paused){ var wasMuted=v.muted; v.muted=true; var p=v.play(); if(p&&p.then) p.then(function(){ setTimeout(function(){ try{v.pause();v.currentTime=0;v.muted=wasMuted;}catch(e){} },600); }).catch(function(){}); } }); } catch(e){}
  }
  if(depth<2) try { root.querySelectorAll('iframe').forEach(function(f){ try { if(f.contentDocument) clicked += autoClickPlayButtons(f.contentDocument,depth+1,allowVideoPlayFallback); } catch(e){} }); } catch(e){}
  return clicked;
}
function pauseAllPlayingVideos(root,depth) {
  root = root||document; depth = depth||0;
  var paused = 0;
  try { root.querySelectorAll('video').forEach(function(v){ if(!v.paused){ try{v.pause();paused++;}catch(e){} } }); } catch(e){}
  if(depth<2) try { root.querySelectorAll('iframe').forEach(function(f){ try{if(f.contentDocument) paused+=pauseAllPlayingVideos(f.contentDocument,depth+1);}catch(e){} }); } catch(e){}
  return paused;
}
...
function runAutoClickAndRescan(silent) {
  var before = urls.size, clicked = 0;
  blockPopupsDuring(2500,function(){ clicked = autoClickPlayButtons(document,0,!silent); });
  setTimeout(function() {
    scan(document,'autoclick-rescan');
    var found = urls.size - before;
    if(found>0) {
      toast('▶ Tự động Play: tìm thêm '+found+' luồng mới');
      if(document.getElementById('__uvd__')) buildUI();
      setTimeout(function(){ var n = pauseAllPlayingVideos(); if(n>0) toast('⏸ Đã tạm dừng video gốc, xem qua player script cho ổn định'); },800);
    } else if(!silent) toast(clicked>0?'Đã bấm Play nhưng chưa thấy link mới, thử lại sau vài giây':'Không tìm thấy nút Play trên trang này');
  },1200);
}
...
// ========== M3U8 MASTER PARSER ==========
function parseM3U8Master(url,cb) {
  fetch(url,{headers:{'Referer':pageInfo.referer}}).then(function(r){return r.text()}).then(function(text) {
    if(!text.includes('#EXT-X-STREAM-INF')){cb(null);return;}
    var qualities=[], lines=text.split('\n');
    for(var i=0;i<lines.length;i++) {
      if(lines[i].startsWith('#EXT-X-STREAM-INF')) {
        var info=lines[i], next=(lines[i+1]||'').trim();
        if(next&&!next.startsWith('#')) {
          ...
          var streamUrl=next;
          if(!streamUrl.startsWith('http')) { var base=url.substring(0,url.lastIndexOf('/')+1); streamUrl=base+streamUrl; }
    ...
    cb(qualities);
  }).catch(function(e){console.error(e);cb(null);});
}
...
function shareUrl(url) {
  if(navigator.share) navigator.share({title:pageInfo.title,url:url}).catch(function(){toast('Không thể chia sẻ')});
  else toast('Thiết bị không hỗ trợ chia sẻ');
}
function addToHistory(url,type) {
  data.history=data.history||[]; data.history.unshift({url:url,type:type,title:pageInfo.title,host:pageInfo.host,timestamp:Date.now()});
  if(data.history.length>50) data.history=data.history.slice(0,50); storage.set(data);
}
...
function addRipple(e) {
  var btn=e.currentTarget, ripple=document.createElement('span'); ripple.className='uvd-ripple';
...
function lockOrientation(video) {
  ...
  var target = video.videoHeight>video.videoWidth?'portrait':'landscape';
...
// ========== PLAYER STATE ==========
var playerState = {
  overlay:null,mini:null,video:null,hls:null,qualities:[],currentQuality:0,speed:1,
  isMinimized:false,url:'',type:'',resolution:'',bandwidth:0,_displayedResolution:'',
  onFullscreenChange:null,menuOpen:false,guardInterval:null
};
...
function showVideoPlayer(url,type) {
  ...
  var overlay=document.createElement('div'); overlay.id='__uvd_player_overlay__';
  ...
  document.body.appendChild(overlay); playerState.overlay=overlay;
  ...
  var titleInfo=document.createElement('div'); titleInfo.className='uvd-title-info'; titleInfo.style.cssText='min-width:0;flex:1;';
  ...
  var minVideoBtn=document.createElement('button'); minVideoBtn.className='uvd-btn uvd-btn-sm'; minVideoBtn.style.cssText='background:rgba(255,255,255,0.15);color:#fff;font-size:12px;'; minVideoBtn.textContent='⛶'; minVideoBtn.onclick=minimizePlayer;
  btnGroup.appendChild(minVideoBtn);
  var closeBtn=document.createElement('button'); closeBtn.id='__uvd_player_close__'; closeBtn.className='uvd-btn uvd-btn-sm'; closeBtn.style.cssText='background:rgba(255,0,0,0.3);color:#fff;'; closeBtn.textContent='✕';
  btnGroup.appendChild(closeBtn); titleRow.appendChild(btnGroup); header.appendChild(titleRow);
  var toolbar=document.createElement('div'); toolbar.style.cssText='display:flex;flex-wrap:wrap;gap:6px;margin-top:8px;';
  var qualityBtn=document.createElement('button'); qualityBtn.className='uvd-btn uvd-btn-sm'; qualityBtn.style.cssText='background:rgba(255,255,255,0.1);color:#fff;font-size:12px;'; qualityBtn.textContent='Chất lượng';
  qualityBtn.onclick=function(){ if(playerState.qualities.length>0) showQualitySubMenu(); else toast('Không có chất lượng để chọn'); };
  toolbar.appendChild(qualityBtn);
  var speedBtn=document.createElement('button'); speedBtn.className='uvd-btn uvd-btn-sm'; speedBtn.style.cssText='background:rgba(255,255,255,0.1);color:#fff;font-size:12px;'; speedBtn.textContent='Tốc độ'; speedBtn.onclick=showSpeedSubMenu;
  toolbar.appendChild(speedBtn);
  var fsBtn=document.createElement('button'); fsBtn.className='uvd-btn uvd-btn-sm'; fsBtn.style.cssText='background:rgba(255,255,255,0.1);color:#fff;font-size:12px;'; fsBtn.textContent='Toàn màn hình';
  fsBtn.onclick=function(){ var vw=document.getElementById('__uvd_video_wrapper__'); var fs=vw.requestFullscreen||vw.webkitRequestFullscreen||vw.mozRequestFullScreen||vw.msRequestFullscreen; if(fs) fs.call(vw); };
  toolbar.appendChild(fsBtn);
  if(type==='MP4'||type==='MKV'||type==='WEBM') {
    var dlBtn=document.createElement('button'); dlBtn.className='uvd-btn uvd-btn-sm'; dlBtn.style.cssText='background:rgba(34,197,94,0.3);color:#fff;font-size:12px;'; dlBtn.textContent='Tải xuống';
    dlBtn.onclick=function(){ var a=document.createElement('a'); a.href=url; a.download=pageInfo.title+'.'+type.toLowerCase(); document.body.appendChild(a); a.click(); a.remove(); toast('Đang tải xuống...'); };
    toolbar.appendChild(dlBtn);
  }
  if(type==='M3U8'||type==='MPD') {
    var shareBtn=document.createElement('button'); shareBtn.className='uvd-btn uvd-btn-sm'; shareBtn.style.cssText='background:rgba(139,92,246,0.3);color:#fff;font-size:12px;'; shareBtn.textContent='Chia sẻ'; shareBtn.onclick=function(){shareUrl(url);};
  ...
  var video=document.createElement('video'); video.id='__uvd_player_video__'; video.style.cssText='max-width:100%;max-height:100%;width:auto;height:auto;display:block;object-fit:contain;';
  video.setAttribute('controls',''); video.setAttribute('playsinline',''); video.setAttribute('webkit-playsinline',''); video.setAttribute('crossorigin','anonymous');
  videoWrapper.appendChild(video); overlay.appendChild(videoWrapper); playerState.video=video;

  var footer=document.createElement('div'); footer.style.cssText='padding:8px 16px;background:rgba(0,0,0,0.7);border-top:1px solid rgba(255,255,255,0.1);font-size:12px;color:#aaa;display:flex;justify-content:space-between;flex-shrink:0;';
  ...
  function showQualitySubMenu() { /* giữ nguyên từ V6.2 */ }
  function showSpeedSubMenu() { /* giữ nguyên từ V6.2 */ }

  var isHls = url.includes('.m3u8')||url.includes('m3u8'), activeHls=null;
  function updateTitleDisplay() { /* giữ nguyên */ }
  function formatBytes(b) { /* giữ nguyên */ }
  function updateSizeEstimate() { /* giữ nguyên */ }
  ...
  function onMetadataLoaded() { /* giữ nguyên */ }
  video.addEventListener('loadedmetadata',onMetadataLoaded);
  video.addEventListener('durationchange',updateSizeEstimate);
  video.addEventListener('timeupdate',function(){ var t=video.currentTime,d=video.duration; if(d) document.getElementById('__uvd_player_time__').textContent=formatTime(t)+' / '+formatTime(d); updateTitleDisplay(); });
  function formatTime(sec){ var m=Math.floor(sec/60),s=Math.floor(sec%60); return (m<10?'0':'')+m+':'+(s<10?'0':'')+s; }

  if(isHls) {
    if(window.Hls&&Hls.isSupported()) {
      activeHls=new Hls(); activeHls.loadSource(url); activeHls.attachMedia(video);
      activeHls.on(Hls.Events.MANIFEST_PARSED,function(){ setTimeout(function(){lockOrientation(video);},100); parseM3U8Master(url,function(q){ if(q&&q.length){ playerState.qualities=q; updateSizeEstimate(); } }); });
      activeHls.on(Hls.Events.LEVEL_SWITCHED,function(e,data){ var lvl=activeHls.levels[data.level]; if(lvl){ playerState.resolution=(lvl.width&&lvl.height)?(lvl.width+'x'+lvl.height):''; playerState.bandwidth=lvl.bitrate||0; } updateTitleDisplay(); updateSizeEstimate(); });
      playerState.hls=activeHls;
    } else if(video.canPlayType('application/vnd.apple.mpegurl')) { video.src=url; }
    else { var s=document.createElement('script'); s.src='https://cdn.jsdelivr.net/npm/hls.js@latest'; s.onload=function(){showVideoPlayer(url,type);}; document.head.appendChild(s); return; }
  } else { video.src=url; }

  document.getElementById('__uvd_player_close__').onclick=function(){ closePlayer(); document.getElementById('__uvd_stream_list__').style.display='block'; };
  function onFullscreenChange(){ var fs=!!(document.fullscreenElement||document.webkitFullscreenElement||document.mozFullScreenElement||document.msFullscreenElement); if(fs) lockOrientation(video); else unlockOrientation(); }
  playerState.onFullscreenChange=onFullscreenChange;
  document.addEventListener('fullscreenchange',onFullscreenChange); document.addEventListener('webkitfullscreenchange',onFullscreenChange); document.addEventListener('mozfullscreenchange',onFullscreenChange); document.addEventListener('MSFullscreenChange',onFullscreenChange);

  if(playerState.guardInterval) clearInterval(playerState.guardInterval);
  playerState.guardInterval = setInterval(function(){
    if(!playerState.overlay) { clearInterval(playerState.guardInterval); playerState.guardInterval=null; return; }
    try{ var all=document.querySelectorAll('video'); for(var i=0;i<all.length;i++){ var v=all[i]; if(v!==playerState.video && !v.paused) v.pause(); } }catch(e){}
  },500);
}

// (Phần 1 kết thúc - phần 2 sẽ bao gồm minimizePlayer, restorePlayer, closePlayer, buildUI, renderStreams, renderSettings...)
...
function closePlayer() {
  if (playerState.overlay) {
    ...
    if (playerState.guardInterval) {
      clearInterval(playerState.guardInterval);
      playerState.guardInterval = null;
    }
    unlockOrientation();
...
// ========== BUILD UI (FULL SCREEN + BO GÓC + MINIMIZE MỚI) ==========
...
.uvd-glass-panel{background:var(--glass);backdrop-filter:blur(28px) saturate(130%);-webkit-backdrop-filter:blur(28px) saturate(130%);border:1px solid var(--border);border-radius:var(--radius-lg);box-shadow:0 20px 50px rgba(0,0,0,0.8);color:var(--text);font-family:-apple-system,BlinkMacSystemFont,'SF Pro Display','Segoe UI',Roboto,sans-serif;font-size:var(--fs-base);padding:0;width:100%;height:100%;position:relative;overflow:hidden}
.uvd-glass-panel::before{content:'';position:absolute;top:0;left:0;right:0;height:1px;z-index:2;background:linear-gradient(90deg,transparent,rgba(109,140,255,0.6),rgba(185,139,255,0.6),transparent);opacity:0.7}
...
.uvd-panel-content{position:relative;z-index:1;display:flex;flex-direction:column;height:100%;min-height:0;padding:16px;padding-top:60px}
.uvd-panel-header{position:absolute;top:0;left:0;right:0;z-index:10;display:flex;justify-content:space-between;align-items:center;padding:12px 16px;background:var(--glass);backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);border-bottom:1px solid var(--border);flex-shrink:0}
.uvd-minimized .uvd-panel-content{padding-top:0}
.uvd-minimized .uvd-panel-header{border-radius:var(--radius-lg);position:relative;height:50px}
.uvd-minimized .uvd-panel-content > *:not(.uvd-panel-header){display:none}
.uvd-minimized .uvd-panel-content{overflow:hidden}
...
function buildUI() {
  ...
  panel.style.cssText = 'position:fixed;top:8px;left:8px;right:8px;bottom:8px;z-index:2147483647;animation:uvdScaleIn 0.4s ease;';
  ...
  // Header chứa 4 nút
  var header = document.createElement('div');
  header.className = 'uvd-panel-header';
  header.innerHTML = 
    ...
    '<div style="display:flex;gap:6px;">' +
      '<button class="uvd-btn-icon" id="__uvd_autoplay__" title="Tự động bấm Play"><span style="font-size:15px;">▶</span></button>' +
      '<button class="uvd-btn-icon" id="__uvd_refresh__" title="Làm mới"><span style="font-size:16px;">↻</span></button>' +
      '<button class="uvd-btn-icon" id="__uvd_minimize__" title="Thu nhỏ / Mở rộng"><span style="font-size:18px;">−</span></button>' +
      '<button class="uvd-btn-icon" id="__uvd_close__" title="Đóng"><span style="font-size:16px;">×</span></button>' +
    '</div>';
...
function showCommandPicker(url, type) {
  ...
  var html = '<div class="uvd-glass-panel" style="max-width:600px;margin:auto;border-radius:var(--radius-lg);">';
...
function showEditor(text) {
  ...
  overlay.className = 'uvd-overlay';
  overlay.innerHTML = 
    '<div class="uvd-glass-panel" style="max-width:600px;margin:auto;border-radius:var(--radius-lg);">' +
...
function showQualityPicker(url) {
  var overlay = document.createElement('div');
  overlay.className = 'uvd-overlay';
  overlay.innerHTML = '<div class="uvd-glass-panel" style="max-width:600px;margin:auto;border-radius:var(--radius-lg);">Đang phân tích M3U8...</div>';
  document.body.appendChild(overlay);
  
  parseM3U8Master(url, function(qualities) {
    if (!qualities) {
      overlay.innerHTML = '<div class="uvd-glass-panel" style="max-width:600px;margin:auto;text-align:center;border-radius:var(--radius-lg);">' +
...
function renderSettings(container) {
  ...
  container.innerHTML =
    ...
    '<div class="uvd-section-title"><span class="uvd-section-num">Ad</span> Lọc quảng cáo (Filterlist)</div>' +
    ...
    '<div class="uvd-card uvd-timeline-card">' +
      '<div style="font-size:13px;color:var(--text2);line-height:1.7;">' +
        ...
        '• Nút <strong style="color:var(--accent);">−</strong> trên header chính để thu nhỏ toàn bộ script<br>' +
...
console.log('V6.3 UMP DL - Full screen, minimize mới, filter chữ');
toast('V6.3 sẵn sàng!');
