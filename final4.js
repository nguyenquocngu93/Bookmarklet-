 ...
 * - Profile mặc định với avatar, bookmarklet hướng dẫn
...
function makeCommands(url, type, title) {
  ...
  if (type === 'BLOB') {
    return {
      'warning': { label: '⚠️ Blob không tải trực tiếp được', cmd: 'Mở DevTools → Network → tìm request gốc (m3u8/mp4) hoặc dùng yt-dlp thử với blob: URL.' },
      ...base
    };
  }
  return base;
...
function openPreviewInNewTab(url, type) {
  ...
  if (directTypes.indexOf(type) !== -1) {
    window.open(url, '_blank');
    return;
  }
  if (type === 'M3U8') {
    var html = '<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Video Preview (M3U8)</title>';
    ...
    html += '</head><body>';
    html += '<video id="video" controls playsinline preload="auto" style="width:100%;height:100%;object-fit:contain;"></video>';
    html += '<script>';
...
function enterFullscreenAuto(element, video) {
  ...
  else { setTimeout(afterFullscreen, 100); }
}
function isDocFullscreen() { return !!(document.fullscreenElement || document.webkitFullscreenElement || document.mozFullScreenElement || document.msFullscreenElement); }
...
function lockAutoOrientation(video, wrapper) {
  ...
  if (video.videoWidth && video.videoHeight) { applyLock(); }
  else {
    video.addEventListener('loadedmetadata', applyLock, { once: true });
    setTimeout(function() { if (!__uvdOrientTarget && video.videoWidth && video.videoHeight) applyLock(); }, 400);
  }
  ...
  window.addEventListener('orientationchange', function() { if (__uvdOrientTarget) enforceVisualOrientation(wrapper, __uvdOrientTarget); });
  window.addEventListener('resize', function() { if (__uvdOrientTarget) enforceVisualOrientation(wrapper, __uvdOrientTarget); });
}
...
#__uvd_video_wrapper__:-ms-fullscreen { width: 100vw !important; height: 100vh !important; background: #000 !important; }
...
#__uvd_video_wrapper__:-ms-fullscreen video { width: 100% !important; height: 100% !important; object-fit: contain !important; }
...
function buildUI() {
  ...
  header.style.cssText = 'display:flex;justify-content:space-between;align-items:center;padding-bottom:12px;border-bottom:1px solid var(--uvd-border);margin-bottom:10px;';
  header.innerHTML = 
  ...
  playerContainer.style.cssText = 'display:none;flex-direction:column;height:100%;background:var(--uvd-glass);border-radius:var(--uvd-radius-md);overflow:hidden;border:1px solid var(--uvd-border);';
    ...
    card.innerHTML = 
      ...
      '<div class="uvd-grid-2" style="margin-top:8px;">' +
        ...
        (item.type === 'IFRAME' ? 
          ...
          (isBlob ?
            '<button class="uvd-btn uvd-btn-sm uvd-ripple-btn" data-action="cmd" data-url="' + encodeURIComponent(item.url) + '" data-type="' + item.type + '" style="grid-column:1/3;background:rgba(255,215,0,0.2);">⚠️ Lệnh (blob)</button>' :
            (item.type === 'M3U8' ?
...
console.log('V5.1 Dark Liquid Edition - Final (fix UI, blob, ffplay/mpv)');
toast('V5.1 sẵn sàng — Đã fix giao diện và hỗ trợ blob!');
