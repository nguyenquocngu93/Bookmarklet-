/**
 * Universal Media Player & Downloader - V6.7.13 PRO
 * - FIX BUG "UI CHỒNG X2": auto-click rescan (3 mốc 1.2s/2.2s/3.4s) trước so
 *   sánh số luồng với MỐC GỐC ở mọi lần quét, nên hễ mốc đầu tìm thấy gì là
 *   2 mốc sau cũng tưởng "có luồng mới" (dù không có gì mới thêm) và gọi lại
 *   buildUI() — panel bị vẽ lại + chạy lại animation scale-in liên tiếp, nhìn
 *   như chồng/nháy x2. Đã sửa so sánh với mốc quét NGAY TRƯỚC ĐÓ, chỉ rebuild
 *   khi thật sự có luồng mới kể từ lần quét gần nhất.
 * - GIẢM GIẬT khi thu nhỏ/mở rộng panel: tạm tắt backdrop-filter (phần tốn
 *   compositing nhất) trong lúc height đang animate, bật lại khi đứng yên.
 * - Dọn nốt vài chỗ màu xanh còn sót (toast mặc định, cỡ file trong player,
 *   nút "Chia sẻ" từng dùng tím-chàm) — đồng bộ hết về tông hồng.
 * - Bo tròn thêm: --radius-lg 26px→32px, --radius-md 16px→20px, --radius-sm
 *   12px→14px, khớp luôn góc bo của Cài đặt overlay.
 * - Nền kính chuyển từ đen-xám trung tính sang đen ánh tím-hồng nhẹ (viền
 *   cũng ăn tông hồng nhạt) — chọn cách phối NHẸ thay vì nhuộm hồng đậm toàn
 *   bộ nền, vì nhuộm đậm dễ lặp lại đúng lỗi tương phản chữ vừa sửa ở bản trước.
 * - (6.7.12) Theme hồng sáng cho accent/glow/gradient; fix chữ "Cài đặt" đen
 *   khó đọc; fix 3 tab chia khoảng không đều; giảm chi phí nền "liquid".
