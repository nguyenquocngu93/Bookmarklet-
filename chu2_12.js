 ...
 * - Sửa: Mở iframe trong overlay thay vì tab mới
 * - PerformanceObserver bắt link từ iframe
 * - Tất cả tính năng cũ: player, quality, subtitle, speed, etc.
 */
(function() {
'use strict';
...
function showVideoPlayer(url, type) {
  ...
  footer.innerHTML = '<span id="__uvd_player_status__">Đang tải...</span><span id="__uvd_player_size__">Đang ước tính dung lượng...</span><span id="__uvd_player_time__" style="cursor:pointer;">00:00</span>';
...
// ========== HÀM MỞ IFRAME TRONG OVERLAY (THAY THẾ TAB MỚI) ==========
function openIframeOverlay(url) {
  // Tạo overlay chứa iframe
  var overlayIframe = document.createElement('div');
  overlayIframe.style.cssText = 'position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.85); z-index:2147483649; display:flex; flex-direction:column;';
  
  // Header để đóng
  var closeBar = document.createElement('div');
  closeBar.style.cssText = 'padding:10px; background:rgba(0,0,0,0.6); display:flex; justify-content:space-between; color:#fff; font-size:14px; flex-shrink:0;';
  closeBar.innerHTML = '<span>🔍 Đang xem iframe</span><button style="background:none; border:none; color:#fff; font-size:18px; cursor:pointer;">✕</button>';
  overlayIframe.appendChild(closeBar);
  
  // Khung iframe
  var iframeWrap = document.createElement('div');
  iframeWrap.style.cssText = 'flex:1; display:flex; align-items:center; justify-content:center;';
  var iframe = document.createElement('iframe');
  iframe.src = url;
  iframe.style.cssText = 'width:100%; height:100%; border:none; background:#000;';
  iframe.setAttribute('allowfullscreen', '');
  iframeWrap.appendChild(iframe);
  overlayIframe.appendChild(iframeWrap);
  
  document.body.appendChild(overlayIframe);
  
  // Sự kiện đóng
  closeBar.querySelector('button').onclick = function() {
    overlayIframe.remove();
    // Quét lại để bắt link từ iframe (nếu có)
    setTimeout(function() { scan(document, 'iframe-overlay'); buildUI(); }, 500);
  };
  
  // Click ra ngoài cũng đóng (tuỳ chọn)
  overlayIframe.addEventListener('click', function(e) {
    if (e.target === overlayIframe) {
      overlayIframe.remove();
      setTimeout(function() { scan(document, 'iframe-overlay'); buildUI(); }, 500);
    }
  });
  
  toast('Đã mở iframe, đang theo dõi link...');
}
...
function buildStreamCardHTML(item, i) {
  ...
  if (item.type === 'BLOB') {
    ...
    actionsHtml =
      ...
      (item.type === 'IFRAME' ?
        // ===== THAY ĐỔI Ở ĐÂY: MỞ IFRAME TRONG OVERLAY =====
        '<button class="uvd-btn uvd-btn-sm" data-action="iframe" data-url="' + encodeURIComponent(item.url) + '" style="text-align:center;grid-column:1/3;">Mở iframe tại đây</button>' :
        (item.type === 'M3U8' ?
...
function renderStreams(container, arr) {
  ...
  container.onclick = function(e) {
    ...
    if (actionBtn) {
      ...
      // ===== THAY THẾ HÀNH ĐỘNG IFRAME =====
      else if (action === 'iframe') openIframeOverlay(u2);
