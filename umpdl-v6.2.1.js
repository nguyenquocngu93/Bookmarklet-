/**
 * Universal Media Player & Downloader - V6.2.1
...
window.__uvdSafeOpen = function(url) {
  if (__uvdOriginalWindowOpen) return __uvdOriginalWindowOpen(url, '_blank');
...
function installPopupBlock() {
  ...
  ['click','mousedown','pointerdown','auxclick'].forEach(function(ev){
    document.addEventListener(ev, killBlankLinks, true);
  });
}
...
function uninstallPopupBlock() {
  ...
  ['click','mousedown','pointerdown','auxclick'].forEach(function(ev){
    document.removeEventListener(ev, killBlankLinks, true);
  });
}

function blockPopupsDuring(durationMs, fn) { installPopupBlock(); fn(); }
...
function simulateClick(el) {
  try {
    ...
    ['pointerdown','mousedown','pointerup','mouseup','click'].forEach(function(type){
      try { el.dispatchEvent(new MouseEvent(type, {bubbles:true,cancelable:true,view:window,clientX:x,clientY:y})); }
      catch(e) {
...
function autoClickPlayButtons(root, depth, allowVideoPlayFallback) {
  ...
  selectors.forEach(function(sel){
    try { root.querySelectorAll(sel).forEach(function(el){ if(simulateClick(el)) clicked++; }); } catch(e) {}
  });
    ...
    try {
      root.querySelectorAll('video').forEach(function(v){
        if (v.paused) {
          ...
          var p = v.play();
          if (p && p.then) p.then(function(){ setTimeout(function(){ try{v.pause();v.currentTime=0;v.muted=wasMuted;}catch(e){} },600); }).catch(function(){});
        }
      });
    } catch(e) {}
  }
  try { root.querySelectorAll('iframe').forEach(function(f){ try{if(f.contentDocument) clicked+=autoClickPlayButtons(f.contentDocument,depth+1,allowVideoPlayFallback);}catch(e){} }); } catch(e) {}
  return clicked;
}
...
function pauseAllPlayingVideos(root, depth) {
  ...
  var pausedCount = 0;
  try { root.querySelectorAll('video').forEach(function(v){ if(!v.paused){ try{v.pause();pausedCount++;}catch(e){} } }); } catch(e) {}
  if (depth<2) try { root.querySelectorAll('iframe').forEach(function(f){ try{if(f.contentDocument) pausedCount+=pauseAllPlayingVideos(f.contentDocument,depth+1);}catch(e){} }); } catch(e) {}
  return pausedCount;
}
...
function installMonitor() {
  window.fetch = function() {
    var url = arguments[0];
    if (typeof url === 'string') { if (!isAdUrl(url)) findUrls(url, 'fetch:live'); }
    else if (url && url.url) { if (!isAdUrl(url.url)) findUrls(url.url, 'fetch:live'); }
...
function stopMonitor() { window.fetch = originalFetch; XMLHttpRequest.prototype.open = originalXHROpen; uninstallPopupBlock(); }
...
function runAutoClickAndRescan(silent) {
  ...
  blockPopupsDuring(2500, function(){ autoClickPlayButtons(document,0,!silent); });
  setTimeout(function(){
    scan(document,'autoclick-rescan');
    var after = urls.size, found = after - before;
    if (found > 0) {
      toast('▶ Tự động Play: tìm thêm ' + found + ' luồng mới');
      if (document.getElementById('__uvd__')) buildUI();
      setTimeout(function(){ var n = pauseAllPlayingVideos(); if(n) toast('⏸ Đã tạm dừng video gốc'); },800);
    } else if (!silent) {
      toast('Không tìm thấy nút Play hoặc không có link mới');
    }
  }, 1200);
}
    ...
    for (var i=0; i<lines.length; i++) {
      if (lines[i].startsWith('#EXT-X-STREAM-INF')) {
        ...
        if (next && !next.startsWith('#')) {
          var res = (info.match(/RESOLUTION=(\d+x\d+)/)||[])[1]||'unknown';
          var bw = parseInt((info.match(/BANDWIDTH=(\d+)/)||[])[1]||0);
          var codecs = (info.match(/CODECS="([^"]+)"/)||[])[1]||'';
          var label = res==='unknown'? Math.round(bw/1000)+'kbps' : res.split('x')[1]+'p';
          var streamUrl = next;
          if (!streamUrl.startsWith('http')) streamUrl = url.substring(0, url.lastIndexOf('/')+1) + streamUrl;
          qualities.push({label:label, resolution:res, bandwidth:bw, codecs:codecs, url:streamUrl});
        }
      }
    }
...
function makeCommands(url, type, title) {
  var t = title, ext = type.toLowerCase()==='iframe'?'mp4':type.toLowerCase();
  var ref=pageInfo.referer, origin=pageInfo.origin, ua=pageInfo.userAgent;
  return {
...
function addToHistory(url,type){ data.history.unshift({url,type,title:pageInfo.title,host:pageInfo.host,timestamp:Date.now()}); if(data.history.length>50) data.history=data.history.slice(0,50); storage.set(data); }
...
function toggleFavorite(url,type){
  ...
  storage.set(data); return isFavorite(url);
}
...
function addRipple(e){
  var btn=e.currentTarget, ripple=document.createElement('span');
  ripple.className='uvd-ripple'; var rect=btn.getBoundingClientRect(), size=Math.max(rect.width,rect.height);
...
// ========== ORIENTATION LOCK ==========
function lockOrientation(video){ if(!video||!video.videoWidth) return; try{ screen.orientation.lock(video.videoHeight>video.videoWidth?'portrait':'landscape').catch(function(){}); }catch(e){} }
function unlockOrientation(){ try{ screen.orientation.unlock(); }catch(e){} }

// ========== PLAYER STATE ==========
var playerState = {
  overlay:null, mini:null, video:null, hls:null, qualities:[], currentQuality:0,
  speed:1, isMinimized:false, url:'', type:'', resolution:'', bandwidth:0,
  _displayedResolution:'', onFullscreenChange:null
};
...
function showVideoPlayer(url, type) {
  ...
  playerState.url = url; playerState.type = type; playerState._displayedResolution = '';
  pauseAllPlayingVideos();

  var overlay = document.createElement('div');
  overlay.id = '__uvd_player_overlay__';
  overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.85);backdrop-filter:blur(8px);z-index:2147483648;display:flex;flex-direction:column;';
  document.body.appendChild(overlay);
  playerState.overlay = overlay;

  var header = document.createElement('div');
  header.style.cssText = 'padding:10px 16px;background:rgba(20,22,30,0.65);backdrop-filter:blur(20px);display:flex;flex-direction:column;flex-shrink:0;border-bottom:1px solid rgba(255,255,255,0.2);';
  ...
  minVideoBtn.className = 'uvd-btn uvd-btn-sm'; minVideoBtn.textContent = '⛶'; minVideoBtn.onclick = minimizePlayer;
  btnGroup.appendChild(minVideoBtn);
  var closeBtn = document.createElement('button');
  closeBtn.id = '__uvd_player_close__'; closeBtn.className = 'uvd-btn uvd-btn-sm'; closeBtn.textContent = '✕';
  ...
  var qualityBtn = document.createElement('button'); qualityBtn.textContent = 'Chất lượng';
  qualityBtn.className = 'uvd-btn uvd-btn-sm'; qualityBtn.onclick = function(){ if(playerState.qualities.length) showQualitySubMenu(); else toast('Không có chất lượng để chọn'); };
  toolbar.appendChild(qualityBtn);
  var speedBtn = document.createElement('button'); speedBtn.textContent = 'Tốc độ';
  speedBtn.className = 'uvd-btn uvd-btn-sm'; speedBtn.onclick = showSpeedSubMenu;
  toolbar.appendChild(speedBtn);
  var fsBtn = document.createElement('button'); fsBtn.textContent = 'Toàn màn hình';
  fsBtn.className = 'uvd-btn uvd-btn-sm'; fsBtn.onclick = function(){ var w=document.getElementById('__uvd_video_wrapper__'); (w.requestFullscreen||w.webkitRequestFullscreen||function(){}).call(w); };
  toolbar.appendChild(fsBtn);
  if (type === 'MP4' || type === 'MKV' || type === 'WEBM') {
    var downloadBtn = document.createElement('button'); downloadBtn.textContent = 'Tải xuống';
    downloadBtn.className = 'uvd-btn uvd-btn-sm'; downloadBtn.onclick = function(){ var a=document.createElement('a'); a.href=url; a.download=pageInfo.title+'.'+type.toLowerCase(); a.click(); toast('Đang tải xuống...'); };
    toolbar.appendChild(downloadBtn);
  }
  if (type === 'M3U8' || type === 'MPD') {
    var shareBtn = document.createElement('button'); shareBtn.textContent = 'Chia sẻ';
    shareBtn.className = 'uvd-btn uvd-btn-sm'; shareBtn.onclick = function(){ shareUrl(url); };
  ...
  videoWrapper.style.cssText = 'flex:1;display:flex;align-items:center;justify-content:center;background:#000;';
  var video = document.createElement('video');
  video.id = '__uvd_player_video__'; video.setAttribute('controls',''); video.setAttribute('playsinline',''); video.setAttribute('crossorigin','anonymous');
  ...
  footer.style.cssText = 'padding:8px 16px;background:rgba(0,0,0,0.7);font-size:12px;color:#aaa;display:flex;justify-content:space-between;';
  footer.innerHTML = '<span id="__uvd_player_status__">Đang tải...</span><span id="__uvd_player_size__" style="color:#8ab4ff;">Đang ước tính...</span><span id="__uvd_player_time__"></span>';
...
function minimizePlayer() {
  ...
  if (video.videoWidth) { try { ctx.drawImage(video, 0, 0, 160, 90); } catch(e) {} }
...
function closePlayer() {
  if (playerState.overlay) {
    ...
    if (playerState.video) { playerState.video.pause(); playerState.video.src = ''; }
...
.uvd-glass-panel{background:var(--glass);backdrop-filter:blur(28px);border:1px solid var(--border);border-radius:var(--radius-lg);box-shadow:0 20px 50px rgba(0,0,0,0.8),inset 0 1px 0 rgba(255,255,255,0.08);color:var(--text);font-family:-apple-system,BlinkMacSystemFont,sans-serif;font-size:var(--fs-base);padding:16px;position:relative;overflow:hidden;max-width:1000px;margin:auto}
.uvd-glass-panel::before{content:'';position:absolute;top:0;left:8%;right:8%;height:1px;background:linear-gradient(90deg,transparent,rgba(109,140,255,0.6),rgba(185,139,255,0.6),transparent);opacity:0.7}
...
.uvd-btn{background:var(--glass-hi);border:1px solid var(--border);color:var(--text);padding:9px 16px;border-radius:var(--radius-md);font-weight:600;font-size:var(--fs-base);cursor:pointer;transition:all 0.2s;backdrop-filter:blur(10px);text-align:center;position:relative;overflow:hidden;display:inline-block;box-shadow:0 3px 10px rgba(0,0,0,0.4),inset 0 1px 0 rgba(255,255,255,0.08);line-height:1.3}
.uvd-btn:hover{background:rgba(255,255,255,0.10);border-color:rgba(255,255,255,0.20);transform:translateY(-1px);box-shadow:0 5px 14px rgba(0,0,0,0.5),inset 0 1px 0 rgba(255,255,255,0.12)}
.uvd-btn:active{transform:scale(0.96)}
.uvd-btn-sm{padding:7px 12px;font-size:var(--fs-sm);border-radius:var(--radius-sm)}
.uvd-btn-icon{background:var(--glass-hi);border:1px solid var(--border);color:var(--text);width:34px;height:34px;border-radius:var(--radius-sm);cursor:pointer;display:inline-flex;align-items:center;justify-content:center;transition:all 0.2s;backdrop-filter:blur(10px);position:relative;overflow:hidden;box-shadow:0 3px 8px rgba(0,0,0,0.35),inset 0 1px 0 rgba(255,255,255,0.08)}
...
.uvd-card{background:var(--card-bg);border:1px solid var(--border);border-radius:var(--radius-md);padding:14px;margin-bottom:10px;font-size:var(--fs-base);backdrop-filter:blur(6px);box-shadow:inset 0 1px 0 rgba(255,255,255,0.04);transition:transform 0.3s,box-shadow 0.3s,background 0.3s;animation:uvdCardEnter 0.4s ease both}
.uvd-card:hover{transform:translateY(-3px);box-shadow:0 14px 30px rgba(0,0,0,0.7),inset 0 0 0 1px rgba(109,140,255,0.30),0 0 24px rgba(109,140,255,0.08);background:rgba(255,255,255,0.05);border-color:rgba(109,140,255,0.25)}
.uvd-type-badge{display:inline-block;padding:4px 12px;border-radius:var(--radius-sm);font-size:var(--fs-xs);font-weight:700;background:linear-gradient(135deg,rgba(109,140,255,0.22),rgba(185,139,255,0.18));color:var(--accent);border:1px solid rgba(109,140,255,0.28)}
.uvd-url-box{background:rgba(0,0,0,0.5);border-radius:var(--radius-sm);padding:12px;font-family:monospace;font-size:var(--fs-sm);word-break:break-all;color:var(--text2);max-height:100px;overflow-y:auto;border:1px solid rgba(255,255,255,0.04)}
...
.uvd-tabbar{display:flex;gap:2px;padding:4px;background:rgba(255,255,255,0.04);border:1px solid var(--border);border-radius:999px;margin-bottom:10px;flex-shrink:0;position:relative}
.uvd-tab-indicator{position:absolute;top:4px;bottom:4px;left:4px;border-radius:999px;background:var(--grad-liquid);z-index:0;box-shadow:0 3px 12px rgba(109,140,255,0.45);transition:transform 0.4s,width 0.4s}
.uvd-tab{position:relative;z-index:1;flex:1;background:transparent;border:none;color:var(--text2);font-weight:600;font-size:var(--fs-sm);padding:9px 6px;border-radius:999px;cursor:pointer;transition:color 0.25s;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
...
.uvd-glass-panel.uvd-minimized #__uvd_tabbar,
.uvd-glass-panel.uvd-minimized #__uvd_info_row,
.uvd-glass-panel.uvd-minimized #__uvd_content_wrapper,
.uvd-glass-panel.uvd-minimized #__uvd_footer,
.uvd-glass-panel.uvd-minimized #__uvd_author { display: none !important; }
...
function minimizeScriptPanel() {
  ...
  if (btn) { btn.innerHTML = '🗖'; btn.title = 'Mở rộng Script'; btn.onclick = restoreScriptPanel; }
}
function restoreScriptPanel() {
  ...
  var btn = document.getElementById('__uvd_minimize_script__');
  if (btn) { btn.innerHTML = '🗕'; btn.title = 'Thu nhỏ Script'; btn.onclick = minimizeScriptPanel; }
}
...
function buildUI() {
  ...
  panel.style.cssText = 'position:fixed;top:15px;left:15px;right:15px;bottom:15px;z-index:2147483647;animation:uvdScaleIn 0.4s;';

  var liquidBg = document.createElement('div'); liquidBg.className = 'uvd-liquid-bg'; panel.appendChild(liquidBg);
  var content = document.createElement('div'); content.className = 'uvd-panel-content'; panel.appendChild(content);
  ...
  header.innerHTML = 
    ...
    '<div style="display:flex;gap:6px;">' +
      '<button class="uvd-btn-icon" id="__uvd_autoplay__" title="Tự động Play">▶</button>' +
  ...
  var tabbar = document.createElement('div'); tabbar.className = 'uvd-tabbar'; tabbar.id = '__uvd_tabbar';
  var indicator = document.createElement('div'); indicator.className = 'uvd-tab-indicator'; indicator.id = '__uvd_tab_indicator__';
  ...
  tabList.forEach(function(t) {
    var b = document.createElement('button'); b.className = 'uvd-tab'; b.dataset.tab = t.id; b.textContent = t.text; tabbar.appendChild(b);
  });
  content.appendChild(tabbar);

  function moveIndicatorTo(btn) { if(btn){ indicator.style.width=btn.offsetWidth+'px'; indicator.style.transform='translateX('+btn.offsetLeft+'px)'; } }

  var info = document.createElement('div'); info.id = '__uvd_info_row';
  ...
  var contentWrapper = document.createElement('div'); contentWrapper.id = '__uvd_content_wrapper';
  contentWrapper.className = 'uvd-scroll'; contentWrapper.style.cssText = 'flex:1;overflow:hidden;position:relative;min-height:0;';
  var streamList = document.createElement('div'); streamList.id = '__uvd_stream_list__';
  streamList.className = 'uvd-scroll'; streamList.style.cssText = 'overflow-y:auto;height:100%;padding-right:4px;';
  contentWrapper.appendChild(streamList);
  content.appendChild(contentWrapper);

  var footer = document.createElement('div'); footer.id = '__uvd_footer';
  ...
  ['TXT','JSON','M3U','CSV'].forEach(function(f) {
    var btn = document.createElement('button'); btn.className = 'uvd-btn uvd-btn-sm'; btn.textContent = f;
    btn.style.flex = '1 0 auto'; btn.onclick = function() { exportData(f.toLowerCase()); };
    footer.appendChild(btn);
  });
  content.appendChild(footer);

  var author = document.createElement('div'); author.id = '__uvd_author';
  ...
  document.querySelectorAll('.uvd-btn, .uvd-btn-icon, .uvd-tab').forEach(function(btn){ btn.addEventListener('click', addRipple); });

  var currentTab = 'streams';
  function renderTab(tabId) {
    currentTab = tabId;
    document.querySelectorAll('[data-tab]').forEach(function(t) {
      if (t.dataset.tab === tabId) { t.classList.add('uvd-tab-active'); moveIndicatorTo(t); }
      else t.classList.remove('uvd-tab-active');
    });
    streamList.style.display = 'block'; streamList.innerHTML = '';
  ...
  document.querySelectorAll('[data-tab]').forEach(function(t){ t.onclick = function(){ renderTab(this.dataset.tab); }; });
  renderTab('streams');
  window.addEventListener('resize', function(){ moveIndicatorTo(document.querySelector('.uvd-tab.uvd-tab-active')); });
  ...
  document.getElementById('__uvd_autoplay__').onclick = function(){
    ...
    toast(n>0 ? 'Đã thử bấm Play ('+n+' nút)' : 'Không tìm thấy nút Play, thử đặt selector riêng');
  ...
  document.getElementById('__uvd_referer__').onclick = function(){
    ...
    if (newRef) { pageInfo.referer = newRef; this.textContent = newRef;
      ...
      storage.set(data); toast('Đã lưu referer cho '+pageInfo.host); }
  };
  document.getElementById('__uvd_playsel__').onclick = function(){
    ...
    if (newSel !== null) {
      ...
      storage.set(data); this.textContent = newSel || '(chưa đặt · bấm để thêm)';
      if (newSel) { toast('Đã lưu selector cho '+pageInfo.host); autoClickPlayButtons(document,0,false); setTimeout(function(){ buildUI(); },1000); }
      else toast('Đã xóa selector riêng');
    }
  };

  window.__uvd_showPlayer = function(url, type){ showVideoPlayer(url, type); };
}

// ========== RENDER FUNCTIONS ==========
function renderStreams(container, arr) {
  if (!arr.length) { container.innerHTML = '<div style="text-align:center;padding:40px;color:var(--text2);">Không phát hiện stream nào.</div>'; return; }
  arr.forEach(function(item, i) {
    var card = document.createElement('div'); card.className = 'uvd-card'; var fav = isFavorite(item.url);
    card.innerHTML = 
  ...
  container.querySelectorAll('.uvd-btn, .uvd-fav-btn').forEach(function(b){ b.addEventListener('click', addRipple); });
  container.querySelectorAll('.uvd-fav-btn').forEach(function(b){
    b.onclick = function(){
      var u = decodeURIComponent(this.dataset.url); var isFav = toggleFavorite(u, this.dataset.type);
      this.textContent = isFav ? '★' : '☆'; this.style.color = isFav ? 'var(--gold)' : 'var(--text3)';
    };
  });
  container.querySelectorAll('.uvd-btn[data-action]').forEach(function(b){
    b.onclick = function(){
      var u = decodeURIComponent(this.dataset.url); var action = this.dataset.action; var t = this.dataset.type;
      ...
      else if (action === 'iframe') { window.__uvdSafeOpen(u); }
    };
  });
}
...
function showCommandPicker(url, type) {
  ...
  var overlay = document.createElement('div'); overlay.className = 'uvd-overlay';
  var html = '<div class="uvd-glass-panel" style="max-width:600px;margin:auto;">';
  html += '<div style="font-weight:700;margin-bottom:12px;">Chọn lệnh tải</div><div style="overflow-y:auto;max-height:60vh;">';
  Object.keys(cmds).forEach(function(k) {
    var c = cmds[k];
    html += '<div class="uvd-card"><div style="font-weight:600;color:var(--accent);">' + c.label + '</div><div class="uvd-url-box">' + c.cmd + '</div><button class="uvd-btn uvd-btn-sm cmd-select" data-cmd="' + encodeURIComponent(c.cmd) + '" style="width:100%;">Chỉnh sửa & Copy</button></div>';
  });
  ...
  overlay.querySelectorAll('.cmd-select').forEach(function(b){
    b.onclick = function(){ overlay.remove(); showEditor(decodeURIComponent(this.dataset.cmd)); };
  });
}

function showEditor(text) {
  var overlay = document.createElement('div'); overlay.className = 'uvd-overlay';
  overlay.innerHTML = '<div class="uvd-glass-panel" style="max-width:600px;margin:auto;"><div style="font-weight:700;margin-bottom:8px;">Chỉnh sửa lệnh</div><textarea style="width:100%;height:120px;background:rgba(0,0,0,0.5);border:1px solid var(--border);border-radius:10px;color:var(--text);padding:12px;font-family:monospace;">' + text + '</textarea><div class="uvd-grid-2" style="margin-top:12px;"><button class="uvd-btn uvd-btn-sm" id="__uvd_ed_copy__">Sao chép</button><button class="uvd-btn uvd-btn-sm" id="__uvd_ed_share__" style="background:rgba(139,92,246,0.3);">Chia sẻ</button></div><button class="uvd-btn uvd-btn-sm close-editor" style="width:100%;margin-top:8px;background:var(--danger);">Đóng</button></div>';
  document.body.appendChild(overlay);
  overlay.querySelectorAll('.uvd-btn').forEach(function(b){ b.addEventListener('click', addRipple); });
  document.getElementById('__uvd_ed_copy__').onclick = function(){ copy(overlay.querySelector('textarea').value); overlay.remove(); toast('Đã sao chép!'); };
  document.getElementById('__uvd_ed_share__').onclick = function(){ shareUrl(overlay.querySelector('textarea').value); overlay.remove(); };
...
function showQualityPicker(url) {
  ...
  parseM3U8Master(url, function(qualities) {
    if (!qualities) {
      overlay.innerHTML = '<div class="uvd-glass-panel" style="max-width:600px;margin:auto;text-align:center;"><div style="color:var(--danger);">Không phải Master Playlist</div><button class="uvd-btn uvd-btn-sm close-overlay-btn" style="margin-top:12px;background:var(--danger);width:100%;">Đóng</button></div>';
      overlay.querySelector('.close-overlay-btn').onclick = function(){ overlay.remove(); };
      return;
    }
    var html = '<div class="uvd-glass-panel" style="max-width:600px;margin:auto;"><div style="font-weight:700;margin-bottom:12px;">Chọn chất lượng ('+qualities.length+')</div><div style="overflow-y:auto;max-height:60vh;">';
    qualities.forEach(function(q) {
      html += '<div class="uvd-card"><b>'+q.label+'</b> <span style="color:var(--text3);">'+Math.round(q.bandwidth/1000)+'kbps</span><div class="uvd-grid-3" style="margin-top:8px;"><button class="uvd-btn uvd-btn-sm q-act" data-url="'+encodeURIComponent(q.url)+'" data-action="share" style="background:rgba(139,92,246,0.2);">Chia sẻ</button><button class="uvd-btn uvd-btn-sm q-act" data-url="'+encodeURIComponent(q.url)+'" data-action="play" style="background:rgba(109,140,255,0.25);">Xem</button><button class="uvd-btn uvd-btn-sm q-act" data-url="'+encodeURIComponent(q.url)+'" data-action="cmd">Lệnh</button></div></div>';
    });
    html += '</div><button class="uvd-btn uvd-btn-sm close-overlay-btn" style="width:100%;margin-top:10px;background:var(--danger);">Đóng</button></div>';
    ...
    overlay.querySelectorAll('.q-act').forEach(function(b){
      b.onclick = function(){
        var u = decodeURIComponent(this.dataset.url); var action = this.dataset.action;
        overlay.remove();
        if (action === 'share') shareUrl(u);
...
function renderFavorites(container) {
  if (!data.favorites.length) { container.innerHTML = '<div style="text-align:center;padding:30px;color:var(--text2);">Chưa có yêu thích.</div>'; return; }
  data.favorites.forEach(function(f, i) {
    var card = document.createElement('div'); card.className = 'uvd-card';
    card.innerHTML = 
      ...
      '<div style="margin:4px 0;">' + f.title + '</div><div class="uvd-url-box">' + f.url + '</div>' +
  ...
  container.querySelectorAll('.fav-act').forEach(function(b){
    b.onclick = function(){ var u = decodeURIComponent(this.dataset.url); if(this.dataset.action==='share') shareUrl(u); else { copy(u); toast('Đã sao chép!'); } };
  });
  container.querySelectorAll('.fav-del').forEach(function(b){
    b.onclick = function(){ data.favorites.splice(parseInt(this.dataset.idx),1); storage.set(data); renderFavorites(container); toast('Đã xóa'); };
  });
}

function renderHistory(container) {
  var hist = data.history || [];
  if (!hist.length) { container.innerHTML = '<div style="text-align:center;padding:30px;color:var(--text2);">Chưa có lịch sử.</div>'; return; }
  ...
  document.getElementById('__uvd_clear_hist__').onclick = function(){ if(confirm('Xóa toàn bộ lịch sử?')){ data.history = []; storage.set(data); renderHistory(container); } };
  hist.forEach(function(h) {
    var card = document.createElement('div'); card.className = 'uvd-card';
    card.innerHTML = '<div style="display:flex;justify-content:space-between;"><b style="color:var(--accent);">' + h.type + '</b><span style="font-size:11px;color:var(--text3);">' + new Date(h.timestamp).toLocaleString() + '</span></div><div>' + h.title + '</div><div class="uvd-url-box">' + h.url + '</div>';
    container.appendChild(card);
  });
}
...
function renderSettings(container) {
  ...
  container.innerHTML =
    '<div class="uvd-profile-card"><div class="uvd-profile-avatar">NQ</div><div class="uvd-profile-info"><div class="uvd-profile-name">nguyenquocngu91</div><div class="uvd-profile-role">Bookmarklet Developer · Universal Media Tools</div><div class="uvd-profile-tags"><span class="uvd-tag">UMP DL v' + VERSION + '</span><span class="uvd-tag">Vanilla JS</span><span class="uvd-tag">HLS · M3U8</span><span class="uvd-tag">Adblock</span></div></div></div>' +
    '<div class="uvd-profile-stats" style="grid-template-columns:repeat(4,1fr);"><div class="uvd-stat"><div class="uvd-stat-num">' + totalStreams + '</div><div class="uvd-stat-label">Streams</div></div><div class="uvd-stat"><div class="uvd-stat-num">' + data.favorites.length + '</div><div class="uvd-stat-label">Yêu thích</div></div><div class="uvd-stat"><div class="uvd-stat-num">' + (data.history||[]).length + '</div><div class="uvd-stat-label">Lịch sử</div></div><div class="uvd-stat"><div class="uvd-stat-num" style="color:#ff5d72;">' + __uvdBlockedCount + '</div><div class="uvd-stat-label">Đã chặn popup</div></div></div>' +
    '<div class="uvd-card"><div style="font-weight:600;margin-bottom:8px;">🛡️ Lọc quảng cáo (Filterlist)</div><div style="font-size:12px;color:var(--text2);margin-bottom:8px;">Nhập mỗi dòng một từ khóa hoặc domain (vd: <code>doubleclick.net</code>). Hỗ trợ regex nếu bắt đầu bằng <code>regex:</code>. Các URL chứa pattern sẽ bị bỏ qua.</div><textarea id="__uvd_filter_text__" style="width:100%;height:80px;background:rgba(0,0,0,0.5);border:1px solid var(--border);border-radius:10px;color:var(--text);padding:12px;font-size:12px;">' + (data.filterlist||[]).join('\n') + '</textarea><div class="uvd-grid-2" style="margin-top:8px;"><button class="uvd-btn uvd-btn-sm" id="__uvd_save_filter__">💾 Lưu</button><button class="uvd-btn uvd-btn-sm" id="__uvd_import_filter__">📂 Import file</button></div><div style="margin-top:6px;font-size:11px;color:var(--text3);">Đã chặn <span id="__uvd_blocked_ads__">' + __uvdAdBlockedCount + '</span> URL quảng cáo trong phiên này.</div></div>' +
    '<div class="uvd-card"><div style="font-weight:600;margin-bottom:8px;">Sao lưu & Khôi phục</div><button class="uvd-btn uvd-btn-sm" id="__uvd_backup__" style="width:100%;margin-bottom:6px;">Xuất dữ liệu</button><button class="uvd-btn uvd-btn-sm" id="__uvd_restore__" style="width:100%;margin-bottom:6px;">Nhập dữ liệu</button><button class="uvd-btn uvd-btn-sm" id="__uvd_reset__" style="width:100%;background:var(--danger);">Đặt lại tất cả</button></div>' +
    '<div class="uvd-section-title"><span class="uvd-section-num">1</span> Cài đặt Bookmarklet</div><div class="uvd-card uvd-timeline-card"><div style="font-size:13px;color:var(--text2);line-height:1.7;"><strong style="color:var(--text);">Cách tạo bookmarklet trên Chrome Android:</strong><br>➊ Mở một trang web bất kỳ, bấm vào biểu tượng ⭐ (Bookmark) trên thanh địa chỉ.<br>➋ Chọn <strong>"Chỉnh sửa"</strong> (hoặc "Edit").<br>➌ <strong>Đặt tên</strong> dễ nhớ, ví dụ: <code>UMP DL</code>.<br>➍ <strong>Xóa toàn bộ địa chỉ</strong> trong ô URL, sau đó dán đoạn code sau vào:<br><textarea readonly style="width:100%;height:60px;background:rgba(0,0,0,0.5);border:1px solid var(--border);border-radius:10px;color:var(--text);padding:8px;font-size:10px;margin:6px 0;">' + bookmarkletCode + '</textarea>➎ Bấm <strong>Lưu</strong> (Save).<br><span style="color:var(--accent);">💡 Từ lần sau, bạn chỉ cần gõ tên bookmark (UMP DL) vào thanh địa chỉ rồi chọn nó để kích hoạt.</span><br><span style="color:var(--text3);">Script luôn tự động cập nhật phiên bản mới nhất.</span></div></div>' +
    '<div class="uvd-section-title"><span class="uvd-section-num">2</span> Sử dụng</div><div class="uvd-card uvd-timeline-card"><div style="font-size:13px;color:var(--text2);line-height:1.7;">• Mở trang web có video<br>• Gõ tên bookmark (vd: <code>UMP DL</code>) vào thanh địa chỉ và chọn nó<br>• Chọn stream và bấm <strong style="color:var(--accent);">Xem</strong> để mở player overlay<br>• Trong player: thanh công cụ gồm <strong style="color:var(--accent);">Chất lượng</strong>, <strong style="color:var(--accent);">Tốc độ</strong>, <strong style="color:var(--accent);">Toàn màn hình</strong><br>• Nút <strong style="color:var(--accent);">⛶</strong> cạnh nút đóng để thu nhỏ video<br>• Nút <strong style="color:var(--accent);">▶</strong> trên header: tự động bấm giúp các nút Play ẩn (eporner, JAV...) để link stream lộ ra. Nếu không ăn, đặt CSS selector riêng ở dòng "Play selector" bên dưới tên file.</div></div>' +
    '<div class="uvd-section-title"><span class="uvd-section-num">3</span> Tải video với yt-dlp và Termux</div><div class="uvd-card uvd-timeline-card"><div style="font-size:13px;color:var(--text2);line-height:1.7;"><strong style="color:var(--text);">Cài đặt yt-dlp trên Termux:</strong><br><code class="uvd-inline-code">pkg update && pkg upgrade -y</code><br><code class="uvd-inline-code">pkg install python ffmpeg -y</code><br><code class="uvd-inline-code">pip install yt-dlp</code><br><br><strong style="color:var(--text);">Sau khi cài đặt, copy lệnh từ tab "Lệnh tải":</strong><br>• Mở tab <strong style="color:var(--accent);">Streams</strong>, chọn stream cần tải<br>• Bấm <strong style="color:var(--accent);">Lệnh tải</strong> → chọn lệnh phù hợp<br>• Copy lệnh, mở Termux, dán vào và bấm Enter để tải.<br><br><strong style="color:var(--text);">Lưu ý:</strong> Nhớ cấp quyền lưu file cho Termux (Android 11+):<br><code class="uvd-inline-code">termux-setup-storage</code></div></div>' +
    '<div class="uvd-profile-footer">© ' + new Date().getFullYear() + ' nguyenquocngu91 · UMP DL v' + VERSION + ' · Made for Chrome Android</div>';
  container.querySelectorAll('.uvd-btn').forEach(function(b){ b.addEventListener('click', addRipple); });
  document.getElementById('__uvd_backup__').onclick = function(){ var blob = new Blob([JSON.stringify(data)],{type:'application/json'}); var a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='uvd_backup.json'; a.click(); };
  document.getElementById('__uvd_restore__').onclick = function(){
    var inp = document.createElement('input'); inp.type='file'; inp.accept='.json';
    inp.onchange = function(e){
      var reader = new FileReader();
      reader.onload = function(ev){ try { data = Object.assign(data, JSON.parse(ev.target.result)); storage.set(data); toast('Đã nhập!'); buildUI(); } catch(ex){ toast('File không hợp lệ','var(--danger)'); } };
      reader.readAsText(e.target.files[0]);
    };
    inp.click();
  };
  document.getElementById('__uvd_reset__').onclick = function(){ if(confirm('Xóa toàn bộ dữ liệu?')){ localStorage.removeItem(STORAGE_KEY); data = {favorites:[],siteProfiles:{},history:[],filterlist:[]}; buildUI(); } };
  document.getElementById('__uvd_save_filter__').onclick = function(){
    ...
    storage.set(data); toast('Đã lưu filterlist ('+data.filterlist.length+' mục). Tải lại trang để áp dụng triệt để.');
  };
...
// ========== KÍCH HOẠT ==========
buildUI();
var autoRefresh = setInterval(function(){ if(!document.getElementById('__uvd__')){ clearInterval(autoRefresh); stopMonitor(); } }, 2000);
console.log('V6.2.1 UMP DL - Minimize script tại chỗ, chỉ giữ header');
toast('V6.2.1 sẵn sàng!');
