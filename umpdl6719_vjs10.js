...
function showVideoPlayer(url, type) {
  ...
  overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.96);z-index:2147483647;display:flex;flex-direction:column;align-items:center;justify-content:center;animation:uvdFadeIn 0.3s ease;';
  ...
  // CARD — bo góc 2 góc trên, nền theo màu theme của script (thay vì đen trơn
  // cả màn hình). Cao theo NỘI DUNG (header + video co theo tỉ lệ thật), không
  // còn ép flex:1 chiếm hết chiều cao màn hình nữa — tự loại bỏ khoảng trống
  // thừa quanh video từ gốc, thay vì phải bù bằng hiệu ứng nền động như trước.
  var card = document.createElement('div');
  card.className = 'uvd-player-card';
  overlay.appendChild(card);

  // HEADER — chỉ còn: tiêu đề + nút ⋮ (Chất lượng/Phụ đề) + nút đóng.
  // Dòng 2 gộp loại/độ phân giải/dung lượng (trước nằm ở footer, đã xoá footer).
  var header = document.createElement('div');
  header.id = '__uvd_player_header__';
  header.style.cssText = 'padding:10px 16px;background:rgba(14,16,22,0.5);display:flex;flex-direction:column;flex-shrink:0;transition: opacity 0.3s ease;position:relative;z-index:6;';
  ...
  titleInfo.innerHTML = '<div style="font-weight:600;font-size:14px;color:#fff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">▶ ' + escapeHtml(pageInfo.title) + '</div><div style="font-size:11px;color:var(--text2);margin-top:2px;">' + escapeHtml(type) + '</div>';
  titleRow.appendChild(titleInfo);

  var btnGroup = document.createElement('div');
  btnGroup.style.cssText = 'display:flex;gap:6px;flex-shrink:0;position:relative;';
  var menuBtn = document.createElement('button');
  menuBtn.className = 'uvd-icon-btn';
  menuBtn.textContent = '⋮';
  ...
  videoWrapper.style.cssText = 'display:flex;align-items:center;justify-content:center;background:#000;position:relative;overflow:hidden;width:100%;max-height:75dvh;';
