// ==UserScript==
// @name         DevLib CDN Player - Glass UI v3 Full
// @namespace    http://tampermonkey.net/
// @version      3.0
// @description  Glass player with images tab, preview, ad filter, PiP, playlist, subtitle, cast
// @author       You
// @match        *://*/*
// @grant        GM_getValue
// @grant        GM_setValue
...
(function () {
    ...
    let settings = loadData(STORAGE_KEYS.SETTINGS) || { theme: 'dark', autoplay: false };
    ...
    function scan(doc, src) {
        try {
            ...
            doc.querySelectorAll('img').forEach(img => {
                if (img.src && !isAdOrTracker(img.src)) {
                    addImageUrl(img.src, 'IMG', src+':img');
                }
                if (img.srcset) {
    ...
    // Hook createElement for dynamic images
    ...
    const CSS = `
        ...
        * { box-sizing: border-box; }
        ...
        .dlp-img-item:hover .dlp-img-thumb {
        ...
        .dlp-action-btn .dlp-action-icon {
            font-size: 20px;
            filter: drop-shadow(0 2px 4px rgba(0,0,0,0.4));
        }
        .dlp-action-btn .dlp-action-label {
            font-size: 10px;
            letter-spacing: -0.2px;
            opacity: 0.85;
        }
    ...
    function initUI() {
        ...
        document.body.insertAdjacentHTML('beforeend', `
            ...
            <div id="dlp-prev">
                ...
                <div id="dlp-vid-wrap">
                    <video id="dlp-vid" controls playsinline webkit-playsinline crossorigin="anonymous"></video>
                </div>
    ...
    function initLogic() {
        ...
        function loadSubtitle(url) {
            ...
            // Enable subtitles
            vid.textTracks[0] && (vid.textTracks[0].mode = 'showing');
        ...
        function openPrev(item) {
            ...
            $('dlp-p-acts').innerHTML = `
                <button class="dlp-action-btn copy" id="dlp-pc-cp">
                    <span class="dlp-action-icon">📋</span>
                    <span class="dlp-action-label">Copy</span>
                </button>
                <button class="dlp-action-btn fs" id="dlp-pc-fs">
                    <span class="dlp-action-icon">⛶</span>
                    <span class="dlp-action-label">Full</span>
                </button>
                <button class="dlp-action-btn share" id="dlp-pc-share">
                    <span class="dlp-action-icon">📤</span>
                    <span class="dlp-action-label">Share</span>
                </button>
                <button class="dlp-action-btn dl" id="dlp-pc-dl">
                    <span class="dlp-action-icon">💻</span>
                    <span class="dlp-action-label">DL</span>
                </button>
                <button class="dlp-action-btn pip" id="dlp-pc-pip">
                    <span class="dlp-action-icon">🖼</span>
                    <span class="dlp-action-label">PiP</span>
                </button>
                <button class="dlp-action-btn bm" id="dlp-pc-bm">
                    <span class="dlp-action-icon">⭐</span>
                    <span class="dlp-action-label">Save</span>
                </button>
                <button class="dlp-action-btn pl" id="dlp-pc-pl">
                    <span class="dlp-action-icon">🎵</span>
                    <span class="dlp-action-label">List</span>
                </button>
                <button class="dlp-action-btn sub" id="dlp-pc-sub">
                    <span class="dlp-action-icon">📝</span>
                    <span class="dlp-action-label">Sub</span>
                </button>
                <button class="dlp-action-btn cast" id="dlp-pc-cast">
                    <span class="dlp-action-icon">📺</span>
                    <span class="dlp-action-label">Cast</span>
                </button>
                <button class="dlp-action-btn rot" id="dlp-pc-rot">
                    <span class="dlp-action-icon">⟳</span>
                    <span class="dlp-action-label">Rot</span>
                </button>
                <button class="dlp-action-btn zm" id="dlp-pc-zm">
                    <span class="dlp-action-icon">🔍</span>
                    <span class="dlp-action-label">${zoom}x</span>
                </button>
            `;
            ...
            $('dlp-pc-share').onclick = () => {
                if (navigator.share) {
                    ...
                    window.open(`intent://${cur.url.replace(/^https?:\/\//,'')}#Intent;scheme=https;action=android.intent.action.SEND;type=text/plain;end`);
                }
            };
            ...
            $('dlp-pc-pip').onclick = () => {
                if (document.pictureInPictureElement) {
                    ...
                    $('dlp-vid').requestPictureInPicture().catch(() => toast('PiP not supported'));
                }
            };
            $('dlp-pc-bm').onclick = () => {
        ...
        $('dlp-btn-opt').onclick = (e) => {
            ...
            $('dlp-m-share').onclick = () => { d.classList.remove('on'); 
                navigator.share && navigator.share({url: cur.url}); };
        ...
        $('dlp-bd').onclick = () => {
            ['dlp-cmd','dlp-panel','dlp-prev','dlp-drop'].forEach(id => {
                $(id).classList.remove('on');
                if (id === 'dlp-drop') $(id).style.display = 'none';
            });
            $('dlp-bd').classList.remove('on');
            $('dlp-vid').pause();
            if (screen.orientation && screen.orientation.unlock) screen.orientation.unlock();
        ...
        document.addEventListener('keydown', (e) => {
            ...
            switch(e.key) {
                ...
                case 'ArrowRight': $('dlp-vid').currentTime += 10; break;
                case 'ArrowLeft': $('dlp-vid').currentTime -= 10; break;
                case ' ': e.preventDefault(); $('dlp-vid').paused ? $('dlp-vid').play() : $('dlp-vid').pause(); break;
                case 'm': $('dlp-vid').muted = !$('dlp-vid').muted; break;
            }
        });
    }
