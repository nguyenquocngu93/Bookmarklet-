...
function showVideoPlayer(url, type) {
  ...
  function __uvdApplyPlayerLayout() {
    ...
    if (fs) {
     ...
     if (isPortrait) {
    videoWrapper.style.padding = '0';
    // ... tính boxW, boxH như cũ ...
    playerEl.style.width = boxW + 'px';
    playerEl.style.height = boxH + 'px';

    playerEl.style.borderRadius = '0'; // hoặc '16px' nếu bạn thích
    video.style.borderRadius = playerEl.style.borderRadius;
    ...
    video.style.objectFit = 'cover'; // hoặc contain nếu muốn thấy hết
} else {
    videoWrapper.style.padding = '0';
    playerEl.style.position = 'relative';
    playerEl.style.margin = 'auto';
    ...
    // ✅ Bo góc cho cả container và video – khớp nhau
    playerEl.style.borderRadius = '16px';
    video.style.borderRadius = '16px';

    // ✅ Giữ bóng đổ trên container (để bóng bao quanh toàn bộ khung video hiển thị)
    playerEl.style.boxShadow = FLOAT_SHADOW;

    // ✅ Xoá bỏ nền đen của container – thay bằng trong suốt
    videoWrapper.style.background = 'transparent';
    playerEl.style.background = 'transparent';

    // ✅ Cắt góc thừa do border-radius
    playerEl.style.overflow = 'hidden';
