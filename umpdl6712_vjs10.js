/**
 * Universal Media Player & Downloader - V6.7.12 PRO
 * - MỚI: đổi toàn bộ theme sang HỒNG SÁNG (--accent #ff4fa0, --accent2 #ff8fd6),
 *   áp dụng nhất quán cho mọi glow/shadow/gradient trong toàn bộ giao diện
 *   (không chỉ 1-2 chỗ lẻ tẻ).
 * - FIX: chữ "Cài đặt" trong overlay bị đen khó đọc — do `.uvd-settings-sheet`
 *   chưa từng được set `color` (chỉ `.uvd-glass-panel` có), nên ăn theo màu
 *   chữ mặc định (đen) của trang web. Đã đưa `color`+`font-family` vào thẳng
 *   `.uvd-scope` (áp dụng cho mọi thứ script tạo ra, không riêng panel chính).
 *   Riêng chữ "Cài đặt" dùng màu hồng đặc (không gradient) cho dễ đọc.
 * - FIX: tabbar 3 tab bị chia khoảng không đều — đổi `.uvd-tab` sang
 *   `flex:1 1 0%; min-width:max-content` để tab tự giãn lấp đầy hàng khi đủ
 *   chỗ, nhưng vẫn không bị co chữ (tự chuyển sang cuộn ngang) khi chật.
 * - GIẢM LAG: nền "liquid" trước blur 50px + 2 lớp gradient chạy vô hạn — giờ
 *   còn 1 lớp, blur 28px, và ẨN HẲN (không chỉ dừng animation) khi bật "chế độ
 *   hiệu suất". Hạ mặc định blurIntensity 24→14, effectsIntensity 55→30 cho
 *   cài đặt mới/sau khi Đặt lại. Nếu máy vẫn thấy giật: vào Cài đặt (⚙) kéo
 *   thanh "Cường độ hiệu ứng" xuống thấp hoặc bật "chế độ hiệu suất".
 * - (6.7.11) Fix nghiêm trọng: tự click nhầm nút TXT/M3U/CSV/JSON của chính
 *   mình; biến màu định nghĩa trên :root bị site đích đè lên.
 * - (6.7.10→6.7.9) Auto-click dò theo text khi selector chuẩn không khớp; nút
 *   ⏭ thử lần lượt từng server; tab "Nút đã click" ghi + chặn theo từng nút.
...
.uvd-scope{color:var(--text);font-family:-apple-system,BlinkMacSystemFont,'SF Pro Display','Segoe UI',Roboto,sans-serif;--bg:rgba(3,4,8,0.97);--glass:rgba(12,14,20,0.85);--glass-hi:rgba(255,255,255,0.06);--border:rgba(255,255,255,0.08);--text:#f3f5ff;--text2:#9ca3bd;--text3:#5d6377;--accent:#ff4fa0;--accent2:#ff8fd6;--danger:#ff5d72;--gold:#ffb84d;--success:#34d399;--card-bg:rgba(255,255,255,0.03);--fs-xs:11px;--fs-sm:12px;--fs-base:13px;--fs-md:14px;--fs-lg:16px;--radius-sm:12px;--radius-md:16px;--radius-lg:26px;--grad-liquid:linear-gradient(135deg,var(--accent),var(--accent2));--glow-px:0px;--glow-op:0;--btn-bg:rgba(255,255,255,0.1);--btn-danger-bg:rgba(255,93,114,0.22);--btn-danger-border:rgba(255,93,114,0.4);--btn-success-bg:rgba(52,211,153,0.2);--btn-success-border:rgba(52,211,153,0.4);--btn-accent-bg:rgba(255,79,160,0.28);--btn-purple-bg:rgba(167,139,250,0.24);--btn-gold-bg:rgba(255,184,77,0.26)}
