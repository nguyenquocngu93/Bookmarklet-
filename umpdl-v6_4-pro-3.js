 ...
 * - Trình phát: nhớ vị trí xem dở (resume), tua nhanh chạm đúp ±10s, Picture-in-Picture,
 *   hẹn giờ ngủ, tăng âm lượng (Web Audio boost), tự động phát stream tiếp theo
 * - Cài đặt: tab "Trình phát" riêng (tốc độ/chất lượng mặc định, tiết kiệm data,
 *   auto-fullscreen, màu chủ đề tuỳ chỉnh)
 * - Thanh tab cuộn ngang được, không còn bị bóp chữ khi nhiều tab
...
if (old) old.remove();
var oldMinBtn = document.getElementById('__uvd_min_float__'); // dọn phần tử sót lại từ bản cũ (<=6.2)
...
data.settings = Object.assign({
  ...
  volumeBoostMax: 200,      // %
  autoNext: false,
  reduceMotion: false,      // tắt hiệu ứng chuyển động cho máy yếu
...
// ========== GESTURE: chạm đúp trái/phải để tua ±10s ==========
...
function showVideoPlayer(url, type) {
  ...
  var mainPanel = document.getElementById('__uvd__');
...
.uvd-glass-panel{background:var(--glass);backdrop-filter:blur(14px) saturate(130%);-webkit-backdrop-filter:blur(14px) saturate(130%);border:1px solid var(--border);border-radius:var(--radius-lg);box-shadow:0 20px 50px rgba(0,0,0,0.8),0 0 0 1px rgba(255,255,255,0.03) inset,0 1px 0 rgba(255,255,255,0.08) inset;color:var(--text);font-family:-apple-system,BlinkMacSystemFont,'SF Pro Display','Segoe UI',Roboto,sans-serif;font-size:var(--fs-base);padding:16px;width:100%;position:relative;overflow:hidden;max-width:1000px;margin:auto}
...
.uvd-panel-minimized .uvd-panel-content>*:not(:first-child){pointer-events:none;opacity:0;transition:opacity .18s ease}
.uvd-panel-minimized .uvd-liquid-bg{animation-play-state:paused}
.uvd-reduce-motion, .uvd-reduce-motion *{animation:none!important;transition:none!important;backdrop-filter:none!important;-webkit-backdrop-filter:none!important;}
...
.uvd-btn{background:var(--glass-hi);border:1px solid var(--border);color:var(--text);padding:9px 16px;border-radius:var(--radius-md);font-weight:600;font-size:var(--fs-base);cursor:pointer;transition:all 0.2s;text-align:center;position:relative;overflow:hidden;display:inline-block;box-shadow:0 3px 10px rgba(0,0,0,0.4),0 1px 0 rgba(255,255,255,0.08) inset;line-height:1.3}
...
.uvd-btn-icon{background:var(--glass-hi);border:1px solid var(--border);color:var(--text);width:34px;height:34px;border-radius:var(--radius-sm);cursor:pointer;display:inline-flex;align-items:center;justify-content:center;transition:all 0.2s;position:relative;overflow:hidden;box-shadow:0 3px 8px rgba(0,0,0,0.35),0 1px 0 rgba(255,255,255,0.08) inset}
...
.uvd-card{background:var(--card-bg);border:1px solid var(--border);border-radius:var(--radius-md);padding:14px;margin-bottom:10px;font-size:var(--fs-base);box-shadow:0 1px 0 rgba(255,255,255,0.04) inset;transition:transform 0.3s cubic-bezier(.4,0,.2,1),box-shadow 0.3s ease,background 0.3s ease,border-color 0.3s ease;animation:uvdCardEnter 0.4s ease both;color:var(--text)}
...
.uvd-tabbar{position:relative;display:flex;gap:2px;padding:4px;background:rgba(255,255,255,0.04);border:1px solid var(--border);border-radius:999px;margin-bottom:10px;flex-shrink:0;overflow-x:auto;overflow-y:hidden;-webkit-overflow-scrolling:touch;scrollbar-width:none}
...
.uvd-tab{position:relative;z-index:1;flex:0 0 auto;background:transparent;border:none;color:var(--text2);font-weight:600;font-size:var(--fs-sm);padding:9px 14px;border-radius:999px;cursor:pointer;transition:color 0.25s;white-space:nowrap}
...
function buildUI() {
  ...
  panel.style.cssText = 'position:fixed;top:15px;left:15px;right:15px;bottom:15px;z-index:2147483647;animation:uvdScaleIn 0.4s ease;overscroll-behavior:contain;';
...
function renderPlayerSettings(container) {
  ...
  container.querySelectorAll('#__uvd_accent_grid__ button').forEach(function(btn) {
    btn.onclick = function() {
      s.accent = this.dataset.accent;
      storage.set(data);
      applyAccent(document.getElementById('__uvd__'));
...
function renderSettings(container) {
  ...
  container.innerHTML =
    ...
    '<div class="uvd-card">' +
      ...
      buildToggleRow('__uvd_toggle_reducemotion__', 'Tắt hiệu ứng chuyển động (dành cho máy yếu)', data.settings.reduceMotion) +
      '<div style="font-size:11px;color:var(--text3);margin-top:6px;">Tắt animation, hiệu ứng trượt và làm mờ kính (blur) để đỡ giật trên máy cấu hình thấp.</div>' +
    '</div>' +
    ...
    '<div class="uvd-card uvd-timeline-card">' +
      ...
      '<div class="uvd-code-block"><textarea readonly rows="3">' + bookmarkletCode + '</textarea><button class="uvd-code-copy" data-copy-target="bookmarklet" title="Sao chép">📋</button></div>' +
    ...
    '<div class="uvd-card uvd-timeline-card">' +
      ...
      '<div class="uvd-step"><span class="uvd-step-num">•</span><span class="uvd-step-text">Trong player: thanh công cụ gồm <strong style="color:var(--accent);">Chất lượng</strong>, <strong style="color:var(--accent);">Tốc độ</strong>, <strong style="color:var(--accent);">Toàn màn hình</strong>, <strong style="color:var(--accent);">PiP</strong>, <strong style="color:var(--accent);">Hẹn giờ</strong>, <strong style="color:var(--accent);">Boost</strong></span></div>' +
      '<div class="uvd-step"><span class="uvd-step-num">•</span><span class="uvd-step-text">Chạm đúp 2 lần vào nửa trái/phải video để tua lùi/tiến 10 giây</span></div>' +
      '<div class="uvd-callout"><span class="uvd-callout-icon">▶</span><span>Nút <strong style="color:var(--accent);">▶</strong> trên header: tự động bấm giúp các nút Play ẩn (eporner, JAV...) để link stream lộ ra. Nếu không ăn, đặt CSS selector riêng ở dòng "Play selector" bên dưới tên file.</span></div>' +
    '</div>' +
  ...
  document.getElementById('__uvd_toggle_reducemotion__').onclick = function() {    var isOn = this.classList.toggle('uvd-toggle-on');
    data.settings.reduceMotion = isOn;
    storage.set(data);
    applyMotionPref(document.getElementById('__uvd__'));
    toast(isOn ? 'Đã tắt hiệu ứng chuyển động' : 'Đã bật lại hiệu ứng chuyển động');
  };
...
console.log('V' + VERSION + ' UMP DL PRO - Resume, tua chạm đúp, PiP, boost, hẹn giờ, tab cuộn ngang');
