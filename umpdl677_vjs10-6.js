 ...
 * - MỚI: nút "🖥 Quét server" trên header — với các site kiểu "chọn server 1,
 *   2, 3..." mà mỗi nút chỉ gán src cho iframe (hoặc gọi AJAX) SAU khi bấm,
 *   script sẽ tự nhận diện các nút đó (theo text "Server N" hoặc class/id có
 *   chữ server/mirror/máy chủ) rồi tự bấm lần lượt, đợi ~0.9s mỗi lần cho JS
 *   của site kịp gán link, quét lại sau mỗi lần bấm. Tự động hoá đúng thao
 *   tác tay người dùng vẫn làm — không đọc/can thiệp nội bộ iframe (vẫn bị
 *   Same-Origin Policy chặn với iframe khác gốc như cũ).
...
// Đây không phải lỗi của script quét tĩnh, mà do chính cách site đó dựng —
// giải pháp là tự động giả lập đúng thao tác người dùng sẽ làm: bấm lần lượt
// từng nút, đợi 1 chút cho JS của site kịp gán src, rồi quét lại. Không đọc
// hay can thiệp gì vào nội bộ iframe (vẫn bị Same-Origin Policy chặn như cũ).
function autoClickServersAndScan() {
  var re = /^(server|sv|máy chủ|mirror|nguồn|source)\s*\.?\s*\d*$/i;
  var hayRe = /server|máy.?chủ|mirror|list-server|embed-server/i;
  var candidates = [];
  var seen = [];
  document.querySelectorAll('a,button,li,div,span').forEach(function(el) {
    ...
    var hay = ((el.className && typeof el.className === 'string' ? el.className : '') + ' ' + (el.id || '')).toLowerCase();
    if (re.test(txt) || (hayRe.test(hay) && el.children.length <= 2)) {
      candidates.push(el);
      seen.push(el);
    }
  });
  if (candidates.length === 0) {
    toast('Không tìm thấy nút server nào theo mẫu nhận diện — thử bấm tay từng nút rồi bấm Làm mới.');
    return;
  }
  ...
  function clickNext() {
    if (i >= candidates.length) {
      toast('Đã quét xong ' + candidates.length + ' server.');
      buildUI();
      return;
    }
    var el = candidates[i++];
...
function buildUI() {
  ...
  header.innerHTML = 
    ...
    '<div style="display:flex;gap:6px;">' +
      ...
      '<button class="uvd-btn-icon" id="__uvd_scan_servers__" title="Tự bấm lần lượt các nút Server rồi quét lại">🖥</button>' +
