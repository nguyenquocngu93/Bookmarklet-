 ...
 * - FIX NGHIÊM TRỌNG (nguyên nhân thật của "2 header chồng chéo"): CSS z-index
 *   là số nguyên 32-bit, giá trị hợp lệ tối đa là 2147483647. Script từng dùng
 *   2147483648 (player) và 2147483649 (toast/dialog xác nhận) — VƯỢT giới hạn,
 *   bị trình duyệt tự kẹp về cùng mức 2147483647 như panel chính → panel và
 *   player có z-index NGANG NHAU, thứ tự hiển thị phụ thuộc thuần vào thứ tự
 *   DOM. Nếu panel bị vẽ lại (buildUI) sau khi player đã mở, panel nhảy lên
 *   TRÊN player. Đã kẹp lại toàn bộ về đúng 2147483647.
 * - FIX (nguyên nhân thật của "bấm Xem không vào video"): rescan tự động chạy
 *   nền vẫn gọi buildUI() dù player đang mở, vừa gây toast "tìm thêm nguồn" xen
 *   ngang, vừa là thứ đẩy panel đè lên player (do bug z-index ở trên). Giờ
 *   rescan nền dừng hẳn khi playerState.overlay đang có giá trị (đang xem).
 * - MỚI (phòng hờ kép, không phụ thuộc z-index): panel chính tự ẩn
 *   (visibility:hidden) khi player mở, hiện lại khi đóng player — kể cả
 *   buildUI() bị gọi từ nơi khác trong lúc đang xem cũng không thể đè lên.
 * - REVERT: bỏ thử nghiệm "footer đè lên đáy video" ở 6.7.14 — nó đè trúng
 *   ngay vị trí thanh điều khiển GỐC của Video.js v10 (thư viện ngoài) như bạn
 *   báo. Trả footer về dòng riêng bên dưới, an toàn, không đoán mò toạ độ của
 *   thư viện ngoài nữa. Việc control gốc của Video.js v10 có tự overlay lên
 *   video hay không phụ thuộc layout nội bộ (nhiều khả năng Shadow DOM) của
 *   thư viện đó — không nằm trong tầm kiểm soát CSS của script.
 * - (6.7.13→6.7.14) Fix bug "UI chồng x2" do rescan so sai mốc; tầng đoán rộng
 *   giờ chỉ chạy khi bấm nút thủ công, không tự chạy ngầm.
