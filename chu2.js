/**
 * Universal Video Downloader V2 - Glass Edition + Video.js v10
 * Features: Live monitoring, Multi-quality, Favorites, Themes, Preview,
 *           Export, Site profiles, Share to YTDLnis
 * UI: Glassmorphism (từ UMP DL Glass) + Video.js v10 (module CDN)
...
(function() {
    ...
    function loadVideoJSV10(callback) {
        ...
        // Load Video.js v10 CSS (optional, có thể dùng skin mặc định)
        ...
        script.onload = function() {
            // Kiểm tra custom element đã đăng ký chưa
    ...
    function showPreview(url) {
        ...
        overlay.innerHTML = 
            ...
            '<div style="flex:1;display:flex;flex-direction:column;padding:16px;justify-content:center;">' +
                '<div id="__uvd_pv_container__" style="width:100%;max-height:70vh;border-radius:16px;overflow:hidden;background:#000;">' +
                    '<video id="__uvd_pv__" class="video-js" controls preload="auto" style="width:100%;height:100%;"></video>' +
                '</div>' +
        ...
        loadVideoJSV10(function() {
            try {
                mountVideoJSV10(container, video);
                // Video.js sẽ tự động phát hiện video và controls
                status.innerText = '▶️ Video.js v10 đã mount';
                // Nếu cần bổ sung HLS, có thể dùng plugin @videojs/http-streaming,
                // nhưng Video.js v10 đã tích hợp sẵn HLS/DASH qua video-skin.
            } catch(e) {
                status.innerText = '❌ Lỗi mount Video.js: ' + e.message;
                // Fallback: gắn controls native
                if (!video.parentNode) container.appendChild(video);
                video.controls = true;
                status.innerText = '⚠️ Dùng controls native (fallback)';
            }
        });
        
        closeBtn.onclick = function() {
            // Dọn dẹp custom element
            var player = container.querySelector('video-player');
            if (player) {
                // Có thể dispose nếu cần, nhưng Video.js v10 chưa có API dispose rõ ràng
                // Chỉ cần gỡ element
                player.remove();
            }
            overlay.remove();
    ...
    console.log('✅ Universal DL V2 (Glass + Video.js v10) loaded! Found', urls.size, 'streams initially');
    toast('✨ Glass Edition + Video.js v10 ready!');
})();
