/**
Universal Media Player & Downloader - V5.9.2 (Fix UI & Performance)
- Bỏ icon màu, dùng indicator đơn giản
- Xóa tính năng thu nhỏ script (tiết kiệm RAM)
- Fix layout: không mất nội dung, không giãn
- Error handling nhẹ nhàng hơn, không báo lỗi vội
...
function isAdUrl(url) {
  ...
  for (var i = 0; i < data.filterlist.length; i++) {
    ...
    if (pattern.startsWith('regex:')) {
      try { if (new RegExp(pattern.slice(6), 'i').test(url)) return true; } catch(e) {}
    } else {
...
function scan(doc, src) {
  try {
    ...
    for (var k = 0; k < iframes.length; k++) {
      ...
      } else if (iframeUrl) { __uvdAdBlockedCount++; }
...
function showVideoPlayer(url, type) {
  ...
  videoWrapper.style.cssText = 'flex:1;display:flex;align-items:center;justify-content:center;background:#000;position:relative;overflow:hidden;min-height:0;';
  ...
  footer.style.cssText = 'padding:8px 16px;background:rgba(0,0,0,0.75);border-top:1px solid rgba(255,255,255,0.1);font-size:12px;color:#aaa !important;display:flex;justify-content:space-between;align-items:center;flex-shrink:0;font-family:-apple-system,Segoe UI,sans-serif;';
  footer.innerHTML = '<span id="__uvd_player_status__"><span class="uvd-status-dot uvd-status-loading"></span> Đang tải...</span><span id="__uvd_player_size__" style="color:#8ab4ff !important;">Đang ước tính...</span><span id="__uvd_player_time__"></span>';
  ...
  function updateStatus(text, state) {
    var el = getStatusEl();
    if (!el) return;
    var dotClass = state === 'loading' ? 'uvd-status-loading' : state === 'playing' ? 'uvd-status-playing' : state === 'paused' ? 'uvd-status-paused' : state === 'error' ? 'uvd-status-error' : '';
    el.innerHTML = '<span class="uvd-status-dot ' + dotClass + '"></span> ' + text;
  }
  ...
  function showEnded() {
    updateStatus('Đã phát hết', 'playing');
  }
  function showError() {
    playerState.errorRetryCount++;
    if (playerState.errorRetryCount < 3 && playerState.isBuffering) {
      updateStatus('Đang tải lại...', 'loading');
      setTimeout(function() {
        if (video) {
          video.load();
      ...
      updateStatus('Lỗi phát (bỏ qua)', 'error');
    }
  }

  // Video events - nhẹ nhàng hơn
  ...
  video.addEventListener('loadeddata', function() { playerState.errorRetryCount = 0; });
  ...
  if (isHls) {
    if (window.Hls && Hls.isSupported()) {
      activeHls = new Hls({ maxBufferHole: 0.5, maxMaxBufferLength: 600 });
...
@keyframes uvdDotPulse{0%,100%{transform:scale(1);opacity:0.6}50%{transform:scale(1.3);opacity:1}}
:root{
...
.uvd-status-loading{
  ...
  animation:uvdDotPulse 1.5s ease-in-out infinite;
}
.uvd-status-playing{
  background:#22c55e;
...
.uvd-status-error{
  ...
  animation:uvdDotPulse 1s ease-in-out infinite;
}
`;
...
function buildUI() {
  ...
  panel.style.cssText = 'position:fixed;top:15px;left:15px;right:15px;bottom:15px;z-index:2147483647;animation:uvdScaleIn 0.4s ease;display:flex;flex-direction:column;';
...
var autoRefresh = setInterval(function() {
  if (!document.getElementById('uvd')) {
    clearInterval(autoRefresh);
    stopMonitor();
  }
}, 2000);

console.log('V' + VERSION + ' UMP DL - Fixed UI, removed minimize, better error handling');
