/**
 * Universal Media Player & Downloader - V6.7.5 PRO (cập nhật: blob & iframe)
 ...
 * - THÊM: lấy link thật từ blob (tải về / phát trực tiếp)
 * - THÊM: lấy link video/audio bên trong iframe (cùng origin)
...
// ========== BLOB URL MANAGEMENT ==========
var __uvd_blob_urls = [];
function __uvd_revokeBlobUrls() {
  __uvd_blob_urls.forEach(function(url) { URL.revokeObjectURL(url); });
  __uvd_blob_urls = [];
}
...
function buildStreamCardHTML(item, i) {
  var extraBtn = '';
  if (item.type === 'BLOB') {
    extraBtn = '<button class="uvd-btn uvd-btn-sm" data-action="reallink" data-url="' + encodeURIComponent(item.url) + '" data-type="BLOB" style="background:rgba(255,184,77,0.2);grid-column:1/3;">📎 Lấy link thật (tải blob)</button>';
  } else if (item.type === 'IFRAME') {
    extraBtn = '<button class="uvd-btn uvd-btn-sm" data-action="reallink" data-url="' + encodeURIComponent(item.url) + '" data-type="IFRAME" style="background:rgba(167,139,250,0.2);grid-column:1/3;">🔍 Lấy link trong iframe</button>';
  }
  return (
    '<div class="uvd-card" data-type="' + escapeHtml(item.type) + '" data-url="' + escapeHtml(item.url) + '">' +
      ...
      '</div>' +
      (extraBtn ? '<div class="uvd-grid-2" style="margin-top:6px;">' + extraBtn + '</div>' : '') +
    '</div>'
  );
}
...
function renderStreams(container, arr) {
  ...
  container.onclick = function(e) {
    ...
    if (actionBtn) {
      ...
      if (action === 'share') shareUrl(u2);
      ...
      else if (action === 'play' && t === 'BLOB') {
        // Phát blob: tải về, tạo object URL, rồi mở player
        toast('Đang tải blob để phát...');
        fetch(u2, { headers: { 'Referer': pageInfo.referer } })
          .then(function(res) { return res.blob(); })
          .then(function(blob) {
            var url = URL.createObjectURL(blob);
            __uvd_blob_urls.push(url);
            window.__uvd_showPlayer(url, 'MP4');
          })
          .catch(function(err) {
            toast('Lỗi tải blob: ' + (err.message || ''));
          });
        return;
      }
      ...
      else if (action === 'reallink') {
        var reallinkType = actionBtn.dataset.type;
        if (reallinkType === 'BLOB') {
          toast('Đang tải blob...');
          fetch(u2, { headers: { 'Referer': pageInfo.referer } })
            .then(function(res) { return res.blob(); })
            .then(function(blob) {
              var ext = blob.type.split('/')[1] || 'bin';
              var a = document.createElement('a');
              a.href = URL.createObjectURL(blob);
              a.download = pageInfo.title + '.' + ext;
              a.click();
              URL.revokeObjectURL(a.href);
              toast('Đã tải blob');
            })
        ...
        } else if (reallinkType === 'IFRAME') {
          var iframeEl = document.querySelector('iframe[src="' + u2 + '"]');
          if (iframeEl && iframeEl.contentDocument) {
            var found = [];
            try {
              iframeEl.contentDocument.querySelectorAll('video, source, audio').forEach(function(v) {
                if (v.src) found.push(v.src);
                if (v.currentSrc) found.push(v.currentSrc);
              });
            } catch(e) {}
            if (found.length) {
              var msg = 'Tìm thấy link trong iframe:\n' + found.join('\n');
              alert(msg);
              toast('Đã tìm thấy ' + found.length + ' link');
            } else {
              toast('Không tìm thấy video trong iframe (có thể bị CORS)');
              if (confirm('Không thể quét iframe (CORS). Muốn mở iframe trong tab mới để chạy script?')) {
                window.open(u2, '_blank');
              }
            }
          } else {
            toast('Không truy cập được iframe (CORS)');
            if (confirm('Mở iframe trong tab mới để chạy script?')) {
              window.open(u2, '_blank');
            }
          }
        }
