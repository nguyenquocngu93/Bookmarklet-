/**
 * Universal Media Player & Downloader - V6.7.16 PRO
 * - MỚI: header của player (thanh tiêu đề + toolbar Chất lượng/PiP/Boost...)
 *   tự "nổi" đè vài px lên đỉnh video khi phát hiện video NGANG (video.videoWidth
 *   > videoHeight, dò qua sự kiện loadedmetadata) — chiếm lại phần viền đen
 *   letterbox phía trên thay vì chừa hẳn 1 dòng riêng. Video ĐỨNG thì header tự
 *   giãn về bình thường (không đè). Chỉ động tới header của CHÍNH script,
 *   hoàn toàn không đụng gì tới CSS/skin của Video.js v10 — an toàn hơn thử
 *   nghiệm footer ở 6.7.14 (khi đó đè trúng control gốc của thư viện ngoài).
 * - (6.7.15) Fix nghiêm trọng: z-index vượt giới hạn 32-bit khiến panel chồng
 *   lên player; rescan nền dừng hẳn khi đang xem video; panel tự ẩn khi player mở.
 * - (6.7.13→6.7.14) Fix bug "UI chồng x2"; tầng đoán rộng chỉ chạy khi bấm nút
 *   thủ công; theme hồng sáng; bo tròn thêm; giảm giật khi thu nhỏ.
