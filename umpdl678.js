...
function __uvdMountVjs10(wrapper, video) {
  ...
  function wrapWithSkin() {
    ...
    try {
      ...
      // QUAN TRỌNG: v10 (beta) bị vỡ UI nặng khi video tỉ lệ DỌC (portrait) —
      // control surface render sai kích thước thành khối tròn khổng lồ che
      // gần hết màn hình. Bug nằm trong Shadow DOM của skin, không sửa được
      // từ CSS bên ngoài. Cách xử lý an toàn: kiểm tra tỉ lệ ngay khi có
      // metadata, nếu là video dọc thì tự "gỡ" skin và quay về controls gốc.
      function checkOrientation() {
        ...
        if (video.videoWidth && video.videoHeight && video.videoHeight > video.videoWidth) {
          console.warn('[UMP DL] Video dọc (' + video.videoWidth + 'x' + video.videoHeight + ') — v10 skin bị lỗi layout với tỉ lệ này, tự chuyển về controls gốc');
          if (video.parentNode) video.parentNode.removeChild(video);
