// ==UserScript==
// @name         Universal Video Downloader V3.8 — Premium UI Edition
// @namespace    http://tampermonkey.net/
// @version      3.8
// @description  Giao diện Glassmorphism siêu mượt, tối ưu cảm ứng, hỗ trợ TorrServer
...
(function() {
    ...
    function shareUrl(url) {
        ...
        } else { copy(url); toast('Đã copy - Sẵn sàng tải!', 'info'); }
    }
                ...
                '<div class="__uvd_header_left__">' +
                    '<div class="__uvd_header_text__">' +
                        ...
                        '<div class="__uvd_subtitle__">' +
                            '<span class="__uvd_live_dot__"></span> Thấy: ' + videoCount + ' phim · ' + photoCount + ' ảnh · ' + scriptCount + ' mã nguồn' +
                        '</div>' +
                    '</div>' +
                '</div>' +
    ...
    function getGlobalCSS(t) {
        ...
        .__uvd_preview_header.hidden { opacity: 0; pointer-events: none; }
        ...
        .__uvd_preview_controls__ {
            position: absolute; bottom: 0; left: 0; right: 0; z-index: 10; padding: 20px;
            background: linear-gradient(to top, rgba(0,0,0,0.8), transparent);
            display: flex; gap: 8px; justify-content: center; transition: opacity 0.3s; flex-wrap: wrap;
        }
        .__uvd_preview_controls.hidden { opacity: 0; pointer-events: none; }
        .__uvd_preview_btn__ {
            background: rgba(255,255,255,0.15); border: 1px solid rgba(255,255,255,0.2); color: #fff;
            padding: 8px 16px; border-radius: 20px; font-size: 12px; font-weight: 600; cursor: pointer; transition: all 0.2s;
        }
        .__uvd_preview_btn__.active { background: ${t.primary}; border-color: ${t.primary}; }
        .__uvd_preview_status__ {
            position: absolute; bottom: 100px; left: 50%; transform: translateX(-50%); background: rgba(0,0,0,0.75);
            padding: 6px 16px; border-radius: 12px; color: #fff; font-size: 12px; pointer-events: none; opacity: 0; transition: opacity 0.2s;
        }
    ...
    function bindGlobalEvents() {
        ...
        document.getElementById('__uvd_batch_copy__').addEventListener('click', function() {
            ...
            copy(listUrls.join('\n'));
            toast('Đã copy ' + listUrls.length + ' liên kết đang lọc', 'success');
        });
    ...
    function renderCategory(container, category) {
        ...
        if (!items.length) {
            container.innerHTML = '<div class="__uvd_empty__"><div class="__uvd_empty_icon__">🔍</div><div>Chưa tìm thấy dữ liệu nào phù hợp</div></div>';
            return;
        }
        ...
        items.forEach(function(item, idx) {
            ...
            if (category === 'video' && (item.type === 'M3U8' || item.type === 'MP4')) {
                ...
                if (item.type === 'M3U8') {
                    html += `<button class="__uvd_act_btn__ __uvd_btn_quality__" data-action="quality" data-url="${encodeURIComponent(item.url)}">Độ phân giải</button>`;
                }
            }
            ...
            if (category === 'video') {
                html += `<button class="__uvd_act_btn__ __uvd_btn_cmd__ full" data-action="cmd" data-url="${encodeURIComponent(item.url)}" data-type="${item.type}">Dòng lệnh nâng cao</button>`;
            }
            html += `</div>`;
    ...
    function showQualityPicker(url) {
        ...
        overlay.innerHTML = `<div class="__uvd_overlay_box__"><div class="__uvd_overlay_title__"><span class="__uvd_accent_inline__"></span>Đang phân tích luồng phát...</div></div>`;
        ...
        parseM3U8Master(url, function(qualities) {
            ...
            if (!qualities) {
                box.innerHTML = `
                    ...
                    <p style="margin-bottom:16px;color:#94a3b8;">Không phát hiện danh sách phân giải (Master Playlist). Đây là tệp luồng trực tiếp đơn.</p>
                    <button class="__uvd_act_btn__ __uvd_btn_preview__ full" id="__uvd_close_overlay__">Đóng lại</button>
                `;
                box.querySelector('#__uvd_close_overlay__').onclick = function() { overlay.remove(); };
                return;
            }
            
            var html = `<div class="__uvd_overlay_title__"><span class="__uvd_accent_inline__"></span>Chọn độ phân giải</div>`;
            ...
            html += `<button class="__uvd_act_btn__ __uvd_btn_share__ full" style="margin-top:10px;" id="__uvd_close_overlay__">Đóng danh sách</button>`;
            box.innerHTML = html;
            ...
            box.addEventListener('click', function(e) {
                ...
                else if (btn.dataset.action === 'copy') { copy(qUrl); toast('Đã copy luồng độ phân giải cao', 'success'); }
            });
        });
    }
    ...
    function showPreview(url, type) {
        ...
        overlay.innerHTML = `
            ...
            <div class="__uvd_preview_video_wrap__">
                ...
                <div class="__uvd_preview_status__"></div>
            </div>
        `;
        document.body.appendChild(overlay);
        
        var video = document.getElementById('__uvd_player__');
        var closeBtn = overlay.querySelector('.__uvd_preview_close__');
        var status = overlay.querySelector('.__uvd_preview_status__');
    ...
    function showCommandPicker(url, type) {
        ...
        overlay.addEventListener('click', function(e) {
            ...
            if (btn.dataset.cmd) {
                ...
                toast('Đã copy câu lệnh CLI!', 'success');
            }
        });
    }
    
    // ========== CÁC TAB CHỨC NĂNG BỔ SUNG HOÀN CHỈNH ==========
            ...
            list.forEach(function(item) {
                html += `
                    <div style="background:rgba(255,255,255,0.03); padding:10px; border-radius:12px; border:1px solid rgba(255,255,255,0.05); margin-bottom:8px;">
                        <div style="font-weight:600; font-size:12px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">${escapeHtml(item.name || 'Đang phân tích tên...')}</div>
                        <div style="font-size:10px; color:#64748b; margin-top:4px;">Dung lượng: ${(item.size / 1024 / 1024 / 1024).toFixed(2)} GB</div>
                        <div style="display:flex; gap:6px; margin-top:8px;">
                            <button class="__uvd_act_btn__ __uvd_btn_copy__" style="flex:1; min-height:28px; padding:2px; font-size:11px;" onclick="navigator.clipboard.writeText('${data.torrserverUrl}/play/torrent/${item.hash}/1'); alert('Đã copy link play dòng 1');">Copy Link Play</button>
                        </div>
                    </div>
                `;
            });
    ...
    function renderHistory(container) {
        if (!data.history || data.history.length === 0) {
            container.innerHTML = '<div class="__uvd_empty__">Chưa có lịch sử thao tác</div>';
            return;
        }
    ...
    function renderSettings(container) {
        container.innerHTML = `
            ...
            <div style="margin-top:20px; border-top:1px solid rgba(255,255,255,0.1); padding-top:14px;">
                <button class="__uvd_act_btn__ __uvd_btn_share__ full" id="__clear_all_data__" style="background:#ef4444;">Xóa toàn bộ cấu hình & bộ nhớ cache</button>
            </div>
        `;
        ...
        container.querySelector('#__clear_all_data__').onclick = function() {
            if (confirm('Bạn có chắc chắn muốn làm sạch toàn bộ dữ liệu của script?')) {
    ...
    // Khởi tạo hiển thị giao diện ban đầu
