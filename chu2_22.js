 ...
 * - Giao diện player mới: card bo góc trên (border-radius: 24px 24px 0 0),
 *   nền theo màu script, độ cao tự động theo tỉ lệ video.
 * - Header 2 dòng: tiêu đề + nút 3 chấm (menu chỉ có Chất lượng & Phụ đề)
 *   + nút đóng. Dòng 2 hiển thị độ phân giải, loại video, dung lượng.
 * - Loại bỏ footer, thu nhỏ video, toolbar cũ, thu nhỏ script, nút cài đặt.
 * - Giữ nguyên ambient, HLS, PiP (có sẵn trên thanh Video.js).
...
(function() {
  ...
  function savePlaybackPosition(url, video) {
    ...
    if (pct < 0.02 || pct > 0.95) { delete data.playbackPositions[url]; } else {
  ...
  function clearSleepTimer() {
    if (playerState.sleepTimerId) { clearTimeout(playerState.sleepTimerId);
  ...
  function showSleepMenu() {
    ...
    [0, 15, 30, 45, 60].forEach(function(m) {
      ...
      b.onclick = function() { setSleepTimer(m);
        overlay2.remove(); };
      panel.appendChild(b);
    });
  ...
  function searchSubDL(query, cb) {
    ...
    if (!apiKey) { toast('Chưa có SubDL API Key, xem hướng dẫn trong bảng Phụ đề');
      cb([]); return; }
    console.log('[UMP DL] SubDL: bắt đầu tìm "' + query + '"');
    var settled = false;

    function finish(fn) { if (settled) return;
      settled = true;
  ...
  function downloadSubDLFile(item, cb) {
    ...
    Promise.resolve(reqPromise)
      ...
      .catch(function() { toast('Lỗi tải phụ đề từ SubDL');
        if (cb) cb(false); });
  }
  ...
  function attachPlayerGestures(wrapper, video) {
    ...
    wrapper.addEventListener('touchend', function(e) {
      ...
      if (lastTap.side === side && (now - lastTap.time) < 300) {
        if (side === 'left') { video.currentTime = Math.max(0, video.currentTime - tapSeconds);
          showGestureHint('⏪ -' + tapSeconds + 's'); } else { video.currentTime = Math.min(video.duration || 1e9, video.currentTime + tapSeconds);
          showGestureHint('⏩ +' + tapSeconds + 's'); }
        hideGestureHintSoon();
        lastTap.time = 0;
      } else {
  ...
  function showVideoPlayer(url, type) {
    ...
    overlay.style.cssText =
      ...
      'background:var(--bg, rgba(6,3,8,0.97));animation:uvdFadeIn 0.3s ease;';
    ...
    card.id = '__uvd_player_card__';
    card.style.cssText =
      'display:flex;flex-direction:column;width:95%;max-width:1200px;max-height:95dvh;' +
      'border-radius:24px 24px 0 0;overflow:hidden;' +
      'background:var(--glass, rgba(20,10,22,0.92));' +
      'backdrop-filter:blur(var(--uvd-blur, 20px)) saturate(130%);' +
      '-webkit-backdrop-filter:blur(var(--uvd-blur, 20px)) saturate(130%);' +
      'border:1px solid var(--border, rgba(255,143,214,0.12));' +
      'box-shadow:0 20px 60px rgba(0,0,0,0.8),0 0 0 1px rgba(255,255,255,0.03) inset;' +
      'position:relative;';
    overlay.appendChild(card);
    applyEffectsPref(card);

    // ——— HEADER DÒNG 1 ———
    var header1 = document.createElement('div');
    header1.style.cssText =
      'display:flex;justify-content:space-between;align-items:center;padding:12px 16px;flex-shrink:0;' +
      'background:rgba(0,0,0,0.2);border-bottom:1px solid rgba(255,255,255,0.06);';
    var titleSpan = document.createElement('span');
    titleSpan.style.cssText = 'font-weight:600;font-size:15px;color:var(--text,#fff);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;flex:1;min-width:0;';
    titleSpan.textContent = '▶ ' + escapeHtml(pageInfo.title);
    header1.appendChild(titleSpan);

    var headerActions = document.createElement('div');
    headerActions.style.cssText = 'display:flex;gap:6px;align-items:center;flex-shrink:0;margin-left:12px;';

    // Nút 3 chấm (menu)
    var menuBtn = document.createElement('button');
    menuBtn.className = 'uvd-btn-icon';
    menuBtn.textContent = '⋮';
    menuBtn.title = 'Menu';
    menuBtn.style.cssText = 'width:36px;height:36px;font-size:20px;font-weight:700;background:rgba(255,255,255,0.06);border-radius:50%;border:1px solid rgba(255,255,255,0.08);color:#fff;cursor:pointer;display:flex;align-items:center;justify-content:center;';
    var menuOpen = false;
    ...
    function toggleMenu(e) {
      ...
      menuDropdown.style.cssText =
        'position:absolute;top:calc(100% + 4px);right:0;background:var(--glass, rgba(20,22,30,0.98));' +
        'border-radius:14px;padding:8px 0;min-width:160px;border:1px solid var(--border);' +
        'backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);' +
        'box-shadow:0 12px 40px rgba(0,0,0,0.6);z-index:20;';
      var items = [
        { label: '🎚 Chất lượng', action: function() { closeMenu();
            showQualitySubMenu(); } },
        { label: '💬 Phụ đề', action: function() { closeMenu();
      ...
      items.forEach(function(item) {
        ...
        btn.style.cssText =
          ...
          'color:var(--text,#fff);font-size:13px;text-align:left;cursor:pointer;' +
      ...
      var closeMenuOnClick = function(ev) { if (!menuDropdown || !menuDropdown.contains(ev.target) && ev.target !== menuBtn) { closeMenu(); } };
      setTimeout(function() { document.addEventListener('click', closeMenuOnClick); }, 10);
      menuDropdown._closeListener = closeMenuOnClick;
      headerActions.style.position = 'relative';
      headerActions.appendChild(menuDropdown);
    }

    function closeMenu() {
      menuOpen = false;
      if (menuDropdown) {
        if (menuDropdown._closeListener) document.removeEventListener('click', menuDropdown._closeListener);
        menuDropdown.remove();
        menuDropdown = null;
      }
    }
    ...
    closeBtn.className = 'uvd-btn-icon';
    closeBtn.textContent = '✕';
    closeBtn.title = 'Đóng trình phát';
    closeBtn.style.cssText = 'width:36px;height:36px;font-size:18px;background:rgba(255,93,114,0.2);border-radius:50%;border:1px solid rgba(255,93,114,0.3);color:#fff;cursor:pointer;display:flex;align-items:center;justify-content:center;';
    closeBtn.onclick = function() {
      closePlayer();
      if (document.getElementById('__uvd__')) document.getElementById('__uvd__').style.visibility = '';
    };
    headerActions.appendChild(closeBtn);
    header1.appendChild(headerActions);
    card.appendChild(header1);

    // ——— HEADER DÒNG 2 (resolution, type, size) ———
    var header2 = document.createElement('div');
    header2.style.cssText =
      'display:flex;gap:16px;padding:4px 16px 10px 16px;font-size:12px;color:var(--text2,#9ca3bd);flex-shrink:0;' +
      'background:rgba(0,0,0,0.12);border-bottom:1px solid rgba(255,255,255,0.04);flex-wrap:wrap;';
    var resEl = document.createElement('span');
    resEl.id = '__uvd_player_resolution__';
    resEl.textContent = '—';
    var typeEl = document.createElement('span');
    typeEl.id = '__uvd_player_vtype__';
    typeEl.textContent = type;
    var sizeEl = document.createElement('span');
    sizeEl.id = '__uvd_player_size__';
    sizeEl.textContent = 'Đang ước tính...';
    [resEl, typeEl, sizeEl].forEach(function(el) {
      var wrap = document.createElement('span');
      wrap.style.cssText = 'display:flex;align-items:center;gap:4px;';
      wrap.appendChild(el);
      header2.appendChild(wrap);
    });
    card.appendChild(header2);
    playerState._sizeEl = sizeEl;
    playerState._resolutionEl = resEl;
    playerState._typeEl = typeEl;
    ...
    ambient.id = '__uvd_ambient__';
    ambient.width = 48;
    ambient.height = 27;
    ambient.style.cssText =
      'position:absolute;inset:-15%;z-index:0;filter:blur(50px) saturate(160%) brightness(0.75);' +
      'transform:scale(1.15);pointer-events:none;transition:opacity 0.6s ease;opacity:0;';
    var ambientCtx = ambient.getContext('2d');
    videoWrapper.appendChild(ambient);

    var video = document.createElement('video');
    video.id = '__uvd_player_video__';
    video.style.cssText = 'max-width:100%;max-height:100%;width:100%;height:100%;display:block;object-fit:contain;position:relative;z-index:1;';
    ...
    // Ambient loop
    if (!playerState.wasReduceMotion) {
      playerState.ambientTimer = setInterval(function() {
        if (!video.videoWidth || video.paused) return;
        try {
          ambientCtx.drawImage(video, 0, 0, 48, 27);
          ambient.classList.add('uvd-ambient-on');
        } catch (e) {}
      }, 700);
    }

    // ——— CẬP NHẬT ASPECT RATIO CỦA CARD ———
    function updateCardAspect() {
      if (!video.videoWidth || !video.videoHeight) return;
      var ratio = video.videoWidth / video.videoHeight;
      // Giới hạn để không bị quá méo
      if (ratio > 2.8) ratio = 2.8;
      if (ratio < 0.4) ratio = 0.4;
      card.style.aspectRatio = ratio;
    }
    video.addEventListener('loadedmetadata', updateCardAspect);
    video.addEventListener('resize', updateCardAspect);
    // Nếu video đã có metadata ngay lập tức
    if (video.videoWidth && video.videoHeight) updateCardAspect();

    // ——— RESUME, AUTO FULLSCREEN, BOOST ———
    video.addEventListener('loadedmetadata', function onMeta() {
      ...
      if (video.videoWidth && video.videoHeight) {
        ...
        if (playerState._resolutionEl) playerState._resolutionEl.textContent = playerState.resolution;
      }
      // Speed
    ...
    function updateSizeEstimate() {
      var el = playerState._sizeEl;
      if (!el) return;
      if (playerState.hls && playerState.hls.levels && playerState.hls.levels.length) {
        var lvl = playerState.hls.levels[playerState.hls.currentLevel || 0];
        var bw = lvl ? lvl.bitrate : playerState.bandwidth;
        if (bw && video.duration) {
          var bytes = (bw / 8) * video.duration;
          var s = formatBytes(bytes);
          el.textContent = s ? '≈ ' + s : 'Đang ước tính...';
        } else {
    ...
    video.addEventListener('timeupdate', function() {
      ...
      // Cập nhật resolution hiển thị nếu thay đổi
      if (video.videoWidth && video.videoHeight) {
        var res = video.videoWidth + 'x' + video.videoHeight;
        if (playerState._displayedResolution !== res) {
          playerState._displayedResolution = res;
          if (playerState._resolutionEl) playerState._resolutionEl.textContent = res;
        }
      }
    });

    // ——— AUTO-HIDE CONTROLS (giữ nguyên) ———
    function resetHideTimer() {
      if (playerState.pinned || !data.settings.autoHideControls) {
        // Hiện controls (header) và không đặt timer
    ...
    // Khi tương tác với header thì reset
    header1.addEventListener('click', resetHideTimer);
    header2.addEventListener('click', resetHideTimer);
    ...
    function applyDefaultQualityPreference() {
      if (!playerState.hls || !playerState.hls.levels || !playerState.hls.levels.length) return;
      ...
      playerState.hls.currentLevel = bestIdx;
    }
    ...
    function createMenuPanel(title, options, callback) {
      ...
      closeBtn2.className = 'uvd-btn uvd-btn-sm';
      closeBtn2.style.cssText = 'width:100%;margin-top:10px;background:var(--danger);';
    ...
    function showQualitySubMenu() {
      ...
      if (!qualities.length) { toast('Không có chất lượng để chọn'); return; }
    ...
    // ——— LƯU THAM CHIẾU CHO closePlayer ———
    playerState._header1 = header1;
    playerState._header2 = header2;
    playerState._card = card;
    playerState._menuBtn = menuBtn;
    playerState._closeMenu = closeMenu;
  ...
  function closePlayer() {
    if (playerState.overlay) {
      ...
      clearSleepTimer();
      clearTimeout(playerState.hideTimeout);
      if (playerState.ambientTimer) { clearInterval(playerState.ambientTimer);
        playerState.ambientTimer = null; }
      if (playerState.audioCtx) {
      ...
      if (playerState.hls) { playerState.hls.destroy();
      ...
      if (playerState._closeMenu) playerState._closeMenu();
...
#__uvd_ambient__{position:absolute;inset:-15%;z-index:0;filter:blur(50px) saturate(160%) brightness(0.75);transform:scale(1.15);pointer-events:none;transition:opacity 0.6s ease;opacity:0;}
#__uvd_ambient__.uvd-ambient-on{opacity:1;}
`;
  document.head.appendChild(style);

  // ========== BUILD UI (đã xóa nút settings & minimize) ==========
  function buildUI() {
    var arr = [...urls.entries()].map(function(e) {
    ...
    document.getElementById('__uvd_refresh__').onclick = function() { buildUI();
      toast('Đã làm mới'); };
  ...
  function renderStreams(container, arr) {
    ...
    container.onclick = function(e) {
      ...
      if (actionBtn) {
        ...
        else if (action === 'copy') { copy(u2);
          toast('Đã sao chép!'); } else if (action === 'quality') showQualityPicker(u2);
  ...
  function showQualityPicker(url) {
    ...
    parseM3U8Master(url, function(qualities) {
      ...
      qualities.forEach(function(q) {
        ...
        shareBtn.onclick = function() { shareUrl(q.url);
          overlay.remove(); };
        ...
        playBtn.onclick = function() { overlay.remove();
        ...
        cmdBtn.onclick = function() { overlay.remove();
          showCommandPicker(q.url, 'M3U8'); };
      ...
      btn.onclick = function() {
        ...
        switch (btn.id) {
          case '__uvd_toggle_resume__':
            s.resumePlayback = isOn;
            break;
          case '__uvd_toggle_autofs__':
            s.autoFullscreen = isOn;
            break;
          ...
          case '__uvd_toggle_autohide__':
            s.autoHideControls = isOn;
            break;
          case '__uvd_toggle_showremaining__':
            s.showRemainingTime = isOn;
            break;
        }
        storage.set(data);
      };
  ...
  function renderSettings(container) {
    ...
    container.innerHTML =
      ...
      '<div class="uvd-step"><span class="uvd-step-num">•</span><span class="uvd-step-text">Trong player: menu 3 chấm chứa <strong style="color:var(--accent);">Chất lượng</strong> và <strong style="color:var(--accent);">Phụ đề</strong>. Các điều khiển còn lại có sẵn trên thanh Video.js.</span></div>' +
    ...
    container.querySelectorAll('.uvd-code-copy').forEach(function(b) {
      b.onclick = function() {
        if (this.dataset.copyTarget === 'bookmarklet') { copy(bookmarkletCode);
    ...
    document.getElementById('__uvd_restore__').onclick = function() {
      ...
      inp.onchange = function(e) {
        ...
        reader.onload = function(ev) {
          try { data = Object.assign(data, JSON.parse(ev.target.result));
            storage.set(data);
            toast('Đã nhập!');
            buildUI(); } catch(ex) { toast('File không hợp lệ', 'var(--danger)'); }
        };
  ...
  // ========== START ==========
  buildUI();
  console.log('V' + VERSION + ' UMP DL PRO - Giao diện player mới, tối giản, menu 3 chấm');
