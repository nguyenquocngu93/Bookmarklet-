/**
 * Universal Media Player & Downloader - V6.7.18 PRO
 * - MỚI: nền động (ambient) sau video — lấy màu/blur từ chính khung hình video
 *   hiện tại (canvas 48x27 độ phân giải thấp, chỉ vẽ lại ~mỗi 700ms, CSS blur
 *   nặng) để lấp khoảng đen quanh video (giờ video co khít theo tỉ lệ thật ở
 *   6.7.17) thay vì đen trơn — kiểu Apple TV/Spotify Canvas. Tái dùng đúng kỹ
 *   thuật drawImage(video,...) mà nút Screenshot đã dùng ổn định, nên không
 *   phát sinh rủi ro CORS mới. Tự tắt nếu người dùng đã bật "chế độ hiệu suất"
 *   từ trước khi mở player (tôn trọng lựa chọn nhẹ máy).
 * - MỚI: gọn hoá toolbar player — từ các nút "viên thuốc" đầy chữ rời rạc
 *   (Chất lượng/1x/PiP/Hẹn giờ/Boost/Mute/Tự động phát tiếp/📌/Screenshot/Phụ
 *   đề/Chia sẻ...) sang icon nhỏ gọn 36x36, nhóm theo chức năng (Hiển thị |
 *   Âm thanh & bảo vệ | Tiện ích | Phụ đề&Chia sẻ) có dấu phân cách giữa các
 *   nhóm, giữ nguyên toàn bộ chức năng/id/trạng thái, chỉ đổi hình thức.
 * - (6.7.17) <video-player> dùng aspect-ratio thật thay vì ép height:100%.
 * - (6.7.16) Header tự nổi lên video khi video ngang.
 * - (6.7.15) Fix nghiêm trọng z-index vượt giới hạn 32-bit gây panel chồng
 *   player; rescan nền dừng khi đang xem; panel tự ẩn khi player mở.
...
data.settings = Object.assign({
  ...
  allowMinimize: true,    // cho phép nút ▼ thu nhỏ script (tắt nếu hay bị glitch)
...
function showVideoPlayer(url, type) {
  ...
  if (__uvdMainPanel) __uvdMainPanel.style.visibility = 'hidden'; // ẩn hẳn panel khi xem video, không cho chồng lên player dù có chuyện gì xảy ra với z-index

  var overlay = document.createElement('div');
  overlay.id = '__uvd_player_overlay__';
  overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.96);z-index:2147483647;display:flex;flex-direction:column;animation:uvdFadeIn 0.3s ease;';
  ...
  header.style.cssText = 'padding:10px 16px;background:rgba(14,16,22,0.92);display:flex;flex-direction:column;flex-shrink:0;border-bottom:1px solid rgba(255,255,255,0.2);box-shadow:0 4px 20px rgba(0,0,0,0.5);transition: opacity 0.3s ease, margin-bottom 0.25s ease, background 0.25s ease;position:relative;z-index:6;';
  ...
  toolbar.style.cssText = 'display:flex;flex-wrap:wrap;align-items:center;gap:5px;margin-top:8px;transition: opacity 0.3s ease;';

  function mkIconBtn(icon, titleText, id) {
    var b = document.createElement('button');
    b.className = 'uvd-icon-btn';
    b.textContent = icon;
    b.title = titleText;
    if (id) b.id = id;
    return b;
  ...
  function mkDivider() {
    ...
    d.className = 'uvd-toolbar-divider';
    return d;
  }

  // NHÓM 1 — Hiển thị: chất lượng / tốc độ / toàn màn hình / PiP
  var qualityBtn = mkIconBtn('🎚', 'Chất lượng');
  qualityBtn.classList.add('uvd-icon-btn-wide');
  ...
  var speedDecBtn = mkIconBtn('−', 'Giảm tốc độ');
  var speedLabel = document.createElement('span');
  speedLabel.className = 'uvd-icon-btn uvd-icon-btn-wide';
  speedLabel.style.cssText = 'cursor:default;font-size:12px;';
  speedLabel.textContent = '1x';
  speedLabel.id = '__uvd_speed_label__';
  var speedIncBtn = mkIconBtn('+', 'Tăng tốc độ');
  function changeSpeed(dir) {
    var video = playerState.video;
    if (!video) return;
    var rates = [0.25, 0.5, 0.75, 1, 1.25, 1.5, 1.75, 2];
    var idx = rates.indexOf(video.playbackRate);
    if (dir > 0) video.playbackRate = idx < rates.length - 1 ? rates[idx + 1] : 2;
    else video.playbackRate = idx > 0 ? rates[idx - 1] : 0.25;
    speedLabel.textContent = video.playbackRate + 'x';
    toast('Tốc độ: ' + video.playbackRate + 'x');
    resetHideTimer();
  }
  speedDecBtn.onclick = function() { changeSpeed(-1); };
  speedIncBtn.onclick = function() { changeSpeed(1); };
  toolbar.appendChild(speedDecBtn);
  toolbar.appendChild(speedLabel);
  toolbar.appendChild(speedIncBtn);

  var fsBtn = mkIconBtn('⛶', 'Toàn màn hình');
  ...
  if (document.pictureInPictureEnabled) {
    var pipBtn = mkIconBtn('⧉', 'PiP (nổi ngoài trang)');
  ...
  // NHÓM 2 — Âm thanh & bảo vệ: hẹn giờ / boost / mute
  var sleepBtn = mkIconBtn('⏰', 'Hẹn giờ tắt');
  sleepBtn.onclick = showSleepMenu;
  toolbar.appendChild(sleepBtn);

  var boostBtn = mkIconBtn('🔊', 'Tăng âm lượng (Boost)', '__uvd_boost_btn__');
  ...
  var muteBtn = mkIconBtn('🔇', 'Tắt/bật tiếng', '__uvd_mute_btn__');
  var isMuted = false;
  ...
  // NHÓM 3 — Tiện ích: tự động phát tiếp / ghim / screenshot
  var autoNextBtn = mkIconBtn('⏭', 'Tự động phát tiếp', '__uvd_autonext_btn__');
  ...
  var pinBtn = mkIconBtn('📌', 'Ghim thanh điều khiển (không tự ẩn)', '__uvd_pin_btn__');
  ...
  var screenshotBtn = mkIconBtn('📷', 'Chụp ảnh màn hình');
  ...
  // NHÓM 4 — Phụ đề / tải / chia sẻ
  var subtitleBtn = mkIconBtn('💬', 'Phụ đề (thử nghiệm)');
  ...
  if (type === 'MP4' || type === 'MKV' || type === 'WEBM') {
    var downloadBtn = mkIconBtn('⬇', 'Tải xuống');
    downloadBtn.style.background = 'var(--btn-success-bg)';
    downloadBtn.style.border = '1px solid var(--btn-success-border)';
  ...
  if (type === 'M3U8' || type === 'MPD') {
    var shareBtn = mkIconBtn('↗', 'Chia sẻ');
    shareBtn.style.background = 'var(--btn-purple-bg)';
  ...
  // NỀN ĐỘNG (ambient) — lấy màu/blur từ chính khung hình video hiện tại để
  // lấp khoảng trống quanh video (giờ video co khít theo tỉ lệ thật, không
  // còn full-height nữa) thay vì để đen trơn, kiểu Apple TV / Spotify Canvas.
  // Dùng canvas độ phân giải THẤP (48x27) + CSS blur nặng, chỉ vẽ lại mỗi
  // ~700ms (không phải mỗi khung hình) nên rất nhẹ — tái dùng đúng kỹ thuật
  // drawImage(video,...) mà nút Screenshot đã dùng ổn định từ trước.
  ...
  if (!playerState.wasReduceMotion) {
    playerState.ambientTimer = setInterval(function() {
      ...
      } catch (e) {} // nguồn không hỗ trợ CORS đọc pixel — bỏ qua, giữ nền đen mặc định
    }, 700);
  }
  ...
  // Header tự "nổi" đè vài px lên đỉnh video khi video NGANG (thường có viền
  // đen letterbox ở trên/dưới nên đè lên không che mất nội dung thật) — chiếm
  // lại phần không gian đó. Video ĐỨNG thì giữ header giãn bình thường vì
  // thường không dư khoảng trống để đè lên. Chỉ động tới header của CHÍNH
  // script, không đụng gì tới CSS/skin của Video.js.
  function __uvdUpdateHeaderFloat() {
    if (!video.videoWidth || !video.videoHeight) return;
    overlay.classList.toggle('uvd-header-float', video.videoWidth > video.videoHeight);
  ...
  video.addEventListener('loadedmetadata', __uvdUpdateHeaderFloat);
  video.addEventListener('resize', __uvdUpdateHeaderFloat); // đổi chất lượng có thể đổi luôn videoWidth/Height
  __uvdUpdateHeaderFloat(); // phòng khi metadata đã có sẵn (ví dụ resume) trước khi listener kịp gắn

  // FOOTER — trả về dòng riêng bên dưới video (không overlay nữa). Bản 6.7.14
  // từng thử đè footer lên đáy video kiểu kính mờ, nhưng lại đè trúng ngay vị
  // trí thanh điều khiển GỐC của Video.js v10 (thư viện ngoài, layout nội bộ
  // của nó không nằm trong tầm kiểm soát CSS của script) — gây chồng chéo 2
  // lớp control. Quay lại cách an toàn: mỗi phần một hàng riêng, không đoán mò
  // toạ độ của thư viện ngoài nữa.
  var footer = document.createElement('div');
  footer.id = '__uvd_player_footer__';
  footer.style.cssText = 'padding:8px 16px;background:rgba(10,6,14,0.85);border-top:1px solid var(--border);font-size:12px;color:#ccc;display:flex;justify-content:space-between;flex-shrink:0;transition: opacity 0.3s ease;';
  footer.innerHTML = '<span id="__uvd_player_status__">Đang tải...</span><span id="__uvd_player_size__" style="color:#ff9ed6;">Đang ước tính dung lượng...</span><span id="__uvd_player_time__" style="cursor:pointer;">00:00</span>';
...
function closePlayer() {
  if (playerState.overlay) {
    ...
    clearSleepTimer();
    clearTimeout(playerState.hideTimeout);
    if (playerState.ambientTimer) { clearInterval(playerState.ambientTimer); playerState.ambientTimer = null; }
    if (playerState.audioCtx) {
...
.uvd-toolbar-divider{width:1px;align-self:stretch;min-height:34px;background:rgba(255,143,214,0.16);margin:0 2px;flex-shrink:0;}
...
.uvd-header-float #__uvd_player_header__{margin-bottom:-14px;background:linear-gradient(to bottom,rgba(14,16,22,0.94),rgba(14,16,22,0.6));border-bottom-color:transparent;}
.uvd-panel-resizing{backdrop-filter:none!important;-webkit-backdrop-filter:none!important;background:var(--glass)!important;}
...
function buildUI() {
  ...
  header.innerHTML = 
    ...
    '<div style="display:flex;gap:6px;">' +
      ...
      (data.settings.allowMinimize ? '<button class="uvd-btn-icon" id="__uvd_minimize_script__" title="Thu nhỏ Script">▼</button>' : '') +
  ...
  var __uvdMinBtn = document.getElementById('__uvd_minimize_script__');
  if (__uvdMinBtn) __uvdMinBtn.onclick = minimizeScriptPanel;
...
function renderSettings(container) {
  ...
  container.innerHTML =
    ...
    '<div class="uvd-card">' +
      '<div style="font-weight:600;margin-bottom:8px;">📐 Nút Thu nhỏ Script</div>' +
      buildToggleRow('__uvd_toggle_allowmin__', 'Hiện nút ▼ thu nhỏ panel về góc màn hình', data.settings.allowMinimize) +
      '<div style="font-size:11px;color:var(--text3);margin-top:6px;">Tắt đi nếu bạn hay gặp panel bị glitch/không giữ hiệu ứng kính mờ lúc thu nhỏ — panel sẽ luôn hiển thị đầy đủ, chỉ đóng bằng nút ×.</div>' +
    '</div>' +
  ...
  document.getElementById('__uvd_toggle_allowmin__').onclick = function() {
    var isOn = this.classList.toggle('uvd-toggle-on');
    data.settings.allowMinimize = isOn;
    storage.set(data);
    var panelEl = document.getElementById('__uvd__');
    if (!isOn && panelEl && panelEl.classList.contains('uvd-panel-minimized')) restoreScriptPanel(); // đang thu nhỏ mà tắt tính năng thì mở lại luôn
    buildUI(); // vẽ lại header để hiện/ẩn nút ▼ ngay
    toast(isOn ? 'Đã bật nút thu nhỏ Script' : 'Đã tắt nút thu nhỏ Script');
  };
...
function minimizeScriptPanel() {
  ...
  panel.classList.add('uvd-panel-resizing'); // tắt backdrop-filter trong lúc animate — đỡ giật
  ...
  panel.addEventListener('transitionend', function onEnd(e) {
    ...
    panel.classList.remove('uvd-panel-resizing'); // bật lại kính mờ khi đã đứng yên
  });
}
