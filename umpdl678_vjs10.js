 ...
 * - MỚI: tab "Nút đã click" — mỗi lần auto-click bấm trúng 1 nút cụ thể (vd nút
 *   trong nhóm button[id^="video-host-"]), script tự ghi lại đúng nút đó (theo
 *   từng site) vào danh sách, kèm toggle CHẶN/CHO PHÉP. Bật chặn cho nút nào hay
 *   bấm nhầm (vd mở playlist thay vì phát) thì lần auto-click sau sẽ tự bỏ qua
 *   nút đó, không cần đoán selector nữa.
 * - FIX hiệu năng/giật lag trên site nặng quảng cáo:
 *   + panelObserver trước theo dõi document.body {subtree:true} (rất tốn nếu
 *     trang đổi DOM liên tục) dù panel chỉ gắn trực tiếp vào <html> — đổi sang
 *     theo dõi đúng <html>, childList, không subtree.
 *   + Observer chặn tự phát trước xử lý ngay mỗi mutation record — gộp lại xử
 *     lý 1 lần/frame bằng requestAnimationFrame.
 *   + Ripple effect trước gắn listener lên toàn bộ document mỗi lần buildUI(),
 *     giờ chỉ gắn trong phạm vi panel.
 *   + Hiệu ứng nền "liquid" tự tạm dừng animation khi tab trình duyệt bị ẩn.
 * - FIX auto-click: quét lại NHIỀU LẦN trong ~4s sau khi bấm (thay vì 1 lần) vì
 *   nhiều site load link chậm. Lưu ý: click giả lập luôn có isTrusted=false —
 *   vài site cố tình kiểm tra cờ này để chặn kiểu tự động hoá, script không thể
 *   giả mạo được cờ đó nên vẫn có thể phải bấm tay ở site khó.
