// ==UserScript==
// @name         Universal Media Player v20 - Smart Download
// @namespace    http://tampermonkey.net/
// @version      20.0
// @description  Phát hiện media + Gợi ý tải thông minh + Fullscreen landscape
// @author       You
// @match        *://*/*
// @grant        none
...
(function () {
    ...
    let prefs   = loadObj(PREFS_KEY, { autoScan: true, maxHistory: 100 });
    ...
    function addUrl(u, type, source, priority) {
        ...
        if (!urls.has(u) || urls.get(u).priority > priority) {
            ...
            history.add(u);
            if (history.size > prefs.maxHistory) {
                const arr = [...history];
                history = new Set(arr.slice(-prefs.maxHistory));
            }
    ...
    function scanPerf() {
        try {
            performance.getEntriesByType('resource').forEach(e => {
                if (!isNonMedia(e.name)) findUrls(e.name, 'perf');
            });
        } catch(e) {}
    }
    ...
    function analyzeMedia(url) {
        ...
        else if (/\.m4s/i.test(url)) { info.type = 'MPEG-DASH Segment'; info.format = 'm4s'; }
        else if (/iframe/i.test(url) || /embed/i.test(url)) { info.type = 'Iframe Player'; info.format = 'html'; }

        // Suggest tools based on type
        if (info.format === 'm3u8' || info.format === 'mpd') {
            info.tools = ['yt-dlp', 'ffmpeg', 'VLC'];
            info.commands = [
                { tool: 'yt-dlp', cmd: `yt-dlp --referer "${REF}" "${url}"`, desc: 'Tải tự động, chất lượng cao nhất' },
                ...
                { tool: 'Termux', cmd: `pkg install python ffmpeg -y && pip install yt-dlp && yt-dlp --referer "${REF}" "${url}"`, desc: 'Cài đặt và tải trên Android' },
            ];
            info.tips = [
                'HLS/DASH cần yt-dlp hoặc ffmpeg để tải',
                'Có thể chọn quality cụ thể với flag -f',
                'Dùng VLC để stream trực tiếp'
            ];
        } else if (['mp4', 'webm', 'mkv', 'flv'].includes(info.format)) {
            info.tools = ['IDM', 'aria2', 'wget', 'curl'];
            info.commands = [
                { tool: 'aria2', cmd: `aria2c --referer="${REF}" --user-agent="${UA}" "${url}"`, desc: 'Tải đa luồng, nhanh nhất' },
                ...
                { tool: 'IDM', cmd: `IDM: Thêm URL vào IDM với Referer: ${REF}`, desc: 'Dùng IDM GUI trên Windows' },
            ];
            info.tips = [
                'File trực tiếp, có thể tải bằng bất kỳ tool nào',
                'aria2c nhanh hơn wget/curl nhờ đa luồng',
                'Nhớ thêm Referer để tránh bị chặn'
            ];
        } else if (info.format === 'html') {
            info.tools = ['Browser', 'yt-dlp'];
            info.commands = [
                { tool: 'Browser', cmd: `Mở trực tiếp: ${url}`, desc: 'Mở trong tab mới' },
                { tool: 'yt-dlp', cmd: `yt-dlp --referer "${REF}" "${url}"`, desc: 'Thử extract từ iframe' },
            ];
            info.tips = [
                'Đây là iframe player, cần extract URL thật',
                'Thử mở trong tab mới để tìm video thật',
                'Dùng yt-dlp với URL iframe để tự động extract'
            ];
        }
    ...
    const CSS = `
        #u-fab {
            ...
            width:56px; height:56px; background:linear-gradient(135deg,#e53935,#c62828);
        ...
        .dl-cmd-label { color:#888; font-size:10px; font-weight:600; margin-bottom:6px; display:flex; justify-content:space-between; }
        .dl-cmd-text { color:#4caf50; font-family:monospace; font-size:11px; word-break:break-all; line-height:1.5; }
        .dl-cmd-copy { background:#1565c0; border:none; color:white; padding:4px 10px; border-radius:6px; cursor:pointer; font-size:10px; font-weight:600; }
    ...
    function initLogic() {
        ...
        function renderStreams() {
            ...
            if (!items.length) {
                pb.innerHTML = `<div style="color:#555;text-align:center;padding:40px 20px;font-size:13px">
                    Chưa tìm thấy media.<br><br>▶️ Phát video rồi nhấn 🔍 QUÉT</div>`;
                return;
            }

            items.forEach(item => {
                const bc = 'lb-' + (['M3U8','MP4','IFRAME','WEBM','MKV','FLV'].includes(item.type) ? item.type : 'other');
        ...
        function renderDownload() {
            ...
            if (!urls.size) {
                pb.innerHTML = `<div style="color:#555;text-align:center;padding:40px 20px;font-size:13px">
                    Chưa có media để tải.<br><br>📺 Chuyển sang tab Streams để tìm video</div>`;
                return;
            }

            // Auto select best quality
            const best = [...urls.values()].sort((a,b) => a.priority - b.priority)[0];
                    ...
                    ${info.commands.map(cmd => `
                        <div class="dl-cmd">
                            <div class="dl-cmd-label">
                                ...
                                <button class="dl-cmd-copy" data-cmd="${cmd.cmd}">📋 Copy</button>
                            </div>
                ...
                parseBtn.onclick = async () => {
                    ...
                    qList.querySelectorAll('.qs-item').forEach(item => {
                        item.onclick = () => {
                            ...
                            newInfo.commands.forEach(cmd => {
                                cmd.cmd = cmd.cmd.replace(best.url, url);
                            });

                            // Re-render commands section
                            const cmdSection = pb.querySelector('.dl-suggest');
                            const cmdDivs = cmdSection.querySelectorAll('.dl-cmd');
                            cmdDivs.forEach((div, i) => {
                                if (newInfo.commands[i]) {
                                    div.querySelector('.dl-cmd-text').textContent = newInfo.commands[i].cmd;
                                    div.querySelector('.dl-cmd-copy').dataset.cmd = newInfo.commands[i].cmd;
                                }
                            });
                        };
                    });
        ...
        function openCmd(url) {
            ...
            const build = (tool) => {
                const cmds = info.commands.filter(c => c.tool.toLowerCase().includes(tool));

                $('u-cmd').innerHTML = `
                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px">
                        <h4 style="color:#fff;margin:0;font-size:14px">💻 Lệnh ${tool.toUpperCase()}</h4>
                    ...
                    <div style="display:flex;gap:4px;margin-bottom:14px">
                        ${['ytdlp','ffmpeg','termux'].map(t=>`
                            <button style="flex:1;background:${t===tool?'#e53935':'#222'};color:white;border:none;padding:8px;border-radius:8px;cursor:pointer;font-size:11px;font-weight:600" data-tab="${t}">${t.toUpperCase()}</button>
                        `).join('')}
                    </div>
                    ${cmds.length ? cmds.map(cmd=>`
                        <div class="cmd-block">
                            <div class="cmd-label">${cmd.desc}</div>
                            <div class="cmd-row">
                                <textarea class="cmd-ta" rows="2" readonly>${cmd.cmd}</textarea>
                                <button class="cmd-cp">📋</button>
                            </div>
                        </div>`).join('') : '<div style="color:#555;text-align:center;padding:20px">Không có lệnh cho tool này</div>'}
        ...
        $('btn-scan').onclick=()=>{ scan(document,'deep'); scanPerf(); renderPanel(); toast('✅ '+urls.size+' media'); };
